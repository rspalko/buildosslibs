  ...
  initialize_significance_distortion_luts()
{
  for (kdu_int32 n=0; n < SIGNIFICANCE_DISTORTIONS; n++)
    { 
      double val = 0.5 + (double)(n | (1<<DISTORTION_LSBS));
      double v_tilde = val / ((double)(1<<DISTORTION_LSBS));
      assert((v_tilde >= 1.0) && (v_tilde < 2.0));
      double sqe_before = v_tilde*v_tilde;
      double sqe_after1 = (v_tilde-1.5)*(v_tilde-1.5);     // midpoint centroid
      double sqe_after2 = (v_tilde-1.375)*(v_tilde-1.375); // 3/8 centroid
      double sqe_after = 0.75*sqe_after1 + 0.25*sqe_after2;
  ...
  initialize_refinement_distortion_luts()
{
  for (kdu_int32 n=0; n < REFINEMENT_DISTORTIONS; n++)
    {
      double v_tilde = (0.5 + (double) n) / ((double)(1<<DISTORTION_LSBS));
      assert(v_tilde < 2.0);
      double sqe_before1 = (v_tilde-1.0)*(v_tilde-1.0);   // midpoint centroid
      double sqe_before2 = (v_tilde-0.75)*(v_tilde-0.75); // 3/8 centroid
      double sqe_before = 0.75*sqe_before1 + 0.25*sqe_before2;
      v_tilde = (n >> DISTORTION_LSBS)?(v_tilde-1.0):v_tilde;
      assert((v_tilde >= 0.0) && (v_tilde < 1.0));
      double sqe_after1 = (v_tilde-0.5)*(v_tilde-0.5);     // midpoint centroid
      double sqe_after2 = (v_tilde-0.375)*(v_tilde-0.375); // 3/8 centroid
      double sqe_after = 0.75*sqe_after1 + 0.25*sqe_after2;
...
#  define _raw_enc_(coder,symbol) coder.raw_encode(symbol)
 ...
 /* The coding pass functions defined below all return 32-bit integers,
    which represent the normalized reduction in MSE associated with the
    ...
    that the coding of symbols actually increases distortion.
       There are two sets of coding pass functions.  The first set
    accumulate these distortion reductions within an array of integers,
    each element of which represents a 4x4 cell within the code-block.  Each
    stripe in the code-block scan is 4 lines high, except possibly the last
    one, so each such scan stripe contains ceil(W/4) cells, where W is
    the code-block width.  The second set of coding pass functions return
    a single distortion reduction value.
 */


/* ========================================================================= */
/*        Coding pass functions with Cellular distortion accumulation        */
/* ========================================================================= */

/*****************************************************************************/
/* STATIC      encode_sig_prop_pass_raw (cellular distortion)                */
/*****************************************************************************/
...
static void
  encode_sig_prop_pass_raw(mq_encoder &coder, int p, bool causal,
                           ...
                           bool lossless_pass, kdu_int32 *dcells)
{
  ...
  kdu_int32 distortion_change=0, dcell_cnt=0;
  if (lossless_pass)
  ...
  for (r=num_stripes; r > 0; r--, cp += EXTRA_ENCODE_CWORDS, sp += width_by3,
       dcell_cnt += ((-dcell_cnt) & 3)) // Round count up to multiple of 4
    for (c=width; c > 0; c--, sp++, cp++, dcell_cnt++)
      { 
        if (((dcell_cnt & 3) == 0) && (dcell_cnt != 0))
          { // Update the cellular distortion record
            *(dcells++) = distortion_change;
            distortion_change = 0; // Start accumulating from scratch
          }
        if (*cp == 0)
          continue;
        cword = *cp;
...
/*****************************************************************************/
/* STATIC         encode_sig_prop_pass (cellular distortion)                 */
/*****************************************************************************/
...
/*****************************************************************************/
/* STATIC        encode_mag_ref_pass_raw (cellular distortion)               */
/*****************************************************************************/
...
/*****************************************************************************/
/* STATIC          encode_mag_ref_pass (cellular distortion)                 */
/*****************************************************************************/
...
/*****************************************************************************/
/* STATIC         encode_cleanup_pass (cellular distortion)                  */
/*****************************************************************************/
...
/* ========================================================================= */
/*         Coding pass functions with Global distortion accumulation         */
/* ========================================================================= */
...
static void
  find_convex_hull(int pass_lengths[], double pass_wmse_changes[],
                   kdu_uint16 pass_log_slopes[], int num_passes)
{
  /* In the following algorithm, we walk through the passes from z=0 to
     z=num_passes-1.  As we go, we fill in the temporary `pass_slopes'
     array with computed distortion-length slopes and we fill in the
     `pass_log_slopes' array with their logs (generated using `slope_to_log')
     or else with 0.  Zeroes are inserted into the `pass_log_slopes'
     array for points that do not contribute to the convex hull.  To
     determine a slope for pass z, we walk backwards, accumulating pass
     length and distortion changes from z to z', were z' is the first
     pass < z that we come to, that has a valid slope strictly greater than
     that of z.  The slope for pass z is defined as the reduction in
     distortion between z' and z divided by the increase in length between
     z' and z.  This value is converted using `slope_to_log' and recorded
     in the `pass_log_slopes' array.  We must also be careful to check
     that the converted slopes remain monotonically decreasing. */
  
  double pass_slopes[MAX_POSSIBLE_PASSES];
  int z;
  for (z=0; z < num_passes; z++)
    {
      ...
      while (true)
        { 
          delta_L += (double) pass_lengths[z_prime];
          delta_D += pass_wmse_changes[z_prime];
          z_prime--;
          if (delta_D <= 0.0)
          ...
          if (z_prime < 0)
            { // We have gone back all the way to the start
              assert(delta_L > 0.0);
              pass_slopes[z] = delta_D / delta_L;
              pass_log_slopes[z] = slope_to_log(pass_slopes[z]);
              break;
            }
          if (pass_log_slopes[z_prime] == 0)
            continue;
          if ((delta_L == 0.0) || ((pass_slopes[z_prime]*delta_L) <= delta_D))
            pass_log_slopes[z_prime] = 0;
          else
            {
              pass_slopes[z] = delta_D / delta_L;
              pass_log_slopes[z] = slope_to_log(pass_slopes[z]);
              if (pass_log_slopes[z] >= pass_log_slopes[z_prime])
                pass_log_slopes[z_prime] = 0; // Keep the higher quality point
              break;
            }
        }
    }
...
void
  kd_block_encoder::encode(kdu_block *block, bool reversible,
                           double msb_wmse,
                           kdu_uint16 estimated_threshold)
{
  ...
  // Determine the actual number of passes which we can accommodate.
  ...
  do {
      ...
      // if (context_row_gap > num_cols) // Condition will always be true
        {
          kdu_int32 oob_marker =
            OOB_MARKER | (OOB_MARKER<<3) | (OOB_MARKER<<6) | (OOB_MARKER<<9);
      ...
      int bypass_threshold = 0; // Coding pass first affected by bypass mode
      if (block->modes & Cmodes_BYPASS)
        { 
          bypass_threshold = 10; // First bypassed coding pass
          if (block->modes & Cmodes_BYPASS_E1)
            bypass_threshold -= 3;
          if (block->modes & Cmodes_BYPASS_E2)
            bypass_threshold -= 6;
        }
              ...
              if (bypass_threshold)
                {
                  if (z < bypass_threshold)
                    segment_passes = bypass_threshold-z;
          ...
          if (estimated_slope_threshold > 0.0)
            { // See if we can finish up early to save time and memory.
              ...
              */
              ...
              int u, z0;
              double last_deltaD=0.0, last_deltaL=0.0;
              double deltaD, deltaL, best_deltaD=0.0, best_deltaL=0.0;
...
/*****************************************************************************/
/*                    kd_block_encoder::cellular_encode                      */
/*****************************************************************************/

void
  kd_block_encoder::cellular_encode(kdu_block *block, bool reversible,
                                    double msb_wmse, float cell_weights[],
                                    int first_cell_cols, int first_cell_rows,
                                    kdu_uint16 estimated_threshold)
  /* Like `encode', but this function is called when a weighting function
     is available for the distortion values within each 4x4 cell.  The function
     is identical to `encode', except that it uses the cellular coding pass
     functions to accumulate distortion in cells, scaling the cell distortions
     by the values found in `cell_weights' and accumulating to form global
     distortion contributions for each coding pass: the global values are
     finally scaled by `msb_wmse' in the usual way.
        The number of cells across and down the code-block is always given
     by the block's width and height, divided by 4 and rounded up to the
     nearest integer.  In most cases, the `first_cell_cols' and
     `first_cell_rows' values should turn out to be 4, but the initial row
     code-blocks and the initial column of code-blocks can have values in
     the range 1 to 3 if the image origin is not aligned at (0,0) -- not
     very common.  Since these cases can be a nuisance to handle, we just
     use bilinear interpolation to crudely shift the cell weights, so that
     the coding pass functions can be free to accumulate distortions within
     4x4 cells that are aligned with the left and upper edges of the
     code-block. */
{
  double estimated_slope_threshold = -1.0;
  if ((estimated_threshold > 1) && (msb_wmse > 0.0) &&
  ...
  // See if we need to shift the cell weights array a little to accommodate
  // initial cell dimensions other than 4, as explained above.
  int cells_wide=(block->size.x+3)>>2, cells_high=(block->size.y+3)>>2;
  int num_cells=cells_wide*cells_high;
  if (first_cell_cols != 4)
    { // Shift horizontally
      assert((first_cell_cols >= 1) && (first_cell_cols <= 3));
      float *cwp=cell_weights, shift=0.25f*(4-first_cell_cols);
      for (int m=cells_high; m > 0; m--, cwp+=cells_wide)
        for (int n=1; n < cells_wide; n++)
          cwp[n-1] += shift*(cwp[n]-cwp[n-1]);
    }
  if (first_cell_rows != 4)
    { // Shift vertically
      assert((first_cell_rows >= 1) && (first_cell_rows <= 3));
      float *cwp=cell_weights, shift=0.25f*(4-first_cell_rows);
      for (int m=cells_high-1; m > 0; m--, cwp+=cells_wide)
        for (int n=0; n < cells_wide; n++)
          cwp[n] += shift*(cwp[n+cells_wide]-cwp[n]);
    }
  
  // Allocate space on the stack for a number of largish quantities.  These
  // could be placed elsewhere.
  assert(num_cells <= 256);
  kdu_int32 cell_distortions[256]; // We don't need to clear this array
  double pass_wmse_changes[MAX_POSSIBLE_PASSES];
  mqe_state states[18];
  mq_encoder pass_encoders[MAX_POSSIBLE_PASSES];
  // Get dimensions.
        ...
        { 
          ...
          if ((k == 0) && !bypass)
            encode_sig_prop_pass(pass_encoders[z],
                                 ...
                                 samples,context_words,
                                 num_cols,num_stripes,context_row_gap,
                                 lossless_pass,cell_distortions);
          else if (k == 0)
            encode_sig_prop_pass_raw(pass_encoders[z],
                                     p,causal,samples,context_words,
                                     num_cols,num_stripes,context_row_gap,
          ...
          float distortion_change = 0.0f;
          for (int i=0; i < num_cells; i++)
            distortion_change += cell_weights[i] * (float)cell_distortions[i];
          pass_wmse_changes[z] = pass_wmse_scale * distortion_change;
          if ((block->modes & Cmodes_SEGMARK) && (k==2))
            {
              kdu_int32 segmark = 0x0A;
