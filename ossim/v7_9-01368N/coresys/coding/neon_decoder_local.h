/*****************************************************************************/
// File: neon_decoder_local.h [scope = CORESYS/CODING]
// Version: Kakadu, V7.9
...
******************************************************************************/
#ifndef NEON_DECODER_LOCAL_H
#define NEON_DECODER_LOCAL_H
...
#define NEONI_SET_BLOCK_ZERO16(_tgt,_nom_width) /* No NEON accelerator yet. */

#define KD_SET_SIMD_FUNC_BLOCK_ZERO16(_tgt,_nw) \
  { \
    NEONI_SET_BLOCK_ZERO16(_tgt,_nw); \
  }
...
#define NEONI_SET_BLOCK_ZERO32(_tgt,_nom_width) /* No NEON accelerator yet. */

#define KD_SET_SIMD_FUNC_BLOCK_ZERO32(_tgt,_nw) \
  { \
    NEONI_SET_BLOCK_ZERO32(_tgt,_nw); \
  }
...
/*****************************************************************************/
  ...
  extern void
    neoni_xfer_rev_decoded_block16(kdu_int32 *,void **,
                                   int,int,int,int,int,float);
#  define NEONI_SET_BLOCK_XFER_REV16(_tgt,_kmax,_nom_width) \
          if ((kdu_neon_level > 0) && (_nom_width >= 8)) \
            _tgt = neoni_xfer_rev_decoded_block16;
...
#define KD_SET_SIMD_FUNC_BLOCK_XFER_REV16(_tgt,_tr,_vf,_hf,_kmax,_nw) \
  { \
    if (!(_tr || _vf || _hf)) \
      { \
        NEONI_SET_BLOCK_XFER_REV16(_tgt,_kmax,_nw); \
      } \
  }
...
#ifdef KDU_NEON_INTRINSICS
  extern void
    neoni_xfer_rev_decoded_block32(kdu_int32 *,void **,
                                   int,int,int,int,int,float);
#  define NEONI_SET_BLOCK_XFER_REV32(_tgt,_nom_width) \
          if ((kdu_neon_level > 0) && (_nom_width >= 4)) \
            _tgt = neoni_xfer_rev_decoded_block32;
#else // No compilation support for ARM-NEON
#  define NEONI_SET_BLOCK_XFER_REV32(_tgt,_nom_width)
#endif
...
#define KD_SET_SIMD_FUNC_BLOCK_XFER_REV32(_tgt,_tr,_vf,_hf,_nw) \
  { \
    if (!(_tr || _vf || _hf)) \
      { \
        NEONI_SET_BLOCK_XFER_REV32(_tgt,_nw); \
      } \
  }
...
#ifdef KDU_NEON_INTRINSICS
  extern void
    neoni_xfer_irrev_decoded_block16(kdu_int32 *,void **,
                                     int,int,int,int,int,float);
#  define NEONI_SET_BLOCK_XFER_IRREV16(_tgt,_kmax,_nom_width) \
          if ((kdu_neon_level > 0) && (_nom_width >= 8)) \
            _tgt = neoni_xfer_irrev_decoded_block16;
#else // No compilation support for ARM-NEON
#  define NEONI_SET_BLOCK_XFER_IRREV16(_tgt,_kmax,_nom_width)
#endif
...
#define KD_SET_SIMD_FUNC_BLOCK_XFER_IRREV16(_tgt,_tr,_vf,_hf,_kmax,_nw) \
  { \
    if (!(_tr || _vf || _hf)) \
      { \
        NEONI_SET_BLOCK_XFER_IRREV16(_tgt,_kmax,_nw); \
      } \
  }
...
#ifdef KDU_NEON_INTRINSICS
  extern void
    neoni_xfer_irrev_decoded_block32(kdu_int32 *,void **,
                                     int,int,int,int,int,float);
#  define NEONI_SET_BLOCK_XFER_IRREV32(_tgt,_nom_width) \
          if ((kdu_neon_level > 0) && (_nom_width >= 4)) \
            _tgt = neoni_xfer_irrev_decoded_block32;
#else // No compilation support for ARM-NEON
#  define NEONI_SET_BLOCK_XFER_IRREV32(_tgt,_nom_width)
#endif
...
#define KD_SET_SIMD_FUNC_BLOCK_XFER_IRREV32(_tgt,_tr,_vf,_hf,_nw) \
  { \
    if (!(_tr || _vf || _hf)) \
      { \
        NEONI_SET_BLOCK_XFER_IRREV32(_tgt,_nw); \
      } \
  }
  
} // namespace kd_core_simd

#endif // NEON_DECODER_LOCAL_H
