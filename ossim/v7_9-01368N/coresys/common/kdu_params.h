...
class kdu_output {
  ...
  */
  public: // Member functions
    ...
    void fill_bytes(int count, kdu_byte byte)
      {
      /* [SYNOPSIS] Copies `byte' zero or more times into the output.
       [ARG: count] The number of copies of `byte' to emit. */
      while (count > 0)
        { 
          int xfer_bytes = (int)(end_buf - next_buf);
          if (xfer_bytes == 0)
          ...
          xfer_bytes = (count < xfer_bytes)?count:xfer_bytes;
          count -= xfer_bytes;
          memset(next_buf,byte,xfer_bytes);
          next_buf += xfer_bytes;
        }
      }
    void fill_pairs(int count, kdu_byte b0, kdu_byte b1)
      { 
        /* [SYNOPSIS] Copies the byte pair `b0', `b1', in that order, zero or
         more times into the output.
         [ARG: count] The number of byte pairs to emit. */
        while (count > 0)
          { 
            int xfer_bytes = (int)(end_buf - next_buf);
            if (xfer_bytes < 2)
              { 
                if (xfer_bytes == 0)
                  ...
                  { *(next_buf++) = b0;  flush_buf();  *(next_buf++) = b1;
                    count--;  xfer_bytes = (int)(end_buf - next_buf); }
              }
            int xfer_pairs = xfer_bytes >> 1;
            xfer_pairs = (count < xfer_pairs)?count:xfer_pairs;
            count -= xfer_pairs;
            for (; xfer_pairs > 0; xfer_pairs--, next_buf+=2)
              { next_buf[0] = b0; next_buf[1] = b1; }
          }
      }
...
class kdu_params {
  ...
  public: // Lifecycle functions
    ...
    virtual kdu_params *new_object() { return NULL; }
      /* [SYNOPSIS]
           ...
           [//]
           Note: this should ideally be a pure virtual function, rather than
           a function that returns NULL.  However, unfortunately, there is
           no clean way to then avoid applying the dllexport attribute to
           whole classes (as opposed to just member functions) when building
           a Windows DLL with GCC.
      */
  protected:
    ...
    KDU_EXPORT kdu_params *
      ...
      /* [BIND: donate]
         ...
         [ARG: num_comps]
           ...
           diversity, in which case `num_comps' must be 0.  Note also that
           different clusters can have different non-zero values for
           `num_comps'.  Almost always, `num_comps' is either 0 or else
           corresponds to the number of codestream image components.  However,
           for clusters of the `NLT_params' type, `num_comps' is actually the
           number of output image components produced by a multi-component
  ...
  protected: // Specific transcoding members implemented by each derived class
    virtual void copy_with_xforms(kdu_params *source, int skip_components,
                                  ...
                                  bool vflip, bool hflip) { return; };
      /* Each derived class must implement this copy function.
  ...
  public: // Marker oriented attribute access functions
    KDU_EXPORT void
      ...
      /* [SYNOPSIS]
           ...
           [//]
           If `for_reading' is true, the function also sets each
           `kd_attribute::num_marked_records' value to 0.
      */
    KDU_EXPORT bool
      any_changes();
      /* [SYNOPSIS]
  ...
  protected: // Individual marker translators
    ...
    virtual bool
      check_marker_segment(kdu_uint16 code, int num_bytes,
                           kdu_byte bytes[], int &c_idx) { return true; }
    ...
    virtual bool
      read_marker_segment(kdu_uint16 code, int num_bytes,
                          kdu_byte bytes[], int tpart_idx) { return false; }
    ...
    virtual void reset_read_state() { return; }
      /* Only needs to be overridden in a derived parameter object that
         keeps state information based on what marker segments have been
         read already.  This function is called from `kdu_params::clear_marks'
         which is invoked prior to re-translation of a set of marker
         segments. */
  // --------------------------------------------------------------------------
  ...
  public: // String oriented attribute access functions
    ...
    KDU_EXPORT void
      ...
      /* [SYNOPSIS]
           ...
           function is called.
           [//]
           Importantly, this function will not touch attributes whose values
  ...
  public: // Finalization functions
    ...
    KDU_EXPORT void
      ...
      /* [SYNOPSIS]
           ...
           [//]
           This function (and especially the second version below) also plays
           an important role in dealing with parameters that have been
           parsed from one codestream and are subsequently reparsed from a
           different codestream, with potentially different parameters.  This
           is relevant only if `after_reading' is true, in which case the
           function examines the `kd_attribute::num_marked_records' values
           to determine whether any attributes have more records than the
           number that are marked.  This means that more records were set
           for the parameter prior to a call to `clear_marks' than were
           set after the call (this happens for codestreams that have been
           restarted, using `kdu_codestream::restart').  When this condition
           is detected, the `kd_attribute::num_records' value is reduced to
           `kd_attribute::num_marked_records', erasing any surplus attribute
           values that were read from a different codestream, prior to the
           `clear_marks' call.  At the same time, the `changed' flags are
           manipulated so as to ensure that the parameter change becomes
           apparent to the `kdu_codestream' machinery.
         [ARG: after_reading]
  ...
  private: // Parameter attribute data members
    kd_core_local::kd_attribute *attributes; // Points to a list of attributes
    friend struct kd_core_local::kd_attribute;
...
#define Scbr "Scbr" // One record x "II"
...
#define Mprecision "Mprecision" // Multiple records x "I"
  ...
  // Attributes derived from the NLT marker segments, if present in the
  // main codestream header.
#define Ncomponents "Ncomponents" // One record x "I"
#define Nsigned     "Nsigned"     // Multiple records x "B"
#define Nprecision  "Nprecision"  // Multiple records x "I"
...
#define Sprofile_CINEMA2S  ((int) 7)
#define Sprofile_CINEMA4S  ((int) 8)
#define Sprofile_CINEMASS  ((int) 9)
#define Sprofile_IMF       ((int) 10)
...
#define Sextensions_NLT     ((int) 512)
...
#define Sbroadcast_rev      ((int) 1)
  ...
  // Values for the Simf attribute
#define Simf_irrev          ((int) 0)
#define Simf_rev            ((int) 1)

  // Values for the SCpart2_caps attribute
#define SCpart2_caps_EXTENDED_COD ((int) 32768)
...
class siz_params: public kdu_params {
  /* [BIND: reference]
     ...
     transforms.
     [//]
     This object also manages the information associated with a JPEG2000 `CAP'
     (Capabilities) marker segment, if any.  There is at most one of these
     per image, which must appear in the main header.  The CAP marker
     segment is written if `SCpart2_caps' is non-zero; moreover, if the
     capability associated with `SCpart2_caps' is one that cannot be ignored
     by a decoder that does not understand the associated ammendment
     capability, the `Scap' boolean attribute is set to true when writing
     the SIZ marker segment.  During codestream parsing, the `Scap' attribute
     determines what happens if a `CAP' marker segment is encountered that
     defines unrecognized capabilities.  If this happens and `Scap' is true,
     a warning message is generated; otherwise, the unrecognized capabilities
     ...
     [>>] Macro = `Sprofile';
          ...
          value must currently be an integer in the range 0 to 10.  Profile0
          ...
          the `Sextensions' parameter attribute.
             The values 4 and 5 correspond to profile restrictions for
          ...
          are closely relatred and very restrictive subsets of JPEG2000 Part 1,
          defined by various ammendments to the original standard.  
             The values 7 and 8 correspond to scalable versions of the 2K
          and 4K digital cinema profiles, supporting 2 quality layers each,
          while value 9 identifies the "long term storage" digital cinema
          profile, that supports dimensions up to 16384 x 8640 and 5
          quality layers.
             The value 10 is used for the IMF (Interoperable Master Format)
          profiles, which are characterized by main- and sub-levels,
          reversibility (or lack thereof) and three different maximum
          frame sizes (2K, 4K and 8K).  This auxiliary information is
          ...
          user to supply `Creslengths' (and possibly `Cagglengths') attributes
          so as to ensure that compressed data sizes match the
     ...
     [>>] Macro = `Sextensions'; Pattern =
          ...
          `Sextensions_NLT' and `Sextensions_ROI'.
     [>>] Macro = `Sbroadcast';
          ...
          level, which is currently required to lie in the range 0 through 11.
          The profile level identifies the maximum bit-rate (in Mbits/s 
          ...
          L >= 6 means = 2^(L-6)*1600Mbits/s, 2^(L-6)*1200Msamples/s.
             The system does not explicitly impose these constraints, since
          they depend upon the intended frame rate -- this can readily be done
          ...
          `Sprofile' equals `Sprofile_BROADCAST'.  Also, if `Sbroadcast' is
          specified and `Sprofile' is not explicitly specified, the system
          automatically sets `Sprofile' to `Sprofile_BROADCAST'.
     [>>] Macro = `Simf';
          Pattern = "II(irrev=0,rev=1)" --
          This parameter attribute provides detailed profile constraints 
          to accompany the IMF (Interoperable Master Format) profile
          (`Sprofile'=10).  Its three fields have the following interpretation.
             The first field is the `main-level' of the profile, an integer
          in the range 0 to 11 that describes the maximum number of samples
          per second, taking into account frame rate, frame size and component
          sub-sampling rate, as explained in Ammendment 8 of IS15444-1:
          0 means unspecified; 1 means 65Msamples/s; 2 means 130Msamples/s;
          3 means 195Msamples/s; 4 means260Msamples/s; 5 means 520Msamples/s;
          and L >= 6 means 2^(L-6)*1200Msamples/s.  Here 1 Msample/s means
          10^6 samples/s.
             The second field is the profile's sub-level, an integer in
          the range 0 to 9, which may not exceed the larger of 1 and the
          main level minus 2; its purpose is to identify the maximum bit-rate
          associated with the codestreams: 0 means unspecified; other values L
          mean 2^L * 10^8 bits/s.
             The final field indicates whether the profile is irreversible
          (`Simf_irrev') or reversible (`Simf_rev'), where the irreversible
          variant is required to be untiled, while the reversible case allows
          for untiled codestreams or tiled codestreams with tile dimensions
          (`Stiles') of 1024x1024, 2048x2048 or 4096x4096.
             During codestream generation, if `Sprofile' equals `Sprofile_IMF',
          this parameter attribute defaults to the main-level 0, sub-level 0,
          irreversible variant.  Also, if `Simf' is specified and `Sprofile'
          is not explicitly specified, the system automatically sets `Sprofile'
          to `Sprofile_IMF'.
     [>>] Macro = `Scap'; Pattern = "B" --
          ...
          the `SCpart2_caps' attribute.
     [>>] Macro = `SCpart2_caps'; Pattern = "[EXTENDED_COD=32768]" --
          Identifies capability extensions that are found in ammendments to
          Part-2 of the JPEG2000 family of standards.  Currently, only one
          such capability, `SCpart2_caps_EXTENDED_COD', is defined,
          corresponding to the possible presence of block-coder extensions
          in COD/COC marker segments.  This capability was introduced with
          IS15444-2/AMD4.
     ...
     [>>] Macro = `Ssigned'; Pattern = "B",
          ...
          `Ssigned', which happens whenever there is no Part-2 multi-component
          transform (i.e., `Mcomponents'=0).  If there is a multi-component,
          `Nsigned' is usually sufficient to deduce `Msigned' automatically,
          but you will need to explicitly specify `Ssigned'.  If any value for
          `Ssigned' is specified explicitly, all values must be specified --
          missing values are derived by auto-replicating the last value that
          was specified.
     [>>] Macro = `Sprecision'; Pattern = "I",
          ...
          information from input image files are recommended to explicitly set
          `Nprecision' only, leaving the values to be copied to `Sprecision'
          automatically.  If there is a Part-2 multi-component transform
          (`Mcomponents' != 0), however, `Nprecision' is usually sufficient to
          deduce `Mprecision' automatically, but you will need to explicitly
          specify `Sprecision'.  If any value for `Sprecision' is specified
          explicitly, all values must be specified -- missing values are
          derived by auto-replicating the last value that was specified.
     ...
     [>>] Macro = `Mcomponents'; Pattern = "I" --
          ...
          defines the number of output image components, but `Scomponents' can
          be derived automatically from `Ncomponents' if it has a non-zero
          value and `Mcomponents'=0.
     [>>] Macro = `Msigned'; Pattern = "B",
          ...
          `Msigned' to be deduced automatically (so long as `Mcomponents' > 0).
          The only condition under which `Msigned' and `Nsigned' need to be
          explicitly specified separately is where a non-linear point transform
          (see `NLType') needs to change the signed characteristics of its
          samples -- probably not common.  If you do specify any `Msigned'
          values, you need to supply them all, or be content with the
          auto-replicating policy that replicates the last specified value
          as required.
     [>>] Macro = `Mprecision'; Pattern = "I",
          ...
          under which `Mprecision' and `Nprecision' need to be
          explicitly specified separately is where a non-linear point transform
          (see `NLType') needs to change the precision of its samples.
          If you do specify any `Mprecision' values, you need to supply them
          all, or be content with the auto-replicating policy that replicates
          the last specified value as required.
     [>>] Macro = `Ncomponents'; Pattern = "I" --
          ...
          be identical, so it is not strictly necessary to explicitly specify
          `Ncomponents' -- it can be auto-deduced.  If `Mcomponents'=0, the
          values of `Ncomponents' and `Scomponents' must be identical, so
          again it is not strictly necessary to specify both.
     [>>] Macro = `Nsigned'; Pattern = "B",
          [MULTI_RECORD, CAN_EXTRAPOLATE] --
          ...
          Compressors are recommended to explicitly specify `Nsigned', leaving
          the internal machinery to automatically deduce `Msigned' or `Ssigned'
          from `Nsigned' (by copying).  If there is no Part-2 multi-component
          transform, `Ssigned' will be copied from `Nsigned' or vice-versa
          if either is not specified.  If there is a Part-2 multi-component
          transform (i.e., `Mcomponents' > 0), `Msigned' will be copied from
          `Nsigned' or vice-versa if either is not explicitly specified.
          If you do specify any `Nsigned' attribute value, you must specify
          all `Ncomponents' of them or be content with the auto-replicating
          policy that replicates the last specified value as required.
     [>>] Macro = `Nprecision'; Pattern = "I",
          [MULTI_RECORD, CAN_EXTRAPOLATE] --
          Plays the same role as `Mprecision' and `Sprecision', but describes
          the ultimate precision of the samples produced during decompression
          after applying any non-linear point transform.  Compressors are
          recommended to explicitly specify `Nprecision', leaving the internal
          machinery to automatically deduce `Mprecision' or `Sprecision' from
          `Nprecision' (by copying).  If there is no Part-2 multi-component
          transform, `Sprecision' will be copied from `Nprecision' or
          vice-versa if either is not specified.  If there is a Part-2
          multi-component transform (i.e., `Mcomponents' > 0), `Mprecision'
          will be copied from `Nprecision' or vice-versa if either is not
          explicitly specified.  If you do specify any `Nprecision' attribute
          value, you must specify all `Ncomponents' of them or be content with
          the auto-replicating policy that replicates the last specified value
          as required.
     [>>] Macro = `Scbr'; Pattern="IF" --
          ...
          size Bmax of a notional "leaky bucket" that is inserted between the
          incremental flushing process and a constant rate data channel.
          We now briefly explain these concepts.  The packet progression
          sequence for each tile is required to be `Corder_PCRL', and there
          will usually only be one tile.  No extra tile-parts are currently
          allowed, so each tile (usually just one) undergoes a series of flush
          operations that are separated by V rows of lowest resolution
          precincts from the lowest resolution of the first tile's first
          codestream component.  That is, each flush-set commences with the
          first packet from precinct row V*k of the lowest resolution in
          tile-component 0, where k=0,1,2,...  Normally, V will be 1, and
          precinct dimensions will be chosen such that all flush sets involve
          the same number of packets/precincts, except possibly the last one.
          If image components have different sizes, you will most likely need
          to adjust the number of vertical decomposition levels and the
          precinct dimensions on a per-component basis to ensure that all
          components can flush the same number of precincts in each flush
          set.  The leaky bucket size Bmax is measured in terms of lines
          from the first codestream image component.  Bmax need not be
          an integer, but it will be converted internally to a leaky
          bucket size that is measured in bytes.  Bmax must be at
          least as large as the number of lines from the first
          ...
          inter-flush separation model provides a valid mechanism for assessing
          the delay attributed to communication, so long as all flush sets
          (except possibly the last one) have the same structure, so that
          flush sets naturally become available at a constant rate from the
          compressor; an informative error message is generated if this is
          not the case.  The maximum delay associated with the leaky bucket
          model is then Bmax*Weff/Rsample seconds, where Weff is an
          ...
          set, while the EOC marker is included as part of the last flush
          set.  Apart from this, the first flush set of every tile includes
          that tile's tile-part header in its byte count.  According to the
          foregoing model, the leaky bucket may not be filled beyond
          Bmax*Weff*D bytes as a result of any given flush, after which
  ...
  public: // Member functions
    KDU_EXPORT siz_params();
      /* [SYNOPSIS]
           ...
           dimensions.
      */
    KDU_EXPORT virtual ~siz_params();
  protected:
    virtual kdu_params *new_object() { return NULL; }
      /* There can be only one `siz_params' object. */
    KDU_EXPORT virtual void finalize(bool after_reading=false);
    virtual void copy_with_xforms(kdu_params *source, int skip_components,
...
class mct_params: public kdu_params {
  ...
  */
  public: // Member functions
    ...
    KDU_EXPORT virtual ~mct_params();
  protected:
    KDU_EXPORT virtual kdu_params *new_object();
    ...
    virtual bool read_marker_segment(kdu_uint16 code, int num_bytes,
                                     kdu_byte bytes[], int tpart_idx);
    virtual void reset_read_state()
      { // Resets the private counters
...
#define Mstage_collections "Mstage_collections" // Multiple records x "II"
#define Mstage_blocks Mstage_collections
...
class mcc_params: public kdu_params {
  /* [BIND: reference]
     ...
     [>>] Macro = `Mstage_collections'; Pattern = "II", [MULTI_RECORD] --
...
class mco_params: public kdu_params {
  /* [BIND: reference]
     ...
     `Mstage_collections' and `Mstage_xforms' attributes to recover all
     relevant details of the transformation process.
     described by an `atk_params' (Arbitrary Transform Kernels) object.
     [//]
     ...
     [>>] Macro = `Mstages'; Pattern = "I", [MULTI_RECORD] --
          ...
          object whose `Mstage_inputs', `Mstage_outputs', `Mstage_collections'
          and `Mstage_xforms' attributes fully describe the transform stage.
  */
  public: // Member functions
    KDU_EXPORT mco_params();
    KDU_EXPORT virtual ~mco_params();
  protected:
    KDU_EXPORT virtual kdu_params *new_object();
    virtual void finalize(bool after_reading=false);
    virtual void copy_with_xforms(kdu_params *source, int skip_components,
...
/*****************************************************************************/
  ...
  // Cluster name
#define NLT_params "NLT"
  // Attributes recorded in an NLT marker segment
#define NLType     "NLType"     // One record x
                                // "(NONE=0,GAMMA=1,LUT=2,SMAG=3,UMAG=4)"
#define NLTgamma   "NLTgamma"   // One record x "FFFFF" -- E/S/T/A/B
#define NLTlut     "NLTlut"     // One record x "FFII" -- Dmin/Dmax/N/P
#define NLTdata    "NLTdata"    // Multiple records x "F" -- N >= 2 points
#define NLTmake    "NLTmake"    // Multiple records x
                                // "(GAMMA=0,IGAMMA=1,LOG=2,ILOG=3)FFI"
  
  // Values for the NLType attribute
#define NLType_NONE  ((int) 0)
#define NLType_GAMMA ((int) 1)
#define NLType_LUT   ((int) 2)
#define NLType_SMAG  ((int) 3)
#define NLType_UMAG  ((int) 4)
  
  // Values for the NLTmake attribute
#define NLTmake_GAMMA  ((int) 0)
#define NLTmake_IGAMMA ((int) 1)
#define NLTmake_LOG    ((int) 2)
#define NLTmake_ILOG   ((int) 3)
  
class nlt_params: public kdu_params {
  /* [BIND: reference]
     [SYNOPSIS]
     Manages the information associated with a JPEG2000 `NLT'
     (Non-Linear Point Transform) marker segment.  Component-specific,
     tile-specific and tile-component-specific forms are all allowed.
     [//]
     Component-specific forms of the NLT parameters are different in that
     they can be specialized to any of the output components identified via
     `Mcomponents', where a multi-component transform is present, as
     opposed to just the codestream components whose number is defined via
     `Scomponents'.
     [//]
     The cluster name is "NLT", but you are recommended to use the
     macro `NLT_params' in functions which take a cluster name like
     `kdu_params::access_cluster'.
     [//]
     A Kakadu-specific restriction applies to the use of tile- and
     tile-component specific forms.  This restriction arises from
     the fact that each JPEG2000 NLT marker segment specifies output (decoded)
     precision and signed/unsigned information for the components that it
     affects, raising the spectre of output components whose bit-depth or
     signed/unsigned characteristics might change from tile to tile.  This
     is something that would surely break just about any application, since
     codestream tiles should not have any significance to higher layers in
     an image processing system, which do need to know about bit-depth.
     [//]
     For this reason, Kakadu insists that all information concerning output
     component precision and signed/unsigned properties can be deduced
     entirely from the codestream's main header.  This information is
     exposed via the `Nprecision' and `Nsigned' attributes offered by
     `siz_params', that are entirely global.  Kakadu provides no means for
     an application to specify separate NLT decoded precision or signed
     attributes at the tile level.  Moreover, when decoding a codestream
     that has such attributes, any tile-specific precision or signed/unsigned
     overrides will be lost -- usually this should not have any impact on
     the usability of decoded results, though.
     [//]
     If you wish to generate a codestream in which some `Nprecision' or
     `Nsigned' value differs from the corresponding `Mprecision'/`Msigned'
     value (if they exist), or the corresponding `Sprecision'/`Ssigned'
     value, then you must also define an `NLType' attribute that applies
     to this component (either a global `NLType' attribute or a component
     specific one) that is not tile-specific.  It is allowed, however, for
     that `NLType' attribute to specify the degenerate value
     "NONE"=0, in which case the component's sample values are
     stretched linearly, as required, to achieve the decoded precision value.
     The use of the "NONE" `NLType' is something that is not carefully
     prescribed in IS15444-2, but the interpretation applied here seems
     most reasonable.
     [//]
     If you define NLT information that is global or tile-specific, but
     not specific to any component or tile-component, then the `Nprecision'
     and `Nsigned' values must be identical for all components.
     [//]
     The following attributes are defined.  For an explanation of pattern
     strings, consult the comments appearing with `kdu_params::parse_string'.
     [>>] Macro = `NLType';
          Pattern = "(NONE=0,GAMMA=1,LUT=2,SMAG=3,UMAG=4)" --
          This attribute identifies the type of non-linear point transform
          to be applied.  In the case of NONE (`NLType_NONE'=0 in code),
          no additional information is required.  The same is true for
          SMAG (`NLType_SMAG'=3 in code) and UMAG (`NLType_UMAG'=4 in code).
          ...
          with any precisions, but 16 would be common for half-float
          compression.  The SMAG option is allowed only if the input and
          output precisions identified by `Sprecision'/`Mprecision' and
          `Nprecision' are identical and the `Ssigned'/`Msigned' and `Nsigned'
          attributes are both true; in this case, -ve values y are mapped to
          z = -2^{B-1} - y - 1, where B is the component bit-depth.
          The UMAG option has the same constraints, but `Ssigned'/`Msigned'
          and `Nsigned' attributes must both be false; this means that
          true negative samples should never occur, so no transformation
          processing strictly needs to be performed, but numerical
          processing machinery needs to be especially careful not
          ...
          For the other two types, GAMMA (`NLType_GAMMA'=1) and LUT
          (`NLType_LUT'=2), the description is not complete without
          additional information supplied via `NLTgamma' or `NLTlut' and
          `NLTdata' attributes, respectively.  During codestream generation,
          these auxiliary attributes can be derived from `NLTmake', which
          provides simpler methods for specifying useful non-linear transforms.
          Moreover, the `NLType' value itself can usually be automatically
          filled in based on the presence of any of the NLT auxiliary
          attributes.
     [>>] Macro = `NLTgamma';
          Pattern = "FFFFF" -- "E/S/T/A/B" --
     ...
     [>>] Macro = `NLTlut';
          Pattern = "FFII" -- Dmin/Dmax/N/P --
          This attribute serves to dimension the lookup table associated with
          a non-linear point transform having `NLType' = `NLType_LUT' = 2.
     ...
     [>>] Macro = `NLTdata'; Pattern = "F", [MULTI_RECORD] --
          ...
          attribute.
     [>>] Macro = `NLTmake';
          Pattern = "(GAMMA=0,IGAMMA=1,LOG=2,ILOG=3)FFI", [MULT_RECORD] --
          ...
          with a transform type instruction that is one of `NLTmake_GAMMA',
          `NLTmake_IGAMMA', `NLTmake_LOG' or `NLTmake_ILOG', followed by
          two real-valued parameters P1 and P2, and finally an integer number
          of points N in any lookup-based approximation of the operator.
  ...
  */
  public: // Member functions
    KDU_EXPORT nlt_params();
    KDU_EXPORT virtual ~nlt_params();
  protected:
    KDU_EXPORT virtual kdu_params *new_object();
    virtual void finalize(bool after_reading=false);
    virtual void copy_with_xforms(kdu_params *source, int skip_components,
  ...
  public:
    KDU_EXPORT static void
      make_gamma_params(float p1, float p2, float gamma[]);
      /* [SYNOPSIS]
           Converts the p1 and p2 parameters of an `NLTmake' record into the
           E, S, T, A and B parameters (in that order) of the `NLTgamma'
           attribute, recording them in the `gamma' array, that is expected
           to have at least 5 parameters.
           [//]
           While this function is intended primarily for internal purposes,
           it can be used to construct the 5 parameters of an `NLTgamma'
           attribute from the two fundamental parameters p1 and p2,
           generating the others by enforcing continuity constraints.  To
           assist with such usages, we note that:
           [>>] `p1' is the reciprocal exponent 1/E defined in connection with
                the `NLTgamma' attribute, which is usually known as the "gamma"
                values; common gamma values are 2.4 and 2.2, for example.
           [>>] `p2' is the offset parameter `B' defined in connection with
                the `NLTgamma' attribute.  This is sometimes known as the
                "beta" values, which is generally close to zero.  The sRGB
                gamma function, for example, has gamma=2.4 and beta=0.055.
      */
    KDU_EXPORT static void
      apply_fwd_gamma(float lut[], int num_entries, float gamma[]);
      /* [SYNOPSIS]
           This function can be used to apply the gamma function associated
           with an `NLTgamma' attribute to an array of non-negative input
           values that are assumed to be normalized, so that 1.0 corresponds
           to the nominal maximum intensity, while 0.0 corresponds to the
           absence of any light.  Normally, the function would be used to
           construct a lookup table, which is why the array of values is
           named `lut'.  The `gamma' array should have at least 5 entries,
           since it is used to supply the E, S, T, A and B parameters of
           the `NLTgamma' attribute (in that order).
           [//]
           The values found in `lut' on entry are transformed according to
           the forward gamma operator, that maps original image samples z to
           the values y that are subject to linear transformation and coding
           during compression.  For decompression, the `apply_rev_gamma'
           function should be used.
      */
    KDU_EXPORT static void
      apply_rev_gamma(float lut[], int num_entries, float gamma[]);
      /* [SYNOPSIS]
           See `apply_fwd_gamma' for an explanation.  This function transforms
           values found on entry in `lut' according to the reverse gamma
           operator, mapping the sample values y that are recovered from
           decoding and inverse linear transform operators to output sample
           values z, which is the appropriate transformation for a
           decompressor.
      */
  private: // Internal helper functions
    static void apply_fwd_log(float lut[], int num_entries, float Bval);
    static void apply_rev_log(float lut[], int num_entries, float Bval);
      /* These two functions are similar to `apply_fwd_gamma' and
         `apply_rev_gamma', but they replace the gamma function with a log-like
          function.  The `apply_fwd_log' function performs a log-like
          transformation from each entry z in the `lut' to y, where
          y = A*(z/B) if |z|/B <= 1, else y = sign(z)*A*(1 + log(|z|/B)).
          Here, A = 1 / (1-log(B)) and B is the Bval argument.
          The `apply_rev_log' function performs the reverse mapping, from
          y to z. */
  };
...
#define Kkernels "Kkernels"// One record x (I1X1=0,R1X1=1,I2X2=2,R2X2=3,I5X3=4)
...
#define Kcoeffs "Kcoeffs" // Multiple records x "F"
  ...
  // Values for the Kkernels attribute
#define Kkernels_I1X1 ((int) 0)
#define Kkernels_R1X1 ((int) 1)
#define Kkernels_I2X2 ((int) 2)
#define Kkernels_R2X2 ((int) 3)
#define Kkernels_I5X3 ((int) 4)
...
class atk_params: public kdu_params {
  /* [BIND: reference]
     ...
     strings, consult the comments appearing with `kdu_params::parse_string'.
     [>>] Macro = `Kkernels';
          Pattern = "(I1X1=0,R1X1=1,I2X2=2,R2X2=3,I5X3=4)" --
          ...
          but is currently limited to the following options: `Kkernels_R1X1'
          and `Kkernels_I1X1' are reversible and irreversible versions of
          the "lazy wavelet", meaning no transform at all; `Kkernels_I2X2'
          means the irreversible Haar transform; `Kkernels_R2X2' means the
          reversible Haar transform; and `Kkernels_I5X3' means the
          irreversible LeGall 5/3 transform, which is the irreversible
          version of the default Part-1 reversible 5/3 transform.  The
          `Kkernels' attribute itself is not recorded in the codestream,
          but is used to configure `Kreversible', `Ksymmetric', `Kextension',
          `Ksteps' and `Kcoeffs' attributes associated with the same parameter
          instance (the ":I" suffix, as in "Kkernels:I2=I5X3").  The lazy
          wavelets (R1X1 and I1X1) pass image samples directly through to
          the subbands (after de-interleaving) to be either coded directly
          (reversible) or quantized and then coded (irreversible).  These
          can be useful for experimental applications, where a transform of
          interest is performed by a separate application and the interleaved
          subbands are supplied directly to Kakadu for compression, inverting
          the process during decompression.
  ...
  */
  public: // Member Functions
    KDU_EXPORT atk_params();
    KDU_EXPORT virtual ~atk_params();
  protected: // Member functions
    ...
    virtual void finalize(bool after_reading=false);
      /* Call this function through the public "kdu_params::finalize'
         function. */
    virtual void
      copy_with_xforms(kdu_params *source, int skip_components,
                       int discard_levels, bool transpose, bool vflip,
                       bool hflip);
    virtual int
      write_marker_segment(kdu_output *out, kdu_params *last_marked,
                           int tpart_idx);
    virtual bool
      check_marker_segment(kdu_uint16 code, int num_bytes,
                           kdu_byte bytes[], int &c_idx);
...
#define Cvis          "Cvis" // One record x "F"
#define Creslengths   "Creslengths" // Multiple records x "I"
#define Cagglengths   "Cagglengths" // Multiple records x "I"
...
#define Cmodes_BYPASS_E1  ((int) 64)
#define Cmodes_BYPASS_E2  ((int) 128)
...
class cod_params: public kdu_params {
  /* [BIND: reference]
     ...
     [>>] Macro = `Creversible'; Pattern = "B" --
          ...
          `Kreversible' attribute.  Also, some profiles (reversible IMF and
          reversible BROADCAST profiles in particular) demand reversibility,
          which affects the default value for this attribute.
     ...
     [>>] Macro = `Catk'; Pattern = "I" --
          ...
          can also be tile-specific.  To construct an arbitrary transform
          kernel, you can either explicitly provide the transform attributes
          via `Kreversible', `Ksymmetric', `Kextension', `Ksteps' and
          `Kcoeffs', or you can use the convenient `Kkernels' meta-attribute
          to build the parameters for pre-defined transforms that might
          exist; for example, "Kkernels:I3=R2X2" configures the reversible
     ...
     [>>] Macro = `Cblk'; Pattern = "II" --
          Nominal code-block dimensions (must be powers of 2, no less than 4
          and no greater than 1024, whose product may not exceed 4096).
          Actual dimensions are subject to precinct, tile and image
          dimensions. Vertical coordinates appear first. Default block
          dimensions are {64,64}.
     [>>] Macro = `Cmodes'; Pattern =
          "[BYPASS=1|RESET=2|RESTART=4|CAUSAL=8|ERTERM=16|SEGMARK=32|BYPASS_E1=64|BYPASS_E2=128]" --
          ...
          `Cmodes_RESTART', `Cmodes_CAUSAL', `Cmodes_ERTERM',
          `Cmodes_SEGMARK' `Cmodes_BYPASS_E1' and `Cmodes_BYPASS_E2' to set
          and test for specific modes.  Note that BYPASS_E1 and BYPASS_E2
          modes are specific to Part-2 codestreams that conform to
          IS15444-2/AMD4.  These modes are meaningless unless combined with
          `Cmodes_BYPASS', in which case the arithmetic coder is bypassed
          earlier than it otherwise would be in the MR and SP coding passes,
          by 2*`Cmodes_BYPASS_E2' + `Cmodes_BYPASS_E1' bit-planes.
     ...
     [>>] Macro = `Cvis'; Pattern = "F" --
          Activates a human visual contrast masking model that can improve
          visual quality at a given bit-rate, so long as the image is large
          and contains sufficiently diverse content.  This option should
          be more effective with smaller code-block sizes (e.g., Cblk={32,32}),
          but can work quite well even with the default block size of
          64x64.  The single real-valued parameter is interpreted as a
          ...
          smallest subband signal that can be detected; this is expressed
          relative to the unit nominal range convention, in which 0.5 is the
          maximum amplitude one could possibly get in any subband, in response
          to any sinusoidal input stimulus.  At or below this detection
          threshold, the strength of neighbouring subband samples is considered
          to have no impact on the visibility of quantization artifacts.  Above
          the visibility floor, their masking impact is considered to grow as
          the square root of their amplitude.  Another way to understand the
          visibility floor is that its reciprocal is the maximum amount by
          which the distortion contribution from any sample can be magnified
          via the contrast masking mechanism.  For example, a value of 0.01
          yields squared error distortion magnification factors in the range
          2 to 100 (i.e., a range of 50:1), which is equivalent to adjusting
          `Cband_weights' values through a range of roughly 7:1, since those
          weights are squared before being applied to squared error
          distortion contributions.  This option is somewhat experimental
          at the current time.
     [>>] Maro = `Creslengths'; Pattern = "I",
          ...
          Note that each quality layer that is affected by the `Creslengths'
          attribute should be governed by an explicit target length or target
          distortion-length slope in calls to `kdu_codestream::flush' (or
          any of the related functions) -- implicit target lengths
          (corresponding to zero-valued entries in the array of lengths
          passed to `kdu_codestream::flush') may produce unexpected outcomes
          when used in conjunction with the `Creslengths' attribute.  Note
          also that `Creslengths' constraints will not currently be applied
          correctly if incremental codestream flushing is used -- this can
          be arranged if a significant need arises for the combined
  ...
  */
  public: // Member Functions
    KDU_EXPORT cod_params();
    KDU_EXPORT virtual ~cod_params();
  protected: // Member functions
    KDU_EXPORT virtual kdu_params *new_object();
    virtual void finalize(bool after_reading=false);
      /* Call this function through the public `kdu_params::finalize'
...
class ads_params: public kdu_params {
  ...
  */
  public: // Member Functions
    KDU_EXPORT ads_params();
    KDU_EXPORT virtual ~ads_params();
  protected: // Member functions
    KDU_EXPORT virtual kdu_params *new_object();
    virtual void finalize(bool after_reading=false);
      /* Call this function through the public "kdu_params::finalize'
...
class dfs_params: public kdu_params {
  ...
  */
  public: // Member Functions
    KDU_EXPORT dfs_params();
    KDU_EXPORT virtual ~dfs_params();
  protected: // Member functions
    KDU_EXPORT virtual kdu_params *new_object();
    virtual void finalize(bool after_reading=false);
      /* Call this function through the public "kdu_params::finalize'
...
#define Qfix16 "Qfix16" // One record x "(FREE=0,LIMIT=1)"
#define Qweights "Qweights" // One record x "B"
  
  // Values for the Qfix16 attribute
#define Qfix16_FREE          ((int) 0)
#define Qfix16_LIMIT         ((int) 1)
...
class qcd_params: public kdu_params {
  /* [BIND: reference]
     ...
     [>>] Macro = `Qfix16'; Pattern = "(FREE=0,LIMIT=1)" --
          If this attribute is present, the quantization step sizes that are
          otherwise configured in `Qabs_steps' (either directly or with the
          aid of `Qstep' and/or `Qweights') are optimized for 16-bit
          fixed-point processing.  Step sizes are adjusted (mostly downwards)
          so that the quantization and dequantization operators encountered
          in fixed-point processing should involve scaling only by powers of 2.
          If the value of the attribute is `Qfix16_LIMIT', the quantization
          step sizes are additionally adjusted so as to prevent any subband
          from receiving more than 15 magnitude bits (including guard bits),
          ignoring the impact of any max-shift based ROI adjustments
          (see `Rshift').  If the value is `Qfix16_FREE', this additional
          adjustment is not made, allowing some (typically low frequency)
          subbands to receive very fine quantization, perhaps at the expense
          of a more expensive block coding opertion.  The first set of
          adjustments, leaving power of 2 quantization operators, avoids
          the introduction of numerical rounding errors in 16-bit fixed-point
          implementations.  While step sizes generally need to be adjusted,
          there is no strong reason to believe that this should adversely
          affect the performance of irreversible compression when EBCOT's
          PCRD algorithm is used for rate control (the default approach).
          At very high bit-rates, the performance of irreversible compression
          is usually improved by specifying the `Qfix16' attribute.  Also,
          the difference between fixed-point and floating-point decoded
          versions of the same code-stream is reduced by adopting
          `Qfix16'-modified quantization parameters.  This attribute has
          no effect if processing is reversible (`Creversible' is true) or if
          `Qimplicit' is true.
     ...
     [>>] Macro = `Qweights'; Pattern = "B" --
          Used only when subband quantization steps sizes are derived using
          `Qstep' and `Qderived' is false (no).  If the value of this boolean
          parameter attribute is true (yes), the quantization step sizes that
          would otherwise be derived are scaled by the reciprocal of any
          weighting factors supplied via `Cband_weights', `Cweight' and/or
          `Clev_weights'.  The value of this option is that it provides a
          simple way to arrange for the number of discarded coding passes from
          each code-block to be similar amongst all image subbands of all image
          components.  Otherwise, when visual weights are applied, it is
          common to wind up discarding many more coding passes from the
          high frequency subbands than the low frequency subbands, which
          may mean that the encoder does more work than it needs to.
  */
  public: // Member Functions
    KDU_EXPORT qcd_params();
    KDU_EXPORT virtual ~qcd_params();
  protected: // Member functions
    KDU_EXPORT virtual kdu_params *new_object();
    virtual void finalize(bool after_reading=false);
      /* Call this function through the public "kdu_params::finalize'
...
class rgn_params: public kdu_params {
  ...
  */
  public: // Member Functions.
    KDU_EXPORT rgn_params();
    KDU_EXPORT virtual ~rgn_params();
  protected: // Member functions
    virtual void finalize(bool after_reading=false);
...
class poc_params: public kdu_params {
  ...
  */
  public: // Member Functions
    KDU_EXPORT poc_params();
    KDU_EXPORT virtual ~poc_params();
  protected: // Member functions
    KDU_EXPORT virtual kdu_params *new_object();
    virtual void copy_with_xforms(kdu_params *source, int skip_components,
                   int discard_levels, bool transpose, bool vflip, bool hflip);
...
class crg_params: public kdu_params {
  ...
  */
  public: // Member Functions
    KDU_EXPORT crg_params();
    KDU_EXPORT virtual ~crg_params();
  protected: // Member functions
    KDU_EXPORT virtual kdu_params *new_object();
    virtual void copy_with_xforms(kdu_params *source, int skip_components,
                   int discard_levels, bool transpose, bool vflip, bool hflip);
...
#define ORGplt_parts "ORGplt_parts" // One record x "[R=1|L=2|C=4]"
#define ORGtlm_style "ORGtlm_style" // One record x "(implied=0,byte=1,short=2)
                                    //               (short=2,long=4)"
#define ORGtpart_interrupts "ORGtpart_interrupts" // One record x "I"

  // Values for the "ORGtparts" attribute
#define ORGtparts_R           ((int) 1)
#define ORGtparts_L           ((int) 2)
#define ORGtparts_C           ((int) 4)
 // Values for the "ORGplt_parts" attribute
#define ORGplt_parts_R        ((int) 1)
#define ORGplt_parts_L        ((int) 2)
#define ORGplt_parts_C        ((int) 4)
 // Values for the "ORGtlm_style" attribute
#define ORGtlm_style_implied  ((int) 0)
#define ORGtlm_style_byte     ((int) 1)
#define ORGtlm_style_short    ((int) 2)
#define ORGtlm_style_long     ((int) 4)
...
class org_params: public kdu_params {
  /* [BIND: reference]
     ...
     [>>] Macro = `ORGplt_parts'; Pattern = "[R=1|L=2|C=4]" --
          ...
          more of the flags, `ORGplt_parts_R', `ORGplt_parts_L' and
          `ORGplt_parts_C'.  If the `R' flag is supplied, new PLT marker
          segments will be started as required to ensure that no
          PLT marker segment contains length information for packets
          that belong to different resolution levels.  Similarly, the
          `L' and `C' flags cause packet length information to be
          partitioned into distinct PLT marker segments at quality
          layer and codestream image component boundaries, respectively.
          By default, packet length information is packed into the
          smallest possible collection of PLT marker segments.
     [>>] Macro = `ORGgen_tlm'; Pattern = "I" --
          ...
          data is written, does not support repositioning via
          `kdu_compressed_target::start_rewrite'.
     [>>] Macro = `ORGtlm_style';
          ...
          at some point during the compression process.  It is probably
          better to use the macros `ORGtlm_style_implied', `ORGtlm_style_byte'
          or `ORGtlm_style_short' for the first parameter and
          `ORGtlm_style_short' or `ORGtlm_style_long' for the second parameter,
          rather than explicit integers.  The first field
     ...
     [>>] Macro = `ORGtpart_interrupts';
          ...
          the bound supplied via `ORGtpart_interrupts'.  By default, if this
          attribute is not explicitly provided, 200 flush-induced interruptions
          will be allowed, which is large enough to accommodate almost all
          incremental flushing situations and leaves enough remaining
          ...
          using `ORGgen_tlm' to generate tile-part pointer information,
  ...
  */
  public: // Member Functions
    KDU_EXPORT org_params();
    KDU_EXPORT virtual ~org_params();
  protected: // Member functions
    KDU_EXPORT virtual kdu_params *new_object();
    virtual void copy_with_xforms(kdu_params *source, int skip_components,
                   int discard_levels, bool transpose, bool vflip, bool hflip);
