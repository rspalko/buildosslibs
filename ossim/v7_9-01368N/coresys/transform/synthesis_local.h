...
class kd_synthesis : public kdu_pull_ifc_base {
  ...
  private: // Function pointers to allow SIMD acceleration
    void reset_func_ptrs() { interleave_16_func = NULL; }
    union {
      kd_interleave_16_func interleave_16_func; // for fixed-point and shorts
      kd_interleave_32_func interleave_32_func; // for floats and ints
    };
  ...
  /* Notes:
     ...
     aligned at a multiple of 2*`KDU_ALIGN_SAMPLE16' bytes for 16-bit samples,
     or 4*`KDU_ALIGN_SAMPLE32' bytes for 32-bit/sample representations.  This
     alignment is beyond our control, being managed by the
     `kdu_sample_allocator' object; however, we want to ensure that all
     vertical and horizontal processing can be performed on aligned
     boundaries within the buffers.
     ...
     `x_min_pull' - `x_min_buf' is a multiple of the relevant alignment
     boundary (2*`KDU_ALIGN_SAMPLES16' for 16-bit samples and
     4*`KDU_ALIGN_SAMPLES32' bytes for 32-bit samples), so that data
