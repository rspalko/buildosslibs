...
{
  kdu_node node = resolution.access_node();
  kd_synthesis *obj = new kd_synthesis;
  state = obj;
  obj->init(node,allocator,use_shorts,normalization,0,env,env_queue,0);
}
                             ...
                             kdu_thread_queue *env_queue, int flags)
{
  kd_synthesis *obj = new kd_synthesis;
  state = obj;
  obj->init(node,allocator,use_shorts,normalization,pull_offset,
            env,env_queue,flags);
}
                     ...
                     kdu_thread_queue *env_queue, int flags)
{
  ...
  if (hor_xform_exists)
    { 
      ...
      int alignment = (use_shorts)?KDU_ALIGN_SAMPLES16:KDU_ALIGN_SAMPLES32;
  ...
  if (vert_xform_exists)
    {
      ...
      for (s=0; s < num_vert_steps; s++)
        { 
          ...
          step->vert_add_shorts_first = step->hor_add_shorts_first = false;
          step->reset_func_ptrs();
          step->extend = 0; // Not used in the vertical direction
#ifdef KDU_SIMD_OPTIMIZATIONS
          if (use_shorts)
            { KD_SET_SIMD_VLIFT_16_FUNC(step->vlift_16_func,
                                        step->vert_add_shorts_first,
                                        step,true); }
          else
            { KD_SET_SIMD_VLIFT_32_FUNC(step->vlift_32_func,step,true); }
...
#ifdef KDU_SIMD_OPTIMIZATIONS
          if (use_shorts)
            { KD_SET_SIMD_HLIFT_16_FUNC(step->hlift_16_func,
                                        step->hor_add_shorts_first,
                                        step,true); }
          else
            { KD_SET_SIMD_HLIFT_32_FUNC(step->hlift_32_func,step,true); }
#endif // KDU_SIMD_OPTIMIZATIONS
        }
    }
  ...
  if (!reversible)
    {
      ...
      if ((num_vert_steps > 0) && !unit_height)
        { 
          ...
          bibo_in = bibo_prev * vert_bibo_gains[0];
          for (n=0; n < num_vert_steps; n++, bibo_in=bibo_out)
            { 
              bibo_out = bibo_prev * vert_bibo_gains[n+1];
              if (bibo_out > bibo_max)
                bibo_max = bibo_out; // Fit representation to lifting outputs
              if (use_shorts && vert_steps[n].vert_add_shorts_first)
      ...
      if ((num_hor_steps > 0) && !unit_width)
        { 
          ...
          if ((num_vert_steps > 0) &&
            ...
            bibo_prev = vert_bibo_gains[num_vert_steps-1]; // Covers both of
                                                    // the vertical subbands.
          bibo_prev *= normalization; // Account for fact that gains reported
                   // by `kdu_node::get_bibo_gains' assume `normalization'=1

          bibo_in = bibo_prev * hor_bibo_gains[0];
          for (n=0; n < num_hor_steps; n++, bibo_in=bibo_out)
            { 
              bibo_out = bibo_prev * hor_bibo_gains[n+1];
              if (bibo_out > bibo_max)
                bibo_max = bibo_out; // Fit representation to lifting outputs
              if (use_shorts && hor_steps[n].hor_add_shorts_first)
                bibo_in *= 2.0; // Avoid overflow in 16-bit pre-accumulation
              if (bibo_in > bibo_max)
                bibo_max = bibo_in;
            }
...
#ifdef KDU_SIMD_OPTIMIZATIONS
  // The following macros may set the relevant `interleave_...' function ptr
  int ilv_pairs = (x_max_out + 2 - x_min_pull) >> 1;
  if (use_shorts)
    { KD_SET_SIMD_INTERLEAVE_16_FUNC(interleave_16_func,ilv_pairs,
                                     normalizing_upshift); }
  else
    { KD_SET_SIMD_INTERLEAVE_32_FUNC(interleave_32_func,ilv_pairs); }
#endif // KDU_SIMD_OPTIMIZATIONS
  ...
  if (!initialized)
    { // Finish creating all the buffers.
      ...
      for (scan=free_lines; scan != NULL; scan=scan->next)
        scan->create();
      for (int n=0; n < 4; n++)
        if (subbands[n].exists() && !subbands[n].start(env))
          fully_started = false;
      initialized = true;
    }
  else
    { // Visit LL subband last if already initialized
      for (int n=3; n >= 0; n--)
        if (subbands[n].exists() && !subbands[n].start(env))
          fully_started = false;
    }
                  ...
                  for (c=0; c < 2; c++)
                    {
                      ...
                      if (use_shorts)
                        { 
                          kdu_int16 **bf=(kdu_int16 **) vert_source_bufs;
                          for (k=0; k < (int) step->support_length; k++)
                            bf[k]=&(vert_source_vlines[k]
                                    ->cosets[c].get_buf16()->ival);
                          step->perform_lifting_step(bf,
                                    &(vline_in->cosets[c].get_buf16()->ival),
                                    &(vline_out->cosets[c].get_buf16()->ival),
                                    width,off,true);
                        }
                      else
                        { 
                          kdu_int32 **bf=(kdu_int32 **) vert_source_bufs;
                          for (k=0; k < (int) step->support_length; k++)
                            bf[k]=&(vert_source_vlines[k]
                                    ->cosets[c].get_buf32()->ival);
                          step->perform_lifting_step(bf,
                                    &(vline_in->cosets[c].get_buf32()->ival),
                                    &(vline_out->cosets[c].get_buf32()->ival),
                                    width,off,true);
                        }
                    }
                }
    ...
    { // Interleave even and odd cosets
      if (!use_shorts)
        { // Working with 32-bit data
          ...
          assert(normalizing_upshift == 0);
#ifdef KDU_SIMD_OPTIMIZATIONS
          if (interleave_32_func != NULL)
            interleave_32_func(&(sp1->ival),&(sp2->ival),&(dp->ival),k);
          else
#endif // KDU_SIMD_OPTIMIZATIONS
            for (; k--; sp1++, sp2++, dp+=2)
              { dp[0] = *sp1; dp[1] = *sp2; }
...
#ifdef KDU_SIMD_OPTIMIZATIONS
          if (interleave_16_func != NULL)
            interleave_16_func(&(sp1->ival),&(sp2->ival),&(dp->ival),k,
                               normalizing_upshift);
          else
#endif // KDU_SIMD_OPTIMIZATIONS
          if (normalizing_upshift == 0)
  ...
  for (s=num_hor_steps-1; s >= 0; s--)
    {
      ...
      if (use_shorts)
        { // Processing 16-bit samples
          ...
          sp += step->support_min;
#ifdef KDU_SIMD_OPTIMIZATIONS
          if (step->hlift_16_func != NULL)
            step->hlift_16_func(&(sp[-x_off].ival),&(dp[-x_off].ival),
                                width+x_off,step,true);
          else
#endif // KDU_SIMD_OPTIMIZATIONS
        ...
        { // Processing 32-bit samples
          ...
          sp += step->support_min;
#ifdef KDU_SIMD_OPTIMIZATIONS
          if (step->hlift_32_func != NULL)
            step->hlift_32_func(&(sp[-x_off].ival),&(dp[-x_off].ival),
                                width+x_off,step,true);
          else
#endif // KDU_SIMD_OPTIMIZATIONS
