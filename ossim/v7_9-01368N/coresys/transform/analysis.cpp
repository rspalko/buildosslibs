                           ...
                           kdu_thread_queue *env_queue)
{
  ...
  kd_analysis *obj = new kd_analysis;
  state = obj;
  obj->init(node,allocator,use_shorts,normalization,roi,env,env_queue,0);
}
                    ...
                    int flags)
{
      ...
      for (s=0; s < num_vert_steps; s++)
        { 
          ...
          if (use_shorts)
            ...
            { KD_SET_SIMD_VLIFT_32_FUNC(step->vlift_32_func,step,false); }
      ...
      for (s=0; s < num_hor_steps; s++)
        { 
          ...
          if (use_shorts)
            ...
            { KD_SET_SIMD_HLIFT_32_FUNC(step->hlift_32_func,step,false); }
#endif // KDU_SIMD_OPTIMIZATIONS
        }
    }
  ...
  int ilv_pairs = (coset_width[0] + coset_width[1] + 1) >> 1;
  if (use_shorts)
    { KD_SET_SIMD_DEINTERLEAVE_16_FUNC(deinterleave_16_func,ilv_pairs,
                                       normalizing_downshift); }
  else
    { KD_SET_SIMD_DEINTERLEAVE_32_FUNC(deinterleave_32_func,ilv_pairs); }
#endif // KDU_SIMD_OPTIMIZATIONS
...
void
  kd_analysis::start(kdu_thread_env *env)
{
  if (!initialized)
    ...
    { // De-interleave into low- and high-pass cosets
      if (!use_shorts)
        { // Working with 32-bit data
          ...
          assert(normalizing_downshift == 0);
#ifdef KDU_SIMD_OPTIMIZATIONS
          if (deinterleave_32_func != NULL)
            deinterleave_32_func(&(sp->ival),&(dp1->ival),&(dp2->ival),k);
          else
#endif // KDU_SIMD_OPTIMIZATIONS          
            for (; k--; sp+=2, dp1++, dp2++)
              { *dp1 = sp[0]; *dp2 = sp[1]; }
...
#ifdef KDU_SIMD_OPTIMIZATIONS
          if (deinterleave_16_func != NULL)
            deinterleave_16_func(&(sp->ival),&(dp1->ival),&(dp2->ival),k,
                               normalizing_downshift);
          else
#endif // KDU_SIMD_OPTIMIZATIONS
          if (normalizing_downshift == 0)
      ...
      if (use_shorts)
        { // Processing 16-bit samples
          ...
          if (step->hlift_16_func != NULL)
            step->hlift_16_func(&(sp->ival),&(dp->ival),width,step,false);
          else
#endif // KDU_SIMD_OPTIMIZATIONS
        ...
        { // Processing 32-bit samples
          ...
          if (step->hlift_32_func != NULL)
            step->hlift_32_func(&(sp->ival),&(dp->ival),width,step,false);
          else
#endif // KDU_SIMD_OPTIMIZATIONS
          if ((step->support_length==2) &&
