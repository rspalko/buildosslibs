...
// be private to the "kdu_compress" demo app.
...
class kdc_flow_control {
  ...
  public: // Member functions
    kdc_flow_control(kdc_file_binding *files, kdu_codestream codestream,
                     int x_tnum, bool allow_shorts, bool rgb_420_convert,
      ...
      /* Constructs a flow control object for one tile.  The caller might
         ...
         tile-component processing hierarchy.
            If `rgb_420_convert' is true, the first three image components are
         expected to be irreversibly compressed, with the first component
         having exactly twice as many rows and columns as the second and third.
         The source files, in this case, are expected to provide three initial
         components whose dimensions are identical to those of the first
         component.  These lines are read in and converted to YCbCr in-line,
         after which the chrominance components are sub-sampled by 2 in each
         direction by simple box-car averaging, as is common in coders such as
         JPEG.  The sub-sampled chrominance components are generated by the
         `advance_components' function so that `access_compressor_line' sees
         the component data that will actually be passed to the compression
         engine.  A suitable error message will be generated if the expected
  ...
  private: // Helper functions
    void allocate_conversion_src_lines();
      /* If the constructor configured in-line conversion to 4:2:0, this
         function is called both from the constructor and `advance_tile',
         immediately after creation of the analysis `engine'.  The function
         allocates the `conversion_src' line buffers found in each of the
         first three components and checks the tile dimensions, number of
         components and data types for compatibility with 4:2:0 conversion,
         generating an error through `kdu_error' if any inconsistency is
         encountered. */
  private: // Definitions
    struct kdc_component_flow_control {
      public: // Data
        ...
        int initial_lines;
        int remaining_lines;
        kdu_line_buf *line; // Line for actual component to be compressed
      public: // Members used to manage in-line conversion to 4:2:0
        int next_conversion_src; // -1 if not converting to 4:2:0; else 0 or 1
        kdu_line_buf conversion_src[2]; // Separately allocated line buffers
      };
  private: // Data
    ...
    bool convert_rgb_to_420;
    kdu_sample_allocator conversion_allocator; // For `conversion_src' lines
  };
