...
extern KDU_AUX_EXPORT void
  mj2_add_box_descriptions(jp2_box_textualizer &textualizer);
  /* [SYNOPSIS]
       Use this function to augment the `textualizer' with any textualization
       functions that are offered for Motion JPEG2000 box-types.  See
  ...
  */
  ...
  public: // Functions specific to Motion JPEG2000 video tracks
    ...
    KDU_AUX_EXPORT kdu_int16
      ...
      */
    KDU_AUX_EXPORT kdu_int16 get_graphics_mode() const;
      /* [SYNOPSIS]
    ...
    KDU_AUX_EXPORT void
      get_geometry(double &presentation_width,
                   ...
                   double matrix[], bool for_movie=true) const;
      /* [SYNOPSIS]
    ...
    KDU_AUX_EXPORT jp2_colour access_colour();
      ...
      */
    KDU_AUX_EXPORT int get_stream_idx(int field_idx) const;
      /* [SYNOPSIS]
    ...
    KDU_AUX_EXPORT virtual int
      ...
      /* [SYNOPSIS]
           ...
           [>>] Prior to Kakadu version 7.0, decompression processing of
                multiple concurrent codestreams drawn from the same data
                source (e.g., a file) was generally thread safe because all
                codestream processing threads ultimately had to take out a
                single global lock on the `KD_THREADLOCK_GENERAL' mutex
                managed via `kdu_thread_env'.
           [>>] From Kakadu version 7.0, however, the mutex identified by
                `KD_THREADLOCK_GENERAL' is no longer global to a
                `kdu_thread_env' environment, but specific to the combination
                of each `kdu_codestream' and `kdu_thread_env' combination.
                This is important in maximizing multi-threaded throughput on
                modern machines with many CPU cores, but it means that you
                may encounter race conditions in poorly designed code that
                previously appeared to be working fine.
           [>>] In view of these changes, the present function no longer
                provides an optional `kdu_thread_env' argument, since that was
                previously provided only to allow access to the previously
                global `KD_THREADLOCK_GENERAL' mutex for the purpose of making
                the current function thread-safe.  Instead, for internal
                thread safety, the function relies upon the
                `jp2_family_src::acquire_lock' and
                `jp2_family_src::release_lock' functions.
           [>>] For these reasons, if you do intend to work with multiple
                codestreams concurrently, you should make sure that the
                `jp2_family_src' that is passed to `mj2_source::open'
  ...
  public: // Overrides for functions declared in `kdu_compressed_video_source'
    ...
    KDU_AUX_EXPORT virtual kdu_long get_frame_instant();
      ...
      */
    KDU_AUX_EXPORT virtual kdu_long get_frame_instant(int frame_idx);
      /* [SYNOPSIS]
           Same as the first form of `get_frame_instant', but returns
           the frame starting time for a specific frame, identified by
           `frame_idx'.  The result should be the same as that obtained
           by first seeking to `frame_idx' and then invoking the first
           form of the function, but saves the need to change the current
           frame context by invoking `seek_to_frame'.
           [//]
           If the indicated frame does not exist, the function returns 0.
      */
    ...
    KDU_AUX_EXPORT virtual kdu_long get_frame_period();
      /* [SYNOPSIS]
           ...
           period.
      */
    KDU_AUX_EXPORT virtual kdu_long get_frame_period(int frame_idx);
      /* [SYNOPSIS]
           Same as the first form of `get_frame_period', but returns
           the frame period of a specific frame, identified by
           `frame_idx'.  The result should be the same as that obtained
           by first seeking to `frame_idx' and then invoking the first
           form of the function, but saves the need to change the current
           frame context by invoking `seek_to_frame'.
  ...
  // --------------------------------------------------------------------------
  private: // Data
    friend class kd_supp_local::mj_video_track;
    kd_supp_local::mj_video_track *state;
  };
  ...
  */
  ...
  public: // Lifecycle functions
    mj2_source() { state = NULL; }
      ...
      */
    mj2_source(kd_supp_local::mj_movie *state) { this->state = state; }
      /* [SYNOPSIS]
...
class mj2_video_target : public kdu_compressed_video_target {
  /* [BIND: reference]
     [SYNOPSIS]
       ...
       `kdu_codestream::create'.
       [//]
       From KDU7.2, this interface implements the `start_rewrite' and
       `end_rewrite' methods, allowing you to directly generate codestreams
       with TLM marker segments, for example.
  */
  // --------------------------------------------------------------------------
  public: // Functions specific to Motion JPEG2000 video tracks
  ...
  public: // Overrides for functions declared in `kdu_compressed_target'
    KDU_AUX_EXPORT virtual bool start_rewrite(kdu_long backtrack);
      /* [SYNOPSIS]
           Used by `kdu_codestream' to write TLM marker segments if required.
           Refer to `kdu_compressed_target::start_rewrite' for more
           information.
      */
    KDU_AUX_EXPORT virtual bool end_rewrite();
      /* [SYNOPSIS]
           See `kdu_compressed_target::end_rewrite'.
      */
  ...
  */
  ...
  public: // Lifecycle functions
    mj2_target() { state = NULL; }
      ...
      */
    mj2_target(kd_supp_local::mj_movie *state) { this->state = state; }
      /* [SYNOPSIS]
    ...
    KDU_AUX_EXPORT void destroy();
      /* [SYNOPSIS]
           Same as `close' but makes no attempt to save content.  This function
           should normally be used in place of `close' if an error was
