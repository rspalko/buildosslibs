...
#define JPB_UNKNOWN_SPACE            ((kdu_byte) 0x00)
#define JPB_SRGB_SPACE               ((kdu_byte) 0x01)
#define JPB_601_SPACE                ((kdu_byte) 0x02)
#define JPB_709_SPACE                ((kdu_byte) 0x03)
#define JPB_LUV_SPACE                ((kdu_byte) 0x04)
#define JPB_XYZ_SPACE                ((kdu_byte) 0x05)
#define JPB_BT2020_SPACE             ((kdu_byte) 0x06)
#define JPB_ST2084_SPACE             ((kdu_byte) 0x07)
  ...
  */
  ...
  public: // Functions specific to this object
    jpb_source() { state = NULL; }
      ...
      */
    jpb_source(kd_supp_local::jb_source *state) { this->state = state; }
      /* [SYNOPSIS]
    ...
    KDU_AUX_EXPORT kdu_byte get_frame_space() const;
      /* [SYNOPSIS]
           ...
           [>>] `JPB_BT2020_SPACE' -- the RGB colour space defined by ITU
                recommendation BT.2020, nominally for UHDTV, having red,
                green and blue primary chromaticities (x,y) of
                (0.708,0.292), (0.170,0.797) and (0.131,0.046), respectively,
                with a whitepoint of (0.3127,0.3290) and gamma tone mapping
                with gamma=2.22 and beta=0.099 [c.f. sRGB has gamma=2.4 and
                beta=0.055].  Note that this is a wide gammut space, whose
                primaries are monochromatic (single wavelength) light sources.
           [>>] `JPB_ST2084_SPACE' -- the RGB colour space whose primaries
                and whitepoint are identical to the `JPB_BT2020_SPACE' above,
                but with the gamma function replaced by a log-like tone curve
                more suitable for high dynamic range content.
      */
    KDU_AUX_EXPORT bool
      get_mastering_display_info(float &x0, float &y0, float &x1, float &y1,
                                 float &x2, float &y2, float &xw, float &yw,
                                 double &Lmin, double &Lmax);
       /* [SYNOPSIS]
            Returns information about the mastering display device associated
            with the currently open image.  If no image is currently open,
            the function returns information about the next image which will
            be opened by `open_image'.  If there is no mastering display
            information available for the image in question (currently open
            image or next one to be opened if none are open), the function
            returns false, but sets the argument values to reflect the default
            mastering display parameters defined in IS15444-1/AMD8, which
            have in turn been taken from ITU recommendation BT.709-5.
            [//]
            The arguments identify the chromaticities of three primaries, the
            whitepoint, and the minimum and maximum luminance of the display
            in candelas per square metre.  The documentation in IS15444-1/AMD8
            suggests that the order in which primaries should be recorded is
            green, blue and then red, but this is not something you should
            rely upon.  In fact, the default parameters supplied in the
            standard even appear in a different order (red, blue then green).
          [RETURNS]
            Returns true if the currently open frame has a `dmon' box
            (corresponds to `jpb_mastering_display_4cc'), or there is a next
            frame that would be opened by the next call to `open_image' and
            that frame has a `dmon' box.  Otherwise the function returns false.
            However, in both cases, the values returned via the function's
  ...
  public: // Functions specific to this object
    jpb_target() { state = NULL; }
      /* [SYNOPSIS]
           ...
           returns false.  Use `open' to open a new broadcast stream.
      */
    jpb_target(kd_supp_local::jb_target *state) { this->state = state; }
      /* [SYNOPSIS]
           Applications have no meaningful way to invoke this constructor
           except with a NULL argument.
      */
    ...
    KDU_AUX_EXPORT kdu_uint32 get_last_timecode() const;
      ...
      */
    KDU_AUX_EXPORT void
      set_mastering_display_info(float x0, float y0, float x1, float y1,
                                 float x2, float y2, float xw, float yw,
                                 double Lmin, double Lmax);
       /* [SYNOPSIS]
            Calls to this function are optional, allowing you to define the
            colorimetry of the mastering display, to be recorded in the
            metadata of each frame via the optional `dmon' box (identified by
            `jpb_mastering_display_4cc').  The arguments identify the
            chromaticities of three primaries, the whitepoint, and the
            minimum and maximum luminance of the display in candelas per
            square metre.  The documentation in IS15444-1/AMD8 suggests that
            the order of the primaries be green, blue then red.  
            [//]
            If this function is not called, no `dmon' box is written.  Each
            call to the function affects the contents of the `dmon' box written
            to the frame opened by the next call to `close_image'.  If you call
            this function with negative values for any of the parameters,
            or with `Lmin' or `Lmax' equal to 0.0, the writing of `dmon' boxes
            is disabled until the function is called again.
            [//]
            Note that the chromaticity parameters are quantized to the nearest
            multiple of 0.00002, while the luminance parameters are quantized
            to the nearest multiple of 0.0001.  Moreover, chromaticities are
            clipped to the range 0 to 1.0 and luminances are clipped to fit
            within 32-bit unsigned integers after division by 0.0001.
       */
    KDU_AUX_EXPORT bool close();
      /* [SYNOPSIS]
           Destroys the internal object and resets the state of the
