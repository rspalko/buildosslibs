  ...
  kdu_window::parse_context(const char *scan)
{
      ...
      if (range.context_type == KDU_JPIP_CONTEXT_JPXL)
        {
          if (*scan != 's')
            return open_range;
          scan++;
          range.remapping_ids[0] = (int)strtol(scan,&end_cp,10);
          if (end_cp == scan)
            return open_range;
          scan = end_cp;
          if (*scan != 'i')
            return scan;
          scan++;
          range.remapping_ids[1] = (int)strtol(scan,&end_cp,10);
          if (end_cp == scan)
            return open_range;
          scan = end_cp;
        }
  ...
  while (((range.from=(int)strtol(scan,&end_cp,10)) >= 0) &&
         (end_cp > scan))
    {
      scan = end_cp;
      if (*scan == '-')
...
void
  kdu_window::add_metareq(kdu_uint32 box_type, int qualifier, bool priority,
                          int byte_limit, bool recurse, kdu_long root_bin_id,
                          int max_depth)
{
  ...
  if (max_depth < 0)
    max_depth = 0;
  if (qualifier == 0)
    qualifier = KDU_MRQ_DEFAULT;
  kdu_metareq *req;
  for (req=metareq; req != NULL; req=req->next)
    if (((req->box_type == box_type) || (req->box_type == 0)) &&
        (req->root_bin_id == root_bin_id) && (req->max_depth >= max_depth) &&
        (req->priority || !priority) &&
        ((req->qualifier & qualifier) == qualifier) &&
        (req->byte_limit >= byte_limit) && (req->recurse || !recurse))
      break;
  if (req != NULL)
    return;
  req = free_metareqs;
...
/*****************************************************************************/
/*                    kdu_window_model::~kdu_window_model                    */
/*****************************************************************************/
...
int kdu_window_model::get_meta_instructions(kdu_long &bin_id, int buf[])
{
  ...
  for (ip_prev=NULL, ip=meta_head; ip != NULL; ip_prev=ip, ip=ip_next)
    { 
      ...
      if (ip == free_instructions)
        ip = ip_prev; // So `ip_prev' does not change as we go around the loop
    }
  return cnt;
}
