...
class kdws_tree_ctrl;
struct kdws_catalog_state;
struct kdws_text_attributes;
class kdws_catalog;
...
// A special flag, `KDWS_CATFLAG_ASOC_NODE' records whether the node represents
// actual ROI or numlist `jpx_metanode's -- these nodes are automatically
// removed once they have any descendants in the catalog.
#define KDWS_CATFLAG_ASOC_BASE                   2
#define KDWS_CATFLAG_TYPE_MASK                   ((kdu_uint16) 0x0003)
#define KDWS_CATFLAG_ASOC_MASK                   ((kdu_uint16) 0x001C)
#define KDWS_CATFLAG_ASOC_ENTITY                 ((kdu_uint16) 0x0004)
#define KDWS_CATFLAG_ASOC_REGION                 ((kdu_uint16) 0x0008)
#define KDWS_CATFLAG_ASOC_NODE                   ((kdu_uint16) 0x0010)

#define KDWS_CATFLAG_IS_EXPANDED                 ((kdu_uint16) 0x0020)
#define KDWS_CATFLAG_NEEDS_EXPAND                ((kdu_uint16) 0x0040)
#define KDWS_CATFLAG_NEEDS_COLLAPSE              ((kdu_uint16) 0x0080)
#define KDWS_CATFLAG_NEEDS_RELOAD                ((kdu_uint16) 0x0100)
#define KDWS_CATFLAG_DESCENDANT_NEEDS_ATTENTION  ((kdu_uint16) 0x0200)

#define KDWS_CATFLAG_PREFIX_NODE                 ((kdu_uint16) 0x0400)
#define KDWS_CATFLAG_HAS_METADATA                ((kdu_uint16) 0x0800)

#define KDWS_CATFLAG_ANONYMOUS_ENTRIES           ((kdu_uint16) 0x1000)
#define KDWS_CATFLAG_COLLAPSED_DESCENDANTS       ((kdu_uint16) 0x2000)
#define KDWS_CATFLAG_LABEL_CHANGED               ((kdu_uint16) 0x4000)

#define KDWS_CATFLAG_WANTS_EXPAND                ((kdu_uint16) 0x8000)
...
extern bool kdws_catalog_new_data_available(jpx_meta_manager manager);
  /* Returns true if `manager' has some metadata on its "touched" list that
     could be displayed within a `KDWS_catalog' object if it were
     opened. */
...
public: // Member functions
  ...
  bool needs_client_requests_if_expanded();
    /* This function determines whether JPIP client requests ought to be
       ...
       is set and any of the entries of the list into which `cur_entry'
  ...
  bool treat_as_hyperlink();
    /* Returns true if the node represents a label that has one of the
       ...
       suffix. */
  kdws_catalog_entry *get_first_entry()
    { // Find 1st element in list referenced by `cur_entry'
      kdws_catalog_entry *scan = cur_entry;
      if (scan != NULL)
...
public: // Data which relates to the actual represented metadata
  ...
  /* Notes:
   ...
   `kdws_catalog_entry::child_info' list of the entry to which `cur_entry'
   points and `cur_entry' is not permitted to point to any other element
   in the list of entries; the other entries exist only to keep track of the
   metanodes which are associated with this catalog node.  If
   `KDWS_CATFLAG_COLLAPSED_DESCENDANTS' is set,
...
protected: // MFC overrides for catching messages
  virtual BOOL PreTranslateMessage(MSG *pMsg);
    /* Currently, this is used only to catch ENTER key strokes and
       route them to `catalog->user_key_down' -- since otherwise, the
       tree control will just beep at them. */
protected: // MFC overrides for tooltips
...
struct kdws_catalog_state {
  public: // Member functions
    kdws_catalog_state() { pasted_label=NULL; paste_text_colour=RGB(0,0,0); }
    ~kdws_catalog_state() { if (pasted_label != NULL) delete[] pasted_label; }
    void clear_pasted_state()
      { 
        ...
        paste_text_colour = RGB(0,0,0);
      }
    bool have_pasted_state()
      { 
        return ((pasted_label != NULL) || pasted_link.exists() ||
  ...
  public: // Data
    ...
    jpx_metanode incomplete_selection; // Exists if `select_matching_metanode'
      ...
      // the `KDWS_CATFLAG_COLLAPSED_DESCENDANTS' or `KDWS_CATFLAG_ASOC_NODE'
...
struct kdws_text_attributes {
  public: // Member functions
    kdws_text_attributes() { created = false; }
    ~kdws_text_attributes() { clear(); }
    void clear();
    void create(LOGFONT font_attributes, COLORREF text_colour,
                COLORREF backsel_text_colour);
  public: // Data
    bool created;
    COLORREF colour;
    COLORREF backsel_colour; // Background colour used when item selected
    CFont font;
    CFont cut_font; // Font used to display items that have been "cut"
  };
...
public: // Member functions
  ...
  bool user_key_down(UINT nChar, bool shift_key_down, bool control_key_down);
    /* Called when the user presses a key within the tree control. Returns
       ...
       tree control. */
  void user_click();
    /* This function is called when the left or right mouse button is
       pressed within the tree control.  The `user_right_click' function
       may subsequently be called also, but the present function is used
       only to cancel any metadata-driven animation that may be in
       progress. */
  void user_double_click(UINT nFlags);
    /* Called when the user double-clicks the selected item.  `nFlags' is
       the set of auxiliary flags presented to `CTreeCtrl::OnLButtonDblClk'. */
...
private: // Helper functions
  ...
  void process_double_click(kdws_catalog_node *node, bool shift_key_down);
   /* Handles actual mouse double-clicks, as well as synonymous events such
      ...
      The `shift_key_down' argument is true if the Shift key was depressed
...
private: // Text attributes
  kdws_text_attributes prefix_attributes;
  kdws_text_attributes link_attributes;
  kdws_text_attributes grouping_link_attributes;
  kdws_text_attributes alternate_parent_link_attributes;
  kdws_text_attributes hyperlink_attributes; // For labels shown as hyperlinks
  kdws_text_attributes root_attributes[KDWS_CATALOG_NUM_TYPES];
  kdws_text_attributes plain_attributes[4];
    // Note: in the above array, the 4 entries correspond to the four
    // possible combinations of the `KDWS_CATFLAG_REGION_ASSOCIATED' and
    // `KDWS_CATFLAG_ENTITY_ASSOCIATED' flag bits.  `plain_attributes'
...
private: // Catalog state
  ...
  kdws_catalog_state *state; // Mirrors organization in "kdu_macshow", where
    // state variables with constructors need to go in a separate C++ object.
};
