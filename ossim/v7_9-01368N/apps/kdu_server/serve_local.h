...
#define KD_MAX_ACTIVE_CODESTREAMS 64
#define KD_MAX_WINDOW_CODESTREAMS 128
#define KD_MAX_ATTACHED_CODESTREAMS 128
#define KD_MAX_WINDOW_SAMPLES 256000000 /* Samples in active precinct bins */
#define KD_MAX_DETACHED_INTERFACE_MEM 64000000 /* 64MB threshold on the
         interchange and structure codestreams kept with detached streams. */
...
struct kd_stream {
  public: // Member functions
    ...
    void open_tile(kd_tile *tp, bool attach_to_source,
      ...
      /* If `structure_region' is non-NULL, the function opens the
         `kd_tile::structure' interface after first creating (if
  ...
  public: // Codestream interfaces
    ...
    kdu_long detached_interface_mem; // See below
  ...
  /* Notes:
     ...
     `sim_header_span'.
     [//]
     The `detached_interface_mem' member is updated each time the
     `kd_serve::detach_stream' function is called.  It keeps track of the
     total amount of memory associated with the `interchange' and `structure'
     interfaces, which could be released by invoking `deconstruct_interfaces'.
  */
...
class kd_precinct_model { // Holds the cache model for a single precinct
  ...
  private: // Data
    kdu_long state; // Two different interpretations
  };
  /* Notes:
...
struct kd_serve {
  ...
  public: // Utility functions
    ...
    void trim_resources();
      /*  Determines whether or not the `total_detached_interface_mem'
          tally exceeds the `KD_MAX_DETACHED_INTERFACE_MEM' threshold.  If so,
          detached streams are visited in a methodical order, starting from
          the tail (least recently used position) of the
          `mediumweight_unused_streams' list, invoking their
          `deconstruct_interfaces' method, which reduces the value of the
          `total_detached_interface_mem' member. */
    ...
    void detach_stream(kd_stream *stream, bool deleting=false);
      /* Causes `stream' to be detached from the `kdu_serve_target', leaving
         ...
         object's destructor, in which case `deleting' should be true.
            This function recomputes the amount of memory represented by
         the `stream's `interchange' and `structure' codestream interfaces,
         recording the value within `stream->detached_interface_mem' and
         adding it to the local `total_detached_interface_mem' tally. */
  ...
  private: // Helper functions
    void move_stream_to_list(kd_stream *stream, kd_stream * &list);
      /* This function is used to move streams between the various lists
         ...
         prepended to the list headed by `list'.  This means that all lists
         commence with the elements most recently moved to them. */
  //---------------------------------------------------------------------------
