...
#define KD_HANDLED_REQUESTS_STRING \
  JPIP_FIELD_TARGET "," \
  JPIP_FIELD_SUB_TARGET "," \
  JPIP_FIELD_TARGET_ID "," \
  JPIP_FIELD_CHANNEL_ID "," \
  JPIP_FIELD_CHANNEL_NEW "=http," \
  JPIP_FIELD_CHANNEL_NEW "=http-tcp," \
  JPIP_FIELD_CHANNEL_NEW "=http-udp," \
  JPIP_FIELD_CHANNEL_CLOSE "," \
  JPIP_FIELD_REQUEST_ID "," \
  JPIP_FIELD_FULL_SIZE "," \
  JPIP_FIELD_REGION_SIZE "," \
  JPIP_FIELD_REGION_OFFSET "," \
  JPIP_FIELD_COMPONENTS "," \
  JPIP_FIELD_CODESTREAMS "," \
  JPIP_FIELD_CONTEXTS "=jplx," \
  JPIP_FIELD_LAYERS "," \
  JPIP_FIELD_META_REQUEST "," \
  JPIP_FIELD_MAX_LENGTH "," \
  JPIP_FIELD_MAX_QUALITY "," \
  JPIP_FIELD_ALIGN "," \
  JPIP_FIELD_WAIT "," \
  JPIP_FIELD_TYPE "=jpp-stream," \
  JPIP_FIELD_CHUNK_ABANDON "," \
  JPIP_FIELD_BARRIER_ID "," \
  JPIP_FIELD_MODEL "," \
  JPIP_FIELD_NEED "," \
  JPIP_FIELD_PREFERENCES "," \
  JPIP_FIELD_UPLOAD
     ...
     address of our LAN card or with host-id/name dependent information. */
{
  ...
  kdu_int32 host_id = (int)gethostid();
...
/*****************************************************************************/
/* INLINE                read_val_range_or_wild (kdu_long)                   */
/*****************************************************************************/
...
/*****************************************************************************/
/*                kds_chunks_to_abandon::~kds_chunks_to_abandon              */
/*****************************************************************************/
...
/*****************************************************************************/
/*                     kds_chunks_to_abandon::translate                      */
/*****************************************************************************/

const char *
  kds_chunks_to_abandon::translate(const char *string, kds_chunk_gap * &list)
{ 
  list = NULL;
  kds_chunk_gap *elt;
  while (*string != '\0')
    { 
      int qid16=-1, seq_from=-1;
      if ((*string == '*') || (!read_val_or_wild(string,qid16)) ||
          (qid16 > 65535) || (*string != ':'))
        return string;
      string++; // Move over the colon
      if ((*string == '*') || !read_val_or_wild(string,seq_from))
        return string;
      int seq_to = seq_from;
      if (*string == '-')
        { // We have a range of chunk sequence numbers
          string++;
          if (!read_val_or_wild(string,seq_to))
            seq_to = -1;
        }
      if (*string == ',')
        string++;
      ...
      // First make sure we have enough chunk gaps
      if ((head == NULL) || (head == list))
        { // We need to allocate a new chunk gap and move it to the `head'
          elt = new kds_chunk_gap;
          if ((elt->next = head) == NULL)
            tail = elt;
          head = elt;
        }
      assert(head != list);
      
      // Next, strip the `head' from the internal list and move it to the tail
      // position.
      elt = head;
      if (elt != tail)
        { 
      ...
      // Finally, write the chunk gap
      if (list == NULL)
        list = elt;
      
      elt->qid16 = (kdu_uint16) qid16;
      elt->seq_from = seq_from;
      elt->seq_to = seq_to;
    }
  return NULL;
}
                                   ...
                                   bool caseless_targets)
{
  ...
  num_jpip_h_cids = num_jpip_ht_cids = num_jpip_hu_cids = 0;
...
void kd_source_thread::start()
{
  if (!thread.create(source_thread_start_proc,this))
    { 
      mutex.lock();    // Locking/unlocking the mutex forces any call to
      finished = true; // `install_channel' to see the `finished' condition
      mutex.unlock();  // and not interfere further with our state.
      closing();
    }
}
...
void kd_source_thread::closing()
{
  ...
  // Assign all auxiliary channels a maximum of `max_completion_seconds' to
  ...
  for (p=0; p < num_cids; p++)
    if (cids[p].aux_channel != NULL)
      cids[p].aux_channel->set_timeout(max_completion_seconds,false);
  ...
  for (chn=primary_channels; chn != NULL; chn=next_chn)
    {
      ...
      if ((chn->request_cid != NULL) && chn->reply_generated &&
        ...
        chn->channel->set_timeout(max_completion_seconds,false);
      else
  ...
  for (p=0; p < num_cids; p++)
    { 
      ...
      cid->init(); // It is safe to do this without locking the manager mutex
                   // here, because we are already marked as `finished'.
    }
  num_cids = 0;
  ...
  else if ((num_jpip_h_cids+num_jpip_hu_cids) == 0)
    kd_msg << "\t\tChannel transport = \"http-tcp\" ("
           << num_jpip_ht_cids << ")\n";
  else if ((num_jpip_h_cids+num_jpip_ht_cids) == 0)
    kd_msg << "\t\tChannel transport = \"http-udp\" ("
           << num_jpip_hu_cids << ")\n";
  else if ((num_jpip_ht_cids+num_jpip_hu_cids) == 0)
    kd_msg << "\t\tChannel transport = \"http\" ("
           << num_jpip_h_cids << ")\n";
  else
    kd_msg << "\t\tChannel transport = \"http\" ("
           << num_jpip_h_cids << "), \"http-tcp\" ("
           << num_jpip_ht_cids << "), \"http-udp\" ("
           << num_jpip_hu_cids << ")\n";
  ...
  if (total_client_abandoned_bytes > 0)
    kd_msg << "\n\t\t\t(" << total_unacknowledged_bytes
           << " not acknowledged, " << total_client_abandoned_bytes
           << " abandoned by client request)";
  else if (total_unacknowledged_bytes > 0)
    kd_msg << "\n\t\t\t(" << total_unacknowledged_bytes
           << " not acknowledged)";
  kd_msg << "\n";
  
  if ((num_jpip_ht_cids+num_jpip_hu_cids) > 0)
    {
  ...
  if (history != NULL)
    {
      ...
      history->num_jpip_hu_channels = num_jpip_hu_cids;
...
kd_jpip_channel_type
  kd_source_thread::translate_cnew_string(const char *string, bool no_udp)
{
      ...
      for (; *string != '\0'; string=delim)
        {
          ...
          if ((!no_udp) && ((delim-string)==8) &&
              kdcs_has_caseless_prefix(string,"http-udp"))
            { result = KD_CHANNEL_HTTP_UDP; break; }        
          if (((delim-string)==8) &&
...
const char *
  kd_source_thread::assign_new_cid(kd_jpip_channel_type channel_type,
                                   ...
                                   kds_cid *requesting_cid)
{
  ...
  assert((cid->active_window == NULL) && (cid->active_model == NULL) &&
         (cid->active_prefs == NULL) && (cid->active_dependencies == NULL));
  ...
  cid->active_dependencies = new kdu_long[max_cids];
  for (n=0; n < max_cids; n++)
    cid->active_dependencies[n] = -1; // No dependencies right now
  ...
  // Generate a unique `client_id' code
  kd_source_thread *scan;
  do {
    cid->client_id = (kdu_uint32)
      (manager->num_threads + (manager->num_threads<<16));
    cid->client_id += (kdu_uint32)(manager->total_transmitted_bytes +
                                   (manager->total_transmitted_bytes<<12));
    cid->client_id ^= (kdu_uint32) rand();
    cid->client_id ^= (kdu_uint32)(rand() << 11);
    for (scan=source->threads; scan != NULL; scan=scan->next)
      { 
        int p;
        for (p=0; p < scan->num_cids; p++)
          if (scan->cids[p].client_id == cid->client_id)
            break;
        if (p < scan->num_cids)
          break;
      }
  ...
  // Create UDP channel if necessary/possible and take fallback action where
  // required -- we can do all this without holding onto the mutex because
  // we have already reserved the CID.
  kdu_uint16 aux_udp_port = 0;
  if (channel_type == KD_CHANNEL_HTTP_UDP)
    { 
      cid->aux_channel = cserver->create_udp_channel(aux_udp_port);
      if ((cid->aux_channel == NULL) &&
          ((num_cids == 1) || (no_udp_channel_type != KD_CHANNEL_STATELESS)))
        cid->channel_type = channel_type = no_udp_channel_type;
    }
  
  // Generate the `channel_id' string from `client_id' and `channel_type'
  cid->channel_id[0] = 'J';
  cid->channel_id[1] = 'P';
  cid->channel_id[2] = 'H';
  if (channel_type == KD_CHANNEL_HTTP_TCP)
    cid->channel_id[3] = 'T';
  else if (channel_type == KD_CHANNEL_HTTP_UDP)
    cid->channel_id[3] = 'U';
  else
    cid->channel_id[3] = '_';
  write_hex_val32(cid->channel_id+4,cid->client_id);
  write_hex_val32(cid->channel_id+12,source->source_id);
  cid->channel_id[20] = '\0';

  // Make channel-type dependent adjustments
  if (channel_type == KD_CHANNEL_STATELESS)
    { // To ensure that `get_num_available_stateless_channels' works correctly
      // we have to be certain that by the time `is_stateless' becomes true,
      // `max_cids' has assumed its final value, corresponding to the maximum
      // number of stateless CID's we can simultaneously support.  For more
      // understanding, look at how `get_num_available_stateless_channels' is
      // used.
      assert(!is_stateless);
      assert(num_cids == 1);
      mutex.lock();
  ...
  else if (channel_type == KD_CHANNEL_HTTP)
    num_jpip_h_cids++;
  else if (channel_type == KD_CHANNEL_HTTP_TCP)
    { 
      num_jpip_ht_cids++;
      cid->aux_connect_pending = true;
      cid->aux_port = aux_listen_port;
    }
  else if (channel_type == KD_CHANNEL_HTTP_UDP)
    { 
      num_jpip_hu_cids++;
      cid->aux_connect_pending = true;
      cid->aux_port = aux_udp_port;
      if (cid->aux_channel == NULL)
        { // Failed to allocate auxiliary channel and no fallback available 
          close_cid(num_cids-1,0);
          return NULL;
        }
      manager->configure_auxiliary(cid->aux_channel);
    }
...
{
  bool success = false;
  channel->set_timeout(-1.0F,false); // Disable timeouts
  mutex.lock();
  
    // NB: The code below is sensitive because we are locking the source
    // thread's mutex but we are not the source thread.  If the source
    // thread happens to be waiting inside `event.wait', the network
    // service thread may attempt to wake it up via `event.unprotected_set',
    // from inside one of the `kds_jpip_channel' objects.  However, this
    // will involve implicit locking of the source thread's mutex, potentially
    // creating a deadlock if we are in here attempting to lock the same
    // `kds_jpip_channel' object's mutex.  To avoid this, we can avoid
    // calling any `kds_jpip_channel' member function from here, but that is
    // not convenient.  Alternatively, we can ensure that we only invoke
    // `kds_jpip_channel' objects on the new `channel' here and that it has
    // no way of learning about the source thread's mutex.  As it turns out,
    // that is what we do.  As a third solution, we could modify `kdu_event'
    // to add a special function that temporarily disables calls to
    // `unprotected_set' from doing anything until the mode is disabled; this
    // would work because we are going to call `event.protected_set' ourself
    // before unlocking the mutex here.
      ...
      if (success)
        {
          ...
          event.protected_set(); // Wake up the source thread if necessary
...
{ // Note: Must not lock `mutex' here, since the source manager's mutex has
  // been locked by the caller.  Attempting to lock `mutex' could create a
  // deadlock if the source thread is simultaneously attempting to acquire
  // a lock on the source manager's mutex.  However, if this is happening, it
  // cannot itself be waiting within `event.wait', so it is always safe for
  // us to invoke `event.unprotected_set'.
  shutdown_requested = true;
  event.unprotected_set();
}
...
void kd_source_thread::run()
{
  ...
  while (!shutdown_requested)
    { 
      ...
      // Process new requests and check for idle-induced session termination
      ellapsed_microseconds = timer.get_ellapsed_microseconds();
      process_requests();
      if ((last_idle_time >= 0) &&
      ...
      // Check unreliable auxiliary channels for which a client-requested
      // abandonment may be pending.
      for (p=0; p < num_cids; p++)
        { 
          kds_cid *cid = cids + p;
          if (!cid->channel_type.aux_unreliable())
            continue;
          kds_jpip_channel *channel = cid->aux_channel;
          for (int nc=0; nc < num_cids; nc++)
            { 
              kds_cid *dependent_cid = cids+nc;
              if (dependent_cid->response_complete ||
                  !dependent_cid->is_active)
                continue;
              kdu_long max_qid = dependent_cid->active_dependencies[p];
              if (max_qid < -1)
                { // Dependency not yet known -- first time processing request
                  assert(!dependent_cid->blocked_by_dependencies);
                  max_qid = channel->check_pending_abandonment(-1,&event);
                  assert(max_qid >= -1);
                  dependent_cid->active_dependencies[p] = max_qid;
                }
              else if (max_qid >= 0)
                { // Max dependency known -- see if dependency remains                  
                  if (channel->check_pending_abandonment(max_qid,&event) < 0)
                    { // Dependency on `cid' has been cleared
                      dependent_cid->active_dependencies[p] = -1;
                      dependent_cid->blocked_by_dependencies = false;
                          // Even if there are other dependencies, it is
                          // worthwhile unblocking the previously dependent CID
                          // so it can have a go at generating more data.
                    }
                }
            }
        }
      ...
      int suggested_chunk_size=max_chunk_bytes;
      ...
      for (p=0;
        ...
        { 
          kds_cid *cid = cids+p;
          if (cid->aux_connect_pending && (cid->aux_channel != NULL) &&
              cid->aux_channel->complete_udp_connection(cid->channel_id,
                                                        &event))
            { 
              cid->aux_timeout = -1;
              cid->aux_connect_pending = false;
          ...
          if (cid->aux_connect_pending)
            { // Waiting for `aux_channel' to connect
              ...
              suggested_chunk_size = cserver->get_min_suggested_chunk_size();
            }
          else if (cid->active_chunks == NULL)
            { // Let the channel tell us how many bytes to send
              if ((suggested_bytes =
                   data_channel->get_suggested_bytes(&suggested_chunk_size,
                                                     false,&event)) <= 0)
          ...
          if ((cid->active_chunks == NULL) && !cid->blocked_by_dependencies)
            { 
              for (int nc=0; nc < num_cids; nc++)
                { // Retrieve any completed data chunks whose abandonment
                  // status may be critical to correct processing of the
                  // current request.
                  kds_chunk *chunks = NULL;
                  if (nc == p)
                    { 
                      if (data_channel != NULL)
                        chunks = data_channel->retrieve_completed_chunks();
                    }
                  else if (cids[nc].channel_type.aux_unreliable())
                    chunks = cids[nc].aux_channel->retrieve_completed_chunks();
                  if (chunks != NULL)
                    release_chunks(chunks,true);
                }
          ...
          if ((data_channel != NULL) && (cid->active_chunks != NULL))
            sending_cid = cid;
                   ...
                   !cid->blocked_by_dependencies)
            { // CID no longer active
      ...
      if (sending_cid != NULL)
        { // Send data prepared in `sending_cid'
          ...
          if ((chunks =
               data_channel->push_chunks(chunks,sending_cid->active_qid,
                                         content_type,
                                         all_done_with_channel)) != NULL)
            release_chunks(chunks,true);
          if (sending_cid->response_complete)
            {
              if (!sending_cid->channel_type.is_http_aux())
                data_channel->terminate_chunked_data(sending_cid->active_qid);
              detach_active_chn(sending_cid);
              sending_cid->is_active = sending_cid->response_complete = false;
            }
          else if (data_channel->check_impending_response_limit())
            { 
              data_channel->terminate_chunked_data(sending_cid->active_qid,
                                              JPIP_EOR_RESPONSE_LIMIT_REACHED);
              assert(sending_cid->active_chn == NULL);
              sending_cid->is_active = false;
              sending_cid->response_complete = true;
            }
        ...
        { 
          ...
          if (chn->request_qid > 0)
            { 
              ...
              cid->next_qid = chn->request_qid+1;
            }
          ...
          cid->active_chn = chn;
          cid->is_active = true;
          cid->blocked_by_dependencies = false;
          for (int p=0; p < max_cids; p++)
            cid->active_dependencies[p] = (p < num_cids)?-2:-1;
               // Note: we fill out the `active_dependencies' array for all
               // possible CID's, noting that we can never become dependent
               // on a CID which is not yet in use, while we might have a
               // dependency on one which is in use.  The -2 value means that
               // we don't yet know whether we have a dependency or not.  This
               // condition is detected in the `run' function when we search
               // for requests on which this one may depend due to data chunks
               // whose abandonment is pending.
          assert(cid->active_chunks == NULL);
          cid->active_qid = chn->request_qid;
...
bool kd_source_thread::process_requests_to_stage_1(kds_primary_channel *chn)
{
  ...
  if ((req->fields.request_id != NULL) && !is_stateless)
    { 
      kdu_long val;
      const char *cp = req->fields.request_id;
      for (val=0; ((*cp) >= '0') && ((*cp) <= '9'); cp++)
        val = 10*val + (int)((*cp) - '0');
      if (*cp != '\0')
        { 
          replies << "HTTP/1.1 400 \"" JPIP_FIELD_REQUEST_ID "\" request field"
...
bool kd_source_thread::process_request_to_stage_2(kds_primary_channel *chn)
{
  ...
  chn->reply_needs_handled_requests = (req->fields.handled_requests != NULL);
  chn->release_channel_after_response = req->close_connection;
  chn->reply_generated = false;
  ...
  if (!chn->requested_target_compatible)
    { // The session-ID demonstrates compatibility but target does not!
      replies << "HTTP/1.1 503 Request cannot be granted for access to "
                 ...
                 "session.  The request might be legal (if \""
                 JPIP_FIELD_CHANNEL_NEW "\" is also being issued), but this "
                 "server can only associate one logical target with a "
                 "given session.\r\n";
      generate_error_reply(chn,req);
      return false;
    }
  ...
  if ((req->fields.chunk_abandon != NULL) && (!first_request_on_channel) &&
      cid->channel_type.aux_unreliable())
    { 
      kds_chunk_gap *list=NULL;
      const char *failure =
        chunks_to_abandon.translate(req->fields.chunk_abandon,list);
      if (failure != NULL)
        { 
          replies << "HTTP/1.1 400 Malformed \"" JPIP_FIELD_CHUNK_ABANDON
          "=\" request body.  Error at \"" << failure << "\".\r\n";
          generate_error_reply(chn,req);
          return false;
        }
      else if (list != NULL)
        cid->aux_channel->abandon_chunks(list);
    }

  if ((req->fields.barrier_id != NULL) && (!first_request_on_channel) &&
      cid->channel_type.aux_unreliable())
    { 
      kdu_long barrier_qid;
      const char *cp = req->fields.barrier_id;
      for (barrier_qid=0; ((*cp) >= '0') && ((*cp) <= '9'); cp++)
        barrier_qid = 10*barrier_qid + (int)((*cp) - '0');
      if (*cp != '\0')
        { 
          replies << "HTTP/1.1 400 \"" JPIP_FIELD_BARRIER_ID
          "\" request field has illegal parameter string.\r\n";
          generate_error_reply(chn,req);
          return false;
        }
      cid->aux_channel->set_barrier_qid(barrier_qid);
    }
  
  if ((req->fields.channel_new != NULL) && !first_request_on_channel)
    { // Requesting a new channel-ID.
      int p;
      kd_jpip_channel_type ctp, fallback_ctp; // fallback for no UDP
      ctp = translate_cnew_string(req->fields.channel_new,false);
      fallback_ctp = translate_cnew_string(req->fields.channel_new,true);
      const char *new_channel_id = NULL;
      if (ctp != KD_CHANNEL_STATELESS)
        new_channel_id = assign_new_cid(ctp,fallback_ctp,chn->request_cid);
      if ((new_channel_id != NULL) &&
          ((p = find_channel_id(new_channel_id)) >= 0))
        {
          chn->request_cid = cids+p;
          if (chn->request_qid > 0)
            chn->request_cid->next_qid = chn->request_qid;
  ...
  if (req->fields.channel_close != NULL)
    {
      ...
      for (p=0; p < num_cids; p++)
        if ((strcmp(req->fields.channel_close,"*") == 0) ||
          ...
          { 
            if (cids[p].channel_type.aux_unreliable())
              cids[p].close_immediately = true;
            cids[p].close_when_inactive = true; // If using a TCP transport,
             // the channel will not actually be closed until it becomes
             // inactive, which usually means that all the data queued in
             // the TCP channel has been delivered.  However, for UDP
             // transports, it is completely reasonable to close the channel
             // immediately, because there is no longer any way that the
             // client can explicitly "abandon" missing data chunks, so there
             // is no way for it to guarantee ultimate delivery of any of the
             // data which did not arrive before it issued the "cclose"
             // request.  In any event, the actual call to `close_cid' will
             // be issued from within the `run' function, based on the
             // state of these to `close_...' flags.
          }
    }
                                            ...
                                            const kd_request *req)
{
  ...
  replies << "Server: Kakadu JPIP Server "
          << KDU_CORE_VERSION << "\r\n\r\n";
  chn->channel->push_reply(replies,0,chn->request_qid);
  ...
  if ((cid->active_chn != NULL) && !cid->active_chn->reply_generated)
    { 
      ...
      if (chn->reply_needs_target_id)
        replies << "JPIP-" JPIP_FIELD_TARGET_ID ": "
                << source->target_id << "\r\n";
      if (chn->reply_needs_handled_requests)
        replies << "JPIP-" JPIP_FIELD_HANDLED_REQUESTS ": "
                << KD_HANDLED_REQUESTS_STRING;
      if (chn->reply_needs_jpip_channel_id)
        {
          ...
          else if (cid->channel_type == KD_CHANNEL_HTTP_TCP)
            replies << ",transport=http-tcp,auxport=" << cid->aux_port;
          else if (cid->channel_type == KD_CHANNEL_HTTP_UDP)
            replies << ",transport=http-udp,auxport=" << cid->aux_port;
          else
            assert(0);
          replies << "\r\n";
        }
      ...
      if (chn->release_channel_after_response)
        replies << "Connection: close\r\n";
      replies << "\r\n";
      chn->channel->push_reply(replies,0,cid->active_qid);
    ...
    { 
      ...
      assert(cid->is_active && (chn->request_cid == cid) &&
             !cid->channel_type.is_http_aux());
      if (!cid->channel_type.is_http_aux())
        chn->channel->terminate_chunked_data(cid->active_qid,
                                             JPIP_EOR_WINDOW_CHANGE);
      detach_active_chn(cid);
    }
  if (cid->aux_channel != NULL)
    { 
      assert(!cid->response_complete);
      cid->aux_channel->terminate_chunked_data(cid->active_qid,
                                               JPIP_EOR_WINDOW_CHANGE);
    }
  cid->is_active = false;
}
  ...
  if (chn != NULL)
    {
      ...
      if (chn->release_channel_after_response)
        { 
          chn->channel->set_timeout(max_completion_seconds,true);
          release_primary_channel(chn,true);
        }
    }
}
...
void kd_source_thread::close_cid(int which, kdu_byte eor_code)
{
          ...
          if (!chn->reply_generated)
            {
              ...
              if (cid->aux_timeout > 0)
                replies << "HTTP/1.1 503 Server is unwilling to wait any "
                           "longer for attachment of auxiliary channel "
                           "to complete HTTP-TCP/HTTP-UDP transport "
                           "connection; closing down the JPIP channel.\r\n";
            ...
            { // Must be in middle of HTTP-only request
              ...
              if (!cid->response_complete)
                chn->channel->terminate_chunked_data(cid->active_qid,eor_code);
              detach_active_chn(cid);
            }
        }
      if (cid->active_chunks != NULL)
        { 
          if ((cid->aux_channel != NULL) && !cid->aux_connect_pending)
            cid->active_chunks =
              cid->aux_channel->push_chunks(cid->active_chunks,cid->active_qid,
                                            NULL);
          else
            {
              kds_chunk *chnk;
  ...
  if (cid->aux_channel != NULL)
    {
      cid->aux_channel->set_timeout(max_completion_seconds,true);
      release_auxiliary_channel(cid,!cid->channel_type.aux_unreliable());
    }
  if (cid->active_chunks != NULL)
    { // Just in case
      kds_chunk *chnk;
  ...
  cids[num_cids].active_dependencies = NULL;
  cids[num_cids].init();
  manager->mutex.unlock();
  
  // Fix up references into the `cids' array from the primary channels list
  kds_primary_channel *chn, *next_chn;
  for (chn=primary_channels; chn != NULL; chn=next_chn)
    { 
      next_chn = chn->next; // Because `chn' may be deleted by a call to
                            // `recycle_primary_channel' below.
      if (chn->request_cid != NULL)
        { 
          if (chn->request_cid == cid)
            { 
              assert(!chn->request_processed);
              chn->request_cid = NULL; // So call below doesn't access us
                                            ...
                                            int suggested_chunk_size)
{
  ...
  assert(!cid->blocked_by_dependencies);
  ...
  bool prohibit_response_data = ((cid->active_remaining_bytes == 0) &&
                                 !cid->channel_type.is_http_aux());
  if (!cid->channel_type.is_http_aux())
    suggested_chunk_size = 0; // With HTTP-only transport, the client does
                              // all flow control.
  
  bool result=true, window_done=false;
  mutex.unlock();
  try {
    if (need_set_window)
      { 
    ...
    cid->active_chunks =
      generate_increments(suggested_bytes,cid->active_remaining_bytes,
                          ...
                          (cid->channel_type==KD_CHANNEL_HTTP_UDP),
                          cid->window_id,suggested_chunk_size);
    assert(cid->active_chunks != NULL);
    window_done = !get_window(*(cid->active_window),cid->window_id);
    for (int nc=0; window_done && (nc < num_cids); nc++)
      if (cid->active_dependencies[nc] >= 0)
        { // One or more dependencies not satisifed yet
          window_done = false;
          cid->blocked_by_dependencies = true;
        }
    ...
    if (prohibit_response_data)
      { 
        ...
        cid->active_chunks = NULL;
        window_done = false; // Because we don't need to send EOR
      }
    else if (cid->blocked_by_dependencies &&
             (cid->active_chunks->next == NULL) &&
             (cid->active_chunks->num_bytes <=
              cid->active_chunks->prefix_bytes))
      { // Don't want to send EOR message yet, but no actual data to send
        release_chunks(cid->active_chunks);
        cid->active_chunks = NULL;      
      }
  }
  ...
  if (window_done || (cid->active_remaining_bytes <= 0))
...
void kd_source_thread::generate_reply(kds_cid *cid)
{
  ...
  if ((cid->active_chunks == NULL) || cid->channel_type.is_http_aux())
    chn->channel->push_reply(replies,0,cid->active_qid);
  else
    chn->channel->push_reply(replies,-1,cid->active_qid);
  
  chn->reply_generated = true;
  total_serviced_requests++;
  
  if (cid->channel_type.is_http_aux() || (cid->active_chunks==NULL))
    detach_active_chn(cid);
}
    ...
    { 
      ...
      this->total_transmitted_bytes +=
        (chn->channel->get_total_transmitted_bytes() -
         chn->initial_transmitted_bytes);
      assert(total_transmitted_bytes >= 0);
      chn->channel->release();
      chn->channel = NULL;
    }
  if (prev == NULL)
                                                 ...
                                                 bool send_queued_data_first)
{
  ...
  if (send_queued_data_first)
    cid->aux_channel->close(false); // Hard close in case we timed out
  kds_chunk *chunks;
  ...
  this->total_client_abandoned_bytes +=
    cid->aux_channel->get_num_abandoned_bytes();  
  ...
  if (cid->channel_type == KD_CHANNEL_HTTP_UDP)
    cserver->release_udp_port(cid->aux_port);
}
...
kd_source_manager::kd_source_manager(kdu_uint16 aux_listen_port,
                                     ...
                                     float max_completion_seconds,
                                     float max_establishment_seconds,
                                     float max_intrinsic_delay,
                                     bool allow_retransmit,
                                     ...
                                     bool caseless_targets)
{
  ...
  this->max_completion_seconds = max_completion_seconds;
  this->max_establishment_seconds = max_establishment_seconds;
  this->udp_max_intrinsic_delay = max_intrinsic_delay;
  this->udp_allow_retransmit = allow_retransmit;
  this->tcp_prefers_group_based_flow_control = prefer_group_based_flow_ctl;
  shutdown_requested = false;
                                             ...
                                             kdcs_message_block &explanation)
{
  ...
  kd_jpip_channel_type ctp, fallback_ctp; // Fallback is in case no UDP ports
  ctp = kd_source_thread::translate_cnew_string(cnew_string,false);
  fallback_ctp = kd_source_thread::translate_cnew_string(cnew_string,true);
  ...
  if (src == NULL)
    { // Need to open the image
      ...
      src->source_id = (kdu_uint32)
        _addr_to_kdu_int32(&src); // Start with 32 LSB's of address
      kd_source *sscan;
      do {
  ...
  const char *new_channel_id = thrd->assign_new_cid(ctp,fallback_ctp);
                                        ...
                                        kdcs_message_block &explanation)
{
  ...
  if (src == NULL)
    {
      mutex.unlock();
      explanation << "503 Requested/created channel ID no longer exists\r\n";
      return false;
    }
                                      ...
                                      int max_records)
{
  ...
  for (scan=history_head; (scan != NULL) && (max_records > 0);
       scan=scan->next, max_records--, index--)
    {
      ...
      block << "\n";
      if ((scan->num_jpip_h_channels + scan->num_jpip_ht_channels +
           scan->num_jpip_hu_channels) == 0)
        block << "    Channel = <none> (stateless)\n";
      else if ((scan->num_jpip_ht_channels+scan->num_jpip_hu_channels) == 0)
        block << "    Channel = \"http\" ("
              << scan->num_jpip_h_channels << ")\n";
      else if ((scan->num_jpip_h_channels+scan->num_jpip_hu_channels) == 0)
        block << "    Channel = \"http-tcp\" ("
              << scan->num_jpip_ht_channels << ")\n";
      else if ((scan->num_jpip_h_channels+scan->num_jpip_ht_channels) == 0)
        block << "    Channel = \"http-udp\" ("
        << scan->num_jpip_hu_channels << ")\n";
      else
        block << "    Channel = \"http\" ("
              << scan->num_jpip_h_channels << "), \"http-tcp\" ("
              << scan->num_jpip_ht_channels << "), \"http-udp\" ("
              << scan->num_jpip_hu_channels << ")\n";
      ...
      if ((scan->num_jpip_ht_channels+scan->num_jpip_hu_channels) > 0)
        {
...
void kd_source_manager::close_gracefully()
{
  ...
  for (src=sources; src != NULL; src=src->next)
    for (thrd=src->threads; thrd != NULL; thrd=thrd->next)
      thrd->request_shutdown(); // Must call this while mutex locked
  while (num_threads > 0)
    {
      event.reset();
      event.wait(mutex);
