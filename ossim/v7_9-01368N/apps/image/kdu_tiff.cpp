  ...
  kdu_tiffdir::opendir(kdu_compressed_source *in)
{
  ...
  for (; entry_count > 0; entry_count--)
    {
      kdu_uint16 tag=0, data_type=0;
      kdu_long tag_length=0, tag_offset=0;
      if (bigtiff)
        {
...
void
  kdu_tiffdir::write_tag(kdu_uint32 tag_type, int length,
                         const kdu_byte data[])
{
  assert(length >= 0);
  kd_tifftag *elt = find_tag(tag_type);
