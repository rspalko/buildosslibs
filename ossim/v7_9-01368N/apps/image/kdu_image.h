...
class kdu_image_dims {
  ...
  */
  public: // Member functions
    kdu_image_dims()
      { 
        ...
        icc_profile = NULL; icc_profile_len=0;
    ...
    void set_colour_info(int num_colours,
      ...
      { /* [SYNOPSIS]
           ...
           [ARG: num_colours]
             ...
             representation.  If `profile' is non-NULL, the number
             of colours can be retrieved using the profile.
           ...
           [ARG: profile]
             If non-NULL, the `colour_space' argument is ignored.  In this
             case, `profile' must point to a buffer containing a valid ICC
             profile, whose contents are copied internally.  The
             profile's first 4 bytes provide a big-endian representation of
             the length of the profile (as is true for all ICC profiles) and
             the buffer must be at least this long.
        */
        ...
        this->icc_profile_len = 0;
        if (this->icc_profile != NULL)
          { delete[] this->icc_profile;
            this->icc_profile = NULL; }
        if (profile != NULL)
          { 
            icc_profile_len =
              ((((int) profile[0])<<24) + (((int) profile[1])<<16) +
               (((int) profile[2])<< 8) + (((int) profile[3])<< 0));
            if (icc_profile_len < 4)
              icc_profile_len = 0;
            else
              { 
                this->icc_profile = new kdu_byte[icc_profile_len];
                memcpy(this->icc_profile,profile,(size_t)icc_profile_len);
              }
          }
      }
    int get_colour_info(bool &has_premultiplied_alpha,
      ...
      { /* [SYNOPSIS]
           ...
           [ARG: profile_len]
             If the value returned value is positive, the colour space is
             described by an ICC profile, that may subsequently be retrieved
             using the `get_profile' function.  The first `profile_len'
             bytes of the returned buffer represents the complete ICC profile.
        */
        ...
        profile_len = this->icc_profile_len;
        return num_colours;
      }
    const kdu_byte *get_profile()
      { /* [SYNOPSIS]
             Returns a buffer containing the ICC profile whose length is
             identified by the last argument to `get_colour_info'; returns
             NULL if the length of that buffer was not at least 4 -- the
             number of bytes required to embed the length of the profile.
             The first 4 bytes of the returned buffer contains a big-endian
             representation of the length which was returned via
             `get_colour_info'.
        */
        return (icc_profile_len>0)?icc_profile:NULL;
      }
    ...
    void set_forced_precision(int comp_idx, int precision,
      ...
      { int c;
        ...
        if (precision == 0)
          { precision = exponent_bits = 0; align_lsbs = true; }
        if (precision > 64) precision = 64;
        if (exponent_bits < 0) exponent_bits = 0;
        if (exponent_bits > 0) align_lsbs = false;
        int val = (precision < 0)?0x80:(precision&0x7F);
        val |= ((align_lsbs?0:0x100)) | (exponent_bits<<16);
        forced_precisions[comp_idx] = val;
      }
      /* [SYNOPSIS]
           Use this function to set information about component precisions
           you want to force an image file reader or writer to use.
           [//]
           Note that passing `precision'=0 disables any precision forcing for
           component `comp_idx', but passing a negative value for `precision'
           is not exactly the same.  When `precision' is -ve, an image file
           reader or writer is free to select the most appropriate precision,
           but a non-zero `exponent_bits' value can have a major effect upon
           the way in which data is read or written.
           [//]
           For more information about the interpretation of forced precisions,
           see the `get_forced_precision' function.
      */
    int get_forced_precision(int comp_idx, bool &align_lsbs,
                             int &exponent_bits)
      { 
        exponent_bits = 0;  align_lsbs = false;
        if ((comp_idx < 0) || (num_forced_precisions == 0))
          return 0;
        if (comp_idx >= num_forced_precisions)
          comp_idx = num_forced_precisions-1;
        int val = forced_precisions[comp_idx];
        int precision = (val & 0x80)?(-1):(val & 0x7F);
        if (precision != 0)
          { 
            exponent_bits = (val >> 16);
            align_lsbs = (val & 0x100)?false:true;
          }
        return precision;
      }
      /* [SYNOPSIS]
           This function returns 0 if no forced precision information is
           available (i.e., `set_forced_precision' has not been called, or
           it has been called with a precision value of 0).  Positive values
           P returned by the function identify a forced precision for the
           component, whose meaning is explained below.  A negative value
           (only -1) may also be returned, in which case the precision itself
           is not being forced but the `exponent_bits' argument is still
           relevant, as explained below.
           [//]
           If no call to `set_forced_precision' has occurred for `comp_idx',
           the function replicates any information that may have been supplied
           for the closest component c < `comp_idx'.
           [//]
           If an image file reader supports precision forcing, it should
           interpret a positive value returned by this function as the
           ...
           (via `set_bit_depth') to the forced precision.  Negative return
           values from this function mean that the image file reader should
           select the most natural precision to employ, which means that
           there is no precision forcing, except that positive values of
           `exponent_bits' still impact the way in which sample values are
           processed, as described below. 
           [//]
           The precision forcing algorithm depends on the values returned via
           `align_lsbs' and `exponent_bits', with the following interpretation
           for image file readers:
           [>>] If `exponent_bits' = 0 and `align_lsbs' is true, the sample
                values read from the image file are forced to fit into the
                identified precision as integers, without any scaling.  This
                means that a value of 1 in the original file will remain 1
                after precision forcing.  Values which exceed the range which
                can be represented by the forced precision should be
                clipped -- as opposed to just ignoring the most significant
                bits.
           [>>] If `exponent_bits' = 0 and `align_lsbs' is false, the sample
                values read from the image file are converted to integers (if
                necessary) and scaled by a power of 2, such that the most
                significant bit in the original representation corresponds
           ...
           [>>] If `exponent_bits' > 0, the `align_lsbs' value is irrelevant,
                but will be false.  Values read from the image file are
                converted to a floating point format which involves a leading
                sign bit, E exponent bits and then M mantissa bits, where
                M = P-E-1, P is the precision returned by the function and E
                is the `exponent_bits' value.  If the function returns a
                negative value (-1 in practice), the image reader is free to
                select a suitable value for P, which may be useful.  For
                example, if the source image already contains floating point
                sample values, P would most naturally be set to 32.  In any
                case, P must be selected no smaller than E+1.  If the
                function's positive return value is smaller than E+1, an
                error should be generated.  The E-bit exponent is a signed
                quantity that has been offset by 2^{E-1}-1 to yield unsigned
                E-bit values for the floating point representation -- this
                is the common convention for IEEE and other floating point
                standards.  After conversion to this representation, the P-bit
                quantities are re-interpreted as integers which are returned
                via calls to `kdu_image_in::get'.  This mapping of floats to
                integers is intended to work in tandem with Part-2 non-linear
                point transforms (NLT) of the `NLType_SMAG' and `NLType_UMAG'
                types, in which the floating-point bit patterns are
                transformed in a manner which renders them suitable for
                efficient compression.  The floating point representation
                should be described via a "Pixel Format" box within any
                relevant compositing layers of a JPX output file that contains
                imagery compressed in this way.  If `exponent_bits' > 0,
                raw input images are assumed to contain floating point values
                with exactly the same number of exponent bits, but perhaps a
                different number of mantissa bits, since the original
                precision found by calling `get_bit_depth' may differ from
                that returned by this function.  For components that are
                identified as unsigned (see `get_signed'), floating point
                representations are assumed to have a 0 in the sign bit
                position, and may need to be truncated to such a state.  Where
                samples need to be converted from N-bit integers to the P-bit
                floating point representation with E-bit exponent, the
                conversion maps unsigned integers in the range 0 to 2^N-1 to
                floats in the range 0.0 to 1.0, while signed integers in the
                range -2^{N-1} to 2^{N-1}-1 are mapped to floats in the range
                -1.0-2^{1-N} to 1.0.  This conversion is most compatible with
                the way in which JPX files interpret different pixel formats
                and colour spaces.
           [//]
           ...
           writer depends upon the `align_lsbs' and `exponent_bits' values,
           and a negative return value has meaning when accompanied by a
           non-zero `exponent_bits' value, as explained below:
           [>>] If `exponent_bits' = 0 and `align_lsbs' is true, the sample
                values written to the output file as integers are not scaled,
                meaning that a value of 1 (with respect to the `get_bit_depth'
                precision) should be written as a 1 (with respect to the
                forced precision used in the output file).  If the
                `get_bit_depth' value is larger than the forced precision,
                some values may need to be clipped to the range which can be
                accommodated by the `forced_precision'.
           [>>] If `exponent_bits' = 0 and `align_lsbs' is false, the sample
                values written to the output file as integers are scaled, so
                that the most significant bit of the representation used to
                supply data via `kdu_image_out::put' is aligned with the most
                significant bit of the forced precision used to actually write
                the data.  Such scaling is always by a power of 2; rounding to
                the nearest representable value should be employed if LSB's are
                discarded during the shifting process.
           [>>] If `exponent_bits' > 0, the `align_lsbs' value is irrelevant
                but will be false.  Values supplied as absolute integers via
                `kdu_image_out::put' have their bit-patterns re-interpreted
                a floating point values with 1 sign bit, followed by E
                exponent bits and then N-1-E mantissa bits, following all the
                same conventions as described above for image readers, where
                N is the incoming bit-depth returned by `get_bit_depth'.  For
                raw output files, these values are written as P-bit floating
                point values with the same number of exponent bits E, but
                P-1-E mantissa bits, where P is the forced precision value
                returned by this function.  For output file formats that can
                explicitly accommodate the writing of P-bit floating point
                data, the source values are converted (if necessary) to
                accommodate any differences in the number of exponent bits --
                i.e., the E=`exponent_bits' value describes the input samples,
                not necessarily the output samples.  Otherwise, the
                floating point values are converted to a signed or unsigned
                P-bit integer representation, depending on the value returned
                by `get_signed'.  In this latter case, the conversion to
                unsigned integers is such that floats in the range 0.0 to 1.0
                map to integers in the range 0 to 2^P-1; the conversion to
                signed integers is such that floats are scaled by 2^{P-1}-1,
                and clipped to the range -2^{P-1} to 2^{P-1}-1, which roughly
                accommodates floats in the range -1.0 to 1.0.  If the function
                returns a negative value (-1 in practice), the image writer
                is free to select the most appropriate precision for its
                written samples, but all the other implications of a non-zero
                `exponent_bits' field remain.  This can be especially useful,
                since it allows the writer to be informed of the floating
                point nature of the data that it will be passed, while giving
                the writer an opportunity to determine how best to represent
                the samples.
      */
  ...
  private: // Private data
    ...
    int icc_profile_len; // 0, or length of the profile inside `icc_profile'
    kdu_byte *icc_profile; // Exists if non-NULL (first 4 bytes give length)
    jp2_colour_space colour_space;
  ...
  /* Notes:
     ...
     replication, as required.  Each entry is divided into bit-fields so
     as to accommodate various precision forcing options and up to two
     integer-parameters (float forcing requires a positive exponent parameter).
