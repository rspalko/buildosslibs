...
struct kdd_namespace {
  public: // Member functions
    kdd_namespace(const char *string)
      { 
        name = NULL; next = NULL; parent=NULL;
        descendant_head=descendant_tail=NULL;
        if (string != NULL)
          { name = new char[strlen(string)+1]; strcpy(name, string); }
        else
          { name = new char[1]; *name = '\0'; }
        has_bindings = false;
      }
    ~kdd_namespace()
      { // Deletes all descendants recursively
        ...
        assert(parent == NULL); // Application only deletes top-level spaces
        while ((descendant_tail = descendant_head) != NULL)
          {
            descendant_head = descendant_tail->next;
            descendant_tail->parent = NULL;  descendant_tail->next = NULL;
            delete descendant_tail;
          }
      }
    bool is_global() const { return (parent == NULL); }
    bool is_anonymous() const { return (*name == '\0'); }
    void print(ostream &tgt);
      /* Recursive function to print the full namespace to `tgt', including
         a final "::" separator. */
    void enter_namespace(ostream &tgt);
    void leave_namespace(ostream &tgt);
      /* The above two functions print C++ "namespace ... {" opening
         statements to enter each level of the namespace, followed by
         closing parentheses "}" with comments, to leave each level of the
         namespace, respectively.  These are used when assembling new
         class declarations for a namespace within header files. */
    void print_using(ostream &tgt);
      /* Recursively scans all descendants of the current namespace, printing
         "using namespace" statements for each one that `has_bindings'. */
  public: // Data
    char *name; // Empty string for global and anonymous namespaces
    bool has_bindings; // If any bound entity (JAVA/MNI) uses this namespace
    kdd_namespace *parent; // NULL for global namesapce only
    kdd_namespace *descendant_head; // List of sub-namespaces, linked via
    kdd_namespace *descendant_tail; // their `next' members.
    kdd_namespace *next;
  };
...
struct kdd_primitive {
  public: // Member functions
    kdd_primitive()
      { name=NULL; java_name=jni_name=NULL; alias=next=NULL; space=NULL; }
    ~kdd_primitive()
  ...
  public: // Data
    ...
    kdd_namespace *space; // NULL unless this is an alias generated via `enum'
  ...
  public: // Member functions
    ...
    void finalize(kdd_class *classes, kdd_primitive *primitives);
       /* Tries to find any unknown type name in the definitions offered
          ...
          alter the modifiers. */
    void print(ostream &out, const char *name, bool full_namespace=false);
      /* Prints the C++ argument type declaration, including all modifiers and
         ...
         types, `name' should be NULL.  If `full_namespace' is true and
         the object was declared within a defined namespace, the type name
         is printed with the namespace qualifier. */
...
struct kdd_constant {
  public: // Member functions
    kdd_constant()
      { type=NULL; name=html_link=html_offset=NULL; value=NULL;
        space=NULL; next=NULL; }
  ...
  public: // Data
    ...
    char *html_offset;
    char *value;
    kdd_namespace *space; // NULL unless found in an enum body
    kdd_constant *next;
  };
...
struct kdd_variable {
  public: // Member functions
    kdd_variable()
      { name=html_link=html_offset=initializer=NULL;
        next=NULL; owner=NULL; line_number=0; file=NULL; space=NULL;
  ...
  public: // Data
    ...
    kdd_namespace *space; // NULL for function args.
...
struct kdd_func {
  public: // Member functions
    kdd_func()
      { name=html_name=NULL; line_number=overload_index=0; owner=NULL;
        ...
        is_constructor=is_destructor=false; property=NULL;
        file=NULL; space=NULL;  java_name = mni_name = NULL;
        csharp_name = vbasic_name = NULL;  binding = KDD_BIND_TRANSIENT;
        bind_specific=false; num_jni_args = num_mni_args = 0;
      }
    ~kdd_func()
      { kdd_variable *vtmp;
  ...
  public: // Data
    ...
    kdd_file *file; // File in which declaration is found
  kdd_namespace *space; // Namespace in which declaration is found
...
struct kdd_class {
  public: // Member functions
    kdd_class()
      { 
        name=html_name=java_name=mni_name=NULL; line_number = 0;
        file = NULL; space = NULL; next = NULL; functions = NULL;
        variables = NULL; constants = NULL;
    ...
    void print_qualified_name(ostream &out)
      { /* Prints the native class name prefixed by its namespace, if any. */
        if (space != NULL)
          space->print(out);
        out << name;
      }
  ...
  public: // Data
    ...
    kdd_namespace *space; // Namespace containing declaration.
