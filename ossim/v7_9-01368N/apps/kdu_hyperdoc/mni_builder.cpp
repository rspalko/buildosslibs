...
{
  assert((cls->binding == KDD_BIND_IFC) || (cls->binding == KDD_BIND_IFC2));
  ...
  if (cls->binding == KDD_BIND_IFC2)
    { 
      out << "    kdu_int64 " << native_obj << "_P = ";
      if (check_for_null)
        { 
          if (old_syntax)
            out << "((" << mni_prefix << mni_obj << "==NULL)?0:";
          else
            out << "((" << mni_prefix << mni_obj << "==nullptr)?0:";
        }
      out << mni_prefix << mni_obj << "->_pval";
      out << ((check_for_null)?");\n":";\n");
    }
  if (cls->binding == KDD_BIND_IFC)
    ...
    { 
      out << "    ((void **) &" << native_obj << ")[0] = "
          << native_obj << "_S;\n";
      out << "    ((kdu_int64 *) &" << native_obj << ")[1] = "
          << native_obj << "_P;\n";
    }
}
...
static void
  implement_native_ifc_to_mni(kdd_class *cls, const char *native_obj,
     ...
     object's `_kval' member. */
{
  if (cls->binding == KDD_BIND_IFC)
    ...
    { 
      out << "    " << mni_prefix << mni_obj
          << "->_kval = IntPtr(((void **)(&" << native_obj << "))[0]);\n";
      out << "    " << mni_prefix << mni_obj
          << "->_pval = Int64(((kdu_int64 *)(&" << native_obj << "))[1]);\n";
    }
  else
    assert(0);
}
      ...
      if (type.known_class != NULL)
        {
          ...
          if ((cls->binding == KDD_BIND_IFC) || (cls->binding == KDD_BIND_IFC2))
            { // Interface passed either by copy or by reference.
              if (type.by_ref)
                implement_native_ifc_to_mni(cls,arg->name,arg->name,out,"_");
            }
          else if ((type.derefs == 0) && !type.by_ref)
...
static void
  implement_return_assignment(kdd_func *func, ostream &out, bool old_syntax)
{
  kdd_type &return_type = func->return_type;
  if (return_type.is_void())
    return;
  if (return_type.known_class != NULL)
    { // Create a managed object of the relevant class
      ...
      if ((return_type.known_class->binding == KDD_BIND_IFC) ||
          (return_type.known_class->binding == KDD_BIND_IFC2))
        {
          ...
          if (old_syntax)
            ...
            out << "    " << return_type.known_class->mni_name <<
                   " ^_Result = gcnew "
                << return_type.known_class->mni_name << ";\n";
          implement_native_ifc_to_mni(return_type.known_class,
                                      "Result","_Result",out);
        }
        ...
        { 
          if (old_syntax)
            { 
              ...
              out << "    if (Result != NULL)\n"
                     "      _Result = new "
                  << return_type.known_class->mni_name << "(Result);\n";
            }
          else
            { 
              out << "    " << return_type.known_class->mni_name
  ...
  else if (return_type.known_primitive != NULL)
    {
      if (return_type.bind_as_opaque_pointer)
        { // Returned array treated as abstract pointer
          out << "    IntPtr _Result = IntPtr((void *) Result);\n";
        }
      else if (return_type.known_primitive->string_type)
        {
          if (old_syntax)
            out << "    String *_Result = __gc new String((char *) Result);\n";
          else
            out << "    String ^_Result = gcnew String((char *) Result);\n";
        }
      else
        {
          assert(return_type.derefs == 0);
          out << "    " << return_type.known_primitive->mni_name
              << " _Result = Result;\n";
        }
    }
  else if (return_type.unknown_name != NULL)
  ...
  for (arg=func->args; nargs > 0; nargs--, arg=arg->next)
    {
      ...
      if ((type.known_class != NULL) &&
        ...
        { // Interface object passed by value
          ...
          implement_native_ifc_to_mni(type.known_class,
                                      arg->name,arg->name,out,"_");
        }
  ...
  generate_derived_delegator(kdd_class *cls, ostream &out, bool old_syntax)
{
  out << "  class " MNI_DELEGATOR_PREFIX << cls->name
      << " : public ";
  out << AUX_DELEGATOR_PREFIX << cls->name << " {\n";
  out << "  private:\n";
  if (old_syntax)
            ...
            { 
              ...
              if (func->is_constructor)
                { 
                  if ((cls->binding == KDD_BIND_REF) ||
                           ...
                           (cls->binding == KDD_BIND_IFC2))
                    {
                      ...
                      implement_args_pass(func->args,num_args,out);
                      out << ");\n";
                      implement_native_ifc_to_mni(cls,"_kval_tmp","this",out);
                    }
                  else
                    assert(0);
                }
                       ...
                       (cls->binding == KDD_BIND_IFC2))
                {
                  ...
                  implement_args_pass(func->args,num_args,out);
                  out << ");\n";
                  implement_native_ifc_to_mni(cls,"_kval_cast","this",out);
                }
              else
                {
                  implement_result_assignment(func->return_type,out);
              ...
              implement_return_assignment(func,out,old_syntax);
              implement_args_cleanup(func->args,num_args,out);
                           ...
                           const char *aux_dir, bool old_syntax)
{
  ...
  generate_mni_implementation(header_files,classes,spaces,
                              mni_dir,aux_dir,old_syntax);
}
