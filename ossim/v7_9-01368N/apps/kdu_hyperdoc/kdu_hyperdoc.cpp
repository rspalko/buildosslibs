...
/*****************************************************************************/
/* STATIC                        open_namespace                              */
/*****************************************************************************/

static kdd_namespace *
  open_namespace(kdd_namespace *current, char *string)
  /* This function forms the name of a namespace by eliminating any trailing
     whitespace from `string', after which it looks on the list of descendants
     `current' to see if a namespace with the same name already exists.
     Otherwise, it adds the namespace.  In both cases, a pointer to the new
     namespace is returned. */
{
  assert(string != NULL);
  string = skip_white(string);
  char *ch_scan = string + strlen(string);
  while ((ch_scan > string) && (ch_scan[-1] == ' '))
    ch_scan--;
  char save_ch = *ch_scan;  *ch_scan = '\0';
  
  kdd_namespace *elt;
  for (elt=current->descendant_head; elt != NULL; elt=elt->next)
    if (strcmp(string,elt->name) == 0)
      break;
  if (elt == NULL)
    { 
      elt = new kdd_namespace(string);
      if (current->descendant_tail == NULL)
        current->descendant_tail = current->descendant_head = elt;
      else
        current->descendant_tail = current->descendant_tail->next = elt;
      elt->parent = current;
    }
  *ch_scan = save_ch;
  return elt;
...
/*****************************************************************************/
/* STATIC                       close_namespace                              */
/*****************************************************************************/

static kdd_namespace *close_namespace(kdd_namespace *current)
  /* Leaves the environment of the `current' namespace, returning a pointer
     to the parent namespace, which may be the global space.  It should not
     be possible to leave the global  namespace. */
{
...
static kdd_variable *
  start_var(char *string, kdd_variable * &list, kdd_class *owner,
            kdd_file *file, int line_number, kdd_namespace *space)
     ...
     of functions managed by `owner'. */
{
  ...
  *(arg_start++) = *arg_end = '\0';
  ...
  if (*arg_start == '*')
    return NULL; // Function pointer definition; skip these for now 
  ...
  while (arg_start < arg_end)
    {
      ...
      start_var(arg_start,tail->args,NULL,file,line_number,NULL);
      arg_start = delim;
    }
      ...
      if (bind_errors == 0)
        {
          ...
          else if (strcmp(scan,"interface2") == 0)
            { from = delim+1; return KDD_BIND_IFC2; }
            ...
            { kdu_error e;
              e << "Likely syntax error encountered in comment string at line "
                ...
                << "\tThe \"[BIND: ...]\" tag must take one of the forms, "
                   "\"[BIND: interface]\", \"[BIND: interface2]\", "
                   "\"[BIND: reference]\", \"[BIND: copy]\", "
                   "\"[BIND: donate]\", \"[BIND: callback]\" or "
                   "\"[BIND: no-java]\".";
            }
        }
...

{
  ...
  if ((check_val == KDD_BIND_IFC) || (check_val == KDD_BIND_IFC2) ||
      (check_val == KDD_BIND_REF) || (check_val == KDD_BIND_COPY))
    {
      if ((last_class == NULL) || (last_class->binding != KDD_BIND_NONE))
        { kdu_error e;
          e << "Likely syntax error encountered in comment at line "
            ...
            << "\t\"[BIND: interface]\", \"[BIND: interface2]\" and "
               "\"[BIND: reference]\" tags must be associated with a "
               "class/struct declaration -- perhaps the relevant class or "
               "structure has already been assigned a binding.";
        }
...
static kdd_file *
  process_file(char *filename, kdd_primitive *primitives, kdd_class *classes,
               kdd_namespace *global_namespace)
  /* Processes the contents of the file, returning a pointer to the newly
     created `kdd_file' object.  On entry, `classes' points to a list of all
     classes found so far, which is never empty because there is always a
     special class for global definitions.  On entry, `global_namespace'
     points to the root of the namespace hierarchy, that is a single
     namespace with NULL `parent' and `next' members and an empty `name'
     string. */
{
  ifstream stream;
  ...
  kdd_namespace *cur_namespace = global_namespace;
           ...
           from=skip_white(to), start_of_line=false)
        {
          if (in_comment)
          ...
          else if (*from == '}')
            {
              ...
              else if ((class_nesting == 0) &&
                       !cur_namespace->is_global())
                cur_namespace = close_namespace(cur_namespace);
              else
                { // Class/Struct/Union declaration terminator
                  if (class_nesting == 0)
                    { kdu_error e; e << "Unexpected '}' encountered on line "
                      ...
                      "structure definition, a function implementation or "
                      "a namespace scope."; }
                  class_nesting--;
                  if (class_nesting == 0)
                    {
            ...
            { // Statement terminator
              ...
              else if ((strncmp(string,"namespace ",10) == 0) &&
                       (*from == '{'))
                { 
                  cur_namespace = open_namespace(cur_namespace,string+10);
                  statement.reset();
                }
              else if ((strncmp(string,"using ",6) == 0) &&
                       (strncmp(skip_white(string+6),"namespace ",10) == 0) &&
                       (*from == ';'))
                { // For the moment, just discard "using namespace" statements
                  statement.reset();
                }
              else
                {
                          ...
                          else
                            {
                              ...
                              last_variable =
                                start_var(string,classes->variables,classes,
                                          file,line_number,cur_namespace);
                            }
                        }
                    }
...
void
  kdd_type::print(ostream &out, const char *name, bool full_namespace)
{
  ...
  if (known_primitive != NULL)
    { 
      if ((known_primitive->space != NULL) && full_namespace)
        known_primitive->space->print(out);
    ...
    { 
      if ((known_class->space != NULL) && full_namespace)
        known_class->space->print(out);
  ...
  if (known_primitive != NULL)
  ...
  else if (known_class != NULL)
    {
      if ((known_class->binding == KDD_BIND_IFC) ||
          (known_class->binding == KDD_BIND_IFC2))
        {
          if (for_return)
  ...
  if (owner->name != NULL)
    ...
    {       
      out << "<P>[Declared in <A HREF=\"" << file->html_name
          << "\">\"" << file->pathname << "\"</A>]</P>\n";
      if ((space != NULL) && !space->is_global())
        { 
          out << "<P>[<U>c++ namespace</U> = \"";
          space->print(out);
          out << "\"]</P>\n";
        }
    }
      ...
      for (kdd_variable *var=variables; var != NULL; var=var->next)
        if ((name != NULL) || (*(var->description.get()) != '\0'))
          { 
            ...
            out << "</H4><DIV CLASS=\"indented-text\">\n";
            if ((this->name == NULL) && (var->space != NULL))
              { // Global variable with namespace
                out << "<P>[<U>c++ namespace</U> = \"";
                var->space->print(out);
                out << "\"]</P>\n";
              }
...
void kdd_namespace::print(ostream &tgt)
{
  if ((parent != NULL) && !parent->is_global())
    parent->print(tgt);
  if ((name != NULL) && (*name != '\0'))
    tgt << name;
  tgt << "::";
...
/*****************************************************************************/
/*                        kdd_namespace::enter_namespace                     */
/*****************************************************************************/

void kdd_namespace::enter_namespace(ostream &tgt)
{
  if ((parent != NULL) && !parent->is_global())
    parent->enter_namespace(tgt);
  if (name != NULL)
    tgt << "namespace " << name << " {\n";
}

/*****************************************************************************/
/*                        kdd_namespace::leave_namespace                     */
/*****************************************************************************/

void kdd_namespace::leave_namespace(ostream &tgt)
{
  if (name != NULL)
    tgt << "} // namespace " << name << "\n";
  if ((parent != NULL) && !parent->is_global())
    parent->leave_namespace(tgt);
}

/*****************************************************************************/
/*                           kdd_namespace::print_using                      */
/*****************************************************************************/

void kdd_namespace::print_using(ostream &tgt)
{
  if ((parent != NULL) && (name != NULL) && (*name != '\0') && has_bindings)
    { 
      tgt << "using namespace ";
      if (!parent->is_global())
        parent->print(tgt);
  ...
  kdd_namespace *scan;
  for (scan=descendant_head; scan != NULL; scan=scan->next)
    scan->print_using(tgt);
}
  ...
  main(int argc, char *argv[])
{
  extern void make_java_bindings(kdd_file *header_files,
                                 kdd_class *classes, kdd_namespace *spaces,
  ...
  extern void make_managed_bindings(kdd_file *header_files, kdd_class *classes,
                                    kdd_namespace *spaces, const char *mni_dir,
                                    const char *aux_dir, bool old_syntax);
  ...
  kdd_namespace *namespaces = new kdd_namespace(NULL); // Global namespace
  kdd_file *files = NULL;
  for (string=args.get_first(); string != NULL; )
    {
      ...
      if (((delim = strrchr(string,'.')) != NULL) &&
        ...
        {
          kdd_file *new_file =
            process_file(string,primitives,classes,namespaces);
          if (new_file != NULL)
            {
              new_file->next = files;
              files = new_file;
  ...
  if (java_dir != NULL)
    {
      ...
      make_java_bindings(files,classes,namespaces,java_dir,jni_dir,aux_dir);
      delete[] java_dir;
      delete[] jni_dir;
    }
  if (mni_dir != NULL)
    {
      make_managed_bindings(files,classes,namespaces,mni_dir,managed_aux_dir,
                            old_managed_syntax);
      delete[] mni_dir;
    }
  if (java_aux_dir != NULL)
