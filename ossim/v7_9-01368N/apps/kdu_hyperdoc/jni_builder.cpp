     ...
     the callback functions. */
{
  ...
  if (cls->space != NULL)
    cls->space->enter_namespace(out);
  out << "class " JNI_DELEGATOR_PREFIX << cls->name
  ...
  out << "      {\n"
      ...
      << "          throw KDU_NULL_EXCEPTION;\n"
      << "        this->_self = NULL;\n"
      << "        this->_self_class = NULL;\n"
      << "      }\n";
  out << "    ~" JNI_DELEGATOR_PREFIX << cls->name << "()\n"
      << "      {\n"
      << "        if (this->_self != NULL)\n"
      << "          _jniEnv()->DeleteGlobalRef(_self);\n"
      << "        if (this->_self_class != NULL)\n"
      << "          _jniEnv()->DeleteGlobalRef(_self_class);\n"
      << "      }\n";
  out << "    void _init(JNIEnv *__env, jobject _self)\n"
      ...
      << "        if (this->_self_class == NULL)\n"
      << "          {\n"
      << "            jclass _class = __env->GetObjectClass(_self);\n"
      << "            this->_self_class = (jclass) __env->NewGlobalRef(_class);\n"
      << "            __env->DeleteLocalRef(_class);\n"
      << "          }\n"
      << "      }\n";
  ...
  implement_callback_binding(kdd_func *func, ostream &out)
{
  ...
  out << ")\n{\n";
  out << "  if ((_self == NULL) || (_self_class == NULL))\n";
  out << "    throw KDU_NULL_EXCEPTION;\n";
  out << "  try {\n";
  ...
  out << "      {\n";
  out << "        kdu_exception __kdu_exc = convertJavaException(__env,__cbk_exc);\n";
  ...
  generate_jni_header(kdd_class *classes, const char *jni_dir)
{
  ...
  out << "\n";
  out << "class kdu_jni_env_ref : public kdu_core::kdu_thread_object {\n";
  out << "  public: // Member functions\n";
  out << "    kdu_jni_env_ref(JNIEnv *env_ref, JavaVM *jvm_ref)\n";
  out << "      : kdu_core::kdu_thread_object(KDU_JNI_ENV_REF_ID)\n";
  out << "      {\n";
                              ...
                              const char *jni_dir, const char *aux_dir)
{
  ...
  spaces->print_using(out);
  out << "#include \"kdu_elementary.h\"\n";
  out << "using namespace kdu_core;\n"; // Need this for basic types
  ...
  // set whenever a C++ exception is caught. If badAlloc is true, then
  // OutOfMemoryException is thrown, otherwise a KduException is thrown.  If
  // this function is called with badAlloc set to false, then it should only
  // be called in a context where the calling java method is declared as
  // throwing KduException (i.e. regular methods, and not Native_create,
  // Native_destroy, Native_init or Native_init_class methods).
  out << "\n";
  ...
  // Utility function: generateJavaNullArgException
  ...
  // Utility function: convertJavaException is called from within callback
  // functions if an exception is encountered while trying to propagate the
  // call to the relevant derived Java object.
  out << "static kdu_exception convertJavaException(JNIEnv *__env, jthrowable __exc)\n";
  out << "{\n";
  out << "  kdu_exception result = KDU_NULL_EXCEPTION;\n";
  out << "  jclass java_oom_class = __env->FindClass(\"java/lang/OutOfMemoryError\");\n";  
  out << "  jclass kdu_exc_class = NULL;\n";
  out << "  if (__env->IsInstanceOf(__exc,java_oom_class))\n";
  out << "    result = KDU_MEMORY_EXCEPTION;\n";
  out << "  else if ((kdu_exc_class = __env->FindClass(\"kdu_jni/KduException\")) != NULL)\n";
  out << "    {\n";
  out << "      result = KDU_CONVERTED_EXCEPTION;\n";
  out << "      if (__env->IsInstanceOf(__exc,kdu_exc_class))\n";
  out << "        {\n";
  out << "          jfieldID field_id = __env->GetFieldID(kdu_exc_class,\"kdu_exception_code\",\"I\");\n";
  out << "          if (field_id != NULL)\n";
  out << "            result = (kdu_exception) __env->GetIntField(kdu_exc_class,field_id);\n";
  out << "        }\n";
  out << "    }\n";
  out << "  if (java_oom_class != NULL)\n";
  out << "    __env->DeleteLocalRef(java_oom_class);\n";
  out << "  if (kdu_exc_class != NULL)\n";
  out << "    __env->DeleteLocalRef(kdu_exc_class);\n";
  out << "  return result;\n";
  out << "}\n";
  ...
  for (cls=classes; cls != NULL; cls=cls->next)
    { 
      ...
      out << "  jclass lcl_class = __env->FindClass(\"kdu_jni/"
          << cls->java_name << "\");\n";
      out << "  if (lcl_class==NULL) throw KDU_NULL_EXCEPTION;\n";
      out << "  jclass kdu_class = (jclass) __env->NewGlobalRef(lcl_class);\n";
      out << "  __env->DeleteLocalRef(lcl_class);\n";
...
void make_java_bindings(kdd_file *header_files, kdd_class *classes,
                        kdd_namespace *spaces, const char *java_dir,
                        const char *jni_dir, const char *aux_dir)
{
  kdd_class *cls;
  for (cls = classes; cls != NULL; cls=cls->next)
  ...
  generate_jni_implementation(header_files,classes,spaces,jni_dir,aux_dir);
}
