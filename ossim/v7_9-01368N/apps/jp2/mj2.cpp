...
/*****************************************************************************/
/* EXTERN                   mj2_add_box_descriptions                         */
/*****************************************************************************/
namespace kdu_supp {
...
{
  return; // No special MJ2 textualization services implemented yet
}
  ...
  mj_chunk_offsets::load_from_box(jp2_input_box *stco)
{
  ...
  kdu_uint32 i, val1=0, val2=0, chunk_count=total_chunks;
  while (chunk_count > 0)
    {
  ...
  mj_sample_chunks::load_from_box(jp2_input_box *stsc)
{
  ...
  kdu_uint32 ver_flags=0, entry_count=0;
  ...
  kdu_uint32 first_chunk, prev_first_chunk=0;
  kdu_uint32 samples_per_chunk=0, description_idx=0;
                                     ...
                                     kdu_uint32 &sample_in_chunk)
{
  ...
  while ((current != NULL) && (current->run > 0) &&
  ...
  if ((current == NULL) || (current->num_samples < 1))
    { KDU_ERROR(e,0x16021607); e <<
      KDU_TXT("Attempting to access an MJ2 frame for which no chunk "
              "mapping is available in the file!  The file may be "
              "corrupt or invalid.");
    }
...
/*****************************************************************************/
/*                        mj_chunk_buf::start_rewrite                        */
/*****************************************************************************/

bool
  mj_chunk_buf::start_rewrite(kdu_long backtrack)
{
  if ((rewrite != NULL) || (backtrack < 0) || (tail == NULL))
    return false;
  rewrite = tail;  rewrite->rewrite_pos = rewrite->num_elts;
  while (backtrack > 0)
    { 
      if (backtrack <= (kdu_long) rewrite->rewrite_pos)
        { 
          rewrite->rewrite_pos -= (int) backtrack;
          backtrack = 0;
        }
      else
        { 
          backtrack -= rewrite->rewrite_pos;
          rewrite->rewrite_pos = 0;
          rewrite = rewrite->prev;
          if (rewrite == NULL)
            return false;
          rewrite->rewrite_pos = rewrite->num_elts;
        }
    }
  return true;
}
  ...
  mj_chunk_buf::store(const kdu_byte *buf, int num_bytes)
{
  if (rewrite != NULL)
    { // In rewrite section 
      while (num_bytes > 0)
        { 
          int xfer = rewrite->num_elts - rewrite->rewrite_pos;
          assert(xfer >= 0);
          if (xfer == 0)
            { 
              assert(rewrite != tail);
              assert(rewrite->next->prev == rewrite);
          ...
          if (xfer > num_bytes)
            xfer = num_bytes;
          memcpy(rewrite->elts+rewrite->rewrite_pos,buf,(size_t) xfer);
          num_bytes -= xfer; buf += xfer;
          rewrite->rewrite_pos += xfer;
        }
      return 0;
    }
  
  int saved_num_bytes = num_bytes;
  if (head == NULL)
...
kdu_uint32
  mj2_video_source::get_track_idx() const
{
...
kdu_int16
  mj2_video_source::get_compositing_order() const
{
...
kdu_int16
  mj2_video_source::get_graphics_mode() const
{
  return state->track->video_track->graphics_mode;
}
...
void
  mj2_video_source::get_geometry(double &presentation_width,
                                 ...
                                 double matrix[], bool for_movie) const
{
...
void
  mj2_video_source::get_cardinal_geometry(kdu_dims &pre_dims,
                                          ...
                                          bool &hflip, bool for_movie) const
{
...
kdu_long
  mj2_video_source::get_frame_instant(int frame_idx)
{ 
  if ((frame_idx < 0) || (frame_idx >= (int) state->read_state.num_frames))
    return 0;
  mj_sample_times tmp_times;
  tmp_times.make_copy(&state->track->sample_times);
  return (kdu_long) tmp_times.seek_to_sample(frame_idx);
}
...
kdu_long
  mj2_video_source::get_frame_period(int frame_idx)
{ 
  if ((frame_idx < 0) || (frame_idx >= (int) state->read_state.num_frames))
    return 0;
  mj_sample_times tmp_times;
  tmp_times.make_copy(&state->track->sample_times);
  tmp_times.seek_to_sample(frame_idx);
  return (kdu_long) tmp_times.get_period();
}
...
int
  mj2_video_source::get_stream_idx(int field_idx) const
{ 
  ...
  mj2_video_source::open_stream(int field_idx, jp2_input_box *input_box)
{
  ...
  mj_movie *movie = track->movie;
  mj_video_read_state *rs = &(state->read_state);
  jp2_family_src *src = movie->get_src();
  if ((rs->next_frame_idx >= rs->num_frames) ||
      (field_idx < 0) || (field_idx >= (int) rs->fields_per_frame))
    return -1;
  ...
  // Find the relevant codestream's location
  kdu_long field0_len, frame_pos;
  kdu_uint32 frame_idx;
  src->acquire_lock();
  try {
    ...
    field0_len = rs->next_field0_len;
    frame_pos = rs->next_frame_pos;
    frame_idx = rs->next_frame_idx;
  } catch (...) {
    src->release_lock();
    throw;
  }
  ...
  int need_field1_pos = 0;
  if (field_idx == 1)
    { 
      if (field0_len > 0)
        frame_pos += field0_len;
      else
        need_field1_pos = 1;
    }
  for (; need_field1_pos >= 0; need_field1_pos--)
    { // Need to open two frames if we still need to find the field 1 position
      jp2_locator locator;
      locator.set_file_pos(frame_pos);
      if (!input_box->open(src,locator))
        { KDU_ERROR(e,54); e <<
      ...
      field0_len = input_box->get_box_bytes();
      if (need_field1_pos)
        { 
          frame_pos += field0_len;
          input_box->close();
  ...
  if (field_idx == 0)
    { // Try to record the `field0_len' value that we found above in the
      // `rs->next_field0_len' member, in case we need to open field 1 next
      // time.
      src->acquire_lock();
      if (rs->next_frame_idx == frame_idx) // Otherwise somebody changed it
        rs->next_field0_len = field0_len;
      src->release_lock();
    }
...
/*****************************************************************************/
/*                      mj2_video_target::start_rewrite                      */
/*****************************************************************************/

bool
  mj2_video_target::start_rewrite(kdu_long backtrack)
{
  if (!state->field_open)
    return false;
  return state->write_state.chunk_buf.start_rewrite(backtrack);
}

/*****************************************************************************/
/*                       mj2_video_target::end_rewrite                       */
/*****************************************************************************/
  ...
  mj2_video_target::end_rewrite()
{
  if (!state->field_open)
    return false;
  return state->write_state.chunk_buf.end_rewrite();
}
  ...
  mj2_video_target::write(const kdu_byte *buf, int num_bytes)
{
  ...
  int new_bytes = state->write_state.chunk_buf.store(buf,num_bytes);
  state->write_state.field_sizes[state->write_state.field_idx] +=
    (kdu_uint32) new_bytes;
  return true;
}
  ...
  for (scan=state->tracks; scan != NULL; scan=scan->next)
    { 
      ...
      if (vt == NULL)
        continue;
      if (vt->first_codestream_id < 0)
        vt->first_codestream_id = count; // First time filling in details
