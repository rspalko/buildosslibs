...
// be private to the "kdu_expand" demo app.
...
class kde_flow_control {
  ...
  public: // Local definitions: some compilers complain if these are private
    ...
    struct kde_channel {
        ...
        kdu_sample16 *lut16; // Exactly one of these is non-NULL if this
        kdu_sample32 *lut32; // channel uses a palette, else both are NULL.
        kdu_line_buf palette_output; // Created only for use with palettes.
        kdu_image_out writer;
      };
      /* Notes:
            When a palette is used, the `lut16' or `lut32' array is non-NULL
         and points to an array with 2^{palette_bits} entries, where
         `palette_bits' is found in the `source_component' object referenced
         from this channel.  The actual number of palette entries need not be
         a power of 2, in which case the original LUT is extended by
         replicating the last entry.  When palette mapping is involved, the
         `source_component's `palette_indices' line buffer will be created,
         having a 16-bit representation, into which the palette indices will
         be written.  The palette output consists of 16- or 32-bit absolute
         integers, not floating-point or fixed-point relative sample values.
         This allows the palette LUT to preserve floating point bit-patterns
         if a JPX pixel format box indicated that the palette entries are
         float-formatted.. */
