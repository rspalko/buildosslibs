  ...
  print_usage(char *prog, bool comprehensive=false)
{
  ...
  if (comprehensive)
    out << "\tOne or more output files. If multiple files are provided, "
           ...
           "image (`Nprecision' parameter attribute).  You can make this "
           "happen by providing the `-fprec' argument, described below.\n";
  out << "-fprec [<comp-0 precision>][M|L|F<E>][,[<comp-1 precision>]...]\n";
  if (comprehensive)
    out << "\tYou can use this argument to adjust the way in which sample "
           ...
           "comma-separated list of precision specifiers.  The list can "
           "contain more specifiers than the number of image components in "
           "the collection of input files.  If it contains fewer values, the "
           "last value is replicated as required to provide precision forcing "
           "information for all image components.  You can supply the special "
           "value of 0 for any component (including the last one in the "
           "comma-separated list, which gets replicated), meaning that "
           "the original precision should be left untouched.  You can "
           "achieve the same thing by omitting everything for the component, "
           "so long as it is not the last one.  Thus, for example \"16,0,8M\" "
           "has the same meaning as \"16,,8M\".\n"
           "\t\tThe optional suffix L, M or F<E> determines how any precision "
           "forcing conversion is performed.  `M' means sample values (at "
           ...
           "preserved.\n"
           "\t\tThe F<E> suffix means that the decoded sample values, at "
           "their original precision N, should be considered as N-bit "
           "integers whose bits should be re-interpreted as N-bit floating "
           "point values, with E exponent bits and N-1-E mantissa bits.  "
           "This (potentially custom) floating point representation "
           "involves a sign bit, followed by a true (signed) exponent that "
           "has been offset by 2^{E-1}-1 so as to leave an unsigned E-bit "
           "integer, follwed by the N-1-E mantissa bits.  Depending on the "
           "output file format, the re-interpreted floating point values "
           "usually need to be converted to integer image samples during "
           "image writing; the precision of these written image samples is "
           "the P value to which the F<E> suffix is appended.  It is "
           "allowed (even encouraged) to omit the P value altogether, so "
           "that the conversion specifier for a component reduces to \"F5\" "
           "or \"F8\", for example.  In this case, the file writer "
           "is free to select the most appropriate value for P, based on "
           "the file format and (potentially) the original input precision.  "
           "For example, an 8-bit/sample image file format would "
           "automatically select P=8, a raw file writer would automatically "
           "select P=N and a TIFF file writer would automatically select "
           "P=32.\n"
           "\t\tIn the case of signed representations, the conversion from "
           "re-interpreted floats identified via the F<E> expression to "
           "P-bit integer image samples is performed after multiplying by "
           "2^P-1, so that floating point values in the range 0.0 to 1.0 map "
           "to image sample values in the range 0 to 2^P-1.  When writing "
           "signed image sample values, the scaling factor is 2^{P-1}-1, so "
           "that the full range of P-bit signed integers corresponds to "
           "floating point values from (approximately) -1.0 to (exactly) "
           "1.0.  If the output image file can store floating point values "
           "directly, the conversion to scaled integers is not required -- "
           "TIFF files, for example, can record floating point values "
           "directly.  In general, however, the written format may involve "
           "a different number of exponent or mantissa bits -- e.g., "
           "IEEE 32-bit floats might be written from data with N=16 and "
           "E=5 (half-floats).  For raw output files, the N-bit floating "
           "point source values with E exponent bits are converted to P-bit "
           "floating point values with the same number of exponent bits and "
           "then written to the output file.\n"
           "\t\tIn practice, you would not normally specify an F<E> suffixed "
           "precision forcing specifier explicitly, unless you are working "
           "with raw codestreams.  Data that have been compressed as "
           "integer-reinterpreted floats should generally have been "
           "compressed using the Part-2 non-linear transform feature, "
           "represented by NLT marker segments that specify the SMAG type "
           "code (sign-magnitude to 2's-complement transform).  NLT marker "
           "segments require Part-2 support, which means that the resulting "
           "codestream should have been embedded in a JPX file, in which "
           "case the \"Pixel Format\" box should provides sufficient "
           "information for this demo-app to recover the floating point "
           "format.  In this case, the F<E> suffix is effectively added for "
           "you, even if you do not specify `-fprec' at all.\n"
           ...
           "with `Nprecision' = 4 to a PGM file, the samples are clipped to "
           ...
           "you save data with `Nprecision'=11 to a TIFF file, the 11 bits of "
           ...
           "\t\tAnother useful application of the `-fprec' option is to "
           "force raw files containing samples with 17 to 24 bits of "
           "precision to be saved into 32-bit words, rather than 24 bit "
           "words, simplifying the interaction with other applications.\n";
  ...
  if (comprehensive)
    out << "\tUse this argument to gain explicit control over "
           ...
           "processing (`T' suffix) or block decoding operations "
  ...
  if (comprehensive)
    out << "\tTimes end-to-end execution and, optionally, the block decoding "
           ...
           "not be at all reliable unless `-num_threads' is 0 or 1.  Since "
           "the default value for the `-num_threads' argument may be greater "
           "than 1, you should explicitly set the number of threads to 0 "
  ...
  out << "-no_res -- prevents resolution tags from being written\n";
  if (comprehensive)
    out << "\tThis is probably useful only when writing TIFF files that "
           "cannot be opened using OSX Preview due to a bug in that app "
           "which causes it to reject files containing Geo referencing and "
           "resolution tags at the same time.\n";
  out << "-com -- requests the printing of textual codestream comments.\n";
  out << "-no_decode -- can only be used if no `-o' argument is supplied.\n";
  if (comprehensive)
    out << "\tIf this option is supplied, no actual decompression will occur; "
           "only the codestream main header will be parsed for errors.  This "
           "could be useful in conjunction with the `-com' argument, allowing "
           "codestream comments to be printed without unnecessary extra "
           "processing.  By default, decompression is performed even if there "
           "is no output file supplied via `-o'.\n";
  out << "-s <switch file>\n";
  if (comprehensive)
...
static kde_file_binding *
  parse_simple_args(kdu_args &args, char * &ifname,
                    ...
                    int &num_coding_threads, int &double_buffering_height,
                    int &progress_interval, int &cpu_iterations,
                    bool &simulate_parsing, bool &mem, bool &stats,
                    bool &dont_save_resolution, bool &want_comments,
                    bool &no_decode, bool &quiet)
     ...
     block coding processes, respectively. */
{
  ...
  if (args.find("-no_res") != NULL)
    { 
      dont_save_resolution = true;
      args.advance();
    }
  ...
  if (args.find("-no_decode") != NULL)
    { 
      if (files != NULL)
        { kdu_error e; e << "\"-no_decode\" argument can only be used if "
          "no output file has been supplied via \"-o\".\n"; }
      no_decode = true;
      args.advance();
    }
  ...
  parse_forced_precisions(kdu_args &args, kdu_image_dims &idims)
{
  ...
  do {
    ...
    int exponent_bits=0;
    if (len > 0)
      { // Else this component has no precision forcing specification
        size_t prec_len = strspn(string,"0123456789");
        assert(prec_len <= len);
        if (prec_len == 0)
          precision = -1;
        else if ((sscanf(string,"%d",&precision) != 1) ||
                 (precision < 0) || (precision > 64))
          { kdu_error e; e << "Malformed `-fprec' argument.  Forced presision "
            "values should lie in the range 1 to 64, if supplied."; }
        bool have_M=false, have_L=false, have_F=false;
        if ((string[prec_len] == 'M') && (string[prec_len+1] == '\0') &&
          ...
          { kdu_error e; e << "Malformed `-fprec' argument.  Expected a comma "
            "separated list of forcing precision values P, each optionaly "
            "followed by an `L', `M' or `F<E>' suffix, where E in the `F<E>' "
            "suffix must be a positive integer, and P may be omitted only "
            "if `F<E>' is used.  The problem specifier is:\n\t\"" << string <<
            "\".";
          }
        align_lsbs = have_L || !(have_M || have_F);
      }
     ...
     geometry, which is why we need the `transpose' argument. */
{
  ...
  // Start by seeing whether we have capture resolution info, or only display
  // resolution info.
  bool for_display=false, have_absolute_res=true;
  ...
  if (for_display && (ypels_per_metre < 10.0))
    have_absolute_res = false; // Don't trust the absolute resolution info
                          ...
                          bool alpha_is_premultiplied)
{
  ...
  const kdu_byte *icc_profile=NULL;
  if ((space == JP2_iccLUM_SPACE) ||
      (space == JP2_iccRGB_SPACE) ||
      (space == JP2_iccANY_SPACE))
    { 
      colour_space_confidence = 0; // Just means that `space' is incomplete
      icc_profile = colour.get_icc_profile();
    }
                      ...
                      kdu_thread_queue *env_queue)
{
  ...
  this->create_flags = KDU_MULTI_XFORM_DEFAULT_FLAGS;
  if (!allow_shorts)
    create_flags |= KDU_MULTI_XFORM_PRECISE;
  if (skip_ycc)
    create_flags |= KDU_MULTI_XFORM_SKIPYCC;
  if (dwt_double_buffering)
    create_flags |= KDU_MULTI_XFORM_MT_DWT;
  if (env != NULL)
    create_flags |= KDU_MULTI_XFORM_DELAYED_START;
  ...
  this->max_buffer_memory =
    engine.create(codestream,tile,env,env_queue,create_flags,
                  dwt_stripe_height);
  ...
  for (c=0; c < num_channels; c++)
    { 
      ...
      if (channel_mapping.exists())
        {
          int cmp, plt_cmp, stream, fmt, i;
          bool success;

          if ((c==(num_channels-1)) && last_channel_is_alpha)
            success = (alpha_is_premultiplied)?
              channel_mapping.get_premult_mapping(0,cmp,plt_cmp,stream,fmt):
              channel_mapping.get_opacity_mapping(0,cmp,plt_cmp,stream,fmt);
          else 
            success = channel_mapping.get_colour_mapping(c,cmp,plt_cmp,
                                                         stream,fmt);
          if (!success)
            { kdu_error e; e << "Error recovering channel mapping info from "
                "JP2/JPX file.  The file might not have been written "
                "correctly."; }
          ...
          if (plt_cmp >= 0)
            { // Set up palette lookup table.
              ...
              if (palette.get_bit_depth(plt_cmp) <= 14)
                { // Use 16-bit absolute palette
                  chnl->lut16 = new kdu_sample16[(int)(1<<palette_bits)];
                  palette.get_abs_lut(plt_cmp,chnl->lut16);
                  for (i=num_entries; i < (1<<palette_bits); i++)
                    chnl->lut16[i] = chnl->lut16[num_entries-1];
                }
              else
                { // Use 32-bit absolute palette
                  chnl->lut32 = new kdu_sample32[(int)(1<<palette_bits)];
                  palette.get_abs_lut(plt_cmp,chnl->lut32);
                  for (i=num_entries; i < (1<<palette_bits); i++)
                    chnl->lut32[i] = chnl->lut32[num_entries-1];
                }
            }
        }
      else
...
kde_flow_control::~kde_flow_control()
{
  ...
  for (int c=0; c < num_channels; c++)
    { 
      kde_channel *chnl = channels + c;
      if (chnl->lut16 != NULL)
        { delete[] (chnl->lut16); chnl->lut16 = NULL; }
      if (chnl->lut32 != NULL)
        { delete[] (chnl->lut32); chnl->lut32 = NULL; }
    }
  delete[] channels;
  if (engine.exists())
    engine.destroy();
  ...
  for (int c=0; c < num_channels; c++)
    {
      ...
      if (comp->ratio_counter < 0)
        { // May need to perform channel mapping
          if (chnl->lut16 != NULL)
            { 
              kdu_sample16 *lut = chnl->lut16;
          ...
          else if (chnl->lut32 != NULL)
            { 
              kdu_sample32 *lut = chnl->lut32;
              kdu_sample32 *dp = chnl->palette_output.get_buf32();
              kdu_sample16 *sp = comp->palette_indices.get_buf16();
              assert(sp != NULL);
              for (int i=chnl->width; i > 0; i--, sp++, dp++)
                *dp = lut[sp->ival];              
  ...
  kde_flow_control::access_decompressed_line(int channel_idx)
{
  ...
  if (comp->ratio_counter >= 0)
    return NULL;
  if ((chnl->lut16 == NULL) && (chnl->lut32 == NULL))
    return comp->line;
  else
...
int main(int argc, char *argv[])
{
  ...
  kde_file_binding *outputs =
    parse_simple_args(args,ifname,record_stream,max_bpp,
                      ...
                      max_layers,discard_levels,num_threads,num_xform_threads,
                      num_coding_threads,double_buffering_height,
                      progress_interval,cpu_iterations,simulate_parsing,
                      mem,stats,dont_save_resolution,want_comments,
                      no_decode,quiet);
    ...
    { // We have an open JP2/JPX-compatible file.
      ...
      if (raw_codestream >= 0)
      ...
      else
        {
          ...
          int cmp, plt, stream_id, fmt;
          channels.get_colour_mapping(0,cmp,plt,stream_id,fmt);
  ...
  for (n=0; n < num_channels; n++)
    { 
      ...
      int format=JP2_CHANNEL_FORMAT_DEFAULT, fmt_params[3]={0,0,0};
      if (channels.exists())
        { 
          channels.get_colour_mapping(n,cmp,plt,stream_id,format,fmt_params);
      ...
      if (format == JP2_CHANNEL_FORMAT_FLOAT)
        { // Pixel format box indicates decoded integers should be
          // re-interpreted as floats, so we will tell the file writers about
          // this but not commit them to a particular output precision.
          int force_prec=0, exp_bits=0; bool align_lsbs=false;
          if (!(force_prec=idims.get_forced_precision(n,align_lsbs,exp_bits)))
            force_prec = -1; // We preserve force-prec if the user set one
          exp_bits = fmt_params[0]; // Override any user-specified value
          idims.set_forced_precision(n,force_prec,align_lsbs,exp_bits);
        }
    }
  ...
  if (want_alpha && channels.exists())
    { 
      int cmp=-1, plt=-1, stream_id, format=0, fmt_params[3]={0,0,0};
      ...
      if (last_channel_is_alpha)
        { 
          ...
          idims.add_component(dims.size.y,dims.size.x,precision,is_signed);
          if (format == JP2_CHANNEL_FORMAT_FLOAT)
            { // As above, we need to tell the file writer(s) that the
              // channel's samples are to be re-interpreted as floating point
              // values.
              int force_prec=0, exp_bits=0; bool align_lsbs=false;
              if (!(force_prec =
                    idims.get_forced_precision(n,align_lsbs,exp_bits)))
                force_prec = -1; // We preserve force-prec if the user set one
              exp_bits = fmt_params[0]; // Override any user-specified value
              idims.set_forced_precision(n,force_prec,align_lsbs,exp_bits);
            }
  ...
  // Find out which output components from the codestream are actually used;
  // also check for an appropriate handling of components that may have an
  // associated SMAG/UMAG non-linear point transform.
  int num_used_components = 0;
  int *used_component_indices = new int[num_output_channels];
  kdu_params *nlt_cluster=codestream.access_siz()->access_cluster(NLT_params);
  bool nlt_warning_issued=false;
  for (n=0; n < num_output_channels; n++)
    { 
      int c, cmp, format=JP2_CHANNEL_FORMAT_DEFAULT, plt=-1;
      ...
      if ((n==(num_channels-1)) && last_channel_is_alpha)
        ...
        { 
          int stream_id=0;
          channels.get_colour_mapping(n,cmp,plt,stream_id,format);
        }
      else
      ...
      int exp_bits=0;
      bool align_lsbs=false;
      if (!idims.get_forced_precision(cmp,align_lsbs,exp_bits))
        exp_bits = 0; // Just in case
      
      kdu_params *nlt=nlt_cluster->access_relation(-1,cmp,0,true); // read-only
      assert(nlt != NULL);
      int nl_type = 0;
      if (nlt->get(NLType,0,0,nl_type) &&
        ...
        { 
          if (plt >= 0)
            { kdu_warning w; w << "The codestream defines a non-linear point "
              "transform (NLT) of type SMAG or UMAG, which means almost "
              "certainly that the content of that component (i.e., " << cmp <<
              ") is representing floating point data whose bits have been "
              "re-interpreted as integers in sign-magnitude format for "
              "efficient compression.  Yet, the component is being used as "
              "input to a palette lookup table, which makes very little "
              "sense!";
            }
          else if ((exp_bits <= 0) && (outputs != NULL) && !nlt_warning_issued)
            { kdu_warning w; w << "The codestream defines a non-linear point "
              "transform (NLT) of type SMAG or UMAG, which means almost "
              "certainly that the content of that component (i.e. " << cmp <<
              ") is representing floating point data whose bits have been "
              "re-interpreted as integers in sign-magnitude format for "
              "efficient compression.  To get meaningful decompressed output, "
              "you should use the `-fprec' option with the \"F<E>\" form of "
              "specifier, to force recovery of floats from the decompressed "
              "integers.  If unsure of the number of exponent bits, try "
              "expanding with \"-fprec F5\" (half-floats) and \"-fprec F8\" "
              "(IEEE single precision floats) and viewing the resulting "
              "imagery.  If your source is a JPX file with properly embedded "
              "Pixel Format box, you should not be getting this warning.";
              nlt_warning_issued = true;
            }
        }
      else
        { 
          if ((exp_bits > 0) && (plt < 0) && !nlt_warning_issued)
            { kdu_warning w; w << "You appear to have supplied an `-fprec' "
              "specification for output channel " << n << ", using the "
              "\"F<E>\" format to force re-interpretation of decompressed "
              "integer values as floating point bit-patterns.  "
              "This is almost certainly inappropriate, since the associated "
              "output component from the codestream (i.e., " << cmp <<
              ") does not specify a non-linear point transform of the SMAG "
              "or UMAG variety.  You may be getting this warning because "
              "a JPX source file contains a Pixel Format box that wrongly "
              "suggests the floating point re-interpretation of sample values "
      ...
      else if (!no_decode)
        {
          ...
          if (num_xform_threads | num_coding_threads)
            pretty_cout << "\t"
              ...
              << num_coding_threads << " prefer to do block decoding;\n\t"
              << num_threads-num_xform_threads-num_coding_threads
