          ...
          else
            {
              ...
              if ((code == KDU_SOT) || (code == KDU_SOP) ||
                ...
                { // Marker code has associated marker segment.
                  ...
                  int read_length = 0;
                  if (in_box != NULL)
                    read_length = in_box->read(buf,length);
                  else
                    read_length = (int) fread(buf,1,length,in_file);
                  if (read_length != (int) length)
                    { kdu_error e; e << "Marker appears to have corrupt "
                      "length value -- attempted to read past end of file."; }
                }
              seg.end_seg(length);
              if (code == KDU_SOD)
                in_tile_body = true;
