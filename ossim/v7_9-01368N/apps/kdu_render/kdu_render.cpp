...
static void render_demo_1(const char *in, const char *out, float scale,
                          float limiter_wrmse, float limiter_ppi,
                          bool true_zero, bool true_max);
  ...
  print_usage(char *prog, bool comprehensive=false)
{
  ...
  out << "-qlimit <weighted rmse>[@<display ppi>]\n";
  if (comprehensive)
    out << "\tThis argument may be used to introduce a somewhat disciplined "
           "quality limiting procedure into the rendering process, which is "
           "intended to allow lower complexity rendering, in exchange for "
           "some loss of visual quality.  When decompressing content at the "
           "originally intended resolution, quality limiting might not make "
           "much sense, or even much difference (depending on parametesr).  "
           "However, when rendering high resolution content at coarse "
           "(low resolution) scales, the quality of the subband data that "
           "is used is almost invariably much higher than required to "
           "produce a high quality rendition at the small scale.  This is "
           "because the precision with which subbands are compressed "
           "tends to grow as a function of their depth in the "
           "multi-resolution pyramid -- lower frequency subbands require "
           "much higher precision when they are used in the reconstruction "
           "of high resolution content than they do when used in the "
           "reconstruction of lower resolution content.  This is where a "
           "quality limiter can help avoid unnecessary decompression "
           "effort.  The first parameter may be interpreted as a bound on "
           "the root-mean-squared error of the reconstructed result, "
           "assessed with respect to samples that have been normalized "
           "to a unit nominal range (0 to 1).  The second argument, if "
           "present, is used to derive approximate visual weights, that "
           "convert the RMSE bound to a weighted RMSE target, where higher "
           "frequencies may receive a weight less than 1 (modelling "
           "optical attenuation in the human visual system plus reduced "
           "sensitivity to high frequency chromatic content than "
           "high frequency luminance content.  Visual weights are derived "
           "from a \"pixels-per-inch\" specification for display of the "
           "rendered output image, at an assumed viewing distance of "
           "around 15cm.\n";
  ...
  if (comprehensive)
    out << "\tCurrently takes an integer in the range 1 through 6, "
           ...
           "demo 5.  All of the demos essentially do the same thing, but "
           ...
           "environment with one thread of execution for each physical "
           "or virtual CPU that the system can detect.  Finally, demo 6 "
           "is the same as demo 5, except that it runs Kakadu's powerful "
  ...
  out << "-true_zero -- force signed originals to have 0 map to black\n";
  if (comprehensive)
    out << "\tThis and the \"-true_max\" option allow you to control calls to "
           "`kdu_region_decompressor::set_true_scaling'.  The "
           "default procedure is to map originally signed source samples in "
           "such a way that 0 maps to a mid-level grey, allowing "
           "visualisation of both +ve and -ve values; this can be useful when "
           "rendering scientific data.  Signed source samples are strongly "
           "discouraged for content that represents photographic colour, but "
           "if it has been used in this way, the \"-true_zero\" option here "
           "allows you to render it so that 0 corresponds to the natural "
           "black level (or achromatic point for chrominance data) of the "
           "relevant colour space.\n";
  out << "-true_max -- force exact inter-precision mapping of max intensity\n";
  if (comprehensive)
    out << "\tThis and the \"true_zero\" option allow you to control calls to "
           "`kdu_region_decompressor::set_true_scaling'.  The  default "
           "procedure avoids scaling source sample values by factors other "
           "than powers of 2, as often as possible, which means that "
           "the maximum intensity associated with a source sample that has "
           "one precision P, might not correspond exactly with the maximum "
           "intensity of the mapped output at the rendered output bit-depth "
           "B.  This argument allows you to force exact correspondence.\n";
  out << "-cpu [<num_threads>]\n";
  if (comprehensive)
...
int main(int argc, char *argv[])
{
  ...
  float scale=1.0F;
  float limiter_wrmse=-1.0F, limiter_ppi=-1.0F;
  ...
  bool true_zero=false, true_max=false;
  try { // Any errors generated inside here will produce exceptions
      ...
      if (args.find("-qlimit") != NULL)
        { 
          const char *string = args.advance();
          char *end_ptr=NULL;
          if ((string==NULL) ||
              ((limiter_wrmse=(float)strtod(string,&end_ptr)) <= 0.0f) ||
              (end_ptr == string) ||
              ((*end_ptr != '\0') &&
               ((*end_ptr != '@') ||
                ((limiter_ppi=(float)strtod(end_ptr+1,&end_ptr)) <= 0.0f) ||
                (*end_ptr != '\0'))))
            { kdu_error e; e << "The `-qlimit' argument requires one or "
              "two positive real-valued parameters, separated by an `@'."; }
          args.advance();
        }
      ...
      if (args.find("-true_zero") != NULL)
        { 
          args.advance();
          true_zero = true;
        }
    
      if (args.find("-true_max") != NULL)
        { 
          args.advance();
          true_max = true;
        }
      ...
      switch (demo) {
        case 0: render_demo_cpu(in_fname,num_threads,scale,
                                limiter_wrmse,limiter_ppi,
                                true_zero,true_max); break;
        case 1: render_demo_1(in_fname,out_fname,scale,
                              limiter_wrmse,limiter_ppi,
                              true_zero,true_max);  break;
        case 2: render_demo_2(in_fname,out_fname,scale,
                              limiter_wrmse,limiter_ppi,
                              true_zero,true_max);  break;
        case 3: render_demo_3(in_fname,out_fname,scale,ovl_intensity,
                              limiter_wrmse,limiter_ppi,
                              true_zero,true_max);  break;
        case 4: render_demo_4(in_fname,out_fname,scale,ovl_intensity,
                              limiter_wrmse,limiter_ppi,
                              true_zero,true_max);  break;
        case 5: render_demo_5(in_fname,out_fname,scale,ovl_intensity,
                              limiter_wrmse,limiter_ppi,
                              true_zero,true_max);  break;
        case 6: render_demo_6(in_fname,out_fname,scale,ovl_intensity,
                              limiter_wrmse,limiter_ppi,
                              true_zero,true_max);  break;
...
static void
  ...
  /* This function demonstrates decompression of a raw codestream
     ...
     the various overloaded `kdu_region_decompressor::process' functions.
        The last two arguments are used to demonstrate quality limiting
     during the rendering process, allowing speed-quality trade-offs that
     are especially relevant when rendering high resolution content at
     coarse scales (low resolution) where the original compressed quality
     is almost always unnecessarily high.  If `limiter_wrmse' is <= 0, there
     is no quality limiting; if `limiter_ppi' <= 0 no visual weighting is
     applied so that the `limiter_wrmse' is actually a root-mean-squared
     error bound on the rendered content, where samples are interpreted as
     having been normalized to a unit nominal range (0 to 1).  If both
     quantities are positive, the `limiter_ppi' vaue represents an
     assumed display resolution, measured in rendered pixels-per-inch, at
     an assumed viewing distance of 15cm.  The `limiter_ppi' can readily be
     scaled to accommodate different viewing distances, since visual
     resolution depends only on the angle subtended by a pixel at the eye. */
{
  ...
  // Configure quality limiting
  kdu_region_decompressor decompressor;
  if (limiter_wrmse > 0.0f)
    { 
      kdu_quality_limiter limiter(limiter_wrmse,false);
      decompressor.set_quality_limiting(&limiter,limiter_ppi,limiter_ppi);
    }
  
  // Configure intensity scaling -- `kdu_region_compositor' follows these
  // steps internally.
  if (true_zero || true_max)
    decompressor.set_true_scaling(true_zero,true_max);
  else
     ...
     decompressed. */
{
  ...
  if (limiter_wrmse > 0.0f)
    { 
      kdu_quality_limiter limiter(limiter_wrmse,false);
      compositor.set_quality_limiting(&limiter,limiter_ppi,limiter_ppi);
    }
  
  // Configure intensity scaling
  compositor.configure_intensity_scaling(true_zero,true_max);
     ...
     multi-threaded processing environment. */
{
  ...
  kdu_dims image_dims;
  kdu_compositor_buf *buf=NULL;
  try { // Just so we can control the order in which the multi-threaded
        // processing environment and the compositor get destroyed.
  
    // Render the image to the frame buffer created by `kdu_region_compositor'.
    // You can override the buffer allocation method to create frames with the
    // configuration of your choosing.
    kdu_dims new_region;
    bool vflip=true; // Flip to generate BMP buffer in native top-down order
    do { // Loop around in the odd event that the number of discard
      // levels is found to be too large after processing starts.
      compositor.set_scale(false,vflip,false,scale);
      if (!compositor.get_total_composition_dims(image_dims))
        { // The following code adjusts geometry and/or scale in response to
          // some extremely unlikely corner cases.
          int code = compositor.check_invalid_scale_code();
          if (code & (KDU_COMPOSITOR_SCALE_TOO_SMALL |
                      KDU_COMPOSITOR_SCALE_TOO_LARGE))
            scale=compositor.find_optimal_scale(kdu_dims(),scale,scale,scale);
          if ((code & KDU_COMPOSITOR_CANNOT_FLIP) && vflip)
            { 
              kdu_warning w; w << "This Part-2 JPEG2000 image can't be "
                "vertically flipped so as to write a BMP file (these are "
                "upside down) in native order.  Of course we can write it "
                "backwards, but this is a simple demo.  To make sure you get "
                "at least some output the image will be rendered without "
                "flipping -- and hence written upside down.";
              vflip = false;
            }
          else if (code & KDU_COMPOSITOR_CANNOT_FLIP)
            { 
              kdu_error e; e << "The current multi-codestream composition "
                "cannot be rendered in any orientation because it requires "
                "codestreams to be flipped in incompatible ways, while the "
                "codestreams use advanced Part-2 decomposition structures "
                "which happen to make in-place geometric flipping "
                "impossible.";
            }
          compositor.set_scale(false,vflip,false,scale);
          compositor.get_total_composition_dims(image_dims);
  ...
  } catch (kdu_exception exc_code) {
    env.handle_exception(exc_code);
    compositor.set_thread_env(NULL,NULL); // Retract multi-threaded environment
    env.destroy(); // Must do this before deleting resources (see documentation
                   // for `kdu_thread_entity::handle_exception'), which will
                   // happen as the stack frame is unwound.
    throw exc_code;
  } catch (...) { // Handle any other exceptions here
    env.handle_exception(KDU_NULL_EXCEPTION);
    compositor.set_thread_env(NULL,NULL);
    env.destroy();
    throw;
  }
  compositor.set_thread_env(NULL,NULL); // Best to retract the multi-threaded
    // processing environment from `compositor' here before calling
    // `env.destroy', because the `compositor' will not be destroyed until
    // it goes out of scope.  Alternatively, if `compositor' was allocated on
    // the heap, you can destroy it first and then destroy the multi-threaded
    // processing environment.
  env.destroy(); // Not strictly necessary, since the destructor will call it
                 // and there was no exception.

  pretty_cout << "Rendering finished, using "
...
static void
  render_demo_6(const char *in_fname, const char *out_fname, float scale,
                ...
                float limiter_wrmse, float limiter_ppi,
                bool true_zero, bool true_max)
  /* Same as `render_demo_5', except that all the rendering is done to
     ...
     required. */
{
  ...
  compositor.set_thread_env(&env,NULL); // As in demo-5
  ...
  } catch (kdu_exception exc_code) { // Exceptions handled as in demo-5
    env.handle_exception(exc_code);
    compositor.set_thread_env(NULL,NULL);
    env.destroy(); // Must destroy thread-env before destroying resources
    throw exc_code;
  } catch (...) { // Handle any other exceptions here, as in demo-5
    env.handle_exception(KDU_NULL_EXCEPTION);
    compositor.set_thread_env(NULL,NULL);
    env.destroy(); // Must destroy thread-env before destroying resources
    throw;
  }
  compositor.set_thread_env(NULL,NULL); // As in demo-5
  env.destroy(); // Not strictly necessary, since the destructor will call it
                 // and there was no exception.   Where an exception occurs,
                 // however, you must destroy the thread-env before any
                 // resources that might be accessed by worker threads can
                 // be safely deleted.
     ...
     when used with huge images.  No output file is actually written. */
{
  ...
  // Create the code-stream management machinery for the compressed data
  kdu_codestream codestream;
  codestream.create(input,env_ref); // We don't have to set things up for
          // multi-threaded processing until later, so we could leave off the
          // `env' reference here -- one will be passed to the codestream
          // when the first tile is opened by the `decompressor' object below.
  ...
  try { // Catch errors to properly deallocate resources before returning
    ...
    double processing_time = timer.get_ellapsed_seconds();
    ...
    if (env_ref != NULL)
      { 
        env.handle_exception(exc_code);
        env.destroy(); // Must do this before destroying resources after exc.
        env.cs_terminate(codestream); // Not actually required when catching an
                          // exception, but good practice to always call this.
      }
    codestream.destroy();
    if (buffer != NULL) delete[] buffer;
    throw exc_code; // Rethrow the exception
  } catch (...) { // Handle any other exceptions the same way
    if (env_ref != NULL)
      { 
        env.handle_exception(KDU_NULL_EXCEPTION);
        env.destroy();
  ...
  if (env_ref != NULL)
    { 
      env.cs_terminate(codestream);  // Cleanest way to detach codestream
        // from the multi-threaded processing environment.  This is done
        // automatically in `kdu_region_compositor' when the multi-threaded
        // processing environment is retracted -- see demos 5 and 6.
      env.destroy(); // Not really required since there was no exception; done
                     // automatically by `env' destructor.  Where an exception
                     // occurs, however, you must destroy the thread-env before
                     // any resources that might be accessed by worker threads
