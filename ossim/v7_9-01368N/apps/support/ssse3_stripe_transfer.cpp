...
static kdu_byte kd_shuffle_r556[16]; // These are all initialized from
static kdu_byte kd_shuffle_r565[16]; // `ssse3_stripe_transfer_static_init'
static kdu_byte kd_shuffle_r655[16]; // when it is safe to do so.
...
static kdu_byte kd_shuffle_556[16]; // Also initialized from
static kdu_byte kd_shuffle_565[16]; // `ssse3_stripe_transfer_static_init'
...
static kdu_byte kd_shuffle_r4444[16]; // Also initialized from
                                      // `ssse3_stripe_transfer_static_init'
...
/*****************************************************************************/
/* EXTERN              ssse3_stripe_transfer_static_init                     */
/*****************************************************************************/

void ssse3_stripe_transfer_static_init()
{ // Static initializers are potentially dangerous, so we initialize here
  ...
  for (int n=0; n < 16; n++)
    { 
      kd_shuffle_r556[n] = shuffle_r556[n];
      kd_shuffle_r565[n] = shuffle_r565[n];
      kd_shuffle_r655[n] = shuffle_r655[n];
      
      kd_shuffle_556[n] = shuffle_556[n];
      kd_shuffle_565[n] = shuffle_565[n];
      kd_shuffle_655[n] = shuffle_655[n];
      
      kd_shuffle_r4444[n] = shuffle_r4444[n];
    }
}
...
void ssse3_int16_to_uint8_rs_ilv1(kdu_byte *dst, kdu_int16 **src, int width,
                                  ...
                                  int preferences)
{
  assert(width > 16); // Guaranteed by macros in "x86_stripe_transfer_local.h"
  ...
  // Generate all but the last 1 to 16 outputs
  kdu_byte *dp = dst;
  kdu_int16 *sp = src[0];
  if (preferences & KDU_STRIPE_STORE_PREF_STREAMING)
    { // Process first vector and then backtrack to align streaming stores
      int advance = 16 - (_addr_to_kdu_int32(dst) & 15);
      v1=_mm_loadu_si128((__m128i *)(sp+0));
      v2=_mm_loadu_si128((__m128i *)(sp+8));
      ...
      assert(!(_addr_to_kdu_int32(dp) & 15));
      for (; width > 16; width-=16, dp+=16, sp+=16)
        { 
          v1=_mm_loadu_si128((__m128i *)(sp+0));
          v2=_mm_loadu_si128((__m128i *)(sp+8));
          v1 = _mm_adds_epi16(v1,vec_off);   v2 = _mm_adds_epi16(v2,vec_off);
          v1 = _mm_sra_epi16(v1,vec_shift);  v2 = _mm_sra_epi16(v2,vec_shift);
          v1 = _mm_packus_epi16(v1,v2);      v1 = _mm_min_epu8(v1,vec_max);  
          _mm_stream_si128((__m128i *) dp,v1);
        }      
    }
  else
    for (; width > 16; width-=16, dp+=16, sp+=16)
                                ...
                                int preferences)
{
  ...
  kdu_byte *dp = dst;
  for (; width >= 16; width-=16, sp1+=4, sp2+=4, sp3+=4, dp+=48)
    { 
                               ...
                               int preferences)
{
  ...
  if (width > 0)
    { // Take care writing final 1 to 16 output words to avoid buffer overrun
      union { __m128i dqwords[2]; kdu_int16 words[16]; };
      v1=sp[0];                          v2=sp[1];
      v1 = _mm_max_epi16(v1,vec_min);    v2 = _mm_max_epi16(v2,vec_min);
      v1 = _mm_min_epi16(v1,vec_max);    v2 = _mm_min_epi16(v2,vec_max);
                                 ...
                                 int preferences)
{
  ...
  if (width > 0)
    { // Take care writing final 1 to 16 output floats to avoid buffer overrun
      union { __m128 dqwords[4]; float floats[16]; };
      v1f=sp[0];                          v2f=sp[1];
      v3f=sp[2];                          v4f=sp[3];
      v1f = _mm_mul_ps(v1f,vec_scale);    v2f = _mm_mul_ps(v2f,vec_scale);
