...
class kdu_region_animator {
  ...
  */
  ...
  public: // Functions used to drive the animation between `start' and `stop'
    ...
    KDU_AUX_EXPORT int
      ...
      /* [SYNOPSIS]
           ...
           frames (see `insert_conditional_frame_step'), generating
           advice regarding the number of quality layers to process (see
           `get_recommended_quality_layers') and providing the application with
           access to the collected statistics.
           [//]
    ...
    KDU_AUX_EXPORT void
      note_frame_generated(double cur_system_time,
                           double next_display_event_time,
                           int max_available_quality_layers=-1,
                           int quality_layer_limit=-1);
      /* [SYNOPSIS]
         ...
         [ARG: max_available_quality_layers]
           For this argument, you should pass the value returned by
           `kdu_region_compositor::get_max_available_quality_layers' after
           rendering of the current animation frame is complete.  This value
           serves as a guide for the recommendations issued by calls to
           `get_recommended_quality_layers' in the future. If the value passed
           for this argument is non-positive, that function will return -1,
           meaning that no recommendation can be formed.
         [ARG: quality_layer_limit]
           This argument should supply any quality layer limit that was
           employed in the rendering of the frame, as supplied to
           `kdu_region_compositor::set_max_quality_layers'.
           [//]
           If the value supplied here is -ve, no update to the internal
           quality layer recommendation state machine will occur in this
           call, which can be useful during transient conditions when the
           frame rendering operations are non-typical for one reason or
           another.
           [//]
           A value of 0, or one that is greater than or equal to
           `max_available_quality_layers', is interpreted as meaning that
           no quality layer limit was applied.
      */
  //---------------------------------------------------------------------------
  public: // Additional functions required or useful for generating frames
    KDU_AUX_EXPORT int
    ...
    int get_suggested_refresh_action()
      ...
      */
    KDU_AUX_EXPORT int
      get_recommended_quality_layers(float target_rendering_frac);
      /* [SYNOPSIS]
           This function suggests a quality layer limit that can be passed
           to `kdu_region_compositor::set_max_quality_layers' in order to
           encourage timely rendering of animation frames.  The function's
           return value relies upon the information passed to
           `note_frame_generated' after rendering previous frames.  That
           function takes note of the amount of time required to render a
           frame, along with the number of quality layers that were actually
           rendered, generating advice for the next frame (via this function)
           based on a simple internal state machine.
           [//]
           Typically, you would call this function after `advance_animation',
           right before generating a frame.  Once the frame has been
           generated, you should pass any quality layer limit you used
           (e.g., based on the advice from this function) to
           `note_frame_generated', along with the maximum number of quality
           layers that were potentially available for the rendered frame.
           Both pieces of information are used for providing the suggested
           number of quality layers to render in subsequent frames.
           [//]
           The state machine that suggests the number of quality layers to
           render is advanced by `note_frame_generated', but only if a positive
           value is passed for that function's `quality_layer_limit' argument.
           This allows you to suspend updates to the state machine (e.g.
           during transient periods where rendering resolution or quality
           might be suppressed via independent mechanisms) simply by passing
           a non-positive value to that function.
         [RETURNS]
           -1 if no quality layer limit can be recommended for rendering of
           the current frame.  Otherwise, a positive value is returned, that
           can be supplied to `kdu_region_compositor::set_max_quality_layers'
           before rendering the current frame.  There is no guarantee that
           the current frame even has this number of quality layers, of
           course, but that is not a fundamental problem.  The suggested
           value is based upon the expectation that successive frames in an
           animation use quality layers which have a similar interpretation,
           which is a reasonable assumption in almost all cases.
         [ARG: target_rendering_frac]
           This argument supplies threshold on the ratio R between the
           rendering time for a frame and the frame period.  Obviously, this
           ratio must be no larger than 1.0, on average, or else the rendering
           of an animation will consume more processing time than the display
           time.  Typically, you would supply a value somewhat smaller than
           1.0 for this argument, allowing some CPU time to be devoted to
           other activities, such as the management of a user interface.  You
           might choose to set `target_rendering_frac' significantly smaller
           than 1.0 so as to reduce power consumption in a mobile device, for
           example.  In practice, the value supplied for this argument is
           always clipped to the interval [0.05,0.95].
           [//]
           While not required, it is recommended that you supply a consistent
           value each time you call this function, since the supplied
           parameter is used by the state machine that is driven by calls
           to `note_frame_generated'.
      */
    KDU_AUX_EXPORT double get_display_time();
      /* [SYNOPSIS]
           ...
           [//]
           It is expected that the caller places the generated frame into
           ...
           [//]
           Display times are measured relative to the display event timebase.
           The display time of the very first animation frame following a
           call to `start' or a change in animation direction (see
           `set_reverse') is initially set to the `next_display_event_time'
           value passed to `advance_animation'; however, this value is
           subsequently adjusted to the `next_display_event_time' that is
           passed to `note_frame_generated'.  This ensures that the animation
           process can start smoothly from the point at which a rendered
           frame first becomes available.  From this point on, the
           display time origin for the animation remains fixed, with
           frames assigned display times that reflect the source timing
           information, combined with playback rate information, relative to
           this origin.  The only exception to this principle is that calls
           to `retard_animation' actually modify the original of the display
           time base.
           [//]
           In view of the above observation that the display time of the
           very first frame in a video sequence is updated when
           `note_frame_generated' is first called, it is best to derive a
           frame's time stamp by calling `get_display_time' after
           `note_frame_generated'.
      */
    KDU_AUX_EXPORT double get_first_display_time();
      /* [SYNOPSIS]
           Similar to `get_display_time', except that the value returned by
           this function corresponds to the first display time for the frame,
           prior to any frame-step that may have been inserted -- see
           `insert_conditional_frame_step'.  The value returned by this
           function should never be larger than that returned by
           `get_display_time'.  The difference between the return values is
           the amount of the frame's total display time that belonged to
           previous renderings of the frame.  This difference can be of
           interest to applications that dynamically pan through an
           animation, possibly requiring the same frame to be rendered
           multiple times in order to cover the viewport that over the
           pan trajectory.
      */
    ...
    KDU_AUX_EXPORT bool count_metadata_driven_frames(int &count);
      /* [SYNOPSIS]
           ...
           can be returned by `get_metadata_driven_frame_id', returning
  ...
  private: // Helper functions
    friend struct kd_supp_local::kdra_frame;
    kd_supp_local::kdra_frame *get_frame();
      /* Allocates a new `kdra_frame' object, recycling it from the internal
         free list, if possible. */
    void recycle_frame(kd_supp_local::kdra_frame *frm);
      /* So it can be recycled by future calls to `get_frame'. */
    kd_supp_local::kdra_frame *
      insert_new_frame(kd_supp_local::kdra_frame *ref, bool insert_before_ref);
    ...
    bool remove_frame(kd_supp_local::kdra_frame *frm, bool force_remove=false);
    ...
    kd_supp_local::kdra_frame_req *
      append_frame_req(kd_supp_local::kdra_frame *frm);
      /* Appends a new element to the list of `kdra_frame_req' objects
         ...
         `frm'. */
    void remove_frame_req(kd_supp_local::kdra_frame_req *elt,
    ...
    kd_supp_local::kdra_client_req *
      ...
      /* Appends a new element to the list of `kdra_client_req' objects
         ...
         same names. */
    void remove_client_request(kd_supp_local::kdra_client_req *elt);
    ...
    void set_metadata_req_offset(kdu_long request_to_display_offset,
                                 kdu_long cpu_allowance_usecs,
                                 kd_supp_local::kdra_frame *last_frame);
      /* This function is called from `generate_imagery_requests' when it
         fails to fully instantiate the frame following `last_frame' by
         invoking `instantiate_next_frame'.  The idea is that this
         instantiation has most likely failed due to a lack of sufficient
         metadata to identify the constituent elements (e.g., JPX compositing
         layers) for the frame and these are required to form the imagery
         request.  This metadata is requested separately by the
         `generate_metadata_requests' function, which bases its requests
         upon a `display_time_limit' that represents a target display time
         for the requested metadata, provided by the application.  This does
         not account for the extra time required to request imagery once
         the frame metadata is known.  This extra time required to request
         imagery is stored in the `metadata_request_time_offset' member
         variable.  More correctly, the current function calculates the
         extra time required and updates `metadata_request_time_offset' only
         if the calculated value exceeds the last value recorded there.  In
         this way, we encourage metadata requests to get comfortably ahead
         of imagery requests, but only to the extent that this can be
         justified by evidence.  The display time offset value calculated
         by this function is the sum of the following three quantities:
         A) `request_to_display_offset' converted from microseconds;
         B) `cpu_allowance_usecs' converted from microseconds, since the
            response data from a request must arrive early enough to allow
            for rendering of the frame data; and
         C) the smaller of `min_auto_refresh_interval' and the next frame's
            expected duration, since the response associated with the whole
            frame (or at least the first `min_auto_refresh_interval' portion
            of it) needs to have arrived before rendering should commence.
            By "next frame", we mean the one that follows `last_frame'.  If
            `last_frame' is NULL, or `last_frame->next' is NULL, the `head'
            frame's calculated duration is used instead.  If `head' does
            not exist yet either, we simply use `min_auto_refresh_interval'.
         Note that the first two items (A and B) are identical to those used
         by the `post_last_client_request' function.  That function follows
         a similar strategy in estimating the amount by which imagery
         requests need to run ahead of the current display time in order to
         keep up. */
    ...
    kd_supp_local::kdra_frame *
      instantiate_next_frame(kd_supp_local::kdra_frame *base);
      /* The object of this function is to instantiate the frame which is
         ...
         and so forth. */
    void note_new_frame(kd_supp_local::kdra_frame *frm);
    ...
    void reset_quality_layer_recommender()
      { // Resets all parameters associated with the quality layer recommender,
        // except for `target_rendering_frac'.
        last_noted_available_quality_layers = -1;
        last_noted_quality_layer_limit = -1;
        last_noted_rendering_period = 0.0;
        last_noted_display_period = 0.0;
        cum_rendering_period = 0.0;
        cum_display_period = 0.0;
      }
  //---------------------------------------------------------------------------
  ...
  private: // Source-specific machinery
    ...
    kd_supp_local::kdra_frame *incomplete_meta_head; // Head/tail of frame
    kd_supp_local::kdra_frame *incomplete_meta_tail; // sub-list with
                                                     // incomplete metadata.
  //---------------------------------------------------------------------------
  private: // Frame management
    ...
    kd_supp_local::kdra_frame *head; // If metadata-driven all frames are kept
    kd_supp_local::kdra_frame *tail; // on the list; else, generate & discard
                                     // on demand.
    kd_supp_local::kdra_frame *current; // Points to the current animation
                                        // frame
    kd_supp_local::kdra_frame *cleanup; // Points to next frame to consider
                                        // cleaning up
    kd_supp_local::kdra_frame *free_list; // Stores recycled frames
  ...
  private: // Client request management
    ...
    double metadata_request_time_offset; // See `set_metadata_req_offset'
    int client_refresh_state; // Meaningful values: 0, -1, 2; see below
    kdu_window_prefs client_prefs;
    kd_supp_local::kdra_frame_req *frame_reqs;
    kd_supp_local::kdra_frame_req *last_frame_req;
    ...
    kd_supp_local::kdra_client_req *client_requests; // Elements are removed
    kd_supp_local::kdra_client_req *last_client_request; // from head of list
                                           // as soon as response is complete.
  ...
  private: // Statistics
    ...
    double max_rendering_time; // Peak detector with exponential decay applied
      ...
      // `note_frame_generated'.
  //---------------------------------------------------------------------------
  private: // Quality layer suggestion machinery
    double target_rendering_frac; // From `get_recommended_quality_layers'
    int last_noted_available_quality_layers; // These 4 values are derived from
    int last_noted_quality_layer_limit;    // the last `note_frame_generated'
    double last_noted_rendering_period;  // call with `max_quality_layers' > 0.
    double last_noted_display_period;
    double cum_rendering_period; // These accumulate rendering & display times
    double cum_display_period; // starting from the last frame whose rendering
      // fraction (cpu time over display time) was found to exceed the
      // `target_rendering_frac' member.
  };
