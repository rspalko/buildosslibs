...
// Macros that limit worst case resource consumption by the compositor
#define KDRC_MAX_BUFFER_PELS  (((kdu_int64) 1) << 33) /* 8 GigaPels is tons */
#define KDRC_MAX_BUFFER_WIDTH (((kdu_int64) 1) << 28) /* 256 MegaPels wide */

namespace kd_supp_local {
  using namespace kdu_supp;

  // Function pointer prototypes to facilitate SIMD acceleration:
  typedef void (*kdrc_erase_region_func)
    (kdu_uint32 *dst, int height, int width, int row_gap, kdu_uint32 erase);
  typedef void (*kdrc_erase_region_float_func)
    (float *dst, int height, int width, int row_gap, float erase[]);
    /* In the floating-point case, `erase' is a 4-element array of values
       with which to erase each pixel.  Also, in the floating point case,
       `row_gap' counts the number of floating point samples (not
       pixels) between rows of the buffer. */
  
  typedef void (*kdrc_copy_region_func)
    (kdu_uint32 *dst, kdu_uint32 *src, int height, int width,
     int dst_row_gap, int src_row_gap);
  typedef void (*kdrc_copy_region_float_func)
    (float *dst, float *src, int height, int width,
     int dst_row_gap, int src_row_gap);
  
  typedef void (*kdrc_rcopy_region_func)
    (kdu_uint32 *dst, kdu_uint32 *src, int height, int width, int row_gap);
  typedef void (*kdrc_rcopy_region_float_func)
    (float *dst, float *src, int height, int width, int row_gap);
    /* These are the same as the copy_region functions, except that the copy
       is performed backwards.  This is important only in cases where the `src'
       and `dst' arguments point to a common block of physical memory, where
       a forward copy would overwrite values that have not yet been copied.
       The function can only meaningfully be called where the `src' and `dst'
       buffers have the same row-gap. */
  
  typedef void (*kdrc_blend_region_func)
    (kdu_uint32 *dst, kdu_uint32 *src, int height, int width,
     int dst_row_gap, int src_row_gap);
  typedef void (*kdrc_blend_region_float_func)
    (float *dst, float *src, int height, int width,
     int dst_row_gap, int src_row_gap);
  
  typedef void (*kdrc_premult_blend_region_func)
    (kdu_uint32 *dst, kdu_uint32 *src, int height, int width,
     int dst_row_gap, int src_row_gap);
  typedef void (*kdrc_premult_blend_region_float_func)
    (float *dst, float *src, int height, int width, int dst_row_gap,
     int src_row_gap);
    /* Same as the `blend_region' functions, except that the source pixels
       have already been pre-scaled by the alpha value found in the
       output pixels, so we only need to add in (1-alpha) * the target
       pixel values. */
  
  typedef void (*kdrc_scaled_blend_region_func)
    (kdu_uint32 *dst, kdu_uint32 *src, int height, int width, int dst_row_gap,
     int src_row_gap, kdu_int16 alpha_factor_x128);
  typedef void (*kdrc_scaled_blend_region_float_func)
    (float *dst, float *src, int height, int width, int dst_row_gap,
     int src_row_gap, float alpha_factor);
    /* Same as the blend_region functions, except that the overlay alpha
     channel is scaled by `alpha_factor'; if `alpha_factor' is negative,
     colour channels are inverted.  In the 32-bit pixel version of the
     function, `alpha_factor' has been pre-scaled by 128 and rounded to
     an integer. */
...
struct kdrc_queue {
  ...
  public: // Data
    ...
    kdu_dims notional_buffer_region;
...
struct kdrc_overlay_segment {
  ...
  /* Notes:
     ...
     is false -- in calls to `kdrc_layer::do_composition', segments that
     are invisible are simply skipped during the overlay blending stage.
   */
  ...
  public: // Member functions
    ...
    jpx_metanode search(kdu_coords point);
      /* Does the work of `kdrc_layer::search_overlay', returning a non-empty
         interface if a match is found.  Note that `kdrc_overlay' only
         knows about the rendering scale, whereas `kdrc_stream' and
         `kdrc_layer' know about both the rendering scale and a
         "notional scale".  The `point' with which this function is
         called has been converted to the rendering scale by the caller,
         since `kdrc_layer::search_overlay' receives a `point' that is
         expressed in the notional scale. */
...
struct kdrc_stream_scaling_params {
  public: // Member functions
    kdrc_stream_scaling_params()
      { scale = 0.0f; discard_levels = 0; }
    bool configure(const kdu_dims &full_source_dims,
                   ...
                   int single_component, // -ve if not in single-comp mode
                   kdu_component_access_mode access_mode,
                   kdu_channel_mapping *mapping, // NULL if in single-comp mode
      ...
      /* This function does most of the work of `kdrc_stream::set_scale'.  The
         function's arguments correspond to those of `kdrc_stream::set_scale'
         and member variables of `kdrc_stream' that are passed (the last two
         may be updated to reflect new knowledge of the scales that can be
         supported that is determined while trying to configure the object
         to achieve the supplied `scale'.  Prior to calling this function, the
         `full_source_dims' and `full_target_dims' arguments have been
         adjusted, as required, to ensure that they contain valid regions
         (one or both of them may have been empty on entry to
         `kdrc_stream::set_scale').  If the function fails, it returns false,
         setting `invalid_scale_code' to the relevant value.
             Note that `min_resampling_factor' is a copy of the
         `kdu_region_compositor::scaling_min_resampling_factor' member, which
         is the smallest amount by which a decoded image will be scaled
         (always < 1) before an attempt is made to discard additional DWT
         levels and interpolate up from the resulting smaller decoded image
         resolution.  Values outside the interval (0.5,1.0) are not of
         interest, since it is always preferable to discard a DWT level
         instead of resampling a decoded image to reduce its size to 0.5
         or even less of that decoded. */
  public: // Data
    float scale; // Identifies the global scale that the parameters achieve
    int discard_levels; // Number of levels to discard
    kdu_coords expansion_numerator; // Numerator and denominator for rational
    kdu_coords expansion_denominator; // ref component expansion factors
    kdu_dims full_region; // Full source image region on rendering grid
    kdu_coords compositing_offset; // Rendering origin - compositing origin
  };
  /* Notes:
        This object maintains parameters that describe how a single codestream
     is to be rendered to produce composited imagery at a given `scale'.  The
     `scale' is one of the `notional_scale' or `rendering_scale' parameters
     that are passed to `kdrc_stream::set_scale' -- same parameters that are
     passed to `kdrc_layer::set_scale'.  The `discard_levels' member
     identifies the number of DWT levels that should be discarded from the
     codestream in order to best achieve the desired rendering scale, while
     the `expansion_numerator' and `expansion_denominator' members describe
     additional rational scaling that should be applied to the decoded
     codestream data.  This rational scaling is applied to the so-called
     "reference component" that is configured for the codestream, as found
     within the `kdrc_stream' member variables.  Other components may have
     require different amounts of interpolation, perhaps prior to colour
     conversion.
        To understand the `rendering_dims' and `compositing_offset' members,
     you must first understand that there are several coordinate systems at
     work simultaneously:
     1) There is a global "compositing grid" that depends only upon `scale'.
     2) Each codestream has its own "rendering grid" whose `scale' matches that
        ...
        coordinates used by the `kdrc_stream' object are expressed in terms
        of the rendering grid.  The interpretation of the rendering grid is
        exactly the same as that adopted by the `kdu_region_decompressor'
        object, which is used to do all codestream decompression, scaling
        and colour conversion operations.
     3) Each codestream has its own high-resolution canvas against which
        ...
        dimensions and then expand by the rational factors represented by
        the `expansion_numerator' and `expansion_denominator' members.
     [//]
     The `full_region' member identifies the portion of the code-stream
     image (expressed on the rendering grid) which would be used to generate
     the full composited image at the relevant scale and orientation.  This
     depends upon the `full_source_dims' argument supplied to the
     `kdrc_stream::set_scale' function, which is typically derived from JPX
     compositing instructions that may involve cropping of the source content.
     In the absence of such cropping, `full_region' is the entire codestream
     image region, expressed on the rendering grid at the prevailing
...
class kdrc_stream {
  public: // Member functions
    ...
    void update_quality_limiting();
      /* Called whenever the `kdu_region_compositor::limiter' or associated
         display resolution information may have changed, this function
         passes the changes to `kdu_region_decompressor::set_quality_limiting'.
         The function need not be called when a `kdrc_stream' is first
         constructed, since the constructor automatically passes the global
         `limiter' information along to the `kdu_region_decompressor'. */
    ...
    bool set_scale(kdu_dims full_source_dims, kdu_dims full_target_dims,
      ...
      /* Called after initialization and whenever the image is zoomed,
         ...
         dynamic scaling, so their values should usually be the same each time
         this function is called, regardless of the values taken by the
         remaining arguments.
         ...
         transformations which are to be applied on the compositing grid.
            `notional_scale' and `rendering_scale' represent the amounts by
         which the full target dimensions are to be stretched or shrunk down
         in defining a composited result.  The `notional_scale' is the scale
         adopted by the `kdu_region_compositor' for computing and reporting
         all dimensional parameters, but the `rendering_scale' can be
         different -- this can be of interest if the rendered image samples
         are to be produced at a lower resolution (smaller scale) for
         improved responsiveness while handling interactive panning/flicking
         ...
         discarding and resampling.  The actual location and dimensions of
         the region which can be painted on the compositing grid, accounting
         for all geometric changes, can be recovered by calling
         `find_full_notional_region' or `find_full_rendering_region', as
         appropriate.
            If the requested scale is too small, too large, or the required
         flipping cannot be performed, the function returns false, identifying
         the specific problem by setting one or more flags in
    ...
    void find_supported_scales(float &min_scale, float &max_scale,
                               ...
                               kdu_coords source_sampling,
                               kdu_coords source_denominator);
      /* On entry, `min_scale' and `max_scale' hold what is known so far about
    ...
    bool find_min_max_jpip_woi_scales(double min_scale[], double max_scale[]);
      /* Returns false without doing anything unless the object has valid
         scale parameters installed via `set_scale'.  If it does, the function
         returns true after  modifying the supplied `min_scale' and `max_scale'
         values so as to ensure that a compatible JPIP request, which scales
         the total composition dimensions accordingly when forming its "fsiz"
         request field, will include just the resolution levels which are
         required from the underlying codestream.
            It is assumed that JPIP requests are to be formed for the
         `notional_scale' last passed to `set_scale', even though the current
         `rendering_scale' might be different.
         ...
         codestream resolution levels which are used by the notional scale
         (as recorded within the `notional_params' member) to be discarded by a
         ...
         incompatible, in which case the minimum scale should be selected
    ...
    kdu_dims find_full_notional_region(bool apply_cropping) const;
      /* If `apply_cropping' is true, this function just returns the region
         occupied by the stream on the compositing surface, at the
         `notional_scale' last passed to `set_scale'.  Otherwise, it returns
         the region which would be occupied by the stream on the compositing
         surface if it had not been cropped based on source compositing
         specifications (i.e., `full_source_dims') passed to `set_scale'. */
    kdu_dims find_full_rendering_region(bool apply_cropping) const;
      /* As above, but at the `rendering_scale' last passed to `set_scale'. */
    void get_packet_stats(kdu_dims notional_region, int max_region_layers,
      ...
      /* Used to implement `kdu_region_compositor::get_codestream_packets',
         ...
         reconstruction of `notional_region' (expressed on the composition
         canvas, at the `notional_scale' last passed to `set_scale'),
         returning:
            1) the sum of their contributing areas via `precinct_samples' (this
         is figured out using `kdu_resolution::get_precinct_area');
            2) the sum of their contributing packets, limited to at most
         `max_region_layers', if non-zero', (from
         `kdu_resolution::get_precinct_packets') scaled by the
         corresponding precinct areas; and
            3) the sum of the maximum possible number of contributing packets,
         limited to at most `max_region_layers', if non-zero, (from
         `kdu_tile::get_num_layers') scaled by the corresponding precinct
         areas.
            Note that quality layer limits are determined only by
         `max_region_layers' -- the internal `max_display_layers' member is
         ignored.  If `max_region_layers' is 0, no restriction at all is
         imposed on the quality layers that are used to compute the result. */
    void set_buffer_surface(kdu_compositor_buf *buffer,
      ...
      /* Called when the buffered region is panned, or when the buffer must
         ...
         codestream's rendering grid.
            Note also that `compositing_region' is expressed with respect
         to the `rendering_scale' last passed to `set_scale', which might
         be different to the `notional_scale'.
            If `start_from_scratch' is true, no previously generated data can
         be re-used, so generation of content for the new buffer surface must
         start from scratch. */
    kdu_dims map_region(kdu_dims notional_region, bool require_intersection);
      /* Does the work of `kdu_region_compositor::map_region'.  If the
         ...
         region on the codestream canvas.
            Note that `notional_region' is expressed with respect to the
         `notional_scale' last passed to `set_scale'. */
    kdu_dims inverse_map_region(kdu_dims notional_region);
      /* Does the work of `kdu_region_compositor::inverse_map_region'.  The
         ...
         coordinate system.  If the `notional_region' is empty, the region
         in question is considered to be the entire image region on the
         codestream's high resolution grid.
            As with `map_region', `notional_region' is expressed with respect
         to the `notional_scale' last passed to `set_scale'. */
    bool process(int suggested_increment, kdu_dims &new_rendered_region,
      ...
      /* Gives this object the opportunity to process some more imagery,
         ...
         `region_in_process' first.
            The function returns with `new_rendered_region' set to the new
         rectangular region which has just been processed, expressed with
         respect to the compositing grid, at the `rendering_scale' last passed
         to `set_scale'.  If this region is non-empty, the caller may
         ...
         current `rendering_scale', or if any required flipping cannot be
         performed.  In either case, the cause of the problem is identified
         via the `invalid_scale_code' argument.  The returned code contains
         the logical OR of one or more of the single-bit flags,
    ...
    int get_min_unrendered_y()
      { 
        if (buffer == NULL)
          return INT_MIN;
        else
          return (buffer->get_rendered_rows() + buffer_origin.y
                  - rendering_params.compositing_offset.y);
      }
    ...
    int find_non_pending_rects(kdu_dims rendered_region, kdu_dims rects[]);
      /* Used by `kdrc_refresh::adjust' and
         `kdu_region_compositor::find_completed_rects', this function checks
         to see whether all or part of the supplied `rendered_region' will be
         regenerated as a result of future calls to this object's `process'
         function.  There may be up to 6 rectangular subsets of
         `rendered_region' which will not be regenerated in this way; these
         are written to the `rects' array and the number of such rectangles is
         the function's return value.  Accordingly, `rects' should be large
         enough to accommodate up to 6 entries. */
  private: // Helper functions
    ...
    void configure_subsampling();
      /* This function is called from within `init', `set_mode' and
         ...
         `max_discard_levels' cannot possibly exceed 32.
            The function also sets the `max_nonempty_levels' member to the
         number of levels for which the active component has non-zero size,
         since some pathalogical codestreams can yield empty image components
         at resolutions that formally do exist.
...
#define KDRC_PRIORITY_MAX (1<<KDRC_PRIORITY_SHIFT)
    void update_completion_status()
      { /* Updates the `is_complete' and `priority' members based on the areas
        ...
        if ((!active_region) || (active_region == valid_region))
          { 
            is_complete = true; priority = 0;
            if (buffer != NULL)
              buffer->note_new_rendered_rows(active_region.size.y,
                                             active_region.size.y);
            return;
          }
        is_complete = false;
  ...
  private: // Members which are unaffected by zooming, panning or rotation
    ...
    int max_nonempty_levels; // Discarding more leaves the image empty
  ...
  private: // Members which are affected by zooming and rotation
    ...
    kdrc_stream_scaling_params notional_params;  // Derived from notional and
    kdrc_stream_scaling_params rendering_params; // rendering scale factors.
  ...
  private: // Members which relate to panning of the buffered surface; all
           // coordinates here are interpreted at `rendering_params.scale'.
    kdu_compositor_buf *buffer; // Sized for 
    kdu_coords buffer_origin; // Expressed on rendering grid at rendering scale
    kdu_dims active_region; // `rendering_params.full_region' intersected with
                            // the compositor's buffer surface region
  private: // Members used to keep track of incremental rendering; again, all
           // coordinates here are interpreted at `rendering_params.scale'.
  ...
  /* Notes:
     ...
     those attached to inactive layers have their `is_active' member false.
        Like `kdrc_layer' and `kdu_region_compositor' itself, this object
     maintains two potentially different scales, known as "notional" and
     "rendering".  For each scale, there is a `kdrc_stream_scaling_params'
     object that describes how the scaling is to be achieved by a combination
     of resolution level discarding and rational resampling/interpolation of
     the result, as well as the dimensions and location of the result,
     expressed with respect to a so-called "rendering grid".  The rendering
     grid is identical to that used by the underlying `kdu_region_decompressor'
     workhorse that is used for actual rendering -- it is further explained
     in the notes that follow the definition of `kdrc_stream_scaling_params'.
     Importantly, the rendering grid take both scale and orientation
     modifiers into account in exactly the same way that the compositing
     grid adopted by the `kdu_region_compositor' object does, except that
     there is an offset between the rendering grid and compositing grid,
     whose value is also maintained by `kdrc_stream_scaling_params'.
        Apart from `notional_params', the geometric region and location
     information recorded in this object's state relates to the rendering
     scale, corresponding to the conditions captured by `rendering_params'.
        `active_region' represents the subset of `rendering_params.full_region'
     which lies on the `kdu_region_compositor' object's current buffer surface,
     as found in `kdu_region_compositor::rendering_buffer_region'.  This is
     expressed with respect to the rendering grid, which may be offset relative
     to the compositing grid, as explained above.
        `valid_region' identifies the subset of `active_region' for which image
     samples are known to have been generated already.
...
class kdrc_layer {
  public: // Member functions
    ...
    bool set_scale(bool transpose, bool vflip, bool hflip,
                   float notional_scale, float rendering_scale,
                   int &invalid_scale_code, kdu_dims composited_frame_dims);
      /* Sets the orientation and scale at which the image is to be composed.
         The first 5 parameters are derived from the first 5 parameters passed
         to `kdu_region_compositor::set_scale'.  In particular,
         `notional_scale' is that function's `scale' argument, while
         `rendering_scale' is its the product of its `scale' and
         `renderint_scale_adjustment' arguments.
         ...
         the layer is its own frame, `composited_frame_dims' will be empty.
            The function returns false if either of the scale parameters
         is too small, or if any requested flipping operation might not be
         possible.  These conditions are identified by setting
         one or more flags in the `invalid_scale_code' argument.  Note that
         the rendering scale may subsequently be found to be too small, or
         flipping may be found to be impossible while actually processing the
         imagery.  This is because it may not be until then that a
         tile-component with insufficient DWT levels is encountered.
         ...
         buffer type. */
    void apply_layer_region_constraints(kdu_dims &notional_composition_dims,
                                        kdu_dims &rendering_composition_dims)
      { notional_layer_region &= notional_composition_dims;
        rendering_layer_region &= rendering_composition_dims; }
      /* This function is called by `kdu_region_compositor::update_composition'
         after converting any fixed frame dimensions to overall composition
         dimensions at the current scale and orientation.  These overally
         composition dimensions must constrain the apparent size of every
         compositing layer; otherwise, problems can arise with the management
         of buffers and `kdu_compositor_buf::get_rendering_composition_dims'
         might wind up reporting regions that lie slightly outside the
         region returned by `kdu_compositor_buf::get_total_composition_dims',
         even when the rendering and notional scales are identical. */
    void find_supported_scales(float &min_scale, float &max_scale);
    ...
    void get_component_scale_factors(kdrc_stream *stream,
      ...
      /* Used to implement `kdu_region_compositor::get_codestream_info'.
         ...
         that was supplied to `set_scale'.  Note that these scaling factors
         are the notional ones (corresponding to the `notional_scale' parameter
         passed to `set_scale'), as opposed to the rendering scale factors,
         which might be different if the `rendering_scale' and `notional_scale'
         arguments to `set_scale' were different. */
    void set_buffer_surface(kdu_dims rendering_region,
      ...
      /* This function is invoked after `set_scale'.  Thereafter, it is
         ...
         `set_scale'.
            Similarly, the `rendering_region' corresponds to the
         `kdu_region_compositor::rendering_buffer_region' member and is
         expressed relative to the `rendering_scale' passed to `set_scale'.
            `rendering_visible_region' identifies the subset of the
         `rendering_region' that is not completely covered by opaque
         foreground layers.  At most this region must actually be rendered
         ...
         composited image.  This happens if there is only one layer and it
         does not use alpha blending.  In this case, `compositing_row_gap'
         is ignored. */
    ...
    kdu_long measure_visible_area(kdu_dims notional_region,
                                  bool assume_visible_through_alpha);
      /* This function measures the area of the subset of `notional_region'
         within which the current compositing layer is visible.  If
         ...
         regardless of alpha blending.
            Note that the `notional_region' and returned area are expressed
         relative to the `notional_scale' passed to `set_scale', which might
         differ from the current `rendering_scale'. */
    void get_visible_packet_stats(kdrc_stream *stream,
      ...
      /* Used to implement `kdu_region_compositor::get_codestream_packets',
         ...
         subsets of `notional_region' which are considered to be visible.
         These subsets are found by recursive application of the function,
         starting from the imagery layer which contains `stream'.  The
         interpretation of `notional_region' and `max_region_layers' is
         identical to the `region' and `max_region_layers' arguments to the
         `stream->get_packet_stats' function.  Note that the function
         behaves as if content were being rendered at the `notional_scale'
         passed to `set_scale', as opposed to the `rendering_scale', which
         might be different. */
    bool map_region(kdu_dims &notional_region, kdrc_stream * &stream);
      /* Does the work of `kdu_region_compositor::map_region', invoking the
         ...
         available, this function returns false.
            Note that all mapping operations work at the `notional_scale' that
         was supplied to `set_scale', which might be different from the
         `rendering_scale'. */
    float get_opacity(kdu_coords point);
      /* Returns the opacity of the current layer at location `point', the
         ...
         buffer surface -- e.g., when animating a video stream.
            Note that `point' is expressed with respect to the `notional_scale'
         that was passed to `set_scale', which might be different from the
         `rendering_scale'.  Currently, the function just converts `point'
         to the nearest equivalent at the rendering scale, but a slightly
         better method might be to measure the maximum opacity within a
         neighbourhood, where `rendering_scale' is smaller than
         `notional_scale'. */
    ...
    jpx_metanode search_overlay(kdu_coords point, kdrc_stream * &stream,
      ...
      /* Does the work of `kdu_region_compositor::search_overlays'.  Returns
         ...
         in lower layers.
            Note that `point' is expressed with respect to the `notional_scale'
         that was passed to `set_scale', which might be different from the
         `rendering_scale'.  As with `get_opacity', this function currently
         just converts `point' to the nearest equivalent at the rendering
         scale, but a slgightly better method might be to search for all
         points within a neighbourhood, where `rendering_scale' is smaller
         than `notional_scale'. */
    void do_composition(kdu_dims region, kdu_int16 blending_factor_x128,
      ...
      /* Called whenever new data is generated on any layer surface.  If
         ...
         `set_buffer_surface' has not been called.
            The `region' argument here is expressed with respect to the
         `rendering_scale' that was passed to `set_scale'. */
    ...
    bool process_overlay(kdu_dims &new_region, bool &painted_something);
      /* Called from `kdu_compositor::process', this function returns true
         ...
         a more concrete measure of the painting work that was done.
            Note that `new_region' is expressed with respect to the current
         `rendering_scale', not the `notional_scale'. */
    ...
    const kdu_dims &get_notional_layer_region() const
      { return notional_layer_region; } // Whole layer at notional scale
    const kdu_dims &get_rendering_layer_region() const
      { return rendering_layer_region; } // Whole layer at rendering scale
    const kdu_dims &get_rendering_buffer_region() const
      { return buffer_region; }
  private: // Helper functions
    void convert_notional_point(kdu_coords &point)
      { // Converts from `notional_scale' to `rendering_scale'
        if ((notional_scale == rendering_scale) || (notional_scale <= 0.0f))
          return;
        float scale = rendering_scale / notional_scale;
        point.x = (int) floor(0.5 + scale*point.x);
        point.y = (int) floor(0.5 + scale*point.y);
      }
  ...
  private: // Members which are affected by zooming and rotation
    ...
    float notional_scale; // Current notional scale, passed to `set_scale'
    float rendering_scale; // Current rendering scale, passed to `set_scale'
    kdu_dims notional_layer_region;  // Region occupied by the whole layer,
    kdu_dims rendering_layer_region; // at the notional & rendering scales
  private: // Members which relate to panning of the layer's buffered surface
    ...
    kdu_dims buffer_region; // a.k.a. `rendering_visible_region'
    kdu_coords buffer_size; // Actual size of buffer; must span `buffer_region'
  ...
  private: // Members which relate to compositing
    ...
    kdu_dims compositing_buffer_region; // Region occupied by above buffer, at
                                        // the curent `rendering_scale'.
  public: // Public identification information
  ...
  /* Notes:
       ...
       since become available.  This is done using the `change_frame' function.
          The `notional_layer_region' and `rendering_layer_region' members
       identify the full extent of the composited image region associated with
       this ilayer, expressed relative to the compositing grid, at the current
       values of `notional_scale', and `rendering_scale', respectively,
       accounting for any rotation, flipping or transposition.
          The `buffer_region' is always a subset of `rendering_layer_region',
       identifying the region on the compositing grid which is currently
       buffered within the `layer_buffer', expressed with respect to the
       `rendering_scale' -- the `buffer_region' is identical to the
       `kdu_compositor_buf::surface_dims' member of any non-NULL
       `layer_buffer'.  Note that this region may exclude portions of the
       compositing layer which could be rendered, but are completely covered
       by opaque content in layers that are closer to the foreground.
       ...
       at the current `rendering_scale', but may be larger (no smaller) than
       the region represented by `buffer_region'.  This is because some layers
       may occupy only a portion of the composited image.
...
class kdrc_refresh {
  public: // Member functions
    kdrc_refresh()
      { free_elts = list = NULL; min_y_val = min_y_count = 0; }
    ~kdrc_refresh()
      { 
    ...
    bool pop_largest_region(kdu_dims &region);
      /* Called from `kdu_region_compositor::process', this function
         removes the largest region from the refresh list that it can find,
         adjusting the internal state and returning the associated region. */
    bool pop_and_aggregate(kdu_dims &aggregated_region,
                           kdu_dims &new_region, kdu_long &aggregated_area,
                           float aggregation_threshold);
      /* This function is similar to `pop_largest_region', except that it
         pops regions in any order, so long as they intersect with or adjoin
         `aggregated_region'.  The `aggregated_area' argument holds the total
         area associated with aggregated regions from the refresh manager.
         This value should not become smaller than `aggregated_region' *
         `aggregation_threshold'.  If aggregating a region would violate this
         condition, the function returns false.
    ...
    int get_min_unrefreshed_y()
      { // Must not be called unless `is_empty' returns false.
        assert(min_y_count > 0);
        return min_y_val;
      }
  private: // Helper functions
    void recalculate_min_y()
      { 
        min_y_count = min_y_val = 0;
        kdrc_refresh_elt *scan;
        for (scan=list; scan != NULL; scan=scan->next)
          if ((min_y_count == 0) || (scan->region.pos.y < min_y_val))
            { min_y_val = scan->region.pos.y; min_y_count = 1; }
          else if (scan->region.pos.y == min_y_val)
            min_y_count++;
      }
  private: // Data
    kdrc_refresh_elt *free_elts;
    kdrc_refresh_elt *list; // List of regions which need refreshing
    int min_y_val;   // These keep track of the smallest y-coord in any element
    int min_y_count; // on the list and the number of such elements, to drive
                     // calls to `kdu_compositor_buf::note_new_rendered_rows'
  };
  /* Notes:
