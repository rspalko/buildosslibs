...
/******************************************************************************
Description:
   Implements the objects defined in "kdu_vex.h".
******************************************************************************/
...
#  include "x86_vex_transfer_local.h"
using namespace kd_supp_simd;
#else // Define empty accelerator selection macros
...
// As explained in "kdu_vex.h", we include part of the internal implementation
// of the `kdu_stripe_decompressor' API here so as to take advantage of
// SIMD accelerators that are defined there.  This is a bit of a hack that
// we will avoid in the future by using the public `kdu_stripe_decompressor'
// API directly.
//    The imported features that we use here are:
// 1. constant `KDU_STRIPE_STORE_PREF_STREAMING'
// 2. macro function `KDSD_FIND_SIMD_TRANSFER_FUNC()'
// 3. type constants used by the above func, such as `KDSD_BUF8'
#include "../support/stripe_decompressor_local.h"
...
void
  vex_processor::start_frame(vex_frame *frame, kdu_thread_env *env,
                             ...
                             kdu_int32 truncation, bool want_precise,
                             const kdu_push_pull_params *params)
{
  int c;

  this->env = env;
  this->frame = frame;
  this->multi_xform_flags = KDU_MULTI_XFORM_DEFAULT_FLAGS;
  if (want_precise)
    this->multi_xform_flags |= KDU_MULTI_XFORM_PRECISE;
  else
    this->multi_xform_flags |= KDU_MULTI_XFORM_FAST;
  this->multi_xform_params = kdu_push_pull_params();
  if (params != NULL)
    this->multi_xform_params = *params;
  this->processing_stripe_height = 1;  
  if (env != NULL)
    { 
      if (double_buffering_height != 0)
        { 
          this->multi_xform_flags |= KDU_MULTI_XFORM_MT_DWT;
          processing_stripe_height = double_buffering_height;
        }
    }

  int n_comps = frame->codestream.get_num_components(true);
  if ((comp_info != NULL) && (n_comps != num_components))
    { 
      delete[] comp_info;
      comp_info = NULL;
    }
    ...
    { // Decompress a line for each component whose counter is 0
      ...
      for (c=0; c < num_components; c++)
        { 
          ...
          if (frame->stride != 0)
            { // Buffer is interleaved XRGB (otherwise must be planar)
              assert(frame->sample_bytes <= 4);
              if (current_tile->xfer_mono_to_xrgb8 != NULL)
                { 
                  assert(c == 0);
                  current_tile->xfer_mono_to_xrgb8(lines[0]->get_buf(),
                                             frame->buffer+comp->boff,
                                             comp->tile_size.x,
                                             current_tile->xfer_downshift);
                }
              else if (current_tile->xfer_rgb_to_xrgb8 != NULL)
                { 
                  if (c == 2)
                    current_tile->xfer_rgb_to_xrgb8(lines[0]->get_buf(),
                                              lines[1]->get_buf(),
                                              lines[2]->get_buf(),
                                              frame->buffer+comp->boff,
                                              comp->tile_size.x,
          ...
          else if (comp->xfer_planar != NULL)
            { // SIMD accelerated transfer to planar frame buffer
              void *src = lines[c]->get_buf();
              void *dst = frame->buffer+comp->boff;
              comp->xfer_planar(dst,&src,comp->tile_size.x,
                                frame->precision,comp->precision,
                                comp->is_absolute,frame->is_signed,
                                KDU_STRIPE_STORE_PREF_STREAMING);
              comp->boff += (size_t)(comp->comp_size.x*frame->sample_bytes);
            }
          else if (frame->sample_bytes == 1)
            { // Tedious sample-by-sample transfer to planar byte buffer
              transfer_line_to_bytes(*(lines[c]),frame->buffer+comp->boff,
                                     comp->tile_size.x,comp->precision,
                                     frame->is_signed);
            ...
            { // Tedious sample-by-sample transfer to planar word buffer
  ...
  vex_processor::init_tile(vex_tile *tile, vex_frame *frame)
{
  ...
  tile->sample_allocator.restart(); // Not strictly necessary
  tile->engine.create(frame->codestream,tile->ifc,env,tile->env_queue,
                      multi_xform_flags,processing_stripe_height,
                      &(tile->sample_allocator),&multi_xform_params);
  for (int c=0; c < num_components; c++)
    { 
      ...
      comp->is_absolute = tile->engine.is_line_absolute(c);
      comp->using_shorts = !tile->engine.is_line_precise(c);
      comp->xfer_planar = NULL;
      ...
      if ((frame->stride == 0) && (frame->sample_bytes <= 2))
        { 
          int out_buf_type=(frame->sample_bytes==2)?KDSD_BUF16:KDSD_BUF8;
          using namespace kd_supp_local; // Only for the macro below
          KDSD_FIND_SIMD_TRANSFER_FUNC(comp->xfer_planar,
                                       out_buf_type,comp->using_shorts,1,
                                       comp->comp_size.x,frame->precision,
                                       comp->precision,comp->is_absolute);
        }
#endif
    }
  int precision = tile->components[0].precision;
  bool is_absolute = tile->components[0].is_absolute;
  bool using_shorts = tile->components[0].using_shorts;
  tile->xfer_downshift = (is_absolute)?(precision-8):(KDU_FIX_POINT-8);
  tile->xfer_mono_to_xrgb8 = NULL;
  tile->xfer_rgb_to_xrgb8 = NULL;
  if ((frame->stride != 0) && (tile->xfer_downshift >= 0))
    { 
      int width = tile->components[0].comp_size.x;
      if (num_components == 1)
        { VEX_SET_MONO16_TO_XRGB8_FUNC(tile->xfer_mono_to_xrgb8,
                                       frame->alignment,width,
                                       is_absolute,using_shorts); }
      else if ((num_components == 3) &&
               (tile->components[1].using_shorts == using_shorts) &&
               (tile->components[2].using_shorts == using_shorts) &&
               (tile->components[1].comp_size.x == width) &&
               (tile->components[2].comp_size.x == width) &&
               (tile->components[1].precision == precision) &&
               (tile->components[2].precision == precision) &&
               (tile->components[1].is_absolute == is_absolute) &&
               (tile->components[2].is_absolute == is_absolute))
        { VEX_SET_RGB16_TO_XRGB8_FUNC(tile->xfer_rgb_to_xrgb8,
                                      frame->alignment,width,
                                      is_absolute,using_shorts); }
    }
}
...
bool
  vex_frame_queue::init(kdu_compressed_video_source *source,
                        int discard_levels, int max_components,
                        int repeat_factor, int max_active_frames,
                        bool want_display)
{
  if (this->source != NULL)
  ...
  vex_frame_queue::get_processed_frame(vex_frame_memory_allocator *allocator)
{
  ...
  while ((num_active_frames < max_active_frames) && !exhausted)
    { // Need to load another frame
      ...
      kdu_long contents_len = frm->box.get_remaining_bytes();
      if ((contents_len > 0) && (contents_len < (kdu_long)INT_MAX) &&
          (contents_len > (kdu_long)frm->box_buffer_bytes))
        { // See if we need to reallocate `box_buffer'
          if (frm->box_buffer != NULL)
            delete[] frm->box_buffer;
          contents_len += frm->box_buffer_bytes + 4096; // Avoid re-allocs
          if (contents_len > (kdu_long)INT_MAX)
            contents_len = INT_MAX;
          frm->box_buffer_bytes = (int) contents_len;
          frm->box_buffer = new(std::nothrow) kdu_byte[frm->box_buffer_bytes];
        }
      if (!frm->box.load_in_memory(INT_MAX,frm->box_buffer,
                                   frm->box_buffer_bytes))
  ...
  vex_frame_queue::terminate()
{
  mutex.lock();
  terminated = true;
  active_waiting.protected_set(); // Starts the wakeup process for blocked
                                  // engine threads
  mutex.unlock();
}
                                ...
                                size_t &stride, int &alignment)
{
  ...
  if (format == VEX_FRAME_FORMAT_CONTIG)
    { 
      stride = 0;
      alignment = 1024; // Massive value to start with
      frame_bytes = 0;
      for (c=0; c < num_components; c++)
        {
          frame_bytes += sample_bytes * (size_t) component_dims[c].area();
          while (component_dims[c].size.x & (alignment-1))
            alignment >>= 1;
        }
    }
  else if (format == VEX_FRAME_FORMAT_XRGB)
    { 
      alignment = KDU_ALIGN_SAMPLES16*2;
      if (alignment < (KDU_ALIGN_SAMPLES32*4))
        alignment = (KDU_ALIGN_SAMPLES32*4);
      if (alignment < 16)
        alignment = 16;
      ...
      stride += (alignment - (int)stride) & (alignment-1);
  ...
  assert(alignment > 0);
  kdu_byte *handle =
    (kdu_byte *) malloc(frame_bytes+alignment-1); // Allow for alignment
  if (handle == NULL)
    { kdu_error e; e << "Insufficient memory available to allocate "
      "frame buffers."; }
  buffer = handle + ((alignment - _addr_to_kdu_int32(handle)) & (alignment-1));
  return handle;
}
  ...
  vex_engine::run_single_threaded()
{
  ...
  kdu_int32 affinity_context=0;
  cpu_affinity.get_bundle_affinity(0,affinity_mask,affinity_context);
  if (affinity_mask != 0)
    master_thread.set_cpu_affinity(affinity_mask,affinity_context);
  ...
  vex_engine::run_multi_threaded()
{
  kdu_thread_env env;
  env.create();
  env.set_cpu_affinity(cpu_affinity);
  env.set_min_thread_concurrency(thread_concurrency);
  if (worker_yield_frequency >= 0)
    env.set_yield_frequency(worker_yield_frequency);
  ...
  while (!immediate_shutdown_requested)
    {
      ...
      if ((frame == NULL) || immediate_shutdown_requested)
        break;
      try {
        int double_buffering_stripe_height = this->double_buffering_height;
        if ((double_buffering_stripe_height < 0) && (num_threads <= 4))
          double_buffering_stripe_height = 0;
        processor.start_frame(frame,&env,double_buffering_stripe_height,
                              queue->get_truncation_factor(),force_precise,
                              &pp_params);
        do {
          if (immediate_shutdown_requested)
            break;
      ...
      if (immediate_shutdown_requested)
        break;
      env.cs_terminate(frame->codestream);    
      queue->frame_processed(frame);
    }
  env.destroy(); // Automatically detaches any frame codestreams' thread
                 // contexts, so that codestreams that still need to be
                 // destroyed can be without having to first call
                 // `env.cs_terminate'.
}
