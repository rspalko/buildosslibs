...
// Declared here:
namespace kdu_supp_vex { // Worth putting these into their own re-usable space
  class vex_jpx_source; // Identical to `kdv_jpx_source' from "kdu_v_expand"
...
// Function typedefs for SIMD data transfer accelerators specific to this app
typedef void (*vex_mono_to_xrgb8_func)
  (void *src, kdu_byte *dst, int width, int downshift);
  /* This function is used with XRGB display buffers, with 8-bits per channel.
     Each sample is processed in the same way as `vex_int16_to_planar8', but
     then copied into all four channels of the 32-bit XRGB word.  Source
     samples may be 16-bit fixed-point/ints or 32-bit floats/ints, depending
     on the specific function that is bound to this pointer.  If the source
     samples are floats, `downshift' can be ignored, but should be 0. */
typedef void (*vex_rgb_to_xrgb8_func)
  (void *red, void *green, void *blue, kdu_byte *dst, int width,
   int downshift);
  /* This function is the same as `vex_mono_to_xrgb8', except that the
     source samples are drawn from 3 buffers (red, green and blue) and the
     converted bytes are interleaved into the `dst' buffer, whose 32-bit
     pixels hold blue, then green, then red, in that order.  The unused "x"
     channel is filled with ones for convenience. */
  
// Create type definition for fast SIMD data transfer accelerators that
// we may import from the `kdu_stripe_decompressor' implementation by including
// "stripe_decompressor_local.h" directly into the kdu_vex_fast project.  The
// actual inclusion of this header happens inside "kdu_vex.cpp".
// This is really a hack here, since we should instead be using the
// public `kdu_stripe_decompressor' API's directly -- that is what we expect
// to do in future releases, but it is simpler for the moment just to
// borrow the fast data transfer functions defined within that API.
typedef void (*simd_transfer_func)(void *dst, void **src, int width,
...
// tracks.
class vex_jpx_source : public kdu_compressed_video_source {
  public: // Member functions
    vex_jpx_source(jpx_source *src, kdu_uint32 track_idx,
  ...
  /* Notes:
     ...
     `kdu_compressed_video_source'.  This allows the video processing
  ...
  public: // Member functions
    ...
    virtual void *allocate_mem(int num_components, kdu_dims *component_dims,
      ...
      /* This function is used to allocate the surface buffer memory for
         ...
         object.
         [//]
         The `alignment' argument is used to return the largest value of
         A, such that A is a power of 2 and each row of each image component
         commences on an A-byte aligned memory boundary.  This value may be
         as small as 1 (no alignment virtues at all), but is preferably much
         larger (e.g., 16, 32 or even 64).
         [//]
         ...
         [>>] If `format' = `VEX_FRAME_MEMORY_CONTIG', image components
              ...
              value returned via `alignment' may be as small as 1 if component
              dimensions do not work out well.
         [>>] If `format' = `VEX_FRAME_MEMORY_XRGB', there may be no more
              ...
              mutex at the time.  In this case, the `stride' value is set
              in such a way as to allow for a large value of `alignment',
              suitable for SIMD vector processing with long vectors and
              aligned stores.  For this format, the value returned via
              `frame_bytes' represents the accessible number of bytes in the
              `buffer', but is unlikely to be important.
          [//]
...
struct vex_frame {
  public: // Member functions
    vex_frame()
      { box_buffer=NULL; box_buffer_bytes=0;
        buffer=NULL; buffer_handle=NULL; allocator=NULL; engine=NULL;
        alignment=1; aborted=waiting=false; next=NULL; }
    ~vex_frame()
      { 
  ...
  public: // Input box and codestream
    jpx_input_box box; // Holds the compressed data source for the frame
    kdu_codestream codestream; // Created by the queue manager
    kdu_byte *box_buffer;    // Buffer (and its size) that we can re-use with
    int box_buffer_bytes; // `box.load_in_memory' to avoid malloc/free calls
  public: // Frame samples
    int precision;
    bool is_signed;
    ...
    int alignment; // Max A for which image rows all aligned on A-byte boundary
  ...
  /* Notes:
     ...
     by "kdu_v_compress".  In this case, the value of `alignment' may be
     as small as 1 (i.e., no alignment whatsoever), but may be much higher
     if the image dimensions work out well -- note that `alignment' is always
     a power of 2.
     ...
     The value of `stride' is selected so that `alignment' can be at least
     as large as the alignment implied by each of `KDU_ALIGN_SAMPLES16' and
     `KDU_ALIGN_SAMPLES32', as defined in "kdu_arch.h", and in any event
     no smaller than 16.  This type of representation is selected only if the
     `want_display' argument to `vex_frame_queue::init' is true. */
...
class vex_processor {
  public: // Member functions
    ...
    void start_frame(vex_frame *frame, kdu_thread_env *env=NULL,
                     int double_buffering_height=0, kdu_int32 truncation=-1,
  ...
  private: // Declarations
    ...
    struct vex_tcomp {
      ...
      public: // SIMD accelerators (NULL unless accelerator exists)
        simd_transfer_func xfer_planar;
    };
    struct vex_tile {
      public: // Members
      ...
      public: // Data
        ...
        kdu_sample_allocator sample_allocator; // Used by `engine'
      ...
      public: // SIMD accelerators (NULL unless accelerator exists)
        int xfer_downshift; // Single downshift for all components if possible
        vex_mono_to_xrgb8_func xfer_mono_to_xrgb8;
        vex_rgb_to_xrgb8_func xfer_rgb_to_xrgb8;
      };
  ...
  private: // Data
    ...
    vex_frame *frame;
    int multi_xform_flags;
    kdu_push_pull_params multi_xform_params;
  ...
  public: // Member functions
    vex_engine()
      { 
        ...
        num_threads = 0; worker_yield_frequency = -1;
        double_buffering_height = -1; force_precise = false;
