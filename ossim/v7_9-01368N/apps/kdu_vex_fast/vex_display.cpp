                    ...
                    int total_frame_buffers)
{
  ...
  // free list.
  int byte_alignment = KDU_ALIGN_SAMPLES16*2;
  if (byte_alignment < (KDU_ALIGN_SAMPLES32*4))
    byte_alignment = (KDU_ALIGN_SAMPLES32*4);
  if (byte_alignment < 16)
    byte_alignment = 16;
  int pel_alignment = byte_alignment >> 2; // Measured in 4-byte pixels
  for (; total_frame_buffers > 0; total_frame_buffers--)
    {
      ...
      surf->alloc_size.x += // Make buffer width a multiple of `pel_alignment'
        (-surf->alloc_size.x) & (pel_alignment-1);
      surf->alloc_size.x += pel_alignment; // Allow for alignment adjustments
      success =
  ...
  if (display_thread.exists())
    {
      ...
      mutex.lock();
      surface_ready_event.protected_set(); // Force `run_display_loop' to wake
      mutex.unlock();
  ...
  vex_display::push_frame(vex_frame *frame)
{
  ...
  if (ready_tail == NULL)
    {
      surface_ready_event.protected_set();
  ...
  frame->buffer_handle =
    allocate_mem(0,NULL,4,VEX_FRAME_FORMAT_XRGB,frame->buffer,
                 frame->frame_bytes,frame->stride,frame->alignment);
      ...
      if (!display_shutdown_requested)
        {
          ...
          if ((surf->next = free_surfaces) == NULL)
            surface_free_event.protected_set();
          free_surfaces = surf;
          mutex.unlock();
        }
    } while (!display_shutdown_requested);
  mutex.lock();
  surface_free_event.protected_set(); // In case we need to wake the
                                      // management thread
  mutex.unlock();
}
                            ...
                            size_t &stride, int &alignment)
{
  ...
  surf->frame_region.pos.x =
    ((-(int) _addr_to_kdu_long(surf->locked_rect.pBits)) & (alignment-1)) >> 2;
