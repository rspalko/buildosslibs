 ...
 path, based upon the selected branch information returned by the browser. */
{
  ...
  while ((node != NULL) && (node->browser_column < column))
    {
      int branch_idx = (int)[browser selectedRowInColumn:node->browser_column];
...
bool kdms_metanode::is_visible_in_browser()
{
  ...
  if ((parent->parent != NULL) &&
    ...
    return false;
  int first_visible_column = (int)[browser firstVisibleColumn];
  int last_visible_column = (int)[browser lastVisibleColumn];
...
- (kdms_metashow_delegate *)initWithOwner:(kdms_renderer *) renderer
{
  owner = renderer;
  browser = nil;
  info_text_panel = nil;
...
- (void) update_info_text
{
  ...
  if (display_node->box_type == jp2_file_type_4cc)
    { // Read entire box and generate representation from scratch
      ...
      box.close();
      display_node_content_bytes = (int)display_node->leaf_content_bytes;
...
{
  int column = (int)[sender selectedColumn];
  kdms_metanode *node =
    find_selected_node_in_browser_column(tree,sender,column);
  if (node == NULL)
    return;
  int branch_idx = (int)[sender selectedRowInColumn:column];
  for (node=node->children; node != NULL; node=node->next)
...
{
  int column = (int)[browser selectedColumn];
  kdms_metanode *node =
    find_selected_node_in_browser_column(tree,browser,column);
  if (node == NULL)
    return;
  int branch_idx = (int)[browser selectedRowInColumn:column];
  for (node=node->children; node != NULL; node=node->next)
    if (node->browser_row == branch_idx)
      break;
...
- (kdms_metashow *)initWithOwner:(kdms_renderer *)renderer
                   ...
                   andIdentifier:(int)the_identifier
{
  ...
  browser =
    [[NSBrowser alloc] initWithFrame:browser_rect];
  ...
  text_panel_frame =
    [[NSBox alloc] initWithFrame:text_panel_frame_rect];
  ...
  NSScrollView *scroll_view =
    [[NSScrollView alloc] initWithFrame:text_panel_rect];
  ...
  NSColor *text_panel_background_colour =
    [NSColor colorWithCalibratedRed:0.95F green:0.9F blue:1.0F alpha:1.0F];
  ...
  text_panel =
    [[NSTextView alloc] initWithFrame:text_panel_content_rect];
  ...
  delegate =
    [[kdms_metashow_delegate alloc] initWithOwner:renderer];
