...
void kdms_property_node::put_text(const char *string)
{
  ...
  int line_length = (int)(new_line_ptr - buffer);
...
-(kdms_properties_delegate *)initWithCodestream:(kdu_codestream)cs
{
  root = NULL;
  browser = nil;
...
-(NSInteger)browser:(NSBrowser *)sender numberOfRowsInColumn:(NSInteger)column
{
  ...
  if (column == 1)
    {
      int branch_idx = (int)[sender selectedRowInColumn:0];
...
-(IBAction)user_click:(NSBrowser *)sender
{
  if ([sender selectedColumn] != 1)
    return;
  int branch_idx = (int)[sender selectedRowInColumn:0];
  int item_idx = (int)[sender selectedRowInColumn:1];
  kdms_property_node *elt = root;
  ...
  kdms_message_window *wnd =
    [[kdms_message_window alloc] initWithTitle:wnd_title
                                    andMessage:attributed_string];
  [wnd run_modal];
            ...
            codestream_index:(int)cs_idx
{
  ...
  delegate =
    [[kdms_properties_delegate alloc] initWithCodestream:cs];
  if (!delegate)
    return false;
  ...
  // Create browser
  NSBrowser *browser =
    [[NSBrowser alloc] initWithFrame:content_rect];
...
- (kdms_message_window *)initWithTitle:(NSString *)title
                            andMessage:(NSAttributedString *)message
{
  ...
  NSColor *bg_colour =
    [NSColor colorWithCalibratedRed:0.95F green:0.9F blue:0.85F alpha:1.0F];
  NSTextField *text_field =
    [[NSTextField alloc] initWithFrame:content_rect];
