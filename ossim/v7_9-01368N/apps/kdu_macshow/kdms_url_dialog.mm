...
- (NSString *) run_modal
{
  ...
  [transport_popup addItemWithTitle:@"http-udp"];
...
- (IBAction) clicked_choose_cache:(id)sender
{
  ...
  if ([cache_dir length] > 0)
    { 
      NSURL *initial_url=[NSURL fileURLWithPath:cache_dir isDirectory:YES];
      initial_url = [initial_url URLByDeletingLastPathComponent];
      [panel setDirectoryURL:initial_url];
    }
  [panel setAllowedFileTypes:nil];
  if ([panel runModal] == NSModalResponseOK)
    {
      NSURL *cache_url = [[panel URLs] objectAtIndex:0];
      cache_dir = [cache_url path];
      [[cache_dir_field cell] setStringValue:cache_dir];
      bool have_cache_dir = ([cache_dir length] > 0);
      use_cache = have_cache_dir;
      [use_cache_button setEnabled:(have_cache_dir)?YES:NO];
