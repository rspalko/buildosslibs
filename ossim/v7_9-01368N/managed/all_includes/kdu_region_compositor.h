...
class kdu_compositor_buf {
  ...
  */
  public: // Member functions
    kdu_compositor_buf()
      { 
        rendering_scale = 0.0f; previous_rendered_content_valid = false;
        first_new_rendered_row = rendered_rows = 0;
        locked_for_read=locked_for_write=false; internal_buf_handle = NULL;
    ...
    virtual ~kdu_compositor_buf()
      ...
      /* [SYNOPSIS]
           ...
           of this class, be aware of the fact that the internal `buf' and
           `float_buf' arrays will be passed directly to the C++ `delete'
           operator if non-NULL.  This is done for legacy reasons only.  If
           you have done anything other than allocate the buffers using
           `new' (and you really should, because it is always preferable to
           at least align buffers on a 16-byte boundary), be sure to override
           the destructor and set them to NULL before exiting your version
           of the function.
      */
    KDU_AUX_EXPORT void init(kdu_uint32 *buf, int row_gap)
      { 
        ...
        previous_rendered_content_valid = false;
        first_new_rendered_row = rendered_rows = 0;
      }
     /* [SYNOPSIS]
          ...
          bits per sample, starting with Alpha (most significant byte) and
          followed either by Red then Green then Blue or Blue then Green then
          Red, depending on the prevailing colour order convention (see
    ...
    KDU_AUX_EXPORT void init_float(float *float_buf, int row_gap)
     ...
     /* [SYNOPSIS]
          ...
          Alpha, and followed either by Red, Green then Blue, or by
          Blue, Green then Red, depending on the colour order convention
          configured via `kdu_region_compositor::set_colour_order'.
          [//]
    ...
    float get_rendering_scale() { return rendering_scale; }
      /* [SYNOPSIS]
           Returns the scale at which this buffer has been (or is being)
           rendered.  This is the product of the `scale' and
           `rendering_scale_adjustment' parameters that were originally
           passed in the relevant call to `kdu_region_compositor::set_scale'.
      */
    kdu_dims get_rendering_region() { return surface_dims; }
      /* [SYNOPSIS]
           Returns the dimensions and location of the region represented
           by this buffer within the overall composition whose location and
           dimensions are returned by `get_total_composition_dims'.  The
           region returned by this function is expressed relative to the
           rendering scale (see `get_rendering_scale') which depends on
           both the `scale' and `rendering_scale_adjustment' arguments that
           were passed to `kdu_region_compositor::set_scale'.
      */
    kdu_dims get_rendering_composition_dims() { return image_dims; }
      /* [SYNOPSIS]
           Returns the location and size of the total composited image,
           within which the current buffer might only represent a limited
           region of interest, as returned by `get_surface_dims'.  The
           information returned by this function is expressed relative to
           the rendering scale (see `get_rendering_scale') which depends on
           both the `scale' and `rendering_scale_adjustment' arguments that
           were passed to `kdu_region_compositor::set_scale'.  By contrast,
           `kdu_region_compositor::get_total_composition_dims' does not
           depend on the `rendering_scale_adjustment' parameter to
           `kdu_region_compositor::set_scale'.
      */
    bool get_rendering_status(int &num_rendered_rows,
                              int &first_new_row, bool reset)
      { 
        num_rendered_rows = this->rendered_rows;
        first_new_row = this->first_new_rendered_row;
        bool result = this->previous_rendered_content_valid;
        if (reset)
          { this->previous_rendered_content_valid = true;
            this->first_new_rendered_row = this->rendered_rows; }
        return result;
      }
      /* [SYNOPSIS]
           This function is useful for applications that wish to display
           the content of composition buffers that are in the process of
           being rendered -- e.g., after each return from the
           `kdu_region_compositor::process' function.
           [//]
           Upon return, `num_rendered_rows' will be equal to the number of
           initial rows of the buffer that can be considered to hold fully
           rendered content, while all rows from `first_new_row' to
           `num_rendered_rows'-1 can be considered to be newly rendered,
           since the function was last called with `reset'=true.  It is
           often the case that `first_new_row' is equal to the value of
           `num_rendered_rows' from the last call with `reset'=true, but
           this is not necessarily the case.  In particular, if the buffer
           has been refreshed or allocated since the last call to this
           function, `first_new_row' will be 0.
           [//]
           The function's return value is also important.  It lets the caller
           know whether the imagery represented by the buffer has changed in
           some fundamental way, since the last call to this function with
           `reset'=true.  Fundamental changes include changes in the buffer
           size or the true location (on the image) of the buffer surface,
           changes in the rendering scale or orientation, or changes in the
           image components, codestreams or compositing instructions (i.e.,
           composited frame) associated with the buffer.  Changes that are
           not considered fundamental are the removal or introduction of
           metadata overlays, and changes resulting from calls to
           `kdu_region_compositor::refresh'.
           [//]
           A false return means that there has been some fundamental change,
           in response to which the next attempt the application makes to
           display content from the buffer should involve displaying the
           entire buffer (or perhaps uploading it to a graphics texture for
           display), as opposed to displaying/updating just the rows
           delineated by `first_new_row' and `num_rendered_rows'.  The
           content beyond `num_rendered_rows' will, in any case, at least
           hold erased background, so this is usually the least visually
           confusing thing to do.
         [RETURNS]
           False if there has been some fundamental change in the imagery
           represented by the buffer, since the last call to this function
           with `reset'=true.  True means that there has been no such
           fundmental change so that it would be appropriate to update a
           displayed version of the buffer just within the region identified
           by `first_new_row' and `num_rendered_rows'.
         [ARG: num_rendered_rows]
           Used to return the number of initial buffer rows of the buffer that
           have been fully rendered so far.  This value might be zero even if
           some region has been rendered, as returned via the `process'
           function, but the reverse will not be true.
           [//]
           Beyond these rendered rows, the buffer will always hold either
           erased background or meaningful content from previous rendering
           operations with the same fundamental interpretation (possibly
           with/without overlays or with lower quality data).
           [//]
           Rendering is complete once `num_rendered_rows' is equal to
           the height of the region returned by `get_rendering_region'.
         [ARG: first_new_row]
           Index of the first row of the buffer (0 is the index of its first
           row) that has been newly rendered, since the last call to this
           function that specified `reset'=true.  The `first_new_row' is
           guaranteed to be no greater than `num_rendered_rows'; if equal,
           there are no new rendered rows.
         [ARG: reset]
           If true, the internal rendering status is reset so that the next
           call reports only changes that have occurred since this call.
           Otherwise, the next call will return exactly the same information
           as this one, if nothing has changed internally in the meantime.
      */
    int get_rendered_rows() { return this->rendered_rows; }
      /* [SYNOPSIS]
           Returns just the number of fully rendered rows out of the
           complete set of information returned via `get_rendering_status'.
           Rendering is complete once this value equals the height of the
           region returned by `get_rendering_region'.
      */
    bool get_region(kdu_dims src_region, kdu_int32 tgt_buf[],
                    int tgt_offset=0, int tgt_row_gap=0)
      { 
        if ((buf == NULL) || (tgt_buf == NULL)) return false;
        src_region &= accessible_region;
        if (tgt_offset > 0) tgt_buf += tgt_offset;
    ...
    bool get_float_region(kdu_dims src_region, float tgt_buf[],
                          int tgt_offset=0, int tgt_row_gap=0)
      { 
        if ((float_buf == NULL) || (tgt_buf == NULL)) return false;
        src_region &= accessible_region;
        if (tgt_offset > 0) tgt_buf += tgt_offset;
      ...
      /* [SYNOPSIS]
         ...
         [ARG: tgt_buf]
           Buffer into which the results are to be written.  Note that the
           buffer consists of 4 floating point samples per pixel, starting
           with Alpha, and followed either by Red, Green then Blur, or by
           Blue, Green then Red, depending on the colour order convention
           established by `kdu_region_compositor::set_colour_order'.
         [ARG: tgt_offset]
           Number of samples (not pixels) from the start of the supplied
           `tgt_buf' array at which you would like to start writing the data.
           This must not be negative, of course.
         [ARG: tgt_row_gap]
           Separation between rows in the target buffer, measured in samples
           (not pixels).  This cannot be negative.  If zero, the pixels
           corresponding to the `src_region' will be written contiguously
           into the buffer.
      */
  protected:
    virtual void lock_buf(bool read_write)
      ...
      */
  public: // Functions used only inside `kdu_region_compositor'
    void post_init(const kdu_coords &actual_size, void *handle)
      { this->accessible_region.size = actual_size;
        this->internal_buf_handle = handle; }
      /* Called by `kdu_region_compositor::internal_allocate_buffer' once
         the `init' (or `init_float') function has been performed.
         `handle' is non-NULL if the buffer was allocated internally, in which
         case this is the base of the allocated memory block to be deleted
         automatically by the destructor.  If `handle' is NULL, a custom
         override of `kdu_region_compositor::allocate_buffer' actually created
         the memory buffer, passing it to `init' (or `init_float') and so
         deletion must also be handled externally. */
    bool need_external_delete() { return (internal_buf_handle==NULL); }
      /* Called from `kdu_region_compositor::internal_delete_buffer' to
         determine how to delete the buffer.  If the function returns true,
         the buffer needs to be passed to an external override of
         `kdu_region_compositor::delete_buffer'; otherwise, the destructor
         will do everything required. */
    void set_geometry(float scale, const kdu_dims &surface,
                      const kdu_dims &composition_dims)
      { this->rendering_scale=scale; this->surface_dims=surface;
        this->image_dims = composition_dims; }
      /* Supplies the rendering scale, along with the location and size of
         the surface region and of the complete composited image, expressed
         relative to the rendering scale, so that these values can be queried
         by the application via `get_rendering_scale',
         `get_rendering_region' and `get_rendering_composition_dims'. */
    void set_invalid()
      { first_new_rendered_row = rendered_rows = 0;
        previous_rendered_content_valid = false; }
      /* This function is used internally by `kdu_region_compositor' to
         identify the fact that there has been a fundamental change in the
         compositing surface, so that any previously generated content is
         no longer consistent with content that will be rendered in the
         future -- the next call to `get_rendering_status' will return
         false. */
    void renovate()
      { previous_rendered_content_valid = false; first_new_rendered_row = 0; }
      /* This function is used internally by `kdu_region_compositor' whenever
         it moves a compositing layer from the inactive list onto the active
         list, to indicate that that the layer's surface is new from the
         perspective of the application; this is particularly important when
         rendering individual compositing layers, where there is no separate
         compositing buffer. */
    void reset_rendered_rows()
      { first_new_rendered_row = rendered_rows = 0; }
      /* This function is used internally by `kdu_region_compositor' to
         indicate that rendering for a buffer will be starting from scratch.
         It is not required if `set_invalid' has already been called. */
    void note_new_rendered_rows(int first_row, int num_rows)
      { if (first_row < first_new_rendered_row)
          first_new_rendered_row = first_row;
        if (num_rows > rendered_rows)
          rendered_rows = num_rows;
      }
      /* This function is used internally by `kdu_region_compositor' to
         indicate that rendering has just generated finished content for all
         rows from `first_row' to `num_rows'-1.  This affects the number of
         newly rendered rows that will be identified later by calls to
         `get_rendering_status'. */
  private: // Members related to rendering state and geometry
    float rendering_scale; // Scale at which the buffer was/is being rendered
    kdu_dims surface_dims; // Absolute location & size of buffer surface
    kdu_dims image_dims; // Location & size of full image at `rendering_scale'
    int rendered_rows; // Portion of buffer fully rendered so far 
    int first_new_rendered_row; // Used by `get_rendering_status'
    bool previous_rendered_content_valid; // Returned by `get_rendering_status'
  private: // Members configured by `post_init'.
    kdu_dims accessible_region; // Region which can be accessed via the
      // `get_region' function (relative to the buffer's top-left corner).
      // This is used only for access checking; it should have the same
      // size as `surface_dims', but may have a different location.
    void *internal_buf_handle; // Non-NULL if buffer was allocated internally
  protected: // Data
...
class kdu_overlay_params {
  ...
  public: // Member functions
    ...
    int get_max_painting_border() { return max_painting_border; }
      /* [SYNOPSIS]
           Returns a copy of the `max_painting_border' parameter supplied in
    ...
    kdu_uint32 get_aux_param(int n)
      ...
      /* [SYNOPSIS]
           Returns a copy of the `n'th auxiliary painting parameter supplied by
           the most recent call to `kdu_region_compositor::configure_overlays'.
      ...
      */
    KDU_AUX_EXPORT void
      push_aux_params(const kdu_uint32 *aux_params, int num_aux_params);
      /* [SYNOPSIS]
  ...
  private: // Data
    friend class kd_supp_local::kdrc_overlay;
    int codestream_idx;
...
class kdu_region_compositor {
  /* [BIND: reference]
     [SYNOPSIS]
       ...
       in a manner that requires no understanding of the underlying machinery
       that generally resamples, re-orients, discards resolution levels,
       converts colour and so forth.
  */
  //---------------------------------------------------------------------------
  public: // Lifecycle member functions
    KDU_AUX_EXPORT
      ...
      /* [SYNOPSIS]
           ...
           [//]
           If you are supplying a non-NULL `env' argument to this constructor,
           we strongly recommend that you read the description of the
           `set_thread_env' function, since all the constraints that are
           described there also apply when the object is constructed directly
           with a non-NULL `kdu_thread_env' reference.  In particular, it
           is IMPERATIVE that all access to this object's interface functions
           be restricted to the thread that is associated with the non-NULL
           `env' object, at least until `set_thread_env' is called again
           with a different (or NULL) `env' argument.       
           [//]
           If you never retract the multi-threading environment supplied by the
           `env' argument (can be done by passing NULL to `set_thread_env'),
           you should be careful to destroy the present object before
           destroying the `env' object, unless you take steps to first close
           all internal open codestreams by using `remove_ilayer' with
           (`permanent'=true).
      */
    KDU_AUX_EXPORT
    ...
    KDU_AUX_EXPORT virtual void pre_destroy();
      /* [SYNOPSIS]
           ...
           [//]
           Note carefully that this function invokes `flush_composition_queue'.
           According to the documentation for `flush_composition_buffer', a
           supported design pattern is one in which two different threads
           manage access to this object (a processing thread, which does
           almost everything and a display thread which accesses only the
           `inspect_composition_queue', `get_composition_buffer' and
           `pop_composition_buffer' functions, working only with queued
           buffers).  Following this design pattern, you should make sure
           that the display thread, if any, is inactive by the time this
           function is called from the processing thread.
      */
    KDU_AUX_EXPORT void
    ...
    KDU_AUX_EXPORT void
      ...
      */
    KDU_AUX_EXPORT void set_colour_order(kdu_colour_order colour_order);
      /* [SYNOPSIS]
           This function is normally called right after `create' to configure
           the colour channel ordering convention used by all buffers.  It is
           not advisable to change the colour ordering convention after
           rendering has commenced, since this may lead to an unpredictable
           mixture of content rendered with different conventions.
           [//]
           All surface buffers hold four channels per pixel, the first of
           which is always Alpha, followed either by Red then Green then Blue,
           or by Blue then Green then Red, depending on the `colour_order'
           value.
           [>>] If `colour_order' is `KDU_ARGB_ORDER', floating point buffers
                interleave the channels in the order Alpha, Red, Green and
                then Blue, while 32-bit pixel buffers record Alpha in the
                most significant byte, Red in the next most significant byte,
                then Green and finally Blue in the least significant byte.
           [>>] If `colour_order' is `KDU_ABGR_ORDER', floating point buffers
                interleave the channels in the order Alpha, Blue, Green and
                then Red, while 32-bit pixel buffers record Alpha in the most
                significant byte, Blue in the next most significant byte,
                then Green and finally Red in the least significant byte.
           [//]
           The default colour order (if this function is never called) is
           `KDU_ARGB_ORDER'.
      */
    KDU_AUX_EXPORT void set_error_level(int error_level);
      ...
      */
    void
      configure_scaling_params(float min_resampling_factor=0.6f,
                               float max_interp_overshoot=0.4f,
                               int bilinear_expansion_threshold=2)
      { 
        if (min_resampling_factor < 0.55f) min_resampling_factor = 0.55f;
        if (min_resampling_factor > 0.9f) min_resampling_factor = 0.9f;
        if (max_interp_overshoot < 0.0f) max_interp_overshoot = 0.0f;
        if (bilinear_expansion_threshold < 1) bilinear_expansion_threshold = 1;
        this->scaling_min_resampling_factor = min_resampling_factor;
        this->scaling_max_interp_overshoot = max_interp_overshoot;
        this->scaling_bilinear_interp_threshold = bilinear_expansion_threshold;
      }
      /* [SYNOPSIS]
           This function allows you to configure the way in which scaling is
           performed when rendering at scales that are not identical to
           any of the decoding resolutions available from the underlying
           codestreams.  In simple cases, there are a set of natural
           scales generated by discarding DWT levels from the wavelet
           transform representation.  This is the most efficient way to
           render an image at a different scale.  However, this does not
           generally produce all the scales that might be of interest to
           an application: it cannot produce scales that are higher than
           the original image resolution (interpolation); nor can it
           produce scales that are smaller than the resolution of the
           lowest frequency subband (the LL band); and of course it cannot
           produce a continuum of scales, which is often of interest.
           In fact it may not be possible to render complex compositions
           or animations at any scale at all, without scaling at least
           some of the involved codestreams to sizes that are not available
           by simply discarding DWT levels.
           [//]
           In practice, you can pass just about any scale at all to the
           `set_scale' function, letting the internal machinery take care
           of deciding how many DWT levels to discard from each codestream
           and how to resample the resulting decoded imagery using appropriate
           filtering techniques.  This function, however, allows you to
           customize the way in which this is done.  The `max_interp_overshoot'
           and `bilinear_expansion_threshold' arguments correspond exactly to
           the `max_overshoot' and `zero_overshoot_threshold' arguments
           to `kdu_region_decompressor::set_interpolation_behaviour'.  They
           are passed directly to that function whenever a region decompressor
           is configured to do the underlying decompression, scaling and
           colour conversion.  You are generally advised to stick with the
           default values, but this function provides you with a way to
           modify them.  The default value of `bilinear_expansion_threshold'=2
           means that resampling operations that are used to interpolate from
           a given decoded image resolution will use disciplined 6-tap
           interpolation kernels until the expansion factor reaches 2 or more,
           at which point the much simpler bilinear interpolation technique
           takes over.  Bilinear interpolation is the most common method
           employed by graphics engines, but is decidedly inferior to the
           disciplined method that is used here by default.  You might choose
           to set this parameter to 1 for maximum throughput (minimum CPU
           effort) when resampling involves expansion (scaling by more than 1),
           but the disciplined technique is already implemented in a highly
           efficient manner on most common platforms (X86 and ARM-NEON
           platforms in particular).
           [//]
           The `min_resampling_factor' value is the smallest scaling factor
           to be applied to a decoded image resolution if there are still
           more DWT levels that could be discarded.  By default, the
           `min_resampling_factor' is 0.6, which means that the imagery may
           be decoded at a resolution which is as much as 1.667 times higher
           (in both directions) than the final rendered result.
           [//]
           Another way to interpret `min_resampling_factor' is that the
           maximum amount by which a decoded image resolution will be
           expanded through resampling is `min_resampling_factor' / 0.5,
           unless there are no higher decoding resolutions available (through
           adding DWT levels).  The default value for `min_resampling_factor'
           of 0.6 means that no decoded resolution will be interpolated by
           more than a factor of 1.2, unless it is already the highest
           (original) resolution.
           [//]
           The default value of 0.6 tends to lead to very high image quality
           across pretty much all scales, but it can result in the need to
           decode almost 3 times as many samples as the rendered image will
           contain (1 / 0.6^2).  For mobile platforms, or devices with
           retina displays, this is almost certainly overkill; in such
           circumstances, a value of 0.7 or 0.75 is probably more appropriate
           for the `min_resampling_factor', which is a prime reason for
           us to provide this configuration function.
      */
    void
      configure_intensity_scaling(bool true_zero, bool true_max)
      { 
        this->intensity_true_zero = true_zero;
        this->intensity_true_max = true_max;
      }
      /* [SYNOPSIS]
           This function allows you to control how, or if, the underlying
           `kdu_region_decompressor::set_true_scaling' and
           `kdu_region_decompressor::set_white_stretch' functions are called
           when rendering is required.
           [//]
           Passing false for both arguments preserves (or reinstates) the
           default behaviour where `kdu_region_decompressor::set_white_stretch'
           is used to stretch low-precision source data out in to at least a
           full 8-bit dynamic range, but there is no other mechanism used to
           scale intensities where source and rendered output precisions
           differ.  For unsigned source samples, this can lead to tiny
           departures from what an exact rendering should produce, but only in
           less common scenarios.  The default behaviour for signed source
           data (not so common) is to map original 0 values to a mid-level
           grey rather than to black.  This is usually a good starting point
           for visualising scientific data, since it allows both positive and
           negative departures from 0 to be visualised.  Note that this
           default intensity mapping procedure was the only one available
           prior to KDU version 7.8.
           [//]
           For exact intensity mapping, you should pass true for both the
           `true_zero' and `true_max' arguments, but note that this may
           slightly increase the computational workload.  To map signed
           source data in such a way that 0 maps to black rather than a
           mid-level grey, pass `true_zero'=true and (for the highest
           throughput) `true_max'=false.
      */
    KDU_AUX_EXPORT void
      ...
      /* [SYNOPSIS]
           ...
           is passed to `kdu_region_decompressor::set_quality_limiting',
           along with the `hor_ppi' and `vert_ppi' arguments supplied here,
           where it is used the next time that codestream rendering
           commences.  All codestreams use copies of the same quality
           limiter, with the same display resolution parameters.
         [ARG: hor_ppi]
           ...
           `hor_ppi' value supplied here refers to the resolution at which
           the composited samples will be displayed, including all scaling
           factors.
           [//]
           We generally recommend adopting a resolution of 200 ppi if the
           rendered samples are to be displayed pixel-for-pixel on a
           "non-retina" device and 400 to 600 ppi if they are to be displayed
           ...
           [//]
           This argument supplied horizontal resolution information, while
    ...
    void set_process_aggregation_threshold(float threshold)
      ...
      /* [SYNOPSIS]
           ...
           a `new_rendered_region' which incorporates non-novel content between
           such disjoint regions.  In this case, the area occupied by
           supposedly novel content should be no smaller than `threshold' times
           the area of the `new_rendered_region' returned by `process'.
      */
    KDU_AUX_EXPORT void
      set_surface_initialization_mode(bool pre_initialize);
      /* [SYNOPSIS]
           ...
           efficiency.  From version 7.0, the beneficial effects of this
           function have been substantially expanded to avoid all
           incremental painting, composition and buffer erasing operations
           that can be put off until the intermediate layers of a composition
           have been fully generated.  This usually allows us to save a huge
           amount of erasing and blending when generating frames of an
           ...
           function with `pre_initialize' true and then call `refresh',
           ...
           [//]
           If this all seems to hard for you, and you don't care about speed,
           you don't need to bother with this function at all.
      */
  //---------------------------------------------------------------------------
  public: // Functions used to build and tear down compositions
    KDU_AUX_EXPORT kdu_ilayer_ref
      ...
      /* [SYNOPSIS]
           ...
           [>>] `set_frame' constructs all the imagery layers required to
                build up a complete JPX animation frame.  Importantly,
                every frame of a JPX animation has the same dimensions,
                even if the imagery that is composited onto the frames has
                varying dimensions.
           [>>] `add_frame' adds additional frames to a composition that
                was initiated with `set_frame'; the primary applications for
                this are the consruction of multi-frame "contact sheets" or
                temporary frame pairs for page-to-page interactive
                navigation.
           [//]
           ...
           raw codestreams, the only valid `layer_src' value is 0.
           [//]
           For MJ2 data sources, `layer_src' is interpreted as the MJ2 track
           index (as returned by `mj2_video_source::get_track_idx') minus 1, so
           that a value of zero (rather than 1) references the first track.
           [//]
         ...
         [RETURNS]
           ...
           [//]
           In the special case (and only this case) where a JPX/JP2 data
           source is served by a dynamic cache which does not yet contain
           sufficient information to open the compositing layer, the
           function returns a null `kdu_ilayer_ref' instance -- i.e.,
           `kdu_ilayer_ref::is_null' will yield true.
           [//]
           So long as the compositing layer is accessible, this function
           succeeds in creating the new imagery layer, even the relevant
           codestreams' main headers are not yet available -- can happen if
           the content is provided via a dynamic cache.  In the event that
           codestream main headers are not yet available, functions such as
           `find_optimal_scale', `get_max_available_quality_layers',
           `access_codestream', `get_istream_info', `map_region',
           `inverse_map_region' and the like, may all return unreliable
           results.  This is true also for the `find_compatible_jpip_window'
           function, which is important in forming the most efficient JPIP
           queries for a region on the composition surface.  For these reasons,
           you should consider invoking the `waiting_for_stream_headers'
           function to determine the point at which all required codestream
           headers have become available, at which point you may need to
           re-issue previously unreliable calls -- for example, you might
           issue a new JPIP client request at this point, based on
           `find_compatible_jpip_window'.
         [ARG: layer_src]
         ...
         [ARG: transpose]
           ...
           recovered via `mj2_video_source::get_cardinal_geometry', or the
           `jpx_composited_orientation' information recovered via
           `jpx_composition::get_instruction' or
           `jpx_frame_expander::get_member'.
         [ARG: vflip]
           See `transpose'.
         [ARG: hflip]
           See `transpose'.
    ...
    KDU_AUX_EXPORT bool
      ...
      /* [SYNOPSIS]
           ...
           only when the ilayer was created from an MJ2 data source.  The
           "frame" in question here is not a JPX animation frame, but a
           video frame within the MJ2 source.
           [//]
           ...
           `kdu_error'.
         [RETURNS]
           False if the requested frame does not exist.  In this case, the
    ...
    KDU_AUX_EXPORT void set_frame(jpx_frame_expander *expander,
      ...
      /* [SYNOPSIS]
           ...
           layers and compositing instructions associated with the complete
           ...
           of the frame onto this surface.  This means that calls to
           `set_frame' always result in image surfaces that have exactly the
           same dimensions, assuming the data source does not change.  Of
           course the dimensions are subject to changes of scale and
           orientation (see `set_scale'), but navigating between frames
           of fixed dimension is more natural than navigating between
           individual compositing layers, whose dimensions may vary.
           [//]
           ...
           invoking this function.
           [//]
           Like `add_ilayer', this function succeeds even if some or all of
           the required codestreams' main headers are unavailable -- can
           happen where the ultimate source of data is a dynamic cache.
           In this event, functions such as `find_optimal_scale',
           `get_max_available_quality_layers', `access_codestream',
           `get_istream_info', `map_region', `inverse_map_region' and the
           like, may all return unreliable results.  This is true also for
           the `find_compatible_jpip_window' function, which is important
           in forming the most efficient JPIP queries for a region on
           the composition surface.  For these reasons, you should consider
           invoking the `waiting_for_stream_headers' function to determine
           the point at which all required codestream headers have become
           available, at which point you may need to re-issue previously
           unreliable calls -- for example, you might issue a new JPIP
           client request at this point based on `find_compatible_jpip_window'.
         [ARG: offset]
           In most cases `offset' will take its default value of (0,0), and
           indeed you are highly recommended to make sure that this is the
           case unless you intend to form a multi-frame composition by
           following this call with one or more calls to `add_frame'.  In the
           case of multi-frame compositions, however, it can be useful for
           the first frame of the composition to be anchored at some location
           other than the origin of the composition surface, since this is the
           only frame whose members are required to all be available.
           [//]
           The `offset' is interprted as a displacement of the original image
           layers that make up the frame, after application of all relevant
           compositing instructions, as found within `expander'.  
      */
    KDU_AUX_EXPORT bool
      add_frame(jpx_frame_expander *expander, kdu_coords offset);
      /* [SYNOPSIS]
           You can call this function one or more times after `set_frame' to
           build up compositions that include the content from multiple
           original JPX composited frames.  Usually, the `expander' passed to
           this function would be different to that passed to `set_frame',
           or would at least have been reconfigured, so that it represents
           a different JPX source frame.  However, this is not required.
           Also, `offset' will usually be different from that passed to
           `set_frame' so that the added frames can sit side-by-side, or
           within a grid, but any amount of overlap can be introduced between
           frames if you like -- their Z-order follows the order in which
           they are added.
           [//]
           This function is particularly useful for mobile apps, where content
           is usually rendered to a fixed size window (full display).  In this
           case, if the content is rendered at a very small `scale', it is
           often intuitive to lay out multiple frames on a grid and display
           them as a single image -- sometimes called a "contact sheet".  The
           function can also be used to implement interactive "page flipping",
           where consecutive frames are interpreted as pages, and certain types
           of user navigation slide the viewport between the pages.
           [//]
           Regardless of the application, the mechanism is completely general.
         [RETURNS]
           False if any of the compositing layers (members) found within
           `expander' is not yet fully available.  In this case, the function
           still augments the compositing surface, but may not add some
           compositing layers which will eventually belong to that surface.
           Indeed even if `expander' contains no members at all, the surface
           is still augmented to reflect the region that the frame will
           eventually occupy.
         [ARG: expander]
           Unlike `set_frame', the `epander' argument passed to this function
           can be NULL or refer to an object that has an incomplete (or even
           empty) set of members.  This can be useful, since it allows space
           to be reserved within the composition for an incomplete or
           non-existent frame.
         [ARG: offset]
           Interprted as a displacement of the original image layers, after
           application of all relevant compositing instructions,
           as found within `expander'.  Each frame of a JPX source has exactly
           the same original image dimensions, notionally anchored at (0,0),
           but this function constructs the overall composition that is formed
           from a union of such frame rectangles, each displaced by its
           `offset'.  Morever, `offset' may be negative, so the new overall
           composition region is generally larger than that of the nominal
           JPX composition frame that need no longer be anchored at (0,0).
      */
    KDU_AUX_EXPORT bool
      waiting_for_stream_headers();
      /* [SYNOPSIS]
           This function is provided because `set_frame' and `add_ilayer' can
           succeed, even if the relevant codestream headers are not
           available at the time when they are called.  This allows buffers
           to be allocated, `set_scale', `get_total_composition_dims',
           `process' and many other functions to be used successfully,
           even if no codestream content or even codestream headers are
           available yet.  However, the results produced by some functions
           are not necessarily reliable until the codestream headers are
           available.  The most important example of this is the
           `find_compatible_jpip_window' function, which may under-estimate
           or over-estimate the size of the JPIP request window that should
           be issued for a given scale (as set using `set_scale') until
           all relevant codestream main headers have become available.
           [//]
           If the present function returns false, you can be sure that all
           codestream headers are loaded and sufficient information is
           available to reliably map regions to/from the codestream, access
           the codestreams, construct the most compatible JPIP window
  ...
  public: // Functions for adjusting/querying the composed image appearance
    KDU_AUX_EXPORT void
      set_scale(bool transpose, bool vflip, bool hflip, float scale,
                float rendering_scale_adjustment=1.0f);
      /* [SYNOPSIS]
           ...
           [//]
           From KDU-7.6, this function allows you to set two scale factors
           simultaneously, which is very useful for interactive rendering
           applications that are computation-constrained, but may be a little
           confusing.  The `scale' parameter determines the significance of
           all dimensional and location information returned by and/or passed
           into virtually all of the region-compositor's member functions.
           However, the actual composited buffer returned by
           `get_composition_buffer' can be rendered at a different scale
           (usually smaller, to save computational effort during interactive
           re-rendering of content) that is obtained by multiplying `scale' by
           `rendering_scale_adjustment'.  This (potentially different)
           rendering scale, as well as the full image dimensions and buffer
           surface region, expressed relative to the rendering scale, may be
           obtained by querying the `kdu_compositor_buf' member functions
           offered by the composition buffer directly.  The functions of
           interest here are:
           [>>] `kdu_compositor_buf::get_rendering_scale';
           [>>] `kdu_compositor_buf::get_rendering_region'; and
           [>>] `kdu_compositor_buf::get_rendering_composition_dims'.
           [//]
           After calling this function, and before calling `process', you
           must call `set_buffer_surface' to identify the portion of the
           image you want rendered, expressed relative to `scale' and the
           geometry represented by `transpose', `hflip' and `vflip'.  You must
           not assume that the image buffer returned by a previous call to
           `get_composition_buffer' is still valid, so you should generally
           call that function again also to get a handle to the buffer which
           ...
           during any step which they executed internally (this could happen
           half way through processing a region, if a tile-component with
           insufficient DWT levels is encountered, for example).
         [ARG: scale]
         ...
         [ARG: rendering_scale_adjustment]
           This default value of 1.0 for this argument ensures that the
           composition buffer is rendered at the scale supplied via `scale',
           which is what one would normally expect.  It can be useful, however,
           to supply a value less than 1.0 (typically 0.5 or 0.25), in which
           case the composition buffer is dimensioned and rendered as if
           the `scale' argument had been adjusted by this same factor, but
           all dimensions and locations passed across the region-compositor's
           API functions remain consistent with `scale', with the sole
           exception of the `new_rendered_region' argument returned by
           the `process' function and the `invalidated_region'
           argument supplied to the `invalidate_rect' function.
           [//]
           For example, if `scale'=1.0 and `rendering_scale_adjustment'=0.5,
           the total image dimensions returned by `get_total_composition_dims'
           and the buffer surface region requested by `set_buffer_surface' (and
           returned via `get_composition_buffer') are all expressed at full
           scale (i.e., `scale'=1.0), whereas the composition buffer is
           rendered at half scale, having roughly half as many rows and
           columns as one would expect based on the surface region supplied to
           `set_buffer_surface' and returned via `get_composition_buffer'.
           The actual rendered buffer surface dimensions and the total
           composited image region to which it belongs may be retrieved by
           calling member functions offered by the `kdu_compositor_buf'
           object returned by `get_composition_buffer'.
           [//]
           The scale supplied via the `scale' argument dictates all aspects
           of the behaviour of the region-compositor other than actual
           rendering, which is scaled according to the product of `scale'
           and `rendering_scale_adjustment'.  For example, the
           `generate_metareq' function generates JPIP metadata requests that
           are relevant to a particular region of interest, expressed relative
           only to `scale'.  As another example, `find_point' locates the
           imagery layer that contains a particular `point', expressed relative
           only to `scale', even though it may assess visibility using content
           rendered at a different resolution (if `rendering_scale_adjustment'
           is not 1.0).  There are many other API functions that work with
           locations and regions, all of which are expressed relative only to
           `scale', the only two exceptions being the `process' and
           `invalidate_rect' functions which necessarily refer to the
           rendering process itself.
           [//]
           The `rendering_scale_adjustment' argument may take any positive
           value, but the product of `scale' and `rendering_scale_adjustment'
           must ultimately represent a scale at which content can be rendered.
           If not, the `check_invalid_scale_code' function may report that the
           rendering scale is too small or too large and `find_optimal_scale'
           can be used to determine the rendering scale (i.e., the product of
           `scale' and `rendering_scale_adjustment') that should be used to
           avoid invalid rendering conditions.
           [//]
           The reason for providing a separate `rendering_scale_adjustment'
           parameter, that affects only the rendered composition buffers, but
           none of the other dimensional information passed across the
           region-compositor's API functions (apart from `process' and
           `invalidate_rect') is that this greatly simplifies the
           implementation of interactive rendering applications that may
           prefer to temporarily enter a "low-res" rendering state for
           performance reasons (e.g., during a repetitive sequence of user
           interactions that require frequent re-rendering).
      */
    KDU_AUX_EXPORT float
      ...
      /* [SYNOPSIS]
           ...
           `scale_anchor'.
           [//]
           The value returned by this function is a rendering scale, meaning
           that a call to `set_scale' should pass `scale' and
           `rendering_scale_adjustment' arguments whose product is equal to the
           returned value in order to achieve the intended result.
         [ARG: region]
           ...
           sizing information.
           [//]
           Note that the `region' argument is interpreted with respect to
           the `scale', `hflip', `vflip' and `transpose' arguments passed in
           the most recent call to `set_scale', ignoring any
           `rendering_scale_adjustment' argument that may also have been
    ...
    KDU_AUX_EXPORT void set_buffer_surface(kdu_dims region,
      ...
      /* [SYNOPSIS]
           ...
           [//]
           Note also that the supplied `region' (and indeed the surface region
           returned via `get_composition_buffer') are all expressed relative
           to the `scale' argument passed to `set_scale', taking into account
           the `transpose', `vflip' and `hflip' arguments also supplied to
           that function, but ignoring its `rendering_scale_adjustment'
           argument.  If the latter was not equal to 1.0, the actual scale at
           which the buffer is rendered (and hence the actual surface region)
           will be based on the product of the `scale' and
           `rendering_scale_adjustment' arguments passed to `set_scale'.  This
           is explained in the comments accompanying `set_scale'.
           [//]
           Note also that any working buffer previously recovered using
           `get_composition_buffer' will no longer be valid.  You must call
           that function again to obtain a valid buffer.  Nevertheless,
           queued buffers recovered using `get_composition_buffer' remain
           valid.
         [ARG: background]
           ...
           in your application.  Buffers may have either a 32-bit/pixel
           organization or a floating-point organization.  In the former case,
           Alpha, Red, Green and Blue components all lie in the range 0 to 255.
           In the floating-point case, Alpha, Red, Green and Blue components
           all lie in the range 0.0 to 1.0 and the four 8-bit values provided
           via `background' are divided by 255 to obtain their floating-point
           equivalents.
           [//]
           The most significant byte of `background' holds the 8-bit Alpha
           value; the next most significant bytes hold either Red then Green
           then Blue, or Blue then Green then Red, depending on the colour
           order convention established by `set_colour_order'.
           [//]
           ...
           top of a background will not alter the alpha value of the
           background.  It also means that the alpha value will be M (opaque)
           if either the background or the composed imagery are opaque.
      */
    int check_invalid_scale_code() { return invalid_scale_code; }
      /* [SYNOPSIS]
           ...
           the rendering scale requested by the most recent call to `set_scale'
           is too large, too small, or its impact on the overall rendering
           dimensions needs to change slightly, given the limited number of DWT
           levels which can be discarded during decompression of some relevant
           ...
           [>>] `KDU_COMPOSITOR_CANNOT_FLIP' -- remedy: try an alternate
                rendering geometry.
           [//]
           If you need to change the rendering scale in response to
           ...
           [//]
           If the return value is 0, none of the above-mentioned problems has
           been detected.  It is worth noting, however, that the
           `invalid_scale_code' value returned here is reset to 0 each time
           `set_scale' is called, after which an attempt must be made to call
           one of `process', `get_total_composition_dims' or
           `get_composition_buffer' before an error code can be detected.
           [//]
    ...
    KDU_AUX_EXPORT bool get_total_composition_dims(kdu_dims &dims);
      /* [SYNOPSIS]
           ...
           component) together with the `scale', `transpose', `hflip' and
           `vflip' parameters supplied to `set_scale'.  This is the region
           within which the composition buffer's region may roam (for
           interactive panning).
           [//]
         ...
         [RETURNS]
           False if composition cannot be performed at the current rendering
           scale or with the current flipping requirements.  These conditions
           may not be uncovered until some processing has been performed, since
           it may not be until that point that some tile-component of some
           code-stream is found to have insufficient DWT levels.  If this
           happens, you should call `check_invalid_scale_code' and make
           appropriate changes in the scale or rendering geometry before
           proceeding.
      */
    KDU_AUX_EXPORT virtual kdu_compositor_buf *
      get_composition_buffer(kdu_dims &region, bool working_only=false);
      /* [SYNOPSIS]
           ...
           on the composited image, expressed relative to the `scale',
           `transpose', `vflip' and `hflip' arguments supplied to `set_scale'.
           If the `rendering_scale_adjustment' argument to `set_scale' was
           equal to anything other than 1.0, the returned `kdu_compositor_buf'
           object can be expected to have a different size to that identified
           by `region.size' upon return, since `rendering_scale_adjustment'
           information is not allowed to affect any of the dimensions or
           locations passed across this object's API functions; if a
           non-trivial rendering scale adjustment is in force, you can discover
           the actual rendering scale by calling the returned buffer's
           `kdu_compositor_buf::get_rendering_scale', and you can discover the
           actual rendered region and the total composited image region within
           which it resides by calling
           `kdu_compositor_buf::get_rendering_region' and
           `kdu_compositor_buf::get_rendering_composition_dims'.
           [//]
           You should call this function again whenever you do anything which
           may affect the geometry of the composited image.
           ...
           `remove_ilayer'.
           [//]
           In the case where `push_composition_buffer' has been used to
           create a jitter absorbtion queue, there may be many composition
           buffers stored within the system, possibly with different buffer
           regions and/or custom regions.  Amongst these, two are available
           for access: the working buffer (identified as W in the symbolic
           representation provided with the documentation of
           `push_composition_buffer') and the head of the queue (identified by
           Q1 in the symbolic representation provided with the documentation of
           `push_composition_buffer').  The `working_only' argument allows you
           to discriminate between these two buffers.  If `working_only' is
           true, your application will gain access to the current working
           buffer, that is affected by calls to `process'; in multi-threaded
           applications that involve a processing master thread and a separate
           display thread, you should generally set `working_only' to true
           when invoking this function from the processing thread.  If
           `working_only' is false, your application will gain access to Q1,
           if it exists, or else W; this is the buffer that is most likely
           intended for display at the present time.
           [//]
           As explained in the documentation accompanying
           `push_composition_buffer', applications in which the jitter
           absorbtion queue is accessed from multiple threads should provide
           mutual exclusion locks to guard entry and exit from this function
           (typically, but not necessarily by overriding the function), at
           least for the case in which `working_only' is false.
         [RETURNS]
           NULL if there is no pushed composition buffer on the jitter
           absorbtion queue (or `working_only' is true) AND the composition
           cannot be performed at the current scale, or with the current
           flipping requirements -- see the description of
           ...
           [>>] If `kdu_compositor_buf::get_buf' returns non-NULL, the buffer
                ...
                byte in each word represents Alpha, followed either by Red
                then Green then Blue, or by Blue then Green then Red, depending
                on the colour order convention (see `set_colour_order').
           [>>] If `kdu_compositor_buf::get_float_buf' returns non-NULL, the
                buffer uses 4 floating point values to represent each pixel.
                The first one is Alpha, followed either by Red then Green then
                Blue, or by Blue then Green then Red, depending on the
                prevailing colour order defined by `set_colour_order'.
                All sample values have been scaled so to a nominal range of
                0.0 to 1.0, where the maximum amplitude for a colour plane
                is precisely 1.0.
         [ARG: working_only]
           If true, you are requesting access to the internal working buffer
           that is affected by calls to `process'.  Otherwise, you are
           requesting access either to the working buffer or the head of the
           jitter absorbtion queue, if some composition buffer has been pushed
           to the queue via `push_composition_buffer'.
      */
    KDU_AUX_EXPORT virtual bool
      push_composition_buffer(kdu_long custom_stamp, int custom_id_val);
      /* [SYNOPSIS]
           ...
           buffer is appended to the tail of the internal queue, along with
           a record of the surface region represented by the buffer, and
           ...
           of the queue, while `inspect_composition_queue' is used to
           examine the existence of any of the N elements currently on the
           queue, optionally returning the `custom_stamp' or `custom_id_val'
           arguments supplied here.  If you need to store additional state
           information with queued composition buffers, the best way to do
           this is to override the `allocate_buffer' function so as to
           allocate derived versions of the `kdu_compositor_buf' object that
           are capable of storing your custom data.
           [//]
           ...
           unless invoked with the `working_only' argument set to true.
           Otherwise, the `get_composition_buffer' function just returns a
           pointer to the working buffer, W.
           [//]
           Note that the composition buffer queue is not affected by changes
           to the buffer surface, scale, orientation, imagery layers and so
           forth, but some of the compositor's state variables are stored
           at the point when this function is called, so that you can
           determine how best to use (or discard) any queued buffers that
           were generated under different conditions to those currently in
           place.
           [//]
           Thread safety issues: A very natural implementation strategy for
           animation and video rendering is to launch separate threads to
           manage processing and rendering of queued composition buffers.  Of
           course, the processing thread may, itself, manage a set of working
           threads via `kdu_thread_env', but that is not our concern here.  The
           obvious question is whether two threads can safely interact with
           queued buffers via the functions: `push_composition_buffer',
           `pop_composition_buffer', `get_composition_buffer',
           `inspect_composition_queue' and `flush_composition_queue'.  The
           answer is yes, but only if you override these functions or otherwise
           provide appropriate mutual exclusion guarantees.  This is
           deliberately not done in the main implementation, since
           you will most likely want to make provision to record additional
           information about the elements on the composition queue and protect
           this with the same mutual exclusion locks.  To be specific, the
           internal implementation is designed to safely allow the following
           type of multi-threaded access usage:
           [>>] A processing thread is free to use any of the member functions
                offered by this object, but calls to all of the member
                functions identified above that can potentially access the
                composition queue must be protected by a mutex lock.
                Moreover, in the design pattern envisaged here, the processing
                thread always passes `working_only'=true to the
                `get_composition_buffer' function and it never pops all
                buffers from the composition queue or calls
                `flush_composition_queue' unless it can be sure that the
                display thread is inactive.
           [>>] A display thread calls `inspect_composition_queue' to determine
                whether or not there are queued frames.  The display thread
                invokes `get_composition_buffer' to access the head of the
                queue, if there is one, and may invoke `pop_composition_buffer'
                at will, typically leaving at least one buffer on the
                composition queue at all times.
           [//]
           To ensure that the above design pattern is thread safe, the
           internal implementation guarantees that buffers on the composition
           queue (and indeed the queue state itself) will never be touched
           except through calls to the above-mentioned functions, that you
           should protect via mutual exclusion locks.  Moreover, the
           `pop_composition_buffer' function does not actually return any
           storage to the processing thread -- instead, the storage is
           reserved for recycling when the `push_composition_buffer' or
           `flush_composition_queue' function is next called.  That way, there
           are no adverse effects resulting from popping buffers from the
           display thread.
         [RETURNS]
      ...
      */
    KDU_AUX_EXPORT virtual bool
      replace_composition_queue_tail(kdu_long custom_stamp, int custom_id_val);
      /* [SYNOPSIS]
           This function is identical to `push_composition_buffer', except
           that the most recently pushed frame buffer is removed from the
           queue and replaced with this one.  This can be very useful if a
           newly generated composition buffer is found to obviate the need
           from the most recent one, but we do not want to pop all buffers
    ...
    KDU_AUX_EXPORT virtual bool
      ...
      /* [SYNOPSIS]
           ...
           [//]
           See the extensive notes on thread safe access to these functions,
           appearing at the end of the description of
           `push_composition_buffer'.
      */
    KDU_AUX_EXPORT virtual kdu_compositor_buf *
      inspect_composition_queue(int elt, kdu_long *custom_stamp=NULL,
                                int *custom_id_val=NULL,
                                kdu_dims *region=NULL);
      /* [SYNOPSIS]
           Use this function to inspect specific elements on the processed
           composition buffer queue.  For a discussion of this queue, as
           well as thread-safe access procedures, see the discussion
           accompanying the `push_composition_buffer' function.
         [RETURNS]
           NULL if the element identified by `elt' does not exist.  If
           `elt'=0 and the function returns NULL, the composition buffer
           queue is empty.  If the element identified by `elt' exists, the
           function returns a pointer to the relevant composition buffer.
           [//]
           By overriding the `allocate_buffer' function you can arrange
           for the buffers that are actually pushed onto the composition
           queue to be derived objects that carry additional state information.
         [ARG: elt]
         ...
         [ARG: region]
           If non-NULL and the function returns true, this argument is used
           to return the location and size of the relevant buffer surface, as
           configured by `set_buffer_surface' at the time when the buffer
           was generated and then pushed onto the queue.
           [//]
           Note that this `region' is expressed with respect to the `scale'
           value that was supplied to `set_scale' prior to rendering of the
           buffer contents, ignoring the `rendering_scale_adjustment' parameter
           to `set_scale'.  This is consistent with the interpretation of the
           `region' argument returned by `get_composition_buffer'.  To recover
           the buffer region at the actual rendering scale, you can invoke
           the returned buffer's `kdu_compositor_buf::get_rendering_region'
           function.
      */
    KDU_AUX_EXPORT virtual void flush_composition_queue();
      /* [SYNOPSIS]
           Removes all elements from the jitter-absorbtion queue managed
           by `push_composition_buffer' and `pop_composition_buffer'.
           [//]
           As discussed in `push_composition_buffer', for design patterns
           involving a processing master thread and a display master thread,
           the processing thread should be the one that invokes this function,
           but it should do so only when it can be sure that the display
           thread is inactive or appropriately locked, so that flushed buffers
           are not being used for display purposes.
      */
    ...
    KDU_AUX_EXPORT int get_max_available_quality_layers();
      /* [SYNOPSIS]
           ...
           value to be supplied to `set_max_quality_layers'.
         [RETURNS]
           Returns 0 if there are no code-streams currently in use.
           Returns 1 if there is at least one code-stream in use, but no
           tiles of any code-streams have yet been opened -- the number of
           layers is not actually known until tiles are opened.
      */
  //---------------------------------------------------------------------------
  public: // Functions used to control dynamic processing
    KDU_AUX_EXPORT virtual kdu_thread_env *
      ...
      /* [SYNOPSIS]
           ...
           which allows enhanced throughput on multi-processor (or
           hyperthreading) platforms.  Multi-threaded processing may be
           useful even if there is only one physical (or virtual) processor,
           since it allows decompression work to continue while the main
           application is blocked on an I/O condition or other event which
           does not involve the CPU's resources.
           [//]
           After calling this function, it is IMPERATIVE that you access this
           object's member functions only from within the thread that is
           actually associated with the supplied `kdu_thread_env' object.
           For each thread group (created using `kdu_thread_entity::create')
           each thread that participates in the group has a unique
           `kdu_thread_env' object.  In most cases, your application only sees
           the `kdu_thread_env' object that is associated with the creator of
           the thread group, but worker threads created via
           `kdu_thread_entity::add_thread' have their own distinct
           `kdu_thread_env' objects that reference the same underlying thread
           group.  The `kdu_thread_env' objects maintain important thread
           specific state information that must not be accessed from other
           threads.  So, what you should NEVER do is create a thread group
           on one thread, pass its `kdu_thread_env' reference to this function
           (or to the object's constructor), but then access the object's
           member functions from a different thread.
           [//]
           ...
           to this owning thread (i.e., it is the object whose
           `kdu_thread_entity::create' function was used to create the thread
           group).  In this case, all you need to remember to do is to
           retract the multi-threaded processing environment by calling this
           function with a NULL `env' argument (or destroy the present object)
           before invoking `kdu_thread_entity::destroy'.
           [//]
           It is also possible to have one of the auxiliary worker threads
           within your thread group supply its own `kdu_thread_env' reference
           to this function and then that thread can be used to invoke the
           object's interface functions.  To do this, you would generally
           need to derive your own `kdu_thread_queue' object that is
           capable of scheduling jobs; schedule a job to be executed, and
           then arrange for all interaction with this object to be performed
           from within the job function -- see `kdu_thread_job' for more on
           this.  Note that scheduled jobs are passed a pointer to the
           executing thread's `kdu_thread_entity' object, that can be safely
           upcast to a `kdu_thread_env' reference and used with this function,
           so long as the underlying thread group was created by a
           `kdu_thread_env' object invocation of its base
           `kdu_thread_entity::create' function.
           [//]
           The following constraints must be strictly observed when calling
           this function with different `kdu_thread_env' references:
           [>>] The thread identified by the `env' object supplied to this
                function must be the only thread which is used to call any of
           ...
           [>>] Where this function is used to identify that a new thread will
                ...
                will be delivered.
           [>>] Ideally, each thread that has registered itself using this
                function should unregister itself by invoking the function
                with a NULL `env' argument before the multi-threaded processing
                environment is destroyed.  Alternatively, the currently
                registered thread can invoke the present object's `destroy'
                function before destroying the multi-threaded environment.
         ...
         [RETURNS]
           ...
           access control to this object's member functions is assumed to
           be assigned to a new thread of execution.  This is legal
           ...
           releasing controll.  Failure to observe this constraint may
         ...
         [ARG: env_queue]
           ...
           would do this by accessing them from within jobs scheduled via
           the relevant object's `env_queue'.  The application could then
           synchronize its access to composited results by passing the
           relevant queue into the `kdu_thread_entity::join'
           or `kdu_thread_entity::terminate' function.
      */
    KDU_AUX_EXPORT virtual bool
      process(int suggested_increment,
              kdu_dims &new_rendered_region, int flags=0);
      /* [SYNOPSIS]
           ...
           buffered region.  You might choose to use the returned
           `new_rendered_region' to control all painting in your image
           application, except where the screen must be repainted after being
           overwritten by an independent event.  However, an alternate
           (often preferable) approach is to invoke the working buffer's
           `kdu_compositor_buf::get_rendering_status' after each call to
           this function: use of that function can avoid the painting of
           any incomplete data, and hence redundant painting steps.
           [//]
           Note that the `new_rendered_region' returned by this function
           is expressed relative to the rendering scale (the product of the
           two scale parameters passed to `set_scale') unlike all other
           regions passed into or out of `kdu_region_compositor' member
           functions, which depend only on the first scale parameter to
           `set_scale'.  This means that `new_rendered_region' should not
           generally be compared in any way with the surface region passed
           to `set_buffer_surface', since that region may be expressed
           with respect to a different scale, but the `new_rendered_region'
           may be compared with the region returned by the current composition
           buffer's `kdu_compositor_buf::get_surface_region' function.
           [//]
           Note also that the composition buffer may contain
           aggregated novel regions which do not completely cover the
           rectangle identified by `new_rendered_region'; the extent of novelty
           ...
           processing sequence.  Perhaps the most useful flag is
           `KDU_COMPOSIT_DEFER_REGION' which cause the function to return
           an empty `new_rendered_region' (unless all processing is complete),
           but keep track internally of what regions the caller has not yet
           been notified of.  This is can have a number of efficiency
           advantages, especially if there are lots of overlapping composited
           image layers, since compositing does not have to be performed each
           time a piece of an image is generated.
           [//]
           You should note that the function also returns an empty
           `new_rendered_region' until all processing has been completed, if
           the `set_surface_initialization_mode' function has been used to turn
           off surface initialization.  As explained with that function, when
           surfaces are not initialized upon creation, the only point at which
           you can be sure that the composition buffer holds meaningful
           imagery is once all processing is complete; the `process' function
           takes advantage of this to defer all initialization, blending and
           compositing operations until processing is complete, at which point
           this function will start returning non-empty regions.  Typically,
           only one non-empty region will be returned right at the end, but
           you cannot rely upon this.  Normally, when surface initialization
           is disabled, you will wait until all processing is complete before
           displaying everything all at once, so you don't care about the
           contents of `new_rendered_region'.
           [//]
           This function may catch and rethrow an exception of type
           `kdu_exception', generated by a `kdu_error' handler if an error
           occurred in processing the underlying code-stream.
         [RETURNS]
           False only under one of the following conditions.  In all cases,
           `new_rendered_region' is also guaranteed to be empty upon return.
         ...
         [ARG: new_rendered_region]
           Upon successful return, this record holds the location and
           dimensions of a new region, expressed with respect to the rendering
           scale, which is the product of the scale parameters passed to
           `set_scale'.  An interactive application may choose to repaint this
           region immediately.  It is possible that the `new_rendered_region'
           will be empty even though the function returns true, meaning that
           some processing has been done to advance the internal state of the
           rendering/compositing machinery, but this has not resulted in any
           new visible imagery.  In fact, if the `KDU_COMPOSIT_DEFER_REGION'
           flag is supplied, or if `set_surface_initialization_mode' has been
           used to avoid pre-initialization of surfaces, `new_rendered_region'
           will always be empty until the point at which all processing is
           complete, as explained above.
           [//]
           For many rendering applications, it may be more interesting to
           retrieve the rendering status of the composition buffer directly,
           by calling `kdu_compositor_buf::get_rendering_status', rather
           than using the `new_rendered_region' information returned by this
           function.  One reason for this is that `new_rendered_region' can
           include some content that has not yet been fully rendered, in the
           event that multiple disjoint regions were completed by this call,
           depending on the aggregation threshold established by
           `set_process_aggregation_threshold'.  If `new_rendered_region' is
           used to perform incremental updates to a display, it may be that
           some of these updates occur prematurely, whereas the
           `kdu_compositor_buf::get_rendering_status' function identifies only
           content that has been rendered in full.
           [//]
           If you use the `kdu_compositor_buf::get_rendering_status' approach
           to control display updates, you should ignore `new_rendered_region'
           and you should also avoid using the `invalidate_rect' function, for
           the reasons that are explained there -- it may be a good idea to
           consider `invalidate_rect' deprecated, although we have no
           immediate intention of deprecating it.
         [ARG: flags]
           ...
           [>>] `KDU_COMPOSIT_DEFER_PROCESSING' -- If this flag is present,
                ...
                the function returns with an empty `new_rendered_region', and
                so forth.  This flag is ignored if `KDU_COMPOSIT_DEFER_REGION'
                ...
                function returns with an empty `new_rendered_region', it can be
    ...
    KDU_AUX_EXPORT bool refresh(bool *new_imagery=NULL);
      /* [SYNOPSIS]
           ...
           the buffer surface was last rendered and composited.  Note that
           this function also marks any inactive codestreams and compositing
           layers as needing to be refreshed, if and when they are used in
           the future.
           [//]
      ...
      */
    KDU_AUX_EXPORT void invalidate_rect(kdu_dims invalidated_region)
      {
        invalidated_region &= rendering_buffer_region;
        if (!invalidated_region.is_empty())
          find_completed_rects(invalidated_region,active_layers,0);
        processing_complete = false;
      }
      /* [SYNOPSIS]
           NOTE: This function is probably only useful to you if you are
           using the `new_rendered_region' that is produced by calls to
           `process' to incrementally paint composited data to a display.
           If you are using the `kdu_compositor_buf::get_rendering_status'
           method instead to determine what content should be painted to
           a display (generally the preferred approach), calls to this
           function will not generally result in display updates in the
           invalidated region, so it is best to avoid its use.  With this
           in mind, it may be a good idea to consider this function
           deprecated, although we do not have any immediate intention of
           deprecating it.
           [//]
           Causes `invalidated_region' to be added to the internal machinery
           which keeps track of rectangular regions which need to be included
           as `newly_rendered_region' results from calls to `process', before
           `is_processing_complete' can return true.  The explicit invalidation
           of regions at most causes them to be recomposited onto a compositing
           surface -- this is quite different to `refresh', which results in
           complete re-rendering of the content.
           [//]
           ...
           prematurely.
           [//]
           Like `process', this function interprets `invalidated_region'
           with respect to the rendering scale, which is the product of the
           `scale' and `rendering_scale_adjustment' arguments passed in the
           last call to `set_scale'.  All other API functions offered by the
           region-compositor interpret dimensions and locations of their
           arguments and return values with respect only to the `scale'
           parameter passed to `set_scale'.
      */
  ...
  public: // Functions used to identify the elements of the composited image
    ...
    KDU_AUX_EXPORT kdu_istream_ref
      ...
      /* [SYNOPSIS]
           ...
           into the mix by setting the `only_active_istreams' argument to
           false.
           [//]
    ...
    KDU_AUX_EXPORT kdu_codestream
      ...
      /* [SYNOPSIS]
         ...
         [RETURNS]
           ...
           [//]
           The function also returns an empty interface if the underlying
           source of data is a dynamic cache and the relevant codestream's
           main header is not yet available in the cache.
      */
    KDU_AUX_EXPORT int
      ...
      /* [SYNOPSIS]
         ...
         [ARG: components_in_use]
           ...
           used to create this istream and it was supplied with an
           `access_mode' argument of `KDU_WANT_CODESTREAM_COMPONENTS', the
           imagery is rendered directly using one or more codestream image
           components; otherwise, the components in question are output image
           components.
    ...
    KDU_AUX_EXPORT bool
      ...
      /* [SYNOPSIS]
           ...
           [//]
           It should be noted that Pn is not limited by any prevailing quality
           layer limit that may have been imposed by a call to
           `set_max_quality_layers'.  However, if the `max_region_layers'
           argument is non-zero, the value of Pn limited to the smaller of
           `max_region_layers' and the number of actual packets that are
           currently available for the precinct associated with sample n.
           [//]
           ...
           value is recovered using `kdu_tile::get_num_layers'.  As with
           Pn, the value of Ln is prevented from exceeding any non-zero
           `max_region_layers' argument.
           [//]
         ...
         [ARG: max_region_layers]
           As explained above, any prevailing quality layer limit that may have
           been imposed by a call to `set_max_quality_layers', is ignored by
           this function.  However, if the `max_region_layers' argument is
           greater than 0, it is used to limit the number of packets that
           appear to be available for any precinct and also the number of
           packets that appear to be potentially available for any precinct.
           That is, `max_region_layers'>0 imposes a limit on the values of Pn
           and Ln described above, that are used to compute the
           `visible_packet_samples' and `max_visible_packet_samples'
           statistics.
      */
    KDU_AUX_EXPORT kdu_ilayer_ref
      ...
      /* [SYNOPSIS]
           ...
           relevant ilayer; otherwise the returned object is a "null"
           reference, meaning that its `kdu_ilayer_ref::is_null' member
           returns true.
           [//]
    ...
    KDU_AUX_EXPORT kdu_dims
      ...
      /* [SYNOPSIS]
         ...
         [ARG: istream_ref]
           You may obtain a valid `istream_ref' instance from
           `get_next_istream' or any of a range of other functions offered by
           the `kdu_region_compositor' object.
         [ARG: apply_cropping]
           If true, the returned region corresponds to the portion of the
           underlying codestream image which actually contributes to the
           imagery layer.  Note, however, that the imagery may have been
    ...
    KDU_AUX_EXPORT bool
      ...
      /* [SYNOPSIS]
           ...
           the first 3 arguments.  The `fsiz' argument is used to return
           the size of the full image which should be used in a JPIP request.
           This, together with `round_direction', affects the image resolution
           ...
           meaning "round-to-nearest".  However, if one or more codestreams
           ...
           might not be stable until the `waiting_for_stream_headers' function
           ...
           [//]
           As an addendum to the above description, we briefly mention the
           special case where compositions are formed in a manner that cannot
           be explicitly identified in a JPIP request.  Specifically, if
           the composition is constructed from multiple JPX frames, via
           calls to `add_frame', with different positioning offsets, the
           composition dimensions reported by `get_composition_dims' are not
           directly related to the returned `fsiz' value.  Instead, the
           `fsiz' value reported for frame compositions is always an
           appropriately scaled version of the global frame dimensions (see
           `jpx_composition::get_global_info'), reported by the JPX source --
           this is what is expected if a JPIP query is to be expressed in
           terms of frames or compositing instructions.  Moreover, for
           multi-frame compositions, the returned `roi_dims' region is
           set to refer to the entire composition (`roi_dims.pos' is set to
           (0,0) and `roi_dims.size' is set to `fsiz'), ignoring the 
           `region' argument.  This is because JPIP requests associated with
           multi-frame compositions cannot must treat each frame as if it
           occupied the entire surface -- there is no way to express the
           frame layout itself within a JPIP request.  This should still be
           efficient in most cases, since multi-frame compositions are only
           expected to be used where the rendering scale is so small that
           multiple frames can be viewed at once within a single rendered
           viewport -- i.e., there would not normally be a specific spatial
           region of interest in such cases.
         [RETURNS]
         ...
         [ARG: roi_dims]
           ...
           [//]
           If the composition appears to involve multiple composited JPX
           frames, at different locations (or even a single frame that was
           composited with a non-zero location offset passed to `set_frame'),
           the `roi_dims' argument is set to refer to the entire surface.
           That is, `roi_dims.pos' is set to (0,0) and `roi_dims.size' is
           set equal to `fsiz'.  The reasons for this are given above.
         [ARG: round_direction]
         ...
         [ARG: region]
           ...
           `set_buffer_surface'.
      */
    KDU_AUX_EXPORT bool load_metadata_matches();
      /* [SYNOPSIS]
           This function does nothing unless the object was initialized with a
           `jpx_source' object.  In that case, the function invokes
           `jpx_meta_manager::load_matches' passing in arrays that identify
           all codestreams and compositing layers that are in active use.  If
           you are interested in all possible metadata, for some reason, it
           is better to directly call `jpx_meta_manager::load_matches' with
           -1 for the arguments that identify the number of codestreams and
           compositing layers of interest, since that will expand all
           codestream and compositing layer contexts for newly arrived
           metadata.  However, for a source with a massive number of
           codestreams or compositing layers, that could be very tedious.
         [RETURNS]
           True if and only if the call to `jpx_meta_manager::load_matches'
           returns true.
      */
    KDU_AUX_EXPORT int
      generate_metareq(kdu_window *client_window,
                       int anchor_flags, kdu_dims region,
                       int num_box_types, const kdu_uint32 box_types[],
                       int num_descend_box_types,
                       const kdu_uint32 descend_box_types[],
                       bool priority=false,
                       kdu_int32 max_descend_depth=KDU_INT32_MAX);
    /* [SYNOPSIS]
         This function is closely related to `jpx_metanode::generate_metareq',
         except that it operates on the complete metadata hierarchy of any
         JPX data source that is in use, and it builds its metadata requests
         starting from a multitude of "anchor nodes" (`jpx_metanode's), based
         on the `anchor_flags' argument, the `region' argument and the
         codestreams and compositing layers that make up the current
         composition.  The function does almost all the work of bridging the
         gap between imagery of interest (which may be part of a complex
         composited frame within an animation) and the complexities of the
         JPIP "metareq" request field.  If the data source is not a JPX
         file, this function does nothing.
         [//]
         As with `jpx_metanode::generate_metareq', the current function does
         not itself invoke `jpx_meta_manager::load_matches'.  You should do
         this explicitly before calling this function, to make sure you are
         building requests based upon the most recent available metadata.  The
         current object's `load_metadata_matches' function provides a
         convenient way to do this.
         [//]
         Typically, you use the function in the following way.  When
         preparing a `kdu_window' object to pass to `kdu_client::post_window',
         you will generally need to use a combination of direct calls to
         `kdu_client_window::add_metareq', higher level calls to
         `jpx_metanode::generate_metareq' and the current function.  You should
         regenerate the client window using these functions on an episodic
         basis (e.g., whenever new data arrives from the server) because the
         higher level functions progressively discover the requests that need
         to be posed based on data that has arrived.  Generally, the strategy
         would be as follows:
         [>>] Directly call
              `kdu_window::add_metareq(jp2_roi_description_4cc,KDU_MRQ_WINDOW)'
              and
              `kdu_window::add_metareq(jp2_number_list_4cc,KDU_MRQ_STREAM)'
              to include requests for numlist and ROI boxes that are relevant
              to the window of interest.  These become anchor nodes for the
              present function.
         [>>] Invoke `jpx_metanode::generate_metareq(client_window,...)' on
              the root of the metadata hierarchy (returned by
              `jpx_meta_manager::access_root'), to request all "semantically
              top-level" metadata that is of interest to the application (e.g.,
              label nodes, XML nodes, URL nodes), being careful to exclude
              numlists and ROI description boxes from the list of box types
              that are of interest.  By "semantically top-level" we mean that
              the metadata is found within boxes that are not contained within
              any other boxes, apart from association and group boxes.  That
              call automatically figures how to progressively obtain all these
              semantically top-level boxes.  The semantically top-level boxes
              correspond to actual `jpx_metanode' objects that are immediate
              descendants of the root node, because JPX metadata is parsed
              in a manner which flattens out grouping structures that have no
              semantic meaning.  In practice, one might use something like the
              following code fragment to request the semantically top-level
              boxes, noting that the statements may need to be re-issued
              episodically:
              "kdu_uint32 top_types[2]={jp2_label_4cc,jp2_xml_4cc};
               jpx_metanode root=meta_manager.access_root();
               root->generate_metareq(client_window,2,top_types,0,NULL,false);"
         [>>] Use the current function to request additional metadata of
              interest that is descended from the "anchor nodes" obtained as
              above.  This is explained further below.
         [//]
         The `anchor_flags' argument plays a critical role in determining
         which metadata is requested.  The anchor flags that can be
         meaningfully used are drawn from the qualifier flags passed to
         `kdu_window::add_metareq' and have related interpretations.
         Specifically,
         [>>] If the `KDU_MRQ_WINDOW' flag is present, the function builds
              metadata requests starting from `jpx_metanode' anchors which
              correspond to ROI nodes (i.e., `jpx_metanode::get_num_regions'
              returns non-zero) whose associated regions of interest intersect
              with the supplied `region', after mapping from the composited
              image domain to the codestreams with which the ROI description
              box is associated.  This is the same mapping process that is
              carried out by the `paint_overlay' function when rendering
              region-of-interest metadata.  Note that the supplied `region'
              does not need to be related in any way to the current region
              being rendered by the compositor, as determined by
              `set_buffer_surface'.  Indeed if `region' is empty, the entire
              composited image will be taken as the region of interest, which
              is frequently much larger than the surface that is actually
              being rendered.
         [>>] If the `KDU_MRQ_STREAM' flag is present, the function builds
              metadata requests starting from `jpx_metanode' anchors which
              correspond to numlist nodes (`jpx_metanode::get_numlist_info'
              returns true) that refer to codestreams and/or compositing layers
              used by the current composition, within the `region' of interest
              (or the whole image if `region' is empty).  Typically, one would
              not include `jp2_roi_description_4cc' amongst the `box_types' of
              interest, when invoking this function, so the `KDU_MRQ_STREAM'
              and `KDU_MRQ_WINDOW' flags refer to different subsets of the
              metadata hierarchy.  On the other hand, it might be useful to
              include `jp2_number_list_4cc' in the `box_types' of interest
              when invoking this function with the `KDU_MRQ_WINDOW' flag so
              that one can obtain references to "other image entities" that
              might be embedded as a descendent of an ROI description box.
         [>>] If the `KDU_MRQ_GLOBAL' flag is present, the function builds
              metadata requests starting from "semantically top-level" nodes
              other than numlist and ROI nodes that are found within the
              metadata hierarchy, whose box-types match those found in
              the `descend_box_types' array.  This makes sense, because the
              anchor nodes for global metadata are already one below the
              conceptual global anchor point (root of the metadata tree).
              Any occurrence of `jp2_numlist_4cc' or `jp2_roi_description_4cc'
              within the `descend_box_types' array is ignored for the purpose
              of determining the anchor points for global metadata.  Thus, the
              `KDU_MRQ_GLOBAL' flag can do nothing unless `descend_box_types'
              contains at least one other box type.
         [//]
         Note that the `KDU_MRQ_WINDOW' and `KDU_MRQ_STREAM' flags here
         cause metadata requests to be matched to the composited image
         `region', constituent codestreams and compositing layers, whereas
         the same flags passed to `kdu_window::add_metareq' restrict
         requested metadata to the window of interest specified in the
         `kdu_window' object.  These need not be connected, although they
         frequently will be.  In practice, you are relying on the direct call
         to `kdu_window::add_metareq' mentioned above to find the initial ROI
         description boxes and numlist boxes that are relevant to the window
         of interest and then you are relying upon the present function to
         build more detailed metadata requests from an appropriate subset of
         these "anchor nodes".  This subset can be determined very accurately
         because generic JPIP request mechanisms are no longer being relied
         upon to determine exactly which numlist and ROI description boxes are
         relevant to the composited region of interest.
         [//]
         Note also that the `anchor_flags' argument is used only to determine
         the initial anchor nodes from which to build metadata requests.
         The actual requests are then constructed by passing the
         `box_types', `num_box_types', `descend_box_types',
         `num_descend_box_types', `priority' and `max_descend_depth' arguments
         directly to `jpx_metanode::generate_metareq', which is invoked on
         each anchor node.  That function adds metadata requests by issuing
         calls to `client_window::add_metareq' with the `KDU_MRQ_ALL' flag.
         So, for example, if you want to receive metadata that is associated
         with the region of interest, including labels and references to other
         image entities, you might use something like:
         ...
         "kdu_winshow" and "kdu_macshow" applications.  In those applications,
         the function is only used to obtain a first level of box-types that
         are of interest within each of the relevant `KDU_MRQ_WINDOW' and
         `KDU_MRQ_STREAM' anchor nodes (i.e., no recursion via a
         `descend_box_types' array.  These top-level nodes of
         interest (typically label nodes) are used to build the metadata
         catalog side-bar.  The `KDU_MRQ_GLOBAL' option is not used by the
         "kdu_macshow" and "kdu_winshow" applications, because the initial
         call to `jpx_metanode::generate_metareq' mentioned above is
         sufficient to provide all top-level global metadata for the
         catalog side-bar.  Thereafter, mouse clicks or other selection
         activities within the metadata catalog side-bar trigger further calls
         to `jpx_metanode::generate_metareq', as required.  Different
         applications, however, may wish to use the recursion feature of this
         function and/or the `KDU_MRQ_GLOBAL' option, to automatically recover
         larger tracts of relevant metadata without the need for user
         interaction.
       [RETURNS]
         The total number of calls to `client_window::add_metareq' that were
         generated by this function by calls to `jpx_metanode::add_metareq'.
       [ARG: client_window]
         Object whose `kdu_window::add_metareq' function is used to add
         metadata requests for the relevant metadata.
       [ARG: anchor_flags]
         Any or all of the flags `KDU_MRQ_STREAM', `KDU_MRQ_WINDOW' or
         `KDU_MRQ_GLOBAL'.  These flags determine the "anchor nodes", as
         described in detail above.
       [ARG: region]
         Either an empty region (mean the entire composited image surface) or
         else a region of interest, expressed on the composited image
         coordinate system.  This may or may not be the region supplied in
         a current or future call to `set_buffer_surface'.  This region
         affects the way in which "anchor nodes" are determined, for both the
         `KDU_MRQ_WINDOW' and the `KDU_MRQ_STREAM' `anchor_flags'.
       [ARG: num_box_types]
         Number of entries in the `box_types' array.
       [ARG: box_types]
         Array of box types that are of interest within each relevant "anchor
         node".  Same interpretation as in `jpx_metanode::generate_metareq'.
       [ARG: num_descend_box_types]
         Number of entries in the `descend_box_types' array.
       [ARG: descend_box_types]
         Same interpretation as in `jpx_metanode::generate_metareq'.  Note
         that any descending starts from immediate descendents of a matching
         "anchor node" where the descendent has one of the box-types in
         `descend_box_types'.  Note also that this array is array is used
         to determine the "anchor nodes" for the `KDU_MRQ_GLOBAL' option -- see
         above.
       [ARG: priority]
         Same intepretation as in `jpx_metanode::generate_metareq'.
       [ARG: max_descend_depth]
         Same intepretation as in `jpx_metanode::generate_metareq' but measured
         based on each matching "anchor node".  A `max_descend_depth' of 0
         means that only the `box_types' of interest found as immediate
         descendents of an "anchor node" are of interest. A `max_descend_depth'
         of 1 means that the `descend_box_types' array is used to descend once,
         to request immediate descendents of the immediate descendents of
         each matching "anchor node" that match the `descend_box_types'.
         Recursive descent may be disabled either by setting
         `max_descend_depth' to 0 or by setting `num_descend_box_types' to 0
         (or both, of course).  Note, however, that the `KDU_MRQ_GLOBAL'
         option requires `num_descend_box_types' to be non-zero; otherwise,
         there will be no global anchor nodes to start from, following the
         definitions provided above, whereas `max_descend_depth' can still
         be 0 to prevent further recursive descent into global metadata nodes.
     */   
  //---------------------------------------------------------------------------
  public: // Functions used to manage metadata overlays
    KDU_AUX_EXPORT void
      ...
      /* [SYNOPSIS]
           ...
           to return a `new_rendered_region' which is the size of the entire
           composited buffer surface, at the relevant rendering scale, so that
           your application will know that it needs to repaint the whole thing.
           [//]
         ...
         [ARG: aux_params]
           ...
           [>>] Each row m specifies a border size Bm, a sequence of colours
                denoted COLR_m0, COLR_m1, ..., COLR_mBm, and a threshold Tm.
           ...
           [>>] The average ROI "width" is assessed by applying the
                ...
                also that the widths and lengths are assessed after
                transforming the region coordinates according to the current
                rendering scale and geometry.
           [>>] COLR colour values have their alpha (blending) value in the
                most significant byte, followed by RGB or BGR in the remaining
                bytes, depending on the colour channel order that is currently
                in force (see `kdu_region_compositor::set_colour_order').
                When painting overlays for higher precision renditions (e.g.,
                with floating-point buffers) the relevant colour and alpha
           ...
           [>>] If any row m specifies a border size of Bm=0, no further rows
                ...
                parameter following Bm is the interior colour COLR_0.
           [>>] For all other rows m, COLR_m0 denotes the colour to be used
                for painting the interior of the region of interest;
                COLR_m1 denotes the colour used for painting the inner-most
                border pixels; and so forth out to COLR_mBm which is used for
           ...
           [>>] If any `num_aux_params' is such that the available set of
                ...
                colour is missing, it defaults to COLR_m0=0 (totally
                transparent).  If the interior border colour is missing,
                it defaults to a solid blue colour.  If any subsequent border
                colours COLR_mb are missing (1 < b <= Bm), the default border
                colour is obtained by replicating the last available border
                colour but decreasing the alpha component linearly towards 0
                with b.  If the threshold Tm is missing, it is taken to be 0
                (this threshold is satisfied by all possible values of ROI
                "width").
           [//]
    ...
    virtual bool
      custom_paint_overlay(kdu_compositor_buf *buffer, kdu_dims buffer_region,
                           ...
                           kdu_coords expansion_denominator,
                           kdu_coords compositing_offset,
                           kdu_colour_order colour_order) { return false; }
      /* [SYNOPSIS]
         [BIND: callback]
         ...
         [ARG: buffer]
           ...
           [>>] the second most significant byte holds the Red channel value
                if `colour_order' is `KDU_ARGB_ORDER', or the Blue channel
                value if `colour_order' is `KDU_ABGR_ORDER';
           [>>] the next byte holds the Green channel value; and
           [>>] the least significant byte holds the Blue channel value
                if `colour_order' is `KDU_ARGB_ORDER', or the Red channel
                value if `colour_order' is `KDU_ABGR_ORDER'.
           [//]
           If the buffer has a floating-point organization,
           `buffer->get_float_buf' returns a non-NULL pointer, and each
           pixel consists of four successive floating point quantities,
           starting with Alpha and followed either by Red then Green then Blue
           (if `colour_order' is `KDU_ARGB_ORDER'), or by Blue then Green then
           Red (if `colour_order' is KDU_ABGR_ORDER').  All floating point
           quantities have a range of 0.0 to 1.0; in particular, opaque
           overlay content should have an Alpha value of 1.0.
         [ARG: buffer_region]
    ...
    virtual kdu_compositor_buf *
      ...
      /* [SYNOPSIS]
         [BIND: callback]
           ...
           referenced memory buffer is at least aligned on a 16-byte boundary,
           with stride (row-gap) chosen to preserve this alignment for each
           row of the buffer.  You may find it useful to base your buffer
           alignment decisions on the constants `KDU_ALIGN_SAMPLES16' and
           `KDU_ALIGN_SAMPLES32', where the former is appropriate for
           low precision buffers and the latter for high precision floating
           point buffers.  To convert these constants, defined in "kdu_arch.h",
           to byte alignment values, they should be multiplied by 2 and 4,
           respectively.
         [RETURNS]
  ...
  private: // Helper functions
    friend class kd_supp_local::kdrc_stream;
    friend class kd_supp_local::kdrc_layer;
    friend class kd_supp_local::kdrc_overlay;
    ...
    void derive_rendering_buffer_region();
      /* Updates `rendering_buffer_region' based on any changes in the
         `notional_scale', `rendering_scale' or `notional_buffer_region'.  The
         function scales `notional_buffer_region' to accommodate any
         differences between the notional and rendering scale factors, rounding
         outwards where appropriate so that the rendered buffer region is in
         some sense a cover for the notional buffer surface, and then
         intersecting with `rendering_composition_dims'. */
    bool update_scaled_overlay_params();
      /* Analyzes the `nominal_overlay_xxx' members and generates a copy
         of the information for the `scaled_overlay_xxx' members that takes
         into account the ratio between `rendering_scale' and `notional_scale'.
         The idea is that the overlay border should be scaled by this same
         ratio (roughly).  The function returns true if the
         `scaled_overlay_aux_params' array is allocated or re-allocated,
         in which case the change must be passed to the individual
         compositing layers. */
    ...
    bool find_completed_rects(kdu_dims &rendered_region,
                    kd_supp_local::kdrc_layer *start_layer,
                    int start_elt,
                    kd_supp_local::kdrc_layer **first_intersecting_layer=NULL,
                    kd_supp_local::kdrc_layer **last_intersecting_layer=NULL);
      /* This function scans through all region processing elements, starting
         ...
         be examining overlays -- this is done last.
           If the function determines that future processing jobs will produce
         results overlapping with `rendered_region', the incomplete portions of
         `rendered_region' are removed and the function continues processing
         the remaining complete portions of `rendered_region' until all layers
         and layer elements have been examined; where the removal of incomplete
         regions leaves multiple rectangular regions (this can happen at any
         stage in the processing), the function must be recursively invoked on
         each piece.  At the end of this potentially recursive process, the
         original `rendered_region' has been decomposed into a collection of
         supposedly complete rectangular regions (actually, the implementation
         needs to take a conservative position sometimes -- it is not a
         disaster to identify a region as complete even if it is not).  At most
         one of these supposedly completed regions can be returned immediately
         to the application via the `kdu_region_compositor::process' call which
         ...
         via `rendered_region' and the first and last layers with which an
         intersection was discovered are returned via
         *`first_intersecting_layer' and *`last_intersecting_layer'.  In this
         latter case, the function returns true if a completed region is being
         returned via `rendered_region'.
            If `can_skip_surface_initialization' is true, we do not generally
         invoke this function, because all initialization and composition is
         deferred until processing of codestream and overlay buffers is
         complete.
       */
    kdu_istream_ref assign_new_istream_ref();
    kdu_ilayer_ref assign_new_ilayer_ref();
    kd_supp_local::kdrc_stream *
      add_active_stream(int codestream_idx, int colour_init_src,
                        bool single_component_only, bool alpha_only);
    ...
    bool needs_surface_init() { return !can_skip_surface_initialization; }
      /* Used by some `kdrc_layer' member functions to decide whether or not
         surfaces need to be initialized when they are created. */
  ...
  private: // Configuration parameters
    ...
    int error_level;
    float scaling_min_resampling_factor;
    float scaling_max_interp_overshoot;
    int scaling_bilinear_interp_threshold;
    bool intensity_true_zero;
    bool intensity_true_max;
    bool persistent_codestreams;
    int codestream_cache_threshold;
    float process_aggregation_threshold;
    kdu_quality_limiter *limiter;  // Copy of the parameters last passed
    float limiter_ppi_x;           // to `set_quality_limiting'.
    float limiter_ppi_y;
  protected:
    bool in_pre_destroy; // If true `update_composition' fails immediately
  private: // Current composition state
    bool have_valid_scale;
    int max_quality_layers;
    bool vflip, hflip, transpose; // First 3 params passed to `set_scale'
    float notional_scale; // Just the `scale' param passed to `set_scale'
    float rendering_scale; // Product of 2 scale params passed to `set_scale'
    int invalid_scale_code; // Error code from last scale-dependent failure
    kdu_dims fixed_composition_dims; // Unscaled frame dimensions, if any
    kdu_dims notional_composition_dims; // At `notional_scale' and orientation
    kdu_dims rendering_composition_dims; // At `rendering_scale' & orientation
    kdu_compositor_buf *composition_buffer; // NULL if first layer is only one
    kdu_dims notional_buffer_region; // Expressed using `notional_scale'
    kdu_dims rendering_buffer_region; // Expressed using `rendering_scale'
    ...
    bool composition_invalid; // If layers, rendering_scale or surface changed
    ...
    kd_supp_local::kdrc_overlay_expression *overlay_dependencies;
  
    int nominal_overlay_max_painting_border;
    int nominal_overlay_num_aux_params;
    kdu_uint32 *nominal_overlay_aux_params; // Copied from `configure_overlays'
  
    bool scaled_overlay_params_invalid; // If need update_scaled_overlay_params
    int scaled_overlay_max_painting_border; // See below
    int scaled_overlay_num_aux_params; // See below
    kdu_uint32 *scaled_overlay_aux_params; // See below
  
  private: // Resource lists
    kd_supp_local::kdrc_layer *active_layers; // Doubly-linked active
                                              // compositing layers list
    kd_supp_local::kdrc_layer *last_active_layer; // Tail of doubly-linked
                                                  // active layers list
    kd_supp_local::kdrc_layer *inactive_layers; // Singly-linked list of
                                                // layers not in use
    kd_supp_local::kdrc_stream *streams; // Singly-linked list of all istreams
    kd_supp_local::kdrc_refresh *refresh_mgr; // Manages regions which must be
                   // refreshed via calls to `process' and would not otherwise
                   // be refreshed in the course of decompression processing
    kdu_ilayer_ref last_assigned_ilayer_ref;
    kdu_istream_ref last_assigned_istream_ref;
  private: // Composition buffer queue management
    kd_supp_local::kdrc_queue *queue_head; // Least recent element on
                                           // jitter-absorbtion queue
    kd_supp_local::kdrc_queue *queue_tail; // Most recent element on
                                           // jitter-absorbtion queue
    kd_supp_local::kdrc_queue *queue_free; // Recycling list for queue elements
