...
// Cache file handling flags passed to `kdu_client::connect' and several other
// functions, including `kdu_client::set_cache_file_handling'.
#define KDU_CLIENT_FILE_SAVE                  ((int) 0x01)
#define KDU_CLIENT_FILE_LOAD                  ((int) 0x02)
#define KDU_CLIENT_FILE_LOAD_SAVE             ((int) 0x03)
#define KDU_CLIENT_FILE_DELETE                ((int) 0x04)
...
class kdu_cache_file_info {
  /* [BIND: reference]
     [SYNOPSIS]
       This class is provided to encapsulate information retrieved via the
       static `kdu_client::check_cache_file' function, making it easier to
       obtain this information from both native and foreign languages such
       as Java or C#.
       [//]
       At the application level, you construct an instance of this of this
       class and pass it (by reference) to `kdu_client::check_cache_file'.
       If that function returns true, you can obtain meaningful information
       from the header of the cache file by querying the member functions
       provided here.
  */
  public: // Member functions
    kdu_cache_file_info()
      { cache_identifier=NULL; host_name=NULL; target_name=NULL;
        header_bytes=0; preamble_bytes=0; };
      /* [SYNOPSIS]
           After construction, pass the object to
           `kdu_client::check_cache_file'.
      */
    ~kdu_cache_file_info() { reset(); }
    kdu_cache_file_info &operator=(const kdu_cache_file_info &rhs)
      { assert(0); return *this; }
      /* [SYNOPSIS]
         This assignment operator serves to prevent actual copying of one
         `kdu_cache_file_info' object to another.  In debug mode, it raises an
         assertion.
      */
    bool exists() const { return (cache_identifier != NULL); }
      /* [SYNOPSIS]
           Returns true if the object has been successfully initialized
           within a call to `kdu_client::check_cache_file'.
      */
    bool operator!() const { return (cache_identifier == NULL); }
      /* [SYNOPSIS]
           Opposite of `exists'.
      */
    KDU_AUX_EXPORT void reset();
      /* [SYNOPSIS]
           You can use this function to clear the state of the object
           internally, so that its `exists' function returns false.
      */
    const char *get_cache_identifier() const
      { return cache_identifier; }
      /* [SYNOPSIS]
           Returns NULL if the object has not been initialized by a successful
           call to `kdu_client::check_cache_file' or has been `reset' since.
           Otherwise, the function returns a special "cache-identifier"
           string, which can be compared with that returned by an instantiated
           `kdu_client' object's `kdu_client::get_cache_identifier'
           function, as a precursor to either joining with that client object
           or opening a new one.
           [//]
           The returned string contains the server-assigned target-id as its
           suffix, preceded immediately by a punctuation character (typically
           a dash '-'), which is then preceded by a target name that is derived
           either from the resource or the explicit target name passed in the
           original JPIP request, whichever is more informative, and including
           any sub-target information.
      */
    const char *get_host_name() const
      { return (cache_identifier==NULL)?NULL:host_name; }
      /* [SYNOPSIS]
           Returns NULL if the object has not been initialized by a successful
           call to `kdu_client::check_cache_file' or has been `reset' since.
           Otherwise, the function returns a string which identifies
           the original host name (or IP address), including any port suffix
           (colon-separated) that may have been used.
      */
    const char *get_target_name() const
      { return (target_name==NULL)?NULL:target_name; }
      /* [SYNOPSIS]
           Returns NULL if the object has not been initialized by a successful
           call to `kdu_client::check_cache_file' or has been `reset' since.
           Otherwise, the function returns string which has exactly the same
           interpretation as that returned by an active `kdu_client's
           `kdu_client::get_target_name' function.  In many cases this will
           be the same as the JPIP requested resource or target field, but it
           may include sub-target information and is not hex-hex encoded.
           This name is a useful consistent display name to use for the target
           that is represented by the cache file.
      */
    kdu_int32 get_header_bytes() const
      { return header_bytes; }
      /* [SYNOPSIS]
           Returns the number of bytes in the header of the cache file
           (in practice, these bytes are all plain ASCII text).  The header
           is immediately followed by a sequence of data-bin descriptors,
           each of which commences with a few bytes of binary identification
           information which are followed by the data-bin body bytes.
      */
    kdu_int32 get_preamble_bytes() const
      { return preamble_bytes; }
      /* [SYNOPSIS]
           A cache file may contain a preamble that consists of an initial
           subset of datab-bin descriptors, which play a particularly
           important role in summarizing the content.  Typically, the preamble
           should be enough to render a reduced resolution (e.g., thumbnail)
           representation of the first image/frame/codestream in the source.
           [//]
           If there is a preamble, this function returns non-zero, and the
           returned value can be added to that returned by `get_header_bytes'
           to determine the total number of initial bytes from the cache file
           that are required to produce a usable representation of the source.
           Some applications might offer users the ability to truncate old
           cache files to just this minimum length for archival purposes.
      */
  private: // Data
    friend class kdu_client;
    char *cache_identifier;
    char *host_name;
    char *target_name;
    kdu_int32 header_bytes;
    kdu_int32 preamble_bytes;
  };
  ...
  */
  public: // Member functions
    ...
    KDU_AUX_EXPORT static bool
      check_cache_file(const char *filename, kdu_cache_file_info *info);
      /* [SYNOPSIS]
           A call to this static function is a useful precurser to calling
           `open_with_cache_file', since that function either succeeds or else
           generates an error through `kdu_error'.
           [//]
           This function generally needs to open the file at `filename'
           and read its header, after which the file is closed before the
           function returns.
           [//]
           If the check succeeds, returning true, any non-NULL `info'
           object is initialized to provide valuable information derived
           from the cache file's header.  This information is intended to
           be completely consistent with information that could be retrieved
           from a live `kdu_client' object that has been opened using the
           cache file (`kdu_client::open_with_cache_file'), or that may have
           saved its cache contents to the cache file.
           [//]
           Most important is the "cache-identifier" that is supposed to
           uniquely identify the resource/target represented by the cache file.
           This string can be retrieved via `info->get_cache_identifier' and
           then directly compared with the string returned by an instantiated
           `kdu_client' object's `get_cache_identifier' function to determine
           whether the instantiated client is already working with the same
           logical target.  If so, there may be no need for an application to
           construct a new instance of `kdu_client'; instead, it is preferable
           to simply attach (see `kdu_cache::attach_to') to the existing
           client, potentially after augmenting its cache with the contents of
           the file at `filename' -- see `kdu_client::augment_with_cache_file'.
         [RETURNS]
           True if the cache file can be opened and its header can be
           understood.
         [ARG: info]
           This argument may be NULL if you only want to discover whether the
           cache file exists and can be interpreted.  However, you will usually
           want to provide a reference to a `kdu_cache_file_info' object that
           you have instantiated here.  If the function returns false, no
           change will be made to the `info' object.  Otherwise, it will
           be freshly initiated with information recovered from the cache
           file header.
      */
    KDU_AUX_EXPORT void
      ...
      /* [SYNOPSIS]
           ...
           NULL for the `translator' argument.  Apart from this, existing
           translators are removed only by calls to `close'.
      */
   void install_notifier(kdu_client_notifier *notifier)
      { mutex.lock(); this->notifier = notifier; mutex.unlock(); }
      /* [SYNOPSIS]
           ...
           `connect' and is not removed until an explicit call to `close'
           (as opposed to the implied call to `close' within `connect') or
           `install_notifier' is called again.
           [//]
    ...
    KDU_AUX_EXPORT virtual int
      connect(const char *server, const char *proxy,
              ...
              int cache_file_handling=KDU_CLIENT_FILE_LOAD_SAVE);
      /* [SYNOPSIS]
         ...
         [ARG: cache_dir]
           ...
           here.  Cache files written to or read from the cache directory have
           the ".kjc" suffix, which stands for (Kakadu JPIP Cache).
           [//]
           From KDU-7.6 significant support has been added for handling cache
           files. For example, it is possible to open a cache file directly
           (see `open_with_cache_file') augment an existing cache with content
           from another cache file (see `augment_with_cache_file'), or
           reconnect to a JPIP server based on information that may have been
           recovered from a cache file (see `reconnect').  You never need to
           read or understand the cache files at the application level.
           [//]
           Importantly, you can also control which cache file operations are
           performed using the `cache_file_handling' argument.  This work is
           a union of zero or more of the following flags:
           [>>] `KDU_CLIENT_FILE_LOAD' means that a compatible existing cache
                file is loaded, if one is found.
           [>>] `KDU_CLIENT_FILE_DELETE' means that a compatible existing
                cache file is deleted (after any required loading), if one
                is found.
           [>>] `KDU_CLIENT_FILE_SAVE' means that cache files are saved
                (if required) once a JPIP server connection is lost.
           [//]
           The default value for this argument is `KDU_CLIENT_FILE_LOAD_SAVE',
           which is a union of the first two flags.
         [ARG: mode]
         ...
         [ARG: compatible_url]
           ...
           like.
         [ARG: cache_file_handling]
           This argument should be formed from zero or more of the following
           flags.  Note, however, that none of these optionsl are meaningful
           unless a valid `cache_dir' is provided here.
           [>>] `KDU_CLIENT_FILE_LOAD' means that a compatible existing cache
                file is loaded, if one is found in the `cache_dir' directory.
           [>>] `KDU_CLIENT_FILE_DELETE' means that a compatible existing
                cache file is deleted (after any required loading), if one
                is found in the `cache_dir' directory.  No deletion of
                cache files can occur unless communication is established
                and a valid target-id is obtained from the server, since
                that is how compatibility of the cache file is established.
           [>>] `KDU_CLIENT_FILE_SAVE' means that cache files are saved
                (if required) once a JPIP server connection is lost.  The
                cache file is saved to the `cache_dir' directory; this is
                typically done in the background, when the client thread is
                preparing to shut down after the communication session
                completes for one reason or another.
      */
    KDU_AUX_EXPORT virtual int
      open_with_cache_file(const char *filename, const char *cache_dir,
                           int cache_file_handling=KDU_CLIENT_FILE_LOAD_SAVE,
                           bool preamble_only=false);
      /* [SYNOPSIS]
           This function may be used instead of `connect', where content
           retrieved previously has been stored in a cache file with the
           supplied `filename'.  If the function succeeds, it returns the
           initial queue-id, which is invariably 0, although the `is_alive'
           function will immediately return true, as if the caller had
           connected to the original source and then invoked `disconnect'
           some time later.
           [//]
           Like `connect', this function implicitly invokes `close', destroying
           any pre-existing state.  After a successful return, it is possible
           to use the cache contents in exactly the same manner as if
           `connect' had been called and at least one reply had been
           received.  In particular, `is_active' and `target_started'
           return true immediately and functions like `get_target_name' can be
           used to retrieve the name of the logical target that was found
           in the cache file.
           [//]
           As with `connect', the optional `cache_dir' argument to this
           function may be used to identify a directory where cached JPIP
           content is expected to be stored by an application.  This serves two
           purposes:
           [>>] If the client is configured to save cache contents before
                closing, the information supplied via `filename' will be
                stored to a consistently named cache file within the
                `cache_dir' where it can be picked up and re-used by future
                JPIP browsing sessions initiated either by `connect' or
                `open_with_cache_file'.
           [>>] After opening and reading the contents of the supplied
                `filename', the object also checks for any other cache
                file that may exist within `cache_dir', following a consistent
                naming convention, that identifies the same target and
                target-id (supposedly unique).  If such a file is found, its
                contents may be merged with those read from `filename'.
           [//]
           Together, the above features allow the information from multiple
           cache files that all describe the same underlying JPIP target to be
           progressively merged together, so that `cache_dir' always holds
           a file which represents the most complete representation available.
           Moreover, that representation can always be augmented by connecting
           to a JPIP server and requesting additional content.       
           [//]
           Perhaps of greatest interest is the fact that the `reconnect'
           function can be used to re-establish a live connection with the
           original server recorded within the cache file, exactly as if
           a call to `connect' had been followed by `disconnect'.  The results
           from any such re-connection will also be saved to the `cache_dir'
           if one is provided.
           [//]
           This function provides a number of features to accelerate the
           cache file loading process and make the system as responsive as
           possible:
           [>>] If the cache file identified by `filename' contains a preamble
                (a set of high priority data-bins that are specially
                identified), the function only loads this preamble before
                returning.  It launches a separate thread to manage the rest of
                the loading process in the background.  This separate thread
                will post notifications to any installed `kdu_client_notifier'
                object, to indicate progress and completion of the loading
                process, exactly as if there were a live server connection.
                To understand how preambles are defined, review the
                documentation for `set_preserve_window'.
           [>>] Regardless of whether the file contains a preamble or not,
                if there is another compatible cache file, in the directory
                identified by `cache_dir', and `cache_file_handling' includes
                the `KDU_CLIENT_FILE_LOAD' or `KDU_CLIENT_FILE_DELETE' flag,
                that file is also loaded/deleted in the background via a
                separate thread, as mentioned above.  Note that any calls to
                `reconnect' wait until the background thread completes its
                work.
           [>>] If `preamble_only' is true, the function reads only the
                preamble of the file identified by `filename'.  In this case,
                `cache_file_handling' is treated as though it were 0,
                `cache_dir' is treated as though it were NULL, and no
                background thread is launched to perform any loading or
                deletion of other files.  This mode is primarily of interest
                when generating thumbnail/summary views of a source only,
                since it saves the effort of loading the entire cache file
                into memory, when only a small amount of the content is
                required to render the summary view.
         [RETURNS]
           Like `connect', this function returns a valid queue-id if successful
           and this queue-id will inevitably be 0, since that is always the
           first created queue.  New queues may not be created using
           `add_queue', since there is actually no connection.  However, the
           `reconnect' function may be used to establish a connection, after
           which calls to `add_queue' should succeed.
           [//]
           Also, like `connect', if the function does not succeed an error
           is always generated through `kdu_error', which usually results in
           the throwing of an exception (or else the entire process exists if
           there is no error handler that throws exceptions).  This means that
           the function either returns 0 or does not return at all.  To
           avoid errors due to non-existence or incompatibility of the
           cache file, you are advised to call the static `check_cache_file'
           function first.
         [ARG: filename]
           Full or relative path to the file that is to be loaded.  It is
           recommended that you test the file for existence and intelligibility
           using the static `check_cache_file' function before calling here.
           [//]
           The supplied file is treated as read-only and not directly
           overwritten with any content that might be retrieved via a future
           call to `reconnect'.  However, if you supply a non-NULL
           `cache_dir' argument the contents of the supplied file will be
           saved to the `cache_dir', potentially augmented by content from
           a connected server or any pre-existing content read from the
           `cache_dir'.
           [//]
           The only condition under which the file supplied via `filename'
           can be modified is if it is already one of the standard cache
           files maintained by the client in the directory identified by
           a non-NULL `cache_dir' argument.  These follow a standard
           naming convention.
         [ARG: cache_dir]
           As explained, if this argument is non-NULL, it identifies a
           directory in which the client is supposed to be managing cache
           files associated with its JPIP browsing sessions.  Cache contents
           may be saved to this directory and existing files in the directory
           may augment the information supplied via the file found at
           `filename'.
           [//]
           In the special case where `filename' refers to a file that is
           actually located within the `cache_dir' directory, you are
           recommended to use full path names for both `filename' and
           `cache_dir' so that this condition can be recognized.  Otherwise,
           the the that the files are the same might not be detected and
           the content in the cache file might be loaded twice: the second
           load may happen in order to augment the cache representation with
           content that the function mistakenly believes may have been
           obtained from previous browsing sessions.
           [//]
           As with `connect', the way in which files in the cache directory
           are handled depends upon the `cache_file_handling' argument, which
           will normally be passed with the default value of
           `KDU_CACHE_FILE_LOAD_SAVE', a union of the `KDU_CACHE_FILE_LOAD'
           and `KDU_CACHE_FILE_SAVE' flags.
           [//]
           If `preamble_only' is true, the `cache_dir' argument is treated
           as though it were NULL.
         [ARG: cache_file_handling]
           As for `connect', this argument holds a collection of zero or more
           of the following flags, which are meaningful only if a valid
           `cache_dir' is supplied and `preamble_only' is false.
           [>>] `KDU_CLIENT_FILE_LOAD' means that any compatible existing cache
                file found in the `cache_dir' will be loaded and used to
                augment the cache state; that is, after the `filename' file has
                been loaded, the function continues to look for another file
                (if there is one) that represents the same content and is
                found in `cache_dir', using its contents to augment what
                was recovered from the file identified by `filename'.
           [>>] `KDU_CLIENT_FILE_DELETE' means that a compatible existing
                cache file is deleted (after any required loading), if one
                is found in the `cache_dir' directory.  Note that the
                file found within `cache_dir' will be deleted EVEN IF it
                happens to be identical to that identified by `filename'.
           [>>] `KDU_CLIENT_FILE_SAVE' means that a cache file is saved
                to the `cache_dir' directory, unless there is no need to
                do this because `filename' refers already to the file that
                would need to be saved (saving a copy of the supplied
                file to itself would be pointless).  Even in this case,
                the cache will be saved to `cache_dir' if a subsequent
                call to `reconnect' collects new data from a server, or a
                call to `augment_with_cache_file' adds new content from a
                separate file.  The writing itself does not take place until
                a client thread is about to close down (writing then takes
                place in the background) or the `close' function is called
                (explicitly or implicitly).
         [ARG: preamble_only]
           If true, the object is being used only to load the first part of
           the file identified by `filename' for the purpose of generating
           a summary view (e.g., to build a contact sheet or image-picker).
           In this case `cache_dir' is ignored (as if it were NULL) and
           `cache_file_handling' is treated as though it were 0.  With this
           in mind, we do not recommend that you use this mode if you intend
           to issue a call to `reconnect'.
      */
    KDU_AUX_EXPORT void set_cache_file_handling(int handling);
      /* [SYNOPSIS]
           This function can be used to alter the way in which cache files are
           treated, after a call to `connect' or `open_with_cache_file',
           overriding the `cache_file_handling' argument supplied to those
           functions.  The function has no effect unless a non-NULL
           `cache_dir' was supplied to `connect' or `open_with_cache_file',
           since the last call to `close' or since the object was
           constructed.
           [//]
           As in `connect' and `open_with_cache_file', the `handling' value
           should consist of zero or more of the following flags:
           [>>] `KDU_CLIENT_FILE_LOAD' has no effect when used with this
                function, since loading of existing cache files in the
                `cache_dir' directory takes place only in connection with
                calls to `connect' or `open_with_cache_file'.
           [>>] `KDU_CLIENT_FILE_DELETE' means that that any matching cache
                file in the cache directory passed to `connect' or
                `open_with_cache_file' should be deleted, regardless of whether
                it existed before the `connect' or `open_with_cache_file' call
                or was created since then -- this may be used to implement
                "private browsing", or perhaps just to minimize file system
                resource consumption for content that is deemed insufficiently
                valuable.  Note that the deletion activity will take place
                immediately for this option, except where a save or deletion
                operation is already in progress, in which case the deletion
                should occur as soon as that in-progress activity completes.
           [>>] `KDU_CLIENT_FILE_SAVE' means that a cache file should be saved
                to the cache directory supplied to `connect' or
                `open_with_cache_file', except where there is no non-trivial
                target-id available for the JPIP target being used by this
                object, or where an existing cache file is already completely
                up to date.  The saving action will not actually take place
                until a client thread is about to terminate upon disconnection,
                or the `close' function is called (explicitly or implicitly).
                It may be that there is nothing to save at the time when
                this function is called, but this changes after a subsequent
                call to `reconnect' or `augment_with_cache_file', in which
                case the save will still occur.
           [//]
           One particularly important application for this function is to
           avoid redundant cache saving operations when a mobile application
           moves into the background.  Unlike desktop platform, mobile
           devices typically present only one foreground application, to which
           all resources should preferably be dedicated.  It is unhelpful,
           therefore, to have a background application consuming valuable
           bandwidth on JPIP services when it cannot be seen.  As a result,
           if the app is moved to the background (e.g., to receive a phone
           call) the `disconnect' function should be used to gracefully close
           the connection.  However, the expectation is that the connection
           may be restored (using `reconnect') at a later point, which might
           not be far away.  To conserve power, it is best to keep the received
           JPIP data in the memory cache without saving anything, at least
           until memory needs to be reclaimed by other applications.
           [//]
           With the above in mind, you are recommended to call this function
           with `handling'=0 immediately before `disconnect' when your
           application is about to move to the background.  When the
           app is brought into the foreground again, you can call this function
           again, with `handling'=`KDU_CLIENT_FILE_SAVE', and then call
           `reconnect' (you can also do these in reverse order, but that is
           slightly less preferable).  If your background app needs to call
           `close' to free up memory, it can call this function with
           `handling'=`KDU_CLIENT_FILE_SAVE' first, so that the
           `close' function manages the save-to-disk process.
      */
    int get_cache_file_handling() const
      { 
        kdu_int32 flags = cache_state.get();
        int result = 0;
        if (flags & CACHE_STATE_SAVE) result |= KDU_CLIENT_FILE_SAVE;
        if (flags & CACHE_STATE_DELETE) result |= KDU_CLIENT_FILE_DELETE;
        return result;
      }
      /* [SYNOPSIS]
           Returns the current cache file handling value.  In practice, the
           returned value consists of zero or more of only the following
           two flags:
           [>>] `KDU_CLIENT_FILE_DELETE' can be present if it was present
                in the most recent call to `connect', `open_with_cache_file'
                or `set_cache_file_handling', meaning that any compatible
                file in the cache directory passed to the first two of
                those functions either has been deleted or is about to be
                (on in the process of being) deleted.
           [>>] `KDU_CLIENT_FILE_SAVE' can be present if it was present in
                the most recent call to `connect', `open_with_cache_file' or
                `set_cache_file_handling', meaning that a cache file will be
                written to the cache directory passed to the first two of
                these functions, once communication finishes or the `close'
                function is called.
      */
    bool will_lose_data_if_closed() const
      { 
        kdu_int32 flags = cache_state.get();
        if (flags & CACHE_STATE_DELETE)
          return true;
        else if ((flags & CACHE_STATE_SAVE) && (flags & CACHE_STATE_VALID)) 
          return false;
        else
          return ((flags & CACHE_STATE_UPDATED) != 0);
      }
      /* [SYNOPSIS]
           The role of this function is similar to that of
           `get_cache_file_handling', but the return value describes what will
           happen to any content that may currently be in the cache if `close'
           is called, rather than describing cache file manipulations that
           are intended.  For example, if the `get_cache_file_handling'
           function returns 0, there are no scheduled deletes or saves of
           a cache file, but data will only be lost if there have been updates
           to the cache since the last time any cache file was loaded.
           [//]
           One reason for providing this function is that it allows
           memory-challenged applications to determine which `kdu_client'
           objects can be safely closed and later re-opened without
           losing information recovered from a remote server -- recovering
           such information again would incur a communications cost.
      */
    KDU_AUX_EXPORT virtual char *construct_jpip_url() const;
      /* [SYNOPSIS]
           This function returns a string that has been allocated using the
           global C++ vector `new[]' function; it is the caller's
           responsibility to delete the string using `delete[]', unless it
           is NULL.
           [//]
           The objective is to return a full JPIP URL that could
           potentially be used with `connect', regardless of whether the
           `connect' function or the `open_with_cache' function were used
           to get to this point.  In practice, you rarely if ever need to
           rely upon this mechanism to generate a URL that would be passed
           to `connect', since the `reconnect' function is a much better
           choice.  However, the returned URL can be useful in constructing
           an informative display name for the active JPIP client, in a
           manner that is independentt of whether a connection was actually
           established using `connect' or the results of a previous
           connection were re-instated using `open_with_cache_file'.
           [//]
           If the client is not currently active (i.e., no call to
           `connect' or `open_with_cache_file' since construction or the
           last call to `close'), the function returns NULL.
      */
    KDU_AUX_EXPORT virtual bool
      augment_with_cache_file(const char *filename);
      /* [SYNOPSIS]
           This function is similar to `open_with_cache_file', except that it
           can only succeed when invoked on a `kdu_client' object that is
           already open (via `connect' or `open_with_cache_file') and is
           working with the same logical resource as that described by the
           cache file named by `filename'.  Typically, you would not call this
           function without first calling the static `check_cache_file',
           passing it a `kdu_cache_file_info' object and comparing the
           string returned by `kdu_cache_file_info::get_cache_identifier' with
           that returned by this object's `get_cache_identifier' function.  If
           both the comparision reveals identical cache-identifiers, this
           function should succeed, returning true.
           [//]
           What the function actually does is augment the object's cache with
           the content found within the supplied cache file.  This might make
           no difference at all, especially if the supplied cache file is one
           that was originally passed to the current object's
           `open_with_cache_file' function.  If any data-bin is actually
           augmented by the call, and the current object is currently
           connected to a JPIP server, future requests to the server will
           be modified (if relevant) to identify the agumented content in
           JPIP cache model statements, so that it is not redundantly
           communicated.
           [//]
           If the function fails, returning false, there should be no
           adverse impact on the current client or any of its dependencies.
      */
    KDU_AUX_EXPORT virtual int
      reconnect(const char *channel_transport, const char *proxy,
                bool clear_cache=false);
      /* [SYNOPSIS]
           This function can be used in place of `connect' after a
           call to `disconnect', with no intervening call to `close'.  The
           `reconnect' function can be used even if `disconnect' has not been
           explicitly called, which can be useful if the server itself has
           disconnected the client in one way or another.  If the client and
           server are still connected, this function does nothing and the
           client remains connected to the server.
           [//]
           The main differences between `reconnect' and `connect' are as
           follows:
           [>>] `reconnect' does not implicitly call `close', so it does not
                lose the contents of the cache accumulated from a previous
                connection, although this behaviour can be overridden by
                passing `clear_cache'=true.  If you have saved the cache
                contents then `connect' can be used to achieve the same result
                by reloading the cache, but `reconnect' bypasses the otherwise
                unnecessary load/save steps.
           [>>] `reconnect' does not need to be informed of the connection
                details (server, request, etc.), since it already knows them.
           [>>] `reconnect' can be used to re-establish a connection with
                the original server discovered from a cache file passed to
                `open_with_cache_file'.
           [>>] `reconnect' also knows the exact path of any cache file that
                was discovered by `connect' or `open_with_cache_file' -- this
                is a file within the cache directory that may have been
                supplied to `connect' or `open_with_cache_file'.  This is
                the cache file that will be updated if the reconnection is
                successful and the client is configured to save cache
                contents.
           [>>] `reconnect' uses the internally recorded JPIP target-id to
                make sure that the new connection involves exactly the same
                representation of the target resource, corresponding to the
                existing cache contents.  If the server cannot guarantee
                consistency, and `clear_cache' is false, the reconnection
                necessarily fails, resulting in an error message being
                generated (eventually) through `kdu_error', after which the
                `target_incompatible' function will return true; this does not
                stop you from continuing to use the existing cache, but it does
                cause the reconnection attempt to fail.  One reasonable action
                to take after such a condition is discovered would be to call
                this function again with `clear_cache'=true, so that the
                server's current version of the target can be accessed from
                scratch.  By contrast, if `connect' has an existing cache file
                for a resource, but disconvers a target-id inconsistency, the
                existing cache file is not loaded, but the connection is
                established.
         [RETURNS]
           If no error occurs, the function will return 0, which can also
           be interpreted as the first queue-id, as with the `connect'
           function.  Unlike `connect', however, the function may return
           other values, as follows:
           [>>] -1 means that the object is already connected, so nothing
                needed to be done.
           [>>] -2 means that insufficient information is available to
                initiate a connection request -- e.g., neither `connect' nor
                `open_with_cache_file' might have been successfully called, or
                there may have been an intervening `close' call.
           [>>] -3 means that a previous attempt to `reconnect' found an
                incompatible TARGET-ID and `clear_cache' is false, so there is
                no point in trying again; note that in this case,
                `target_incompatible' would also have returned true before this
                function was called and will continue to return true.
           [>>] Other negative values may be used to indicate that the
                function was able to determine immediately that reconnection
                is not possible.
           [//]
           As with `connect', the function may generate an error through
           `kdu_error' is something is clearly wrong (e.g., invalid transport
           string), which will usually result in an exception being thrown.
         [ARG: channel_transport]
           During reconnection, you have the opportunity to request a different
           type of transport.  The interpretation of this argument is
           identical to its namesake in `connect', except that a NULL
           `channel_transport' argument does not mean that communication should
           be stateless; instead, it means that the same type of transport
           should be requested as the one that was used when the client
           was last connected.  Stateless is the default only if the client
           was previously communicating statelessly or has never been
           connected (i.e., if it was activated using `open_with_cache_file').
         [ARG: proxy]
           During `reconnect' you also have the opportunity to set a proxy
           server or change the one that might previously have been in use.
           If this argument is NULL, the proxy settings remain unchanged
           from those established by the last call to `connect' (no proxy
           if `open_with_cache_file' was used to get to this point).
         [ARG: clear_cache]
           This option is provided mainly to deal with the situation in which
           a previous attempt to `reconnect' resulted in the
           `target_incompatible' connection.  If this happens, you can continue
           using the existing cache, backed by any cache file that may exist.
           However, to access the server's current version of the content, you
           need to clear the cache and start from scratch.  There are two ways
           you can do this:
           [>>] Use `connect' to start again -- this will automatically clear
                existing cache contents; or
           [>>] Call this function with `clear_cache'=true.
           [//]
           ...
           [>>] If there is a backing cache file, this function knows exactly
                which file that is and its lack of compatibility, which allows
                it to overwrite the file with new content received from the
                server once a connection is established.  On the other hand,
                `connect' just skips over any incompatible cache file, since
                there may be many cache files that refer to resources with the
                same name that should not necessarily be deleted or
                overwritten.  The result is that simply calling `connect' after
                discovering incompatibility within a `reconnect' call may
                tend pollute the cache directory with out-of-date content.
           [//]
           Note that since this option completely clears the cache, it is
           important that you do not have any other entities reading from it,
           including asynchronous readers that might be using caches that are
           attached to this one via `kdu_cache::attach_to'.
      */
    ...
    const char *get_target_name() const
      { 
        const char *result = processed_target_name;
        if (result == NULL) result = "<no target>";
        return result;
      }
      /* [SYNOPSIS]
           ...
           [//]
           Note that any hex-hex encoding which may have been used to ensure
           URI-legal names in the original call to `connect' has already
           been decoded in the returned string.
      */
    KDU_AUX_EXPORT virtual bool
    ...
    bool is_active() { return active_state; }
      ...
      */
    bool target_started() const { return target_request_successful; }
      /* [SYNOPSIS]
           ...
           true.
           [//]
           This function continues to return true after calls to
           `disconnect' and/or `reconnect', until such time as `close' is
           called.  To determine whether the object is in the process of
           establishing a connection with a server, either for the first or
           a subsequent time (i.e., via `connect' or `reconnect'), evaluate
           `is_alive()' && (`is_reconnecting()' || !`target_started()').
      */
    bool check_stateless() const { return is_stateless; }
      /* [SYNOPSIS]
           You can use this function to determine whether or not the client
           is currently connected in a stateless session with a server.  Calls
           to `connect' or `reconnect' do not immediately change the value
           returned by this function to true, even if a stateful session
           transport is requested from the server.  Instead, the stateless
           condition persists until connection is established and the
           `target_started' function returns true.  At that point, the function
           will continue to return false only if no stateful session was
           granted.  Calls to `close' or `open_with_cache_file' also leave
           the object in the stateless condition.
           [//]
           There is no fundamental reason why an application should need to
           know whether communication is stateful or stateless, but
           stateful communication is significantly more efficient, so this
           information may allow an application to determine whether the
           resource should be accessed from a different server that might
           support stateful communication.
      */
    const char *get_cache_identifier() const
      { return (cache_state.get() & CACHE_STATE_VALID)?cache_identifier:NULL; }
      /* [SYNOPSIS]
           This function is provided to allow a client to determine whether
           or not the logical resource that it is working with is compatible
           with that represented by a separate cache file.  To do this, you
           pass the separate named cache file to `check_cache_file', passing
           it a `kdu_cache_file_info' object, then you compare the string
           returned by `kdu_cache_file_info::get_cache_identifier' with that
           returned by this function.  This function can also be used to
           determine whether two separate `kdu_client' objects are both
           working with the same logical resource/target.
           [//]
           This function cannot return non-NULL unless `target_started' already
           returns true, which means that the first reply has been received
           in response to a `connect' call or there has been a successful
           `open_with_cache_file' call.  Once this has happened, the cache
           identifier can usually be completed based on the `target_id' string
           and then this function can return non-NULL.  Note, however, that if
           the server never returns a usable target-id (something other than 0)
           this function will always return NULL.
           [//]
           If non-NULL, the returned string is an internal resource that should
           remain valid until `close' is called (perhaps implicitly from
           within a call to `connect' or `open_with_cache_file').
      */
    bool is_reconnecting() const { return reconnecting; }
      /* [SYNOPSIS]
           Returns true from the point at which `reconnect' is called to the
           point at which the first reply is received from the server, or
           `close' is called or `disconnect' completes.
           [//]
           This information can be useful, since `target_started' can only
           be used to determine the point at which the first reply is received
           from an original `connect' call, not from a `reconnect' call.
      */
    bool target_incompatible() const { return target_is_incompatible; }
      /* [SYNOPSIS]
           Returns true if the logical target-id returned by a JPIP server
           differs from the current value that was determined when the source
           was first opened or obtained from the original `connect' call.
           This is usually discovered after a `reconnect' call, whereupon
           any subsequent call to `reconnect' is bound to fail.
      */
    KDU_AUX_EXPORT virtual bool is_alive(int queue_id=-1);
      /* [SYNOPSIS]
           ...
           point at which it is created by `connect'/`reconnect' or `add_queue'
           (even though it may take a while for actual communication to be
    ...
    KDU_AUX_EXPORT kdu_long
      ...
      /* [SYNOPSIS]
           ...
           using `post_window'.
           [//]
           Roughly speaking, the horizon is calculated by starting with the
           value Lmax (number of bytes the client aims to request at once,
           either in a single request or a batch of concurrently issued
           requests), adding the intended overlap (number of bytes that
           are still to be received from a previous request or batch of
           requests before a new request is issued), which is Lmax/2 in the
           current implementation, removing any outstanding bytes from earlier
           requests (requested but not yet received), dividing by the
           estimated transmission rate so as to convert from bytes to
           microseconds, then subtracting the service times associated with
           pending requests that have been posted but not yet issued to the
           server.
           [//]
           Beyond the above rough blueprint, the function also takes account
           of accumulated "disparity" between actually received service times
           and requested service times.
           [>>] If the disparity is positive, the service is running behind, so
                requests in the future will have to be issued more quickly
                (with lower byte limits) to catch up.  This means that the
                horizon should be lengthened, because we will need requests
                further into the future in order to accommodate the issuing
                of deliberately foreshortened requests, while continuing to
                keep the channel alive.
           [>>] On the other hand, if the service time disparity is negative,
                the service is running ahead, which often happens when the
                client already has most or all of the content being requested
                within its cache.  In this case, requests will be deliberately
                lengthened in order to restore things; however, if the client
                already has all available content for a decent chunk of an
                animation, this will not help matters, since the server will
                consistently return far less information than the client
                requests, regardless of how much data is requested.  Without
                any corrective efforts, this will result in the current
                function returning a consistently positive horizon, because
                more requests are always required in the vain attempt to keep
                the channel alive.  While this is OK for a streaming video
                service, it is not ideal for an interactive media
                browser, since a large number of requests may need to be
                wiped out (pre-empted) if the client's interests change.
                To address this issue, the present function implicitly applies
                a cap on the degree to which the realized service disparity
                can become negative, reducing the returned horizon so as to
                enforce this cap.  In this way, negative horizons will
                eventually be returned, causing the request queue to empty and
                the service to go idle for a while.  This idle time counts as
                unused service time from the perspective of the flow control
                algorithm, so that the service disparity is gradually restored
                towards zero.  While idling the service may appear to be
                non-optimal, it is expected to lead to better overall
                utilization of resources, since an interactive user's interests
                are likely to change over time, and other JPIP clients (or
                queues within the same client) may be able to better
    ...
    KDU_AUX_EXPORT virtual bool
      get_window_in_progress(kdu_window *window, int queue_id=0,
                             ...
                             bool last_window_if_not_alive=false);
      /* [SYNOPSIS]
           ...
           supplied `window'.  However, if `last_window_if_not_alive' is true
           and there are no request queues still alive, the function treats
           the last window that was in progress for any request queue as the
           one to which the query relates, returning true if there was such
           a window and returning its final state via the `window' argument.
           [//]
         ...
         [ARG: last_window_if_not_alive]
           This argument can be used to discover some useful information if
           all request queues have disappeared (i.e., if `is_alive' would
           return false when invoked with any argument).  Whenever a
           request queue terminates for any reason, its most recent window
           in progress (the window that would have been returned by this
           function) is internally saved.  This is the last window that
           received any reply from the server.  Each queue that terminates,
           having had a window in progress (one with a reply), overwrites the
           internal store.  If there is any request queue still alive, this
           argument is ignored.  If there are none alive and this argument is
           true, the function returns true so long as there was at least some
           queue which had a replied window at the time when it terminated;
           in this event, meaningful information is returned via `window', as
           well as any non-NULL `status_flags' and `custom_id' arguments.
           [//]
           In many cases, the last window in progress before all request queues
           terminated might not be very interesting.  However, if the client
           has been used to issue a one-shot request (i.e., a single request
           queue with just one request and no interaction), this information
           can be quite valuable.  Single-shot requests may be very short
           lived, and the request queue will be terminated as soon as the
           response is complete, which may not give an application the
           opportunity to inspect the window in progress without the aid of
           this option.
      */
    KDU_AUX_EXPORT bool
    ...
    KDU_AUX_EXPORT virtual kdu_long
      ...
      /* [SYNOPSIS]
           ...
           [//]
           In the event that `open_with_cache_file' was used and there has
           been no call to `reconnect', this function actually returns the
           number of bytes read from the file so far, including any bytes
           read from a compatible cache file that may have been found in
           a supplied cache directory.  This simplifies the implementation
           of applications that render content progressively as more data
           arrives in the cache, since they do not need to distinguish
           between content that arrives from file loading and content
           that arrives from a server.
         [ARG: queue_id]
           One of the request queue identifiers returned by the `connect'
    ...
    KDU_AUX_EXPORT virtual void
      set_preserve_window(const kdu_window *window,
                          bool save_cache_files_with_preamble=true);
      /* [SYNOPSIS]
           This function is useful if you are in a memory constrained
           environment and have called (or are planning on calling)
           `kdu_cache::set_preferred_memory_limit'.  In this case, as data
           is added to the cache, there is always the possibility that
           existing content gets evicted, following a least-recently-touched
           policy; in particular, this can potentially happen whenever
           `kdu_cache::add_to_databin' is called.  While this is usually the
           desired behaviour, it can be important to preserve certain types
           of content against such eviction.  The `kdu_cache' object provides
           two basic functions for doing this: `kdu_cache::preserve_databin';
           and `kdu_cache::preserve_stream_class'.  The latter is
           automatically used certain types of key data-bins; in particular,
           the client already arranges to preserve all meta-databins and all
           codestream main header data-bins.  Beyond this, however, the
           former function can be used to describe specific imagery that you
           would like to be preserved; the purpose of this function is to
           make that easy for you.
           [//]
           The behaviour of the function is determined entirely by its
           `window' argument, which describes the content to be preserved via
           a JPIP-compatible window-of-interest.  Typically, `window' will
           describe a WOI that covers the first codestream (raw files) or
           compositing layer (wrapped JP2/JPX files) up to some modest
           resolution that can be used to reconstruct a thumbnail or summary
           view of the source.  Such summary views are very useful, so it
           is natural to want to preserve the associated cache data against
           eviction when memory constraints are encountered.
           [//]
           In order to interpret the `window', it may be necessary to perform
           some context translation (true for everything except raw
           codestreams), so any context translator that you intend to install
           via `install_context_translator' should be installed prior to
           calling this function.
           [//]
           It may be that the supplied `window' cannot be fully interpreted
           until after some more data arrives in the cache.  There is
           absolutely no need to call this function repeatedly as data arrives
           in the cache.  Just call the function once, and an internal copy
           will remain in place until the `window' can be fully translated,
           at which point it will be cleaned up.  Full translation may occur
           upon reading an existing cache file (e.g., if `open_with_cache_file'
           or `augment_with_cache_file' is used, or if a pre-existing
           compatible cache file is discovered upon completing a connection
           with a server).  In many cases, full translation occurs only after
           some content is retrieved from a JPIP server.
           [//]
           You can replace an exising preserve-window with another one,
           calling this function as often as you like, although the effects
           of any previous preserve window will persist, to the extent that
           it has been processed already; since there is no way to know whether
           a previous window has been fully processed, calling this function
           multiple times with different windows may not have a predictable
           effect.  In any case, you also need to be careful not to preserve
           so much content that you render memory constraints ineffective.
           [//]
           Beyond the preservation of data within the cache memory itself,
           this function serves one very other useful purpose: it allows
           cache files to be written with a specifically identified preamble
           that contains all preserved content, after which any remaining
           cache data is stored.  This is relevant only if cache files are
           written (see `set_cache_file_handling', for example), but this is
           expected to be the case in most applications.  If the
           `save_cache_files_with_preamble' argument is true, and the
           supplied `window' does get translated, any written cache file will
           contain a preamble, which alerts readers to the fact that they
           can read just the preamble to get started with rendering content
           for the source, after which they can load in the rest of the
           content.  An application which generates summary images within an
           image-picker window would typically only load the preamble, greatly
           reducing I/O time and energy consumption if the full cache file is
           very large.
         [ARG: window]
           The window-of-interest is copied internally, so you need not
           preserve this object beyond the call.
         [ARG: save_cache_files_with_preamble]
           This argument puts the client into a mode where any cache file that
           is written will be saved with a preamble containing all cache
           data-bins that were flagged for preservation, by this function or
           any other means.  Strictly speaking, if the argument is true, the
           mode transition is deferred until the supplied `window' has been
           fully translated, so that a failed window does not wind up creating
           useless preambles that might deceive applications into believing
           they do not need to load the full cache file.  If the argument is
           false, the preamble-save mode is immediately cancelled internally.
      */
  private: // Startup function
    friend kdu_thread_startproc_result
      KDU_THREAD_STARTPROC_CALL_CONVENTION client_thread_startproc(void *);
  ...
  private: // Internal functions which are called only once per `connect' call
    ...
    void run_load_file_only();// Called from `thread_start' if `load_file_only'
  ...
  private: // Helper functions used to manage primary channel life cycle
    kd_supp_local::kdc_primary *
      add_primary_channel(const char *host, kdu_uint16 default_port,
                          bool host_is_proxy);
      /* Creates a new `kdc_primary' object, adding it to the list.  The new
         ...
         to use the channel. */
    void release_primary_channel(kd_supp_local::kdc_primary *primary);
  ...
  private: // Helper functions used to manage CID (JPIP channel) life cycle
    kd_supp_local::kdc_cid *add_cid(kd_supp_local::kdc_primary *primary,
    ...
    void release_cid(kd_supp_local::kdc_cid *cid);
  ...
  private: // Helper functions used to manage request queue life cycle
    kd_supp_local::kdc_request_queue *
      add_request_queue(kd_supp_local::kdc_cid *cid);
    ...
    void release_request_queue(kd_supp_local::kdc_request_queue *queue);
  ...
  private: // Cache file I/O
    kdu_long load_cache_file_contents(FILE *fp, kdu_int32 max_bytes);
      /* Reads the body of a cache file whose header has already been read
         successfully.  This function does not generate errors or throw
         exceptions; it simply invokes `kdu_cache::add_to_databin' each time
         it encounters new content in the cache file, marking the relevant
         data-bins as augmented if appropriate.
            If `max_bytes' <= 0, the function reads to the end of the file;
         otherwise, it reads a `max_bytes' from the file and then any
         additional bytes required to ensure that it has read a whole
         number of data-bins.
            Returns the total number of bytes read. */
    kdu_int32 count_cache_file_preamble_bins(kdu_int32 &num_preamble_bytes);
      /* This function scans through all data-bins in the cache that are
         flagged for preservation and have data of any form that could be
         saved to the cache, returning the number of such data-bins and
         setting `num_preamble_bytes' equal to the total number of bytes that
         would be required to save these data-bins to a cache file, including
         the header costs.  The function stops counting data-bins and measuring
         their size if the `num_preamble_bytes' value would grow too large
         to store in a 32-bit signed integer. */
    void store_cache_file_contents(FILE *fp, bool has_preamble);
      /* Does the reverse of the above function.  The cache file header
         must be written first.  If `has_preamble' is true, the function
         first scans through all data-bins flagged for preservation, after
         which it scans through all remaining data-bins.  For this to be
         completely robust, it is important that no calls to `add_to_databin'
         occur while the file is being saved. */
  private: // Helper functions for recyclable resources
    friend struct kd_supp_local::kdc_request;
    kd_supp_local::kdc_request *alloc_request();
    void recycle_request(kd_supp_local::kdc_request *req);
    kd_supp_local::kdc_request_dependency *alloc_dependency();
    void recycle_dependencies(kd_supp_local::kdc_request_dependency *list);
    kd_supp_local::kdc_chunk_gap *alloc_chunk_gap();
    void recycle_chunk_gaps(kd_supp_local::kdc_chunk_gap *list);
  private: // Other helper functions
    ...
    char *make_temp_string(const char *src, int max_copy_chars);
      /* Makes a temporary copy of the first `max_copy_chars' characters from
         ...
         long. */
    kd_supp_local::kdc_model_ref *
      add_stream_model_ref(kdu_long codestream_id,
                           kd_supp_local::kdc_model_ref_list *list);
      /* Called when a queue is about to issue a JPIP request for codestream
         content for which cache model manipulation statements might need to
         be generated, or where data-bins may need to be touched to protect
         them against automatic trimming operations, or where a preservation
         flag generator needs to assess data-bins to preserve permanently
         against cache trimming operations.  The function is only called if
         `list' does not already contain a cache model reference for the
         indicated codestream.  The function creates the model, if necessary,
         or retrieves it from the internal active or inactive model list,
         instantiating a reference to the model and hooking the reference into
         both the model's list of references and the `list's list of
         references. */
    void release_stream_model_ref(kd_supp_local::kdc_model_ref *ref);
      /* Called when a request queue or other entity no longer has a need to
         keep an active reference to a codestream model manager.  Every
         model-ref lives on two lists: one belonging to the model (identified
         by `ref->model') that keeps track of references to it; and one
         belonging to a `kdc_model_ref_list' object (identified by `ref->list')
         that manages references to different codestreams from a context
         such as a request queue.  The `ref' object is removed from both of
         its these lists by this function, after which `ref' is recycled
         internally.
            The function also checks the `ref->model->all_marks_removed' flag.
         If this flag is set, it may be possible to completely delete the
         cache model early, rather than just moving it to a list of inactive
         models.  In particular, if the flag is set, the function tries to
         remove any other references to the model, which is generally
         possible unless a `kdc_model_ref_list::can_discard' flag is found to
         be false.  If all references can be removed then the model is
         discarded. */
    int signal_model_corrections(kdu_window &ref_window,
                                 kdcs_message_block &block,
                                 int max_block_length,
                                 kd_supp_local::kdc_request_queue *queue);
      /* Called when generating a request message to be sent to the server.
         ...
         previous session, stored in a `.kjc' file.  In both cases, the
         server cannot be expected to know about any deletions from the
         cache, which may have resulted from explicit calls to
         `kdu_cache::delete_databin' or `kdu_cache::delete_stream_class', or
         which may have resulted from automatic cache trimming operations
         that occur transparently within `kdu_cache::add_to_databin' if a
         preferred memory limit has been installed.
         [//]
         ...
         headers.
         [//]
         The `max_block_length' argument identifies a point at which the
         function is expected to stop adding cache `model' statements
         to the message `block'.  Specifically, after adding
         each cache model manipulation statement to `block' the value of
         `block.get_remaining_length' is tested; if it equals or exceeds
         `max_block_length', the function returns -1, inviting the caller
         to complete the request that is being constructed and use
         `kdc_request_queue::duplicate_request' to make a copy of the
         request that can be issued as soon as practicable so as to 
         continue the cache model signalling process.  The `max_block_length'
         argument is not completely strict, since the function does not
         actually return -1 until after it has added a cache model statement,
         which almost certainly exceeds the limit.  The limit may be further
         exceeded if the model requests are hex-hex encoded by the caller,
         or if the caller adds further fields to the request.  The JPIP
         standards does not formally impose a request limit, but servers
         (including "kdu_server") will usually impose some reasonable
         upper bound to limit the threat posed by DOS attacks.
         [//]
         Returns 0 if no cache model statements were written; 1 if all
         relevant model statements were written without reaching the
         `max_block_length' limit; else -1, as explained above.  If the
         function does return anything other than 0, the request/response
         should not reasonably be considered cacheable by intermediate
         HTTP proxies.
         [//]
         Note that this function is also used to touch any data-bins within
         the cache that may be relevant to the request, if there is a
         preferred memory limit in place and the cache size is getting
         large so that there may be some risk of cache trimming occurring
         as the data returned in response to this quest is added into the
         cache.  Addition/deletion mark analysis for cache model signalling
         and data-bin touching operations are very similar so it is easiest
         to do both types of operation together, especially since at most one
         of the two types of operation is required for any given data-bin.
      */
    bool install_preserve_flags();
      /* This function uses the `preserve_descriptor' member to try to install
         all relevant preservation flags via `kdu_cache::preserve_databin'.
         If sufficient information is available to complete the task, the
         function returns true and the `preserve_descriptor' can be removed
         using `remove_preserve_descriptor'.  Moreover, upon succeeding, the
         function may convert the `save_files_with_preserved_preamble' flag to
         true, if the `preserve_descriptor' had that flag set.  If the
         `preserve_descriptor' does not exist, the function returns false
         immediately. */
    void remove_preserve_descriptor();
      /* Safely removes the `preserve_descriptor', whether it has finished
         installing all relevant preservation flags or not. */
    bool parse_query_string(char *query, kd_supp_local::kdc_request *req,
  ...
  private: // Synchronization and other shared resources
    friend struct kd_supp_local::kdc_request_queue;
    friend class kd_supp_local::kdc_cid;
    friend class kd_supp_local::kdc_primary;
    ...
    kdcs_timer *timer; // Lasts entire life of the `kdu_client'
  ...
  private: // Parameters that control overall behaviour
    char *host_name; // Copy of the `server' string supplied to `connect' --
        // this string retains any hex-hex encoding from its supplier.
    char *proxy_name; // Copy of the `proxy' string supplied to `connect' --
        // this string retains any hex-hex encoding from its supplier.
    char *resource_name; // Resource part of `request' supplied to `connect' --
        // this string retains any hex-hex encoding from its supplier.
    const char *query_ptr; // Configured by `connect' -- points into
        // the `resource_name' buffer and re-usable by `reconnect'.
    char *query_buf; // Copy of `query_ptr' string that may be modified by
    ...
    char *cache_identifier; // Alloc by app; may be completed in client thread
    kdu_interlocked_int32 cache_state; // See below
    bool save_files_with_preserved_preamble; // see `set_preserve_window'
    char *cache_path; // Starts as directory; completed with cache_identifier
    ...
    bool check_for_cache_file; // If should try to find one after first reply
    bool initial_connection_window_non_empty; // Set inside `connect'
    bool reconnecting; // True between `reconnect' and first reply
    bool target_is_incompatible; // True if attempt to `reconnect' found that
       // the server's TARGET-ID differs from that associated with the existing
       // cache contents -- if so, any future `reconnect' call will immediately
       // return an error code, until `close' has erased this condition.
    bool is_stateless; // True until a session is granted
  private: // Members relevant to all request queues within the client thread
    ...
    bool load_file_only; // If `thread' was launched by `open_with_cache_file'
    FILE *file_to_load; // NULL, or open file for `thread' to finish loading;
                        // always NULL unless `load_file_only' is true.
  ...
  private: // Global statistics
    ...
    kdu_long cache_file_loaded_bytes; // Valid only while `load_file_only' true
  ...
  private: // Communication state and resources
    kd_supp_local::kdc_request *free_requests; // List of recycled request
                                               // structures
    kd_supp_local::kdc_request_dependency *free_dependencies; // List of
                                               // recycled dependencies
    kd_supp_local::kdc_chunk_gap *free_chunk_gaps; // List of recycled chunk
                                                   // gaps
    kd_supp_local::kdc_primary *primary_channels; // List of primary
                                                  // communication channels
    kd_supp_local::kdc_cid *cids; // List of JPIP Channel-ID's and associated
                                  // channel state
    kd_supp_local::kdc_request_queue *request_queues; // List of all alive
                                                      // request queues
  ...
  private: // Final state before request queues terminate
    kdu_window final_window;
    bool have_final_window;
    bool final_window_was_completed;
    kdu_long final_window_custom_id;
  private: // Model manager resources
    kd_supp_local::kdc_preserve_descriptor *preserve_descriptor;
    kd_supp_local::kdc_model_manager *active_models;
    kd_supp_local::kdc_model_manager *inactive_models_head; // oldest is here
    kd_supp_local::kdc_model_manager *inactive_models_tail; // newest is here
    int num_active_models;   // Number of models on the `active_models' list
    int num_inactive_models; // Number of models on the `inactive_models' list
    kd_supp_local::kdc_model_ref *free_model_refs; // Recycling list
    int num_active_model_refs; // Num refs allocated but not yet released
  private: // Temporary resources
    int max_scratch_chars; // Includes space for any null-terminator
    char *scratch_chars;
    int max_scratch_ints;
    int *scratch_ints;
  private: // Bit-field constants used with the `cache_state' member, see below
    static const kdu_int32 CACHE_STATE_VALID     = 0x01;
    static const kdu_int32 CACHE_STATE_EXISTS    = 0x02;
    static const kdu_int32 CACHE_STATE_UPDATED   = 0x04;
    static const kdu_int32 CACHE_STATE_SAVE      = 0x08;
    static const kdu_int32 CACHE_STATE_SAVING    = 0x10;
    static const kdu_int32 CACHE_STATE_DELETE    = 0x20;
    static const kdu_int32 CACHE_STATE_DELETING  = 0x40;
    static const kdu_int32 CACHE_STATE_IGNORE    = 0x80;
  };
  /* Implementation Notes:
     ...
     list) or any request queue.
        The `cache_state' interlocked variable plays an important role in
     managing the reading, writing and deleting of cache files, while
     also identifying the validity of the `cache_identifier' and
     `cache_path' strings.  The variable holds the following flag bits:
     * Bit 0 (`CACHE_STATE_VALID'):
       0 = the `cache_identifier' string has not yet been completed; we must
           still be waiting for a non-trivial `target_id' string to become
           available, which might never happen if the server does not assign
           target-id's -- in that case it is not safe to use cache files.
       1 = `cache_identifier' and (if non-NULL) `cache_path' are both valid
           strings.
     * Bit 1 (`CACHE_STATE_EXISTS'):
       0 = no cache file found matching `cache_path'
       1 = file at `cache_path' exists (pre-existing or written by us)
     * Bit 2 (`CACHE_STATE_UPDATED'):
       0 = any cache file at `cache_path' is up-to-date with cached data
       1 = cache file at `cache_path' needs to be rewritten to be current
     * Bit 3 (`CACHE_STATE_SAVE'):
       0 = no save required
       1 = cache file should be saved (may happen when client thread is
           finishing up, or else when the application calls `close')
     * Bit 4 (`CACHE_STATE_SAVING'):
       0 = no saving is in progress
       1 = cache file saving is in progress; this can be true at the same time
           as `CACHE_STATE_SAVE', since after saving is complete, there
           may be subsequent updates to the cache which should be saved in
           the future.
     * Bit 5 (`CACHE_STATE_DELETE'):
       0 = no delete required
       1 = cache file should be deleted (may happen when client thread is
           finishing up, when the application calls `close', or when an
           in-progress load completes)
     * Bit 6 (`CACHE_STATE_DELETING'):
       0 = no delete is in progress
       1 = cache file deletion is in progress.  It is possible for the
           `CACHE_STATE_DELETE' flag to be preserved while deleting a
           cache file, but it probably makes no difference if it is removed
           either, since there is no way for the `CACHE_STATE_EXISTS' flag
           to be re-instated.
     * Bit 7 (`CACHE_STATE_IGNORE'):
       0 = cache files which are found to exist within the `cache_path' are
           loaded wherever appropriate.
       1 = cache files which are found are not loaded, but may be deleted
           or saved over.
  */
