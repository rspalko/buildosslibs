  ...
  */
  public: // Data
    ...
    kds_chunk *next;
      ...
      */
  private: // Data
    friend struct kd_serve_local::kd_serve;
    friend class kdu_supp::kdu_serve;
    friend class kd_serve_local::kd_chunk_server;
    kd_serve_local::kd_chunk_binref *bins; // Used to keep track of what is in
                       // a chunk, in case it gets abandoned and we have to
                       // erase its contributions from the cache model.
  };
...
/*****************************************************************************/
/*                          kds_entity_container                             */
/*****************************************************************************/

struct kds_entity_container {
  ...
  /* This is used inside `kds_image_entities' to re-interpret codestream
     and compositing layer indices.
        If a non-NULL `container' reference is found in `kds_image_entities':
     1) a codestream entity that is found to be less than `num_top_codestreams'
     is left unchanged; 2) other codestream entities are expected to
     lie in the range `num_top_codestreams' to `num_top_codestreams' +
     `num_base_codestreams'-1.  In this latter case, the codestream entity is
     re-interpreted by adding `first_base_codestream'-`num_top_codestreams'
     and replicating the resulting index indefinitely out to a maximum value
     of `max_codestream'.
        The same considerations apply to the interpretation of compositing
     layer entities, except that the relevant parameters are `num_top_layers',
     `first_base_layer', `num_base_layers' and `max_layer'.
        The `kds_image_entities::universal_flags' member is also re-interpreted
     with respect to the container.  Specifically, a `kds_image_entities'
     object has a non-NULL `container' member, its `universal_flags' are
     interpreted as matching all non-top-level codestreams or compositing
     layers (as appropriate) that are defined by the container. */
...
class kds_image_entities {
  ...
  */
  public: // Member functions
    ...
    bool test_codestream(int test_idx)
      { 
        if (container != NULL)
          { 
            if (test_idx >= container->num_top_codestreams)
              { 
                if ((test_idx > container->max_codestream) ||
                    ((test_idx-=container->first_base_codestream) < 0))
                  return false;
                if (universal_flags & 0x01000000) return true;
                if (container->num_base_codestreams == 1) test_idx = 0;
                else test_idx %= container->num_base_codestreams;
                test_idx += container->num_top_codestreams;
              }
          }
        else if (universal_flags & 0x01000000)
          return true;
        int idx, n;
        for (n=0; n < num_entities; n++)
          if ((((idx=entities[n]) & 0x00FFFFFF)==test_idx) && ((idx>>24)==1))
            return true;
        return false;
      }
    bool test_codestream_range(kdu_sampled_range &rg)
      { 
        int idx, max_idx, n, from=rg.from, to=rg.to;
        if (container != NULL)
          { // Start by restricting to the top level
            if (to >= container->num_top_codestreams)
              to = container->num_top_codestreams-1;
          }
        else if (universal_flags & 0x01000000)
          return true;
        if (from <= to)
          for (n=0; n < num_entities; n++)
            if ((((idx = entities[n]) >> 24) == 1) &&
                ((idx &= 0x00FFFFFF) >= from) && (idx <= to) &&
                ((rg.step == 1) || (((idx-rg.from)%rg.step) == 0)))
              return true;
        if ((container == NULL) || (from > container->max_codestream))
          return false;
        // Now restrict to container-defined codestreams
        to=(rg.to>container->max_codestream)?(container->max_codestream):rg.to;
        to -= container->first_base_codestream;   // Make `from' and `to'
        from -= container->first_base_codestream; // relative indices
        if (to < 0) return false;
        if (from < 0) from = (rg.step==1)?0:(from % rg.step);
        if (to < from) return false; // No container-defined streams in `rg'
        if (universal_flags & 0x01000000) return true;
        for (n=0; n < num_entities; n++)
          if ((((idx=entities[n]) >> 24) == 1) &&
              ((idx = (idx&0x00FFFFFF) - container->num_top_codestreams) >= 0))
            { // Match k,m s.t. if idx+k*num_base_codestreams = from+m*rg.step
              for (max_idx=to-idx, idx=from-idx; idx <= max_idx; idx+=rg.step)
                if ((idx>=0) && ((idx % container->num_base_codestreams) == 0))
                  return true;
            }
        return false;
      }
    bool test_compositing_layer(int test_idx)
      { 
        if (container != NULL)
          { 
            if (test_idx >= container->num_top_layers)
              { 
                if ((test_idx > container->max_layer) ||
                    ((test_idx-=container->first_base_layer) < 0))
                  return false;
                if (universal_flags & 0x02000000) return true;
                if (container->num_base_layers == 1) test_idx = 0;
                else test_idx %= container->num_base_layers;
                test_idx += container->num_top_layers;
              }
          }
        else if (universal_flags & 0x02000000)
          return true;
        int idx, n;
        for (n=0; n < num_entities; n++)
          if ((((idx=entities[n]) & 0x00FFFFFF)==test_idx) && ((idx>>24)==2))
            return true;
        return false;
      }
    bool test_compositing_layer_range(kdu_sampled_range &rg)
      { 
        ...
        if (container != NULL)
          { // Start by restricting to the top level
            if (to >= container->num_top_layers)
              to = container->num_top_layers-1;
          }
        else if (universal_flags & 0x02000000)
          return true;
        if (from <= to)
          for (n=0; n < num_entities; n++)
            if ((((idx = entities[n]) >> 24) == 2) &&
                ((idx &= 0x00FFFFFF) >= from) && (idx <= to) &&
                ((rg.step == 1) || (((idx-rg.from)%rg.step) == 0)))
              return true;
        if ((container == NULL) || (from > container->max_layer))
          return false;
        // Now restrict to container-defined compositing layers
        to = (rg.to > container->max_layer)?(container->max_layer):rg.to;
        to -= container->first_base_layer;   // Make `from' and `to'
        from -= container->first_base_layer; // relative indices
        if (to < 0) return false;
        if (from < 0) from = (rg.step==1)?0:(from % rg.step);
        if (to < from) return false; // No container-defined layers in `rg'
        if (universal_flags & 0x02000000) return true;
        for (n=0; n < num_entities; n++)
          if ((((idx=entities[n]) >> 24) == 2) &&
              ((idx = (idx&0x00FFFFFF) - container->num_top_layers) >= 0))
            { // Match k,m s.t. if idx+k*num_base_layers = from+m*rg.step
              for (max_idx=to-idx, idx=from-idx; idx <= max_idx; idx+=rg.step)
                if ((idx>=0) && ((idx % container->num_base_layers) == 0))
                  return true;
            }
        return false;
      }
  protected: // Data
    ...
    int universal_flags; // 0x01000000=match all codestreams;
                         // 0x02000000=match all layers; may have both
    kds_entity_container *container; // Re-interprets the image entity values
                                     // if non-NULL.
  };
...
#define KDS_METASCOPE_INCLUDE_FIRST_SUBBOX      ((kdu_int32) 0x00040)
    ...
    kdu_int32 flags;
      /* [SYNOPSIS]
           ...
           [>>] `KDS_METASCOPE_MANDATORY' -- If this flag is present and the
                metadata matches the current client request, the
           ...
           [>>] `KDS_METASCOPE_INCLUDE_NEXT_SIBLING' -- If this flag is
                ...
                in a JP2 header box.
           [>>] `KDS_METASCOPE_INCLUDE_FIRST_SUBBOX' -- If this flag is
                present, whenever the present group is considered relevant
                to a client request, the entire first sub-box is also
                considered relevant -- note that this first sub-box is
                guaranteed not to be a placeholder in this case.  This
                flag is used by JCLX (Compositing Layer Extensions) and
                J2CX (Multiple Codestream) boxes to indicate that their
                respective "info" sub-boxes are required to meaningfully
                interpret the contents of any current and subsequent boxes
                of the same type at the same level of the file.
           ...
           [>>] `KDS_METASCOPE_IS_CONTEXT_SPECIFIC_DATA' -- This flag may
                ...
                to true if the group contains an `nlst' box whose contents
  ...
  public: // Member functions
    ...
    virtual bool
      ...
      /* [SYNOPSIS]
           ...
           in code length (measured in bytes).  These values should generally
           lie in the interval 0 to 65535, even if they are returned as
           integers.  However, this range might occasionally be violated in
           order for the internal machinery to guarantee that the log slope
           values decrease montonically -- that may require some adjustment
           to the originally recorded values.
         [RETURNS]
           False if the codestream does not exist or if no R-D information
           is available for it.
      */
    virtual kdu_codestream
      attach_to_codestream(int codestream_id,
                           kd_serve_local::kd_serve *thread_handle)=0;
      /* [SYNOPSIS]
      ...
      */
    virtual void
      detach_from_codestream(int codestream_id,
                             kd_serve_local::kd_serve *thread_handle)=0;
      /* [SYNOPSIS]
    ...
    virtual void
      ...
      /* [SYNOPSIS]
           ...
           codestreams before calling this function again.  For this
           ...
           will be thrown.
           [//]
           You should note that the internal implementation may do a variety
           of things, including the taking of an exclusive lock on all
           codestreams, regardless of which ones are actually requested.
        [ARG: thread_handle]
           Must be the same address which was passed into the
...
class kdu_serve {
  /* [BIND: reference]
     [SYNOPSIS]
       ...
       generated.
       [//]
       From Kakadu version 6.5 on, the `kdu_serve' object commits to
       maintaining the state set up in each window context by `set_window',
       until such time as `window_finished' is called (if at all), even if
       intervening calls to `generate_increments' dispatch all content
       relevant to the window of interest.  That is, a window of interest is
       kept open even after all data has been generated by calls to
       `generate_increments'.  This is done because more data might need
       to be generated in the event that a call to `release_chunks'
       returns some abandoned data chunks.  By adopting this policy, it is
       possible for a server application to defer its processing of data
       chunk abandonment requests from a client until it can be sure that
       recent transmissions (or retransmissions) of those chunks will not
       arrive.  In practice, there is no need to call `window_finished' at
       all, but it can provide a valuable way to release temporary resources
       for use within other window contexts.
       [//]
  ...
  public: // Member functions
    ...
    KDU_AUX_EXPORT bool
      ...
      /* [SYNOPSIS]
         ...
         [RETURNS]
           True if the window is still incomplete, in the sense that not all
           data which is relevant to the window has yet been packaged up
           in calls to `generate_increments'.  Note, however, that even
           if this function returns false, meaning that the window is
           complete, future calls to `release_chunks' may provide information
           about generated data chunks which have been abandoned, as a
           result of which a call to the present function may return true
           and future calls to `generate_increments' may be required to
           regenerate some of the abandoned data.
         [ARG: context_id]
    ...
    KDU_AUX_EXPORT kds_chunk *
      ...
      /* [SYNOPSIS]
           ...
           application.
           [//]
           If `recommended_chunk_size' is > 0, the function will endeavour
           to pack messages into chunks of the indicated size, rather than
           using all bytes that can be stored in each `kds_chunk' object.
           This is especially useful for HTTP-TCP and HTTP-UDP transports
           defined by JPIP, where smaller data chunks may provide better
           channel state feedback or better throughput in the face of potential
           packet loss.
           [//]
           Prior to returning, the `kdu_serve' object updates its internal
           cache model, based on the assumption that all of the chunks will be
         ...
         [ARG: context_id]
           ...
           window context.
         [ARG: suggested_chunk_size]
           This argument is ignored unless positive, in which case if
           `suggested_chunk_size' is smaller than the `max_chunk_size'
           value passed to `initialize', the internal machinery will attempt
           to pack JPIP messages into smaller chunks.  This can be useful,
           because the best chunk size to use may depend upon channel
           conditions, which vary dynamically.  For example, if data chunks
           are transmitted over a lossy channel (e.g., using UDP), the chunk
           size (related to packet size) generally affects loss probability.
           In JPIP HTTP-TCP or HTTP-UDP transport protocols, the chunk size
           determines the granularity at which channel state information
           can be obtained through chunk acknowledgements.  Obviously, then,
           smaller chunks may allow better channel state estimation,
           particularly if the data rate is low.
      */
    KDU_AUX_EXPORT void window_finished(int context_id);
      /* [SYNOPSIS]
           This function may be called to clean up temporary resources
           that may have been set up during calls to `generate_increments'.
           This can be particularly beneficial when there are multiple
           window contexts in use.  For that reason, the `context_id'
           argument to this function deliberately has no default.
           [//]
           Typically, you would call `set_window', followed by one or more
           calls to `generate_increments'.  If a new window of interest
           becomes available (typically from a client request), you may
           just install the new window with a call to `set_window' and
           then continue calling `generate_increments'.  On the other hand,
           if there is no new window of interest and all relevant data has
           been generated by calls to `generate_increments' (i.e., if
           `get_window' returns false), it may be advisable to call this
           function to clean up resources.  Note carefully that even if
           `get_window' returns false, it is possible that a future call
           to `release_chunks' returns some abandoned data chunks so that
           future calls to `generate_increments' with the same window
           of interest will return more data.  This situation can be
           desirable in the context of lossy transmission schemes.
      */
    KDU_AUX_EXPORT void
