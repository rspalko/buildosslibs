...
class kdu_servex : public kdu_serve_target {
  ...
  */
    ...
    KDU_AUX_EXPORT virtual
      kdu_codestream
        attach_to_codestream(int codestream_id,
                             kd_serve_local::kd_serve *thread_handle);
      /* [SYNOPSIS]
  ...
  private: // Helper functions
    friend struct kd_serve_local::kdsx_open_file;
    friend class kd_serve_local::kdsx_image_entities;
    friend class kd_serve_local::kdsx_stream;
    friend struct kd_serve_local::kdsx_stream_suminfo;
    friend struct kd_serve_local::kdsx_metagroup;
    ...
    kd_serve_local::kdsx_entity_container *add_container(int num_jpch,
                                                         int num_jplh);
    ...
    kd_serve_local::kdsx_stream *add_stream(int stream_id);
    ...
    kd_serve_local::kdsx_image_entities *get_temp_entities();
      /* Used when building scope for `kdsx_metagroup'. */
    kd_serve_local::kdsx_image_entities *
      commit_image_entities(kd_serve_local::kdsx_image_entities *tmp,
                            kd_serve_local::kdsx_entity_container *container);
    ...
    kd_serve_local::kdsx_image_entities *
      ...
      /* Returns the element of a cached image entities list whose index
         ...
         `container_ref_id'-1. */
    kd_serve_local::kdsx_open_file *get_open_file(const char *fname);
  ...
  private: // Data
    ...
    kd_serve_local::kd_serve *locking_thread_handle; // Uniquely identifies
       // thread that has locked one or more streams for access.
    int num_locked_streams;
    kd_serve_local::kdsx_open_file *locked_files; // Files used by streams
                                                  // locked for access
    kd_serve_local::kdsx_open_file *used_files_head; // Files not locked but
    kd_serve_local::kdsx_open_file *used_files_tail; // with at least one user;
                                             // head is considered the LRU elt.
    kd_serve_local::kdsx_open_file *unused_files_head; // Files that are open
    kd_serve_local::kdsx_open_file *unused_files_tail; // but not used; hhead
                                             // is considered the LRU element.
    ...
    kd_serve_local::kdsx_metagroup *metatree;
    FILE *meta_fp; // Open file pointer for use in meta-data accesses.
    kd_serve_local::kdsx_stream *stream_head; // List of available code-streams
    kd_serve_local::kdsx_stream *stream_tail; // Last element in above list
    kd_serve_local::kdsx_stream **stream_refs; // Array of ptrs to codestreams
    int max_stream_refs; // Size of above array
    kd_serve_local::kdsx_entity_container *container_head;
    kd_serve_local::kdsx_entity_container *container_tail;
    kd_serve_local::kdsx_entity_container **container_refs;
    int num_containers;
  private: // Default codestream summary data
    kd_serve_local::kdsx_stream_suminfo *default_stream_suminfo;
  private: // Top-level image entitites and context mappings
    kd_serve_local::kdsx_image_entities *tmp_entities; // Active temp entities
    kd_serve_local::kdsx_image_entities *free_tmp_entities; // Free temp list
    kd_serve_local::kdsx_image_entities *committed_entities_list;
    int num_committed_entities; // Becomes number of elements in below array
    kd_serve_local::kdsx_image_entities **committed_entity_refs; // Pointers
                                                        // into committed list
    kd_serve_local::kdsx_context_mappings *top_context_mappings;
    kd_supp_local::j2_data_references data_references;  
