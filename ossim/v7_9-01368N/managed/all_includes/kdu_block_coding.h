...
class kdu_block_encoder_base {
  ...
  protected:
    ...
    virtual void cellular_encode(kdu_block *block, bool reversible,
                                 double msb_wmse, float cell_weights[],
                                 int first_cell_cols, int first_cell_rows,
                                 kdu_uint16 estimated_slope_threshold=0)
      { encode(block,reversible,msb_wmse,estimated_slope_threshold); }
      /* Must be overridden in order to actually use the cellular weighting
         functions. */
  };
  ...
  */
  ...
  void cellular_encode(kdu_block *block, bool reversible,
    ...
    /* [SYNOPSIS]
         ...
         impact of each coding pass by applying a set of additional distortion
         weights, as found in the `cell_weights' array.  The code-block is
         partitioned into cells whose nominal size is 4x4, except that the
         cells at the left, right, top and/or bottom edges may be smaller.
         The `first_cell_cols' and `first_cell_rows' arguments give the
         widths of the left-most cells on each line and the heights of the
         cells at the top of the code-block, respectively, from which the
         dimensions of all other cells are readily computed.  The total number
         of cells across and down the code-block can be determined by dividing
         its dimensions by 4 and rounding upwards; also, at most one of the
         left or right edges may have reduced width cells and at most one of
         the top or bottom edges may have reduced height cells.
    */
      state->cellular_encode(block,reversible,msb_wmse,cell_weights,
                             first_cell_cols,first_cell_rows,
